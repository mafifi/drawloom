...
async function start(registration, identity) {
  ...
  for (let attempt = 0; attempt < 8; attempt++) {
    try {
      return await registration.orchestrator.start(identity, nightloomWorkflow, {
        batchSize: 1,
        maxBytes: 65_536,
      });
    } catch (error) {
...
async function resultWithState(registration, runId) {
  try {
    ...
    throw new Error(
      `Nightloom workflow failed: ${String(error)}; snapshot=${JSON.stringify(snapshot)}; steps=${JSON.stringify(steps.steps)}`,
      { cause: error },
    );
  }
}
...
async function waitForLine(child, expected) {
  ...
  await new Promise((resolveReady, reject) => {
    const timer = setTimeout(
      () =>
        reject(new Error(`Timed out waiting for ${expected}; output=${output}; errors=${errors}`)),
      60_000,
    );
    ...
    child.once("exit", (code, signal) =>
      reject(
        new Error(
          `Fixture exited before ${expected}: ${code}/${signal}; output=${output}; errors=${errors}`,
        ),
      ),
    );
  });
...
test("real known-running assessment resumes the same workflow and original deadline after Temporal restart", {
  timeout: 120_000,
}, async () => {
  const root = await mkdtemp(join(tmpdir(), "drawloom-nightloom-running-"));
  let manager, provider;
  try {
    provider = createSqliteKnowledge({
      databasePath: join(root, "knowledge.sqlite"),
      authorizer,
      resolveResource: resource,
    });
    const ref = { type: "source", origin: "nightloom-integration", id: "running", revision: "r1" };
    await provider.intake.ingest(subject, {
      operation: "upsert",
      expectedRevision: null,
      record: {
        ref,
        body: "Public synthetic continuation evidence.",
        status: "active",
        confidence: {},
    ...
    const assessment = {
      ...
      async reconcile(_owner, identity) {
        ...
        identities.push(identity);
        return complete
          ? { kind: "completed", ...identity, proposals: [] }
          : { kind: "running", ...identity };
      },
      async cancel(_owner, identity) {
        throw Error(`unexpected cancellation ${identity.requestId}`);
      },
    };
    ...
    while (submissions === 0 && Date.now() < until)
    ...
    while (reconciliations === 0 && Date.now() < resumedUntil)
      await new Promise((resolve) => setTimeout(resolve, 20));
...
test("real cancellation during durable assessment sleep retains uncertainty until exact cancellation settles", {
  timeout: 60_000,
}, async () => {
  ...
  try {
    ...
    await provider.intake.ingest(subject, {
      ...
      record: {
        ref,
        body: "Public cancellation evidence.",
        status: "active",
        confidence: {},
        provenance: { producer: { type: "test", id: "fixture" }, inputs: [] },
      },
    ...
    const assessment = {
      async assess(_owner, request) {
        submissions++;
        identity = { requestId: request.requestId, payloadFingerprint: request.payloadFingerprint };
        return { kind: "running", ...identity };
      },
      async reconcile(_owner, request) {
        assert.deepEqual(request, identity);
        return { kind: "running", ...request };
      },
      async cancel(_owner, request) {
        cancellations++;
        assert.deepEqual(request, identity);
    ...
    const coordinator = createNightloomCoordinator({
      ...
      maintenance: provider.maintenance,
      subject,
      store: coordinatorStore(createNodeJsonStore(join(root, "coordinator"))),
      readiness: () => registration.readiness(),
      clock: { now: () => Date.UTC(2026, 8, 15) },
    });
    ...
    for (;;) {
      ...
      if (
        state.steps.some((step) => step.status === "completed" && step.result?.kind === "running")
      )
        break;
    ...
    while (!finishCancel && Date.now() < until)
    ...
    finishCancel();
    while ((await receipts.load(identity.requestId)).state !== "cancelled" && Date.now() < until)
      await new Promise((resolve) => setTimeout(resolve, 10));
...
async function stopOwnedProcessTree(child) {
  ...
  await Promise.race([
    ...
    new Promise((_, reject) =>
      setTimeout(() => reject(new Error("Fixture process did not exit")), 5_000),
    ),
  ]);
  const deadline = Date.now() + 5_000;
  while (descendants.some(alive) && Date.now() < deadline)
  ...
  while (descendants.some(alive) && Date.now() < killedDeadline)
    await new Promise((resolveWait) => setTimeout(resolveWait, 25));
  if (descendants.some(alive))
    throw new Error(
      `Owned fixture descendants did not stop: ${descendants.filter(alive).join(",")}`,
    );
}
...
test("real Nightloom keeps an actively interrupted assessment unknown without resubmission or publication", {
  timeout: 120_000,
}, async () => {
  ...
  try {
    child = spawn(
      process.execPath,
      [resolve("apps/desktop/tests/fixtures/nightloom-interrupted-assessment.mjs"), root],
    ...
    const maintenance = {
      ...
      async publish(owner, input) {
        publications += 1;
        return provider.maintenance.publish(owner, input);
      },
    };
    const assessment = {
      async assess() {
        submissions += 1;
        throw new Error("recovery must not submit the assessment again");
      },
      async reconcile(_owner, identity) {
...
test("real Nightloom safely reconciles a durable receipt and publishes one exact claim across reopen", {
  timeout: 120_000,
}, async () => {
  ...
  try {
    ...
    assert.equal(
      (
        await provider.intake.ingest(subject, {
          ...
          record: {
            ref: sourceRef,
            body: "public evidence to reconcile",
            status: "active",
            confidence: { value: "fixture" },
            provenance: { producer: { type: "test", id: "fixture" }, inputs: [] },
          },
          links: [],
        })
          ...
          proposals: [
            {
              record: {
                ref: claimRef,
                body: "Recovered from one interrupted assessment.",
                status: "active",
                freshness: "current",
                confidence: { value: "deterministic" },
                provenance: {
                  producer: { type: "assessment", id: "nightloom-integration" },
                  inputs: [sourceRef],
                },
              },
              expectedRevision: null,
              links: [{ from: claimRef, to: sourceRef, relation: "support" }],
            },
          ],
        };
      },
    ...
    assert.deepEqual(await resultWithState(registration, firstRun), {
      kind: "deferred",
      processed: 0,
      remaining: true,
    });
    ...
    assert.deepEqual(await provider.retrieval.get(subject, claimRef), {
      kind: "ok",
      record: {
        ref: claimRef,
        body: "Recovered from one interrupted assessment.",
...
test("real Nightloom controller persists pause across service restart and resumes explicit work", {
  timeout: 120_000,
}, async () => {
  ...
  try {
    ...
    assert.equal(
      (
        await provider.intake.ingest(subject, {
          ...
          expectedRevision: null,
          record: {
            ref: { type: "source", origin: "nightloom-integration", id: "paused", revision: "r1" },
            body: "paused public evidence",
            status: "active",
            confidence: { value: "fixture" },
            provenance: { producer: { type: "test", id: "fixture" }, inputs: [] },
          },
    ...
    const assessment = {
      ...
      async reconcile(_owner, identity) {
        return { kind: "completed", ...identity, proposals: [] };
      },
      async cancel(_owner, identity) {
        return { kind: "too_late", ...identity };
      },
    ...
    const activate = async () => {
      ...
      const coordinator = createNightloomCoordinator({
        ...
        readiness: () => registration.readiness(),
        clock: { now: () => Date.UTC(2026, 8, 15) },
        settings: { ...DEFAULT_NIGHTLOOM_SETTINGS, pendingThreshold: 1 },
      });
      return { registration, coordinator };
    };
    let active = await activate();
...
test("real Nightloom preserves a terminal uncertain assessment without reconciliation, publication or resubmission", {
  timeout: 120_000,
}, async () => {
  ...
  try {
    ...
    const accepted = await provider.intake.ingest(subject, {
      ...
      expectedRevision: null,
      record: {
        ref: { type: "source", origin: "nightloom-integration", id: "uncertain", revision: "r1" },
        body: "uncertain public evidence",
        status: "active",
        confidence: { value: "fixture" },
        provenance: { producer: { type: "test", id: "fixture" }, inputs: [] },
      },
    ...
    assert.deepEqual(await resultWithState(registration, runId), {
      kind: "deferred",
      processed: 0,
      remaining: true,
    });
    ...
    assert.deepEqual(await registration.orchestrator.result(reopened), {
      kind: "deferred",
      processed: 0,
      remaining: true,
    });
