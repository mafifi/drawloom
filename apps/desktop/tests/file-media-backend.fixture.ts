...
const backend: PluginBackendFactory = async ({ configuration }) => {
  const config = z
    .object({ approved: z.string().url(), blocked: z.string().url() })
    .parse(configuration);
  ...
  registerAppResource(server, "Media check", uri, {}, async () => ({
    contents: [
      {
        uri,
        mimeType: RESOURCE_MIME_TYPE,
        text: html,
        _meta: { ui: { csp: { resourceDomains: [config.approved] } } },
      },
    ],
  }));
  registerAppTool(
      ...
      workbenches: [
        {
          ...
          description: "Public synthetic browser fixture",
          tools: [],
          skills: [],
        },
      ],
