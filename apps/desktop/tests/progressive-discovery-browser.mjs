...
try {
  for (const [colorScheme, width] of [
    ...
    await page.route("**/api/discovery?*", async (route) => {
      ...
      await route.fulfill({
        json: {
          ...
          categories: [{ kind: "app", status: ready ? "available" : "loading" }],
    ...
    try {
      ...
      await page.getByText("Ready local skill", { exact: true }).waitFor({ timeout: 2000 });
      await page.getByText("Native skill", { exact: true }).waitFor({ timeout: 3000 });
      await page.getByText("Native skill", { exact: true }).click();
      await page.getByRole("button", { name: /Remove Native skill from synthetic/ }).waitFor();
      await message.fill("@");
      await page.getByRole("button", { name: "Load more apps", exact: true }).click();
      await page.getByText("Later app", { exact: true }).waitFor();
      assert.equal(pageCalls, 1, "Only explicit Load more fetches another page");
      assert.equal(
        await page.getByRole("button", { name: "Load more apps", exact: true }).count(),
        0,
      );
      assert.equal(
      ...
      assert.equal(
        await page.getByText("Wrong conversation skill", { exact: true }).count(),
        0,
      ...
      await page.getByRole("button", { name: "Refresh discovery", exact: true }).click();
      await page.getByText("Discovery temporarily unavailable", { exact: true }).waitFor();
      await page
        .getByRole("button", {
