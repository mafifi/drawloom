...
const browser=await chromium.launch({headless:true,...(process.env.DRAWLOOM_BROWSER_EXECUTABLE ? {executablePath:process.env.DRAWLOOM_BROWSER_EXECUTABLE} : {channel:'chrome'})});
...
try{
  ...
  await context.request.post(fixture.origin+'/api/command',{headers:{origin:fixture.origin},data:{kind:'rename_conversation',conversationId:fixture.conversationId,title:'Media reference fixture'}});
  await page.goto(fixture.origin);
  const card=page.getByRole('main').getByRole('region',{name:'Synthetic working video',exact:true});
  ...
  for(const theme of ['light','dark']){
    ...
    const video=workspace.locator('video');await video.waitFor();
  ...
  const downloaded=page.waitForEvent('download');await workspace.getByRole('link',{name:'Save file'}).click();
  ...
  await page.getByRole('main').getByRole('button',{name:'New conversation',exact:true}).last().click();
  ...
  await page.getByRole('main').getByRole('button',{name:'Media reference fixture',exact:true}).click();
