...
if (!DRAWLOOM_FILE_TEST_METADATA || !DRAWLOOM_PLAYWRIGHT_PATH)
  throw Error("Set fixture metadata and installed Playwright paths");
...
await context.addCookies([
  {
    name: "drawloom_" + new URL(fixture.origin).port,
    value: new URL(fixture.url).searchParams.get("token"),
    url: fixture.origin,
  },
]);
...
page.on("request", (r) => {
  if (r.url().includes("/api/files?"))
    requests.push({ url: new URL(r.url()).pathname, range: r.headers().range });
});
const report = { videoBytes: (await stat(fixture.video)).size, themes: [], requests, errors };
try {
  await context.request.post(fixture.origin + "/api/command", {
    headers: { origin: fixture.origin },
    data: { kind: "select_conversation", conversationId: fixture.conversationId },
  });
  await context.request.post(fixture.origin + "/api/command", {
    headers: { origin: fixture.origin },
    data: {
      ...
      title: "Media reference fixture",
    },
  });
  await page.goto(fixture.origin);
  const card = page
    .getByRole("main")
    .getByRole("region", { name: "Synthetic working video", exact: true });
  const workspace = page.getByRole("complementary", { name: "Artifact and details" });
  await card.waitFor();
  assert.equal(requests.length, 0, "no eager working-file read");
  for (const theme of ["light", "dark"]) {
    await page.emulateMedia({ colorScheme: theme });
    await card.getByRole("button", { name: "Open in workspace", exact: true }).click();
    const video = workspace.locator("video");
    await video.waitFor();
    await video.evaluate((v) =>
      v.readyState >= 1
        ...
        : new Promise((resolve, reject) => {
            ...
            v.addEventListener("error", () => reject(Error("Video decode failed")), { once: true });
          }),
    );
    ...
    await page.waitForFunction(
      () => {
        ...
        return v && !v.seeking && v.currentTime >= 19.9;
      },
      {},
      { timeout: 10000 },
    );
  ...
  await unknown.getByRole("button", { name: "Open in workspace", exact: true }).click();
  const downloaded = page.waitForEvent("download");
  await workspace.getByRole("link", { name: "Save file" }).click();
  ...
  await invalid.getByRole("button", { name: "Open in workspace", exact: true }).click();
  await workspace.getByText("This file could not be loaded.", { exact: false }).waitFor();
  await page.getByRole("button", { name: "Close artifact pane", exact: true }).click();
  ...
  const state = (await (await context.request.get(fixture.origin + "/api/state")).json()).sections;
  if (!state.projects.some((project) => project.directory === fixture.other)) {
  ...
  await page.getByLabel("Conversation provider", { exact: true }).click();
  await page.getByRole("option", { name: "Synthetic provider", exact: true }).click();
  await page
    .getByRole("main")
  ...
  assert.equal(
    await page.getByRole("region", { name: "Synthetic working video", exact: true }).count(),
    0,
  );
  await page
    .getByRole("textbox", { name: "Message", exact: true })
    .fill("Second project keeps its own draft");
  await page.getByRole("button", { name: "Public media", exact: true }).focus();
  await page.keyboard.press("Enter");
  await page
    .getByRole("main")
    .getByRole("button", { name: "Media reference fixture", exact: true })
    .click();
  await card.waitFor();
  ...
  const opened = await context.request.post(fixture.origin + "/api/command", {
    headers: { origin: fixture.origin },
    data: { kind: "create_conversation", workbenchId: "media-check", provider: "synthetic" },
  });
  assert.equal(opened.status(), 200);
  const owner = await opened.json();
  const mounted = await context.request.post(fixture.origin + "/api/view-session", {
    headers: { origin: fixture.origin },
    data: { conversationId: owner.selectedId, viewId: "media-check.view", action: "open" },
  });
  ...
  await page.evaluate(
    ({ id }) => {
      ...
      frame.id = "media-csp-check";
      frame.sandbox.add("allow-scripts");
      frame.src = "/api/views/media-check.view?conversationId=" + id;
  ...
  await frame.waitForFunction(
    () =>
      ...
      document.querySelector("#approved")?.naturalWidth === 1 &&
      document.querySelector("#project")?.readyState >= 1,
  );
  assert.deepEqual(
    await frame.evaluate(() => ({
      remoteScript: Boolean(window.remoteScriptRan),
      blockedImage: document.querySelector("#blocked").naturalWidth,
    })),
    { remoteScript: false, blockedImage: 0 },
  );
  ...
  assert.equal(
    (await context.request.get(scopedUrl, { headers: { range: "bytes=0-1" } })).status(),
    403,
  );
  await page.goto(fixture.origin);
  await card.waitFor();
  ...
  console.error("Original browser failure:", String(error));
  console.error(
    JSON.stringify({
      requests,
      media: await page
