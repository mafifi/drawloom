...
if (!DRAWLOOM_PLAYWRIGHT_PATH || !DRAWLOOM_INPUT_TEST_URL || !DRAWLOOM_INPUT_TEST_TOKEN)
  throw Error("Supply browser runtime and disposable authenticated desktop.");
  ...
  for (const [colorScheme, width] of [
    ...
    try {
      ...
      const form = (requestId) => ({
        requestId,
        source: "package:public-stationery:choices",
        invocationId: "invocation-" + requestId,
        operationId: "synthetic-operation",
        params: {
          mode: "form",
          message: "Choose stationery for the public example.",
          requestedSchema: {
            type: "object",
            properties: {
              paper: {
      ...
      await page.route("**/api/state*", (route) =>
        route.fulfill({
          json: { kind: "snapshot", token: `form-${++revision}`, sections: snapshot, removed: [] },
        }),
      );
      await page.route("**/api/command", async (route) => {
        const command = route.request().postDataJSON();
        commands.push(command);
        assert.equal(command.kind, "elicitation");
        assert.equal(command.conversationId, snapshot.selectedId);
        if (command.result.action === "accept") {
          assert.deepEqual(command.result.content, {
            paper: "lined",
            copies: 3,
            recycled: true,
            colours: ["blue", "red"],
        ...
        } else if (command.result.action === "decline")
          snapshot.elicitations = [form("form-cancel")];
      ...
      await page.screenshot({
        path: `/tmp/drawloom-elicitation-${colorScheme}-${width}.png`,
        fullPage: true,
      });
      ...
      await page
        .getByText("Choose stationery for the public example.", { exact: true })
        .waitFor({ state: "hidden" });
      assert.deepEqual(
        commands.map((command) => command.result.action),
      ...
      console.log(
        JSON.stringify({
          ...
          width,
          typedForm: true,
          actions: commands.map((command) => command.result.action),
