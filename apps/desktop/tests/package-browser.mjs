...
const {
  DRAWLOOM_PLAYWRIGHT_PATH,
  DRAWLOOM_INPUT_TEST_URL,
  DRAWLOOM_INPUT_TEST_TOKEN,
  DRAWLOOM_PACKAGE_FIXTURE,
} = process.env;
if (
  !DRAWLOOM_PLAYWRIGHT_PATH ||
  !DRAWLOOM_INPUT_TEST_URL ||
  ...
  throw Error("Supply disposable host, package fixture and browser runtime.");
...
try {
  for (const [colorScheme, width] of [
    ...
    await context.addCookies([
      { name: "drawloom_" + url.port, value: DRAWLOOM_INPUT_TEST_TOKEN, url: url.origin },
    ]);
    ...
    try {
      ...
      await packages
        .getByRole("textbox", { name: "Package folder" })
        .fill(DRAWLOOM_PACKAGE_FIXTURE);
      // The initial installation read disables actions until its result arrives.
      await page.waitForFunction(() =>
        [...document.querySelectorAll("button")].some(
          (button) => button.textContent?.trim() === "Inspect package" && !button.disabled,
        ),
      );
      await packages.getByRole("button", { name: "Inspect package", exact: true }).focus();
      await page.keyboard.press("Enter");
      await packages
        .getByText("1 skills · 1 servers · Standard package", { exact: true })
        .waitFor();
      ...
      const backend = packages.getByRole("checkbox", {
        name: "Trust this package’s backend to execute in the host process",
      });
      assert.equal(await backend.getAttribute("aria-checked"), "false");
      const enabled = packages.getByRole("checkbox", {
        name: "Activate configured servers and skills on next restart",
      });
      ...
      await packages
        .getByText("Saved. Restart Drawloom to apply these changes.", { exact: true })
        .waitFor();
      assert.equal(await serverChoice.getAttribute("aria-checked"), "false");
      await packages
        .getByRole("textbox", { name: "Package folder" })
        .fill("/a/nonexistent/synthetic/package");
      ...
      const installation = {
        id: "03f41a9c-8308-4bb0-b00d-00a4c77a2abb",
        root: "/synthetic/oauth-package",
        name: "OAuth presentation fixture",
        enabled: true,
        ...
        status: "partial",
        diagnostics: [],
        connections: [{ name: "remote", status: "auth-required", transport: "streamable-http" }],
      };
      const authActions = [];
      let statusState = "denied";
      await page.route("**/api/packages", (route) => route.fulfill({ json: [installation] }));
      await page.route("**/api/packages/oauth", (route) => {
        const input = route.request().postDataJSON();
        authActions.push(input.action);
        assert.equal(input.id, installation.id);
        assert.equal(input.server, "remote");
        const state =
        ...
        return route.fulfill({
          json: {
            ...
            ...(input.action === "connect"
              ...
              : {}),
            ...(input.action === "reconnect" ? { restartRequired: true } : {}),
          },
        });
      });
      await packages.getByRole("button", { name: "Refresh", exact: true }).click();
      await packages
        .getByRole("button", { name: "Preconfigured OAuth client", exact: true })
        .click();
      await packages
        .getByRole("textbox", { name: "Local registration JSON file" })
        .fill("/synthetic/client-registration.json");
      await packages
        .getByRole("button", { name: "Import client registration", exact: true })
        .focus();
      await page.keyboard.press("Enter");
      await packages.getByText("remote · auth-required · disconnected", { exact: true }).waitFor();
      assert.equal(
        await packages.getByRole("textbox", { name: "Local registration JSON file" }).inputValue(),
        "",
      );
      ...
      assert.equal(
        await packages.getByRole("link", { name: "Continue sign-in in your browser" }).count(),
        0,
      );
      ...
      await packages.getByRole("button", { name: "Check sign-in", exact: true }).click();
      await packages
        .getByText("remote · auth-required · refresh-failed", { exact: true })
        .waitFor();
      await packages.getByRole("button", { name: "Reconnect", exact: true }).click();
      await packages
        .getByText("Restart Drawloom to load this server’s newly discovered contributions.", {
          exact: true,
        })
        .waitFor();
      ...
      console.log(
        JSON.stringify({
          ...
          backendUntrusted: true,
          failureVisible: true,
          simulatedOAuthControls: authActions,
