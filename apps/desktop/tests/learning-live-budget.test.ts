...
function native(
  ...
  const rpc: RpcTransport = {
    async request(method) {
      ...
      if (method === "turn/interrupt" && options.turns)
        options.turns = options.turns.map((t) => ({ ...t, status: "interrupted" }));
      return {};
    },
    notify() {},
    respond() {},
...
test("failed foreground and assessor sends consume the same allowance before transport dispatch", async () => {
  ...
  const a = native({ fail: true }),
    b = native();
  const foreground = budget.wrap(a.rpc, "foreground"),
    assessor = budget.wrap(b.rpc, "assessment");
  await foreground.request("thread/start", {});
  for (let i = 0; i < 12; i++) {
    try {
      await (i % 2 ? assessor : foreground).request(i % 2 ? "turn/steer" : "turn/start", {
        threadId: "owned",
      });
    } catch {}
  }
...
test("deadline remains fixed across connections and interrupts an unacknowledged native submission", async () => {
  ...
  const rpc = budget.wrap(a.rpc, "assessment");
  ...
  await expect(
    budget.wrap(native().rpc, "foreground").request("turn/steer", { threadId: "owned" }),
  ).rejects.toThrow("deadline");
  expect(stopped).toBe(1);
  expect(budget.state.firstSubmissionAt).toBe(100);
  expect(budget.state.attempts[0]?.outcome).toBe("uncertain");
  expect(budget.state.threads).toEqual(["owned"]);
  budget.dispose();
});
...
test("durable receipt failure prevents model dispatch", async () => {
  const a = native();
  const budget = createLiveBudget({
    initial: { threads: ["owned"], attempts: [] },
    persist() {
      throw Error("disk full");
    },
  });
  await expect(
    budget.wrap(a.rpc, "foreground").request("turn/start", { threadId: "owned" }),
...
test("restoring a receipt preserves consumed attempts and the original deadline", async () => {
  ...
  first.dispose();
  const restored = createLiveBudget({
    initial: structuredClone(first.state),
    now: () => now,
    persist() {},
  });
  await restored.wrap(native().rpc, "assessment").request("turn/start", { threadId: "owned" });
  await expect(
    restored.wrap(native().rpc, "foreground").request("turn/start", { threadId: "owned" }),
  ).rejects.toThrow("allowance");
  now = 600_001;
  await expect(
    restored.wrap(native().rpc, "foreground").request("turn/start", { threadId: "owned" }),
  ).rejects.toThrow("deadline");
  expect(restored.state.firstSubmissionAt).toBe(1);
  restored.dispose();
});
...
test("expiry still interrupts when persistence fails, without throwing from the timer", async () => {
  ...
  const budget = createLiveBudget({
    persist() {
      if (broken) throw Error("disk failed");
    },
    schedule: (callback) => {
      expire = callback;
      return () => {};
  ...
  const rpc = budget.wrap(native().rpc, "foreground");
  await rpc.request("thread/start", {});
  await rpc.request("turn/start", { threadId: "owned" });
  broken = true;
...
test("unowned submissions and uncertain thread creation stop before model work", async () => {
  ...
  await expect(
    budget.wrap(n.rpc, "foreground").request("turn/start", { threadId: "someone-elses-thread" }),
  ).rejects.toThrow("owned");
  expect(n.calls).toEqual([]);
  const broken = {
    ...n.rpc,
    request: async () => {
      throw Error("lost thread/start reply");
    },
  };
  await expect(budget.wrap(broken, "foreground").request("thread/start", {})).rejects.toThrow(
    "lost",
  );
  expect(budget.state.uncertainThreadStarts).toBe(1);
  await expect(budget.wrap(n.rpc, "foreground").request("thread/start", {})).rejects.toThrow(
    "uncertain",
  );
  budget.dispose();
});
...
test("cleanup interrupts owners, closes them before archive, and retains unresolved identities", async () => {
  ...
  const retained = await cleanupLiveThreads(["owned"], {
    attempts: [
      {
        role: "foreground",
        method: "turn/start",
...
test("owner-close or status failure cannot archive a thread", async () => {
  ...
  const results = await cleanupLiveThreads(["owned"], {
    connect: async () => a.rpc,
    save() {},
    closeOwners: async () => {
      throw Error("still open");
    },
  });
  expect(results[0]?.outcome).toBe("retained-uncertain");
  expect(a.calls).not.toContain("thread/archive");
});

test("cleanup includes native identities discovered while owners finish closing", async () => {
  const ids: string[] = [];
  const n = native();
  const results = await cleanupLiveThreads(ids, {
    connect: async () => n.rpc,
    save() {},
    closeOwners: async () => {
      ids.push("owned");
  ...
  expect(n.calls).toContain("thread/archive");
});
...
test("evidence write failure still closes owners and prevents archive", async () => {
  ...
  expect(n.calls).not.toContain("thread/archive");
});
...
test("deadline interruption reaches the owning transport, never a different recovery process", async () => {
  ...
  const result = await cleanupLiveThreads(budget.state.threads, {
    attempts: budget.state.attempts,
    connect: async () => recoveryRpc,
    save() {},
    closeOwners: () => rpc.close(),
  });
  expect(result[0]?.outcome).toBe("archived");
  expect(recovery.calls).not.toContain("turn/interrupt");
  budget.dispose();
});
...
test("an old completed turn cannot resolve a later unacknowledged start and the owner is closed", async () => {
  ...
  expire();
  await expect(submission).rejects.toThrow("deadline");
  await budget.waitForExpiry();
  expect(owner.calls).toContain("close");
  const recovery = native({ turns: [{ id: "old", status: "completed" }] });
  const result = await cleanupLiveThreads(budget.state.threads, {
    attempts: budget.state.attempts,
    connect: async () => recovery.rpc,
    save() {},
    closeOwners: async () => {},
  });
  expect(result[0]?.outcome).toBe("retained-uncertain");
  expect(recovery.calls).not.toContain("thread/archive");
  budget.dispose();
});
...
test("bounded owner reconciliation closes the owner when its status cannot be established", async () => {
  ...
  const owner = {
    ...n.rpc,
    request: async (method: string, params: unknown) =>
      method === "thread/turns/list" ? new Promise<never>(() => {}) : n.rpc.request(method, params),
    close: async () => {
      ownerClosed = true;
    },
  };
  ...
  const rpc = budget.wrap(owner, "foreground");
  await rpc.request("thread/start", {});
  await rpc.request("turn/start", { threadId: "owned" });
  expire();
  await budget.waitForExpiry();
