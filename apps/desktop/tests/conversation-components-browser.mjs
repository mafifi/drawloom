...
const {
  ...
  DRAWLOOM_UI_EVIDENCE_DIR: root,
} = process.env;
if (!DRAWLOOM_PLAYWRIGHT_PATH || !DRAWLOOM_INPUT_TEST_URL || !DRAWLOOM_INPUT_TEST_TOKEN || !root)
  throw Error("Set Playwright path, disposable host URL/token and UI evidence directory.");
...
try {
  ...
  await context.addCookies([
    {
      name: "drawloom_" + new URL(host.origin).port,
      value: DRAWLOOM_INPUT_TEST_TOKEN,
      url: host.origin,
    },
  ]);
  ...
  assert.equal(
    ...
    "article",
    "Message keeps named article semantics",
  );
  assert.equal(
    await message.locator('[data-slot="bubble"] [data-slot="attachment"]').count(),
    0,
  ...
  assert.equal(userStyle.font, "15px");
  const files = Array.from({ length: 7 }, (_, i) => ({
    name: `reference-${i}.txt`,
    mimeType: "text/plain",
    buffer: Buffer.from(`Synthetic reference ${i}`),
  }));
  await page.getByLabel("Choose attachments", { exact: true }).setInputFiles(files);
  await page
    .locator('[data-slot="attachment"][data-state="uploading"]')
    .first()
  ...
  assert.ok(
    ...
    "Attachment group actually overflows horizontally",
  );
  await group.focus();
  await page.keyboard.press("End");
  await page
    .getByRole("textbox", { name: "Message", exact: true })
    .fill("Choose the strongest references, then revise the introduction.");
  for (const theme of ["light", "dark"]) {
    await page.emulateMedia({ colorScheme: theme });
    await page.waitForFunction(
      (theme) =>
        getComputedStyle(document.querySelector('[data-slot="attachment"]')).backgroundColor ===
  ...
  await page.getByLabel("Choose attachments", { exact: true }).setInputFiles({
    name: "pending.txt",
    mimeType: "text/plain",
    buffer: Buffer.from("Slow synthetic upload"),
  });
  ...
  assert.equal(
    await pending
      .locator('[data-slot="attachment-title"]')
      .evaluate((el) => getComputedStyle(el).animationName),
    "none",
    "Reduced motion stops pending title shimmer",
  );
  assert.equal(
    await pending
      .locator('svg[role="status"]')
      .evaluate((el) => getComputedStyle(el).animationName),
    "none",
    "Reduced motion stops pending spinner",
  );
  release();
  ...
  console.log(
    JSON.stringify({
      ...
      restoredTitle: true,
      separateBubble: true,
      primaryColour: userStyle.background,
      bodyFont: userStyle.font,
      horizontalAttachments: true,
