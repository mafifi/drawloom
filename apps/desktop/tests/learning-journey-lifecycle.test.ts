...
function ownedPids(root: string) {
  return execFileSync('ps', ['-axo', 'pid=,command='], { encoding: 'utf8' }).split('\n')
    .filter(line => line.includes('sidecar.js') && line.includes(root)).map(line => Number(line.trim().split(/\s+/, 1)[0]));
}
...
test('rejected desktop construction closes its already running knowledge sidecar', async () => {
  ...
  try {
    ...
    await expect(fixture.open({ root: join(root, 'knowledge'), workingDirectory: root }, async client => {
      emergencyClose = () => client.close();
      await client.status();
      pids = ownedPids(root); expect(pids).toHaveLength(1);
    ...
    await fixture.close();
  } finally { await emergencyClose?.(); await rm(root, { recursive: true, force: true }); }
});
...
test('fixture cleanup rejects while its owned client cannot close and retains the handle for recovery', async () => {
  ...
  try {
    await expect(fixture.open({ root: join(root, 'knowledge'), workingDirectory: root }, async client => {
      ...
      client.close = async () => { if (!permitClose) throw Error('Controlled cleanup failure'); await emergencyClose!(); };
      await client.status(); pids = ownedPids(root); expect(pids).toHaveLength(1);
