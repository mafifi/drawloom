...
async function waitForInput(run) {
  for (let attempt = 0; attempt < 200; attempt++) {
    ...
    if (["failed", "cancelled"].includes(snapshot.status))
      throw Error(`Unexpected run status: ${snapshot.status}`);
...
try {
  ...
  assert.equal(
    ...
    "ready",
    "The installed desktop loader must attach actual workflow handlers",
  );
  ...
  const run = await registration.orchestrator.start(
    "specimen-001",
    registration.registry.workflows[0],
    "  moss  ",
  );
  ...
  assert.deepEqual(
    preparedOwners.map((owner) => [owner.projectId, owner.installationId, owner.packageDirectory]),
    [
      [projectId, installationId, pkg],
      [projectId, installationId, pkg],
    ],
  );
