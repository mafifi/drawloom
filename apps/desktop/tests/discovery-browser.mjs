...
if (!DRAWLOOM_PLAYWRIGHT_PATH || !DRAWLOOM_INPUT_TEST_URL || !DRAWLOOM_INPUT_TEST_TOKEN)
  throw Error("Set Playwright and disposable host URL/token.");
...
const entries = Array.from({ length: 4301 }, (_, index) => ({
  id: "app:" + index,
  origin: "test",
  kind: "app",
  name: "Discovery test app " + index,
  description: index === 4300 ? "Beyond initial page" : "",
  scope: "workspace",
...
async function check(viewport, colorScheme, label) {
  ...
  await page.route("**/api/discovery?*", (route) =>
    route.fulfill({
      json: { entries: catalogue, categories: [], experimentalPluginDiscovery: false },
    }),
  );
  try {
    ...
    assert.equal(
      await page.getByRole("complementary", { name: "Artifact and details" }).count(),
      0,
    );
    ...
    await page
      .getByLabel("Find plugins, apps, tools and skills", { exact: true })
      .fill("Beyond initial page");
    ...
    assert.equal(
      await page
        .locator('[data-slot="command-item"][aria-selected="true"]')
        .innerText()
        .then((value) => value.includes("Discovery test app 4300")),
      true,
    );
    ...
    await text.press("ArrowDown");
    assert.ok(
      (await page.locator('[data-slot="command-item"][aria-selected="true"]').innerText()).includes(
        "Chosen skill",
      ),
    );
    await text.press("Enter");
    ...
    for (const name of ["Send message", "Agent provider", "Execution review"]) {
      ...
      assert.ok(
        ...
        `${name} must not be clipped at ${viewport.width}px`,
      );
    }
