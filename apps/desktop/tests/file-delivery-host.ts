...
const telemetry = initializeObservability({
  mode: "recording",
  serviceName: "drawloom.file-verification",
  exporters: {
    traces: {
      export(spans: ReadableSpan[], done) {
        for (const s of spans)
          if (s.name === "host.file.deliver")
            deliveries.push({
              bytes: s.attributes["drawloom.file.bytes"],
              cached: s.attributes["drawloom.file.cached"],
              outcome: s.attributes["drawloom.outcome"],
              milliseconds: s.duration[0] * 1000 + s.duration[1] / 1e6,
            });
        done({ code: 0 });
      },
      async shutdown() {},
...
const allowed = Bun.serve({
  ...
  fetch(request) {
    ...
    originRequests.push("approved:" + path);
    return new Response(path === "/script" ? "window.remoteScriptRan=true" : png, {
      headers: { "Content-Type": path === "/script" ? "text/javascript" : "image/png" },
    });
  },
});
    ...
    extensions: {
      "org.drawloom": {
        ...
        workbenches: [
          {
            id: "media-check",
            title: "Media policy check",
            openingTool: { server: "media", tool: "media.open" },
          },
        ],
      },
    },
...
await installations.configure(installation, {
  enabled: true,
  trustedBackend: true,
  servers: [],
  ...
  entries: [
    {
      ...
      role: "assistant",
      state: "complete",
      text: "Public working-file references. Open only the media you want to inspect.",
      assets: [],
      resources: [
        {
          id: "video",
          title: "Synthetic working video",
          source: "synthetic",
          status: "unavailable",
        ...
        {
          id: "unknown",
          title: "Unknown working format",
          source: "synthetic",
          status: "unavailable",
          uri: pathToFileURL(join(work, "unknown.custom")).href,
        ...
        },
        {
          ...
          source: "synthetic",
          status: "unavailable",
          uri: pathToFileURL(join(work, "invalid.mp4")).href,
          mimeType: "video/mp4",
        },
      ],
    },
...
const rangeUrl =
  host.origin +
  "/api/files?" +
  new URLSearchParams({ conversationId: state.selectedId, path: "large.bin" });
for (let i = 0; i < 30; i++) {
  const started = performance.now();
  const response = await fetch(rangeUrl, {
    headers: { cookie, range: `bytes=${i * 1024 * 1024}-${i * 1024 * 1024 + 65535}` },
  });
  if (response.status !== 206 || (await response.arrayBuffer()).byteLength !== 65536)
    throw Error("Range verification failed");
...
await writeFile(
  join(root, "range-measurements.json"),
  JSON.stringify(
    {
