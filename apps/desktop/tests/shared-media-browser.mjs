...
await page.addInitScript(() => {
  ...
  window.animationProbe = [];
  Element.prototype.animate = function (...args) {
    const result = animate.apply(this, args);
    window.animationProbe.push({ animation: result, element: this });
    return result;
  };
});
const errors = [];
page.on("pageerror", (e) => {
  ...
  console.error("Browser exception", e.stack);
});
const consoleMessages = [];
page.on("console", (m) => {
  if (["error", "warning"].includes(m.type())) consoleMessages.push(m.text());
});
const report = {
  themes: [],
  errors,
  browser: "Chrome via Playwright; Browser plugin not available",
...
try {
  ...
  assert.equal(
    await frame.getByLabel("Unsaved note").inputValue(),
    "Keep this until I choose to reopen",
  );
  assert.equal(
    await frame.locator("#returned").evaluate((n) => n.naturalWidth),
    0,
    "existing frame was not silently reloaded",
  );
  report.explicitReopen = { notice: true, unsavedPreserved: true };
  console.log("Notice shown without losing draft");
  await page.screenshot({
    path: join(fixture.root, "shared-media-notice.png"),
    fullPage: true,
    animations: "disabled",
  });
  await page.getByRole("button", { name: "Reopen workbench", exact: true }).focus();
  ...
  await page.waitForFunction(
    () => document.querySelectorAll('iframe[title="consumer"]').length === 0,
  );
  ...
  await (await (await card.locator("iframe").elementHandle()).contentFrame()).waitForFunction(
    () => document.querySelector("#media").naturalWidth === 1,
  );
  ...
  for (const theme of ["light", "dark"]) {
    ...
    await page.screenshot({
      path: join(fixture.root, `shared-media-${theme}.png`),
      fullPage: true,
      animations: "disabled",
    });
  ...
  await page
    .frameLocator('iframe[title="Remote preview: Expired image"]')
    .getByText("This media could not be loaded.", { exact: false })
    .waitFor();
  report.expiryFeedback = true;
  await page.setViewportSize({ width: 390, height: 844 });
  await page.screenshot({
    path: join(fixture.root, "shared-media-narrow.png"),
    fullPage: true,
    animations: "disabled",
  });
  ...
  report.pageIdentity = true;
  report.meaningfulContent = true;
  report.noFrameworkOverlay = true;
  const unexpected = consoleMessages.filter(
    (message) =>
      !/Content Security Policy|Content-Security-Policy|Failed to load resource|violates.*directive/i.test(
        message,
      ),
  );
  ...
  await writeFile(join(fixture.root, "shared-media-results.json"), JSON.stringify(report, null, 2));
  ...
  console.error(
    ...
    await page.evaluate(() =>
      window.animationProbe
        .filter((x) => x.element.tagName === "IFRAME")
        .map(({ animation: a, element }) => ({
          state: a.playState,
  ...
  await page.screenshot({ path: join(fixture.root, "shared-media-failure.png"), fullPage: true });
