...
if (
  ...
  throw Error("Supply a disposable host, browser runtime and evidence directory.");
...
try {
  for (const width of [1280, 390]) {
    ...
    await page.route("**/api/orchestration/owners?*", (route) =>
      route.fulfill({
        json: [{ installationId: "documents", title: "Documents", readiness: { status: "ready" } }],
      }),
    );
    await page.route("**/api/orchestration/runs?*", (route) =>
      route.fulfill({ json: { runs: [run] } }),
    );
    await page.route("**/api/orchestration/steps?*", (route) =>
      route.fulfill({
        json: new URL(route.request().url()).searchParams.has("cursor")
          ? { steps: [{ stepId: "Assemble document", attempts: 2, status: "completed" }] }
          : {
              steps: [{ stepId: "Prepare reference", attempts: 1, status: "completed" }],
              cursor: "second",
            },
      }),
    );
    await page.route("**/api/orchestration/runs", async (route) => {
      ...
      if (command.action === "respond")
        return route.fulfill({
          status: 400,
          json: {
            error: "Check the requested JSON and submit only if the input is still pending.",
          },
        });
    ...
    assert.ok(
      commands.every(
        (command) =>
          command.projectId && command.installationId === "documents" && command.runId === "run",
      ),
    );
  ...
  console.log(
    "Workflow presentation: desktop/light and mobile/dark passed; input error, steps, cancellation and no stale controls.",
  );
} finally {
  await browser.close();
}
