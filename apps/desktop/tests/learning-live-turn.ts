...
export async function sendLiveTurn(input: {
  ...
  const submissionOffset = input.submissions.length,
    messageOffset = input.messages.length;
  await input.command({
    kind: "send",
  ...
  const submitted = z
    .array(
      z.object({
        ...
        response: z.object({ turn: z.object({ id: z.string().min(1) }) }),
      }),
    )
    .length(1)
    .parse(
      input.submissions
        .slice(submissionOffset)
        .filter((value) => value.role === "foreground" && value.method === "turn/start"),
    )[0]!;
  const threadId = submitted.params.threadId,
    turnId = submitted.response.turn.id;
  const matching = () =>
    input.messages.slice(messageOffset).filter(({ role, message }) => {
      ...
      return (
        ...
        (message.method === "turn/completed"
          ? value.data.turn?.id === turnId
          : value.data.turnId === turnId)
      );
    });
  await input.until(
    async () =>
      matching().some(
        ({ message }) =>
          message.method === "turn/completed" &&
          z
            .object({ turn: z.object({ status: z.enum(["completed", "failed", "interrupted"]) }) })
