...
if (!DRAWLOOM_PLAYWRIGHT_PATH || !DRAWLOOM_UI_URL || !DRAWLOOM_UI_EVIDENCE_DIR) throw Error('Set installed Playwright path, disposable host URL and evidence directory.');
...
try {
  ...
  const pluginOwner = { installationId: 'public-fixture', title: 'Project documents', readiness: { status: 'ready' } };
  ...
  for (const first of ['knowledge', 'plugin']) {
    ...
    await page.route('**/api/orchestration/runs?*', route => route.fulfill({ json: { runs: [] } }));
    await page.goto(url.origin);
    await page.getByRole('button', { name: 'Activity', exact: true }).click();
    await page.getByRole('button', { name: first === 'knowledge' ? hostOwner.title : pluginOwner.title, exact: true }).click();
    releaseSlow();
    await page.getByRole('button', { name: first === 'knowledge' ? pluginOwner.title : hostOwner.title, exact: true }).waitFor();
    ...
    await page.unroute('**/api/orchestration/owners?*'); await page.unroute('**/api/knowledge-activity/owner*'); await page.unroute('**/api/orchestration/runs?*');
  }
  ...
  for (const tab of ['Search', 'Sources', 'Settings']) {
    await page.getByRole('tab', { name: tab, exact: true }).click(); assert.equal(await warning.isVisible(), true); assert.equal(await warning.count(), 1);
  }
  await page.screenshot({ path: `${DRAWLOOM_UI_EVIDENCE_DIR}/setup-warning-after-pause-and-save.png` });
  ...
  for (const clear of ['success', 'stop']) {
    ...
    await page.route('**/api/knowledge', async route => {
      ...
      const statusResponse = command.action === 'source'
        ? await page.request.post(url.origin + '/api/knowledge', { headers: { origin: url.origin }, data: { action: 'status' } }) : response;
      ...
      value.source = { projectId: 'synthetic', enabled: !cleared || clear === 'success', state: !cleared ? 'unavailable' : clear === 'success' ? 'ready' : 'stopped', message: !cleared ? 'Installed Git source is unavailable.' : '' };
    ...
    await page.getByRole('tab', { name: 'Sources', exact: true }).click();
    await page.getByRole('button', { name: clear === 'success' ? 'Connect project source' : 'Stop collection', exact: true }).click();
    await warning.waitFor({ state: 'detached' });
    await page.unroute('**/api/knowledge');
    await page.getByRole('tab', { name: 'Settings', exact: true }).click();
  }
  assert.deepEqual(errors, []);
  console.log(JSON.stringify({ knowledgeFirstSelectionRetainsPlugin: true, pluginFirstSelectionRetainsKnowledge: true, setupWarningSurvivesPauseSaveAndRefresh: true, tabContinuity: true, configuredWarningDeduplicated: true, authoritativeSuccessAndStopClearWarning: true, pageErrors: errors }));
