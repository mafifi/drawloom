...
const { DRAWLOOM_PLAYWRIGHT_PATH, DRAWLOOM_INPUT_TEST_URL, DRAWLOOM_INPUT_TEST_TOKEN } =
  process.env;
if (!DRAWLOOM_PLAYWRIGHT_PATH || !DRAWLOOM_INPUT_TEST_URL || !DRAWLOOM_INPUT_TEST_TOKEN)
  throw Error("Set the Playwright path and disposable input test host URL/token.");
...
try {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    assert.deepEqual(
      errors,
      [],
      "file selection must never write a nonempty filename into input.value",
    );
    assert.equal(
      await input.inputValue(),
      "",
      "consumer can clear file input for same-file reselection",
    );
    await page
      .getByRole("button", { name: "Remove input-binding-proof.png", exact: true })
      .waitFor();
    await page
      .getByRole("button", { name: "Importing attachments", exact: true })
      .waitFor({ state: "hidden" });
    const preview = page
      .getByRole("list", { name: "Attachments", exact: true })
      .getByRole("img", { name: "input-binding-proof.png", exact: true });
  ...
  for (const theme of ["light", "dark"]) {
    ...
    for (const event of ["paste", "drop"]) {
      ...
      await page.locator(event === "paste" ? "#message-draft" : "form.composer").evaluate(
        (element, { event, name }) => {
          const transfer = new DataTransfer();
          transfer.items.add(new File(["Synthetic attachment"], name, { type: "text/plain" }));
      ...
      await page
        .getByRole("button", { name: "Importing attachment", exact: true })
        .waitFor({ state: "hidden" });
      await remove.click();
    }
    let fail = true;
    await page.route("**/api/import?*", (route) =>
      fail
        ? route.fulfill({ status: 400, json: { error: "Synthetic upload failure" } })
        : route.continue(),
    );
    await input.setInputFiles({
      name: "retry.txt",
      mimeType: "text/plain",
      buffer: Buffer.from("Retry reference"),
    });
    await page
      .getByRole("list", { name: "Attachments", exact: true })
      .getByText("Synthetic upload failure", { exact: true })
      .waitFor();
    fail = false;
  ...
  console.log(
    JSON.stringify({
      fileSelection: "pass",
      clearing: "pass",
      sameFileReselection: "pass",
      textBinding: "pass",
      attachmentReferenceReload: "pass",
      pasteAndDrop: "pass",
      uploadFailureAndRetry: "pass",
      lightAndDark: "pass",
      pageErrors: errors,
    }),
  );
