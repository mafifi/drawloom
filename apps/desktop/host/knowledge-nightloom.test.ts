...
test("Nightloom registers once as a host capability and attaches only its bounded handlers", async () => {
  ...
  const engine: Orchestrator = {
    ...
    async get(runId) { return { runId, identity: "identity", workflow: "nightloom.maintenance", version: "1", status: phase, cancellationRequested: false, childRunIds: [], unresolvedEffects: unresolved ? ["assessment"] : [], stepsTruncated: false, pendingInputs: [], steps: [] }; },
    async getSteps() { return { steps: [] }; }, async list() { return { runs: [] }; }, async result(): Promise<JsonValue> { if (deferred) return { kind: "deferred", processed: 0, remaining: true }; return { kind: "completed", processed: 1, remaining: false, checkpoint: "done" }; }, async respond() {}, async cancel() {},
  };
  ...
  const nightloom = createKnowledgeNightloom({ service, store: coordinatorStore, packageDirectory: "/installed/nightloom",
    settings: async () => ({ automaticContext: false, captureOutcomes: false, automaticCuration: true, embeddingModel: "qwen3-embedding-0.6b-gguf", assessmentModel: "gpt-5.6-terra", assessmentTimeoutMs: 300_000, maxAutomaticStartsPerDay: 6, maxAutomaticMillisecondsPerDay: 1_800_000 }),
  ...
  const host = createKnowledgeHost({ service: client, store: memoryStore(), selectedProjectId: () => undefined,
    sourceForProject: async () => { throw Error("No source requested"); }, nightloom });
  try {
...
test("owned scheduling is serial, default-off, pause-aware and stopped before close", async () => {
  ...
  const service: NightloomKnowledgeService = {
    async maintenanceStatus() { if (releaseStatus) await new Promise<void>(resolve => { releaseStatus = resolve; }); return { kind: "ok", pendingUnits: 50, checkpoint: "status" }; },
    maintenancePending: unavailable, maintenancePublish: unavailable, maintenanceRelease: unavailable,
    search: unavailable, get: unavailable, expand: unavailable, evidence: unavailable, export: unavailable,
    async assess() { throw Error("external double must not be submitted by fake workflow"); },
    async reconcile() { throw Error("external double must not be called by fake workflow"); },
    async cancelAssessment() { throw Error("external double must not be cancelled by fake workflow"); },
  };
  const engine: Orchestrator = {
    async start(identity) { starts++; return `host/${identity}`; },
    async get(runId) { return { runId, identity: "same", workflow: "nightloom.maintenance", version: "3", status: phase, cancellationRequested: false, childRunIds: [], unresolvedEffects: [], stepsTruncated: false, pendingInputs: [], steps: [] }; },
    async result() { return { kind: "completed", processed: 1, remaining: false, checkpoint: "done" }; },
    async getSteps() { return { steps: [] }; }, async list() { return { runs: [] }; }, async respond() {}, async cancel() {},
  };
  const host = createKnowledgeNightloom({ service, store: memoryStore(), packageDirectory: "/public/double", settings: async () => configuration,
    scheduleTick(callback) { expect(scheduled).toBeUndefined(); scheduled = callback; return () => { scheduled = undefined; }; },
    prepareHost: async () => ({ orchestrator: engine, registry: { workflows: [], tasks: [] }, readiness: () => ({ status: "ready" }), async attach() {}, async close() { closes++; } }),
  });
  const fire = async () => { const callback = scheduled!; scheduled = undefined; await callback(); };
  host.startScheduling(); host.startScheduling();
  await fire(); expect(starts).toBe(0);
  configuration = { ...configuration, automaticCuration: true };
  await fire(); expect(starts).toBe(1);
  await fire(); expect(starts).toBe(1);
  phase = "completed";
  configuration = { ...configuration, automaticCuration: false };
  await fire(); expect(starts).toBe(1);
  configuration = { ...configuration, automaticCuration: true };
  await host.pause(); await fire(); expect(starts).toBe(1);
  await host.resume();
  releaseStatus = () => {};
  const pending = fire();
  for (let count = 0; count < 10; count++) await Promise.resolve();
  const closing = host.close();
  await Promise.resolve(); expect(closes).toBe(0);
  releaseStatus(); await pending; await closing;
  expect(closes).toBe(1); expect(scheduled).toBeUndefined();
  host.startScheduling(); expect(scheduled).toBeUndefined();
});
