...
export function createHistoryCoordinator(store: ConversationHistoryStore, conversationId: string) {
  ...
  const mergeAssets = (left: Asset[], right: Asset[]) => [
    ...new Map([...left, ...right].map((asset) => [asset.key, asset])).values(),
  ];
  ...
  function failed() {
    error =
      "History is not synchronized. The last saved messages are retained; execution is not retried.";
  }
        ...
        for (const update of updates) {
          ...
          entries.push(
            HistoryEntrySchema.parse({
              ...content,
              ...(previous?.selections && !content.selections
                ? { selections: previous.selections }
                : {}),
              ...(previous?.resources && !content.resources
                ? { resources: previous.resources }
                : {}),
              ...(previous?.operationId && !content.operationId
  ...
  return {
    ...
    async write(entry: Omit<HistoryEntry, "position">) {
      pending.set(entry.id, {
        ...entry,
        assets: mergeAssets(pending.get(entry.id)?.assets ?? [], entry.assets),
      });
    ...
    async writeAsset(id: string, operationId: string, asset: Asset) {
      ...
      pending.set(
        id,
        previous
          ? { ...previous, assets: mergeAssets(previous.assets, [asset]) }
          : {
              id,
              operationId,
              role: "assistant",
    ...
    flush,
    async synchronize(
      reader: ConversationHistoryReader | undefined,
          ...
          try {
            ...
            if (!reader) {
              await store.commit(conversationId, {
                ...
                sync: {
                  sync: "unsupported",
                  message: "This provider does not expose conversation history.",
                },
              });
              return;
            }
            const batch = HistoryReadBatchSchema.parse(
              await reader.read(
                {
            ...
            const committed = await store.commit(conversationId, {
              ...
              checkpoints: batch.checkpoints.map((checkpoint) => ({
                ...checkpoint,
            ...
            if (direction === "latest" && batch.hasMore) {
              if (committed.revision === status.revision)
                throw new HistoryReadError("error", "History reconciliation made no progress.");
              more = true;
            }
            error = "";
          } catch (cause) {
            ...
            if (cause instanceof HistoryReadError) {
              try {
                ...
                const message = {
                  ...
                  error:
                    "History synchronization failed. Saved messages are retained; execution is not retried.",
                }[cause.status];
                await store.commit(conversationId, {
                  expectedRevision: status.revision,
                  sync: { sync: cause.status, message },
                });
                error = "";
              } catch {
                failed();
      ...
      await serial(async () => {
        try {
          ...
          await store.commit(conversationId, {
            ...
            sync: {
              sync: "unavailable",
              message: "Provider unavailable. Saved history is still readable.",
            },
          });
        } catch {
          failed();
