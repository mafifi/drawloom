...
const outcomes = new WeakMap<
  Span,
  "ok" | "error" | "denied" | "cancelled" | "unknown" | "timeout"
>();
export function observeOutcome(
  outcome: "ok" | "error" | "denied" | "cancelled" | "unknown" | "timeout",
) {
  const span = trace.getActiveSpan();
  if (span) {
    outcomes.set(span, outcome);
    span.setAttribute("drawloom.outcome", outcome);
  }
}
export function observeCache(hit: boolean, count?: number) {
  const span = trace.getActiveSpan();
  span?.setAttribute("drawloom.cache.hit", hit);
...
export async function observed<T>(
  ...
  return tracer.startActiveSpan(name, { attributes }, async (span) => {
    ...
    try {
      ...
      logs.getLogger("drawloom.desktop").emit({
        eventName: failed ? "operation.failed" : "operation.completed",
        severityNumber: failed ? SeverityNumber.ERROR : SeverityNumber.INFO,
        attributes: { "drawloom.outcome": outcome },
      });
      const meter = metrics.getMeter("drawloom.desktop");
      meter
        .createHistogram("operation.duration", { unit: "ms" })
        .record(performance.now() - start, {
          "drawloom.operation.name": name,
          "drawloom.outcome": outcome,
        });
      meter
        .createCounter("operation.count")
        .add(1, { "drawloom.operation.name": name, "drawloom.outcome": outcome });
      span.end();
    }
  });
}
...
export function observedRpc(rpc: RpcTransport): RpcTransport {
  return {
    ...
    request: (method, params) =>
      observed("agent.request", { "rpc.method": rpcMethods.has(method) ? method : "other" }, () =>
  ...
  return tools.map((tool) => ({
    ...
    execute: (input, execution) =>
      observed(
        "tool.execute",
        {
          "drawloom.invocation.id": execution.invocationId,
          "drawloom.operation.id": execution.operationId,
        },
        async () => tool.execute(input, execution),
      ),
  }));
}
export function observedToolGateway(
  gateway: ToolGateway,
  operations?: ReturnType<typeof createOperationTelemetry>,
): ToolGateway {
  const owners = new Map<ToolBinding, string>();
  return {
    ...gateway,
    bind(operationId) {
      const binding = gateway.bind(operationId);
      owners.set(binding, operationId);
      return binding;
    },
    revoke(binding) {
    ...
    invoke: (binding, name, args, signal) => {
      const run = () =>
        trace.getTracer("drawloom.desktop").startActiveSpan("tool.invoke", async (span) => {
          try {
...
export function createOperationTelemetry() {
  ...
  const end = (id: string, outcome: "ok" | "error" | "unknown" | "cancelled") => {
    for (const [key, value] of approvals)
      if (value.operationId === id) {
        value.span.setAttribute("drawloom.outcome", "unknown");
        value.span.end();
  ...
  return {
    begin(id: string) {
      ...
      const span = trace
        .getTracer("drawloom.desktop")
        .startSpan("agent.operation", { attributes: { "drawloom.operation.id": id } });
      running.set(id, { span, context: trace.setSpan(context.active(), span) });
    },
    run<T>(id: string, fn: () => T): T {
      return context.with(running.get(id)?.context ?? context.active(), fn);
    },
    end,
    signal(signal: AgentSessionSignal) {
      if (signal.kind === "approval.requested") {
        ...
        const span = trace
          .getTracer("drawloom.desktop")
          .startSpan("agent.approval.wait", {}, owner.context);
        ...
        const pending = approvals.get(signal.approvalId);
        pending?.span.end();
        approvals.delete(signal.approvalId);
    ...
    close() {
      for (const id of running.keys()) end(id, "unknown");
    },
  };
}
const methods: Record<string, string> = {
  ...
  readResource: "host.resource.read",
  readDiscoveredResource: "host.resource.read",
  openListedResource: "host.resource.read",
  resourcePage: "host.resource.read",
  importAssetStream: "host.asset.write",
  viewRequest: "mcp.request",
  viewInteraction: "host.command",
  inspectPackage: "host.package.inspect",
  packageAction: "host.package.activate",
...
export function instrumentApplication<T extends object>(app: T): T {
  return new Proxy(app, {
    get(target, key, receiver) {
      ...
      return name
        ? (...args: unknown[]) => observed(name, {}, async () => Reflect.apply(value, target, args))
        : value.bind(target);
    },
  });
}
export function observedAssets<T extends { read(key: string): Promise<Uint8Array> }>(assets: T): T {
  return {
    ...assets,
    read: (key: string) => observed("host.asset.read", {}, () => assets.read(key)),
  };
}
...
export async function observedHttp(
  request: Request,
  ...
  return context.with(parent, () =>
    trace.getTracer("drawloom.desktop").startActiveSpan(
      "http.request",
      {
