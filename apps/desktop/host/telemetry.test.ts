...
test("file delivery records transferred bytes without file paths or content", async () => {
  ...
  const response = await fileResponse(
    new Request("http://localhost/api/files?path=SECRET"),
    {
      size: 100,
      async *stream() {
    ...
    { mediaType: "video/mp4", immutable: false },
  );
...
test("direct MCP App calls receive host trace context in standard metadata, never arguments", async () => {
  ...
  const uri = "ui://synthetic/editor.html";
  let incoming: unknown;
  server.registerResource("editor", uri, {}, async () => ({
    contents: [{ uri, mimeType: "text/html;profile=mcp-app", text: "<p>Editor</p>" }],
  }));
  server.registerTool(
    "edit",
    { inputSchema: { text: z.string() }, _meta: { ui: { resourceUri: uri } } },
    async (args, extra) => {
      incoming = { args, metadata: extra._meta };
  ...
  try {
    await observed("host.command", {}, () =>
      app.callTool({
        name: "edit",
        arguments: { text: "SECRET" },
        _meta: { traceparent: "untrusted-app-parent" },
      }),
    );
    const span = exporter.getFinishedSpans().find((s) => s.name === "mcp.request")!;
    expect(incoming).toEqual({
      args: { text: "SECRET" },
      metadata: { traceparent: `00-${span.spanContext().traceId}-${span.spanContext().spanId}-01` },
    });
    expect(span.parentSpanContext?.spanId).toBe(
      exporter
        .getFinishedSpans()
        .find((s) => s.name === "host.command")!
        .spanContext().spanId,
    );
  } finally {
    await app.close();
...
test("concurrent observed calls retain their own child spans and never capture argument or error content", async () => {
  ...
  for (const call of calls)
    expect(
      roots.some(
        (s) =>
          s.spanContext().spanId === call.parentSpanContext?.spanId &&
          s.spanContext().traceId === call.spanContext().traceId,
      ),
    ).toBe(true);
  expect(JSON.stringify(spans.map((s) => [s.attributes, s.events, s.status]))).not.toContain(
    "SECRET_PROMPT",
  );
});
...
test("a denied gateway call creates no execution span and never invokes its handler", async () => {
  ...
  expect(spans.find((s) => s.name === "tool.invoke")?.attributes["drawloom.outcome"]).toBe(
    "denied",
  );
...
test("application instrumentation preserves method receiver and returned state", async () => {
  ...
  const app = {
    ...
    viewSession() {
      return { mountId: "synchronous" };
    },
  };
  const wrapped = instrumentApplication(app);
  expect(await wrapped.discover()).toBe(3);
  expect(await wrapped.snapshot()).toBe(3);
  expect(wrapped.viewSession()).toEqual({ mountId: "synchronous" });
  await observedHttp(new Request("http://localhost/api/history/changes"), async () => {
    await wrapped.historyChanges();
    return new Response(null, { status: 204 });
  });
  await observedHttp(new Request("http://localhost/api/knowledge?query=SECRET"), async () => {
    await wrapped.knowledgeCommand();
    return new Response(null, { status: 204 });
  });
  const spans = exporter.getFinishedSpans();
  expect(spans.map((s) => s.name)).toEqual([
    "host.discovery",
    "host.knowledge.command",
    "http.request",
  ]);
...
test("the real synthetic desktop exposes discovery and history boundaries without content", async () => {
  ...
  try {
    ...
    expect(names).toContain("host.discovery");
    expect(names).toContain("host.history.page");
  } finally {
    await app.close();
  }
});
...
test("HTTP propagation keeps only W3C trace identity and sanitizes routes", async () => {
  ...
  const response = await observedHttp(
    new Request("http://localhost/api/discovery?conversationId=SECRET", {
      headers: { traceparent: `00-${traceId}-${parentId}-01`, baggage: "secret=SECRET" },
    }),
    () => observed("host.discovery", {}, async () => new Response(null, { status: 204 })),
  );
  ...
  expect(spans.find((s) => s.name === "host.discovery")?.parentSpanContext?.spanId).toBe(
...
test("standard MCP metadata continues the invocation without changing tool arguments", async () => {
  ...
  const active = await activatePackage(
    ...
    { dataRoot: "/unused", installationId: "test", selectedServers: ["remote"] },
  );
  try {
    await observed("tool.execute", {}, async () => {
      await active.servers
        .get("remote")!
        .client.callTool({ name: "plain", arguments: { text: "SECRET" } });
    });
    const message = seen.find((m) => m.method === "tools/call")!;
    expect(message.params?.arguments).toEqual({ text: "SECRET" });
    const call = exporter
      .getFinishedSpans()
      .find((s) => s.name === "mcp.request" && s.attributes["rpc.method"] === "tools/call")!;
...
test("approval waits and detached gateway work retain operation ownership and close explicitly", async () => {
  ...
  operations.signal({
    ...
    request: {
      ...
      approvalId: "one",
      summary: "SECRET",
      options: [{ optionId: "yes", label: "SECRET" }],
    },
  });
  await operations.run(op, () => observed("tool.execute", {}, async () => {}));
  operations.signal({ kind: "approval.resolved", approvalId: "one" });
  operations.end(op, "ok");
  operations.close();
  const spans = exporter.getFinishedSpans();
  const operation = spans.find((s) => s.name === "agent.operation")!;
  expect(operation).toBeDefined();
  for (const name of ["tool.execute", "agent.approval.wait"])
    expect(spans.find((s) => s.name === name)?.parentSpanContext?.spanId).toBe(
