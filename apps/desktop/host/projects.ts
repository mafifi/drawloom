...
export async function bindProjectDirectory(
  ...
  if (!isAbsolute(directory))
    throw new ProjectDirectoryError("Choose an absolute project directory");
  const [canonical, protectedRoot] = await Promise.all([
    realpath(directory),
    realpath(internal),
  ]).catch(() => {
    throw new ProjectDirectoryError(
      "The project folder is unavailable. Reconnect it or choose an existing folder. Saved history remains available.",
    );
  });
  if (
...
export async function verifyProjectDirectory(
  ...
  if (
    ...
    throw new ProjectDirectoryError(
      "The project directory has changed. Restore the original folder or add the replacement as a new project.",
    );
