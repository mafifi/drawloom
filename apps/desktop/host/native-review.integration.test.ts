...
for (const mode of ["approve", "deny", "delegated", "revoked"] as const) {
  test.skipIf(process.env.DRAWLOOM_LIVE_NATIVE_REVIEW !== "1")(
    `live native ${mode} gates the existing MCP gateway`,
    async () => {
      const root = await mkdtemp(join(tmpdir(), "drawloom-native-review-"));
      let calls = 0,
        grant = true,
        text = "Original synthetic passage";
      ...
      const gateway = createLocalToolGateway({
        tools: [
          defineTool({
            name: "text.revise",
            description:
              "Replace an in-memory synthetic test passage. No external effects or business acceptance.",
      ...
      const mcp = await createMcpToolServer({
        ...
        invoke: (meta, name, args, signal) => bridge.call(meta, name, args, signal),
      });
      const driver = createCodexDriver({
          ...
          rpc.request = async (method, params) => {
            ...
            if (method === "thread/start") {
              ...
              const servers = z
                ...
                .parse(status);
              expect(
                Object.keys(servers.data.find((server) => server.name === "drawloom")?.tools ?? {}),
              ).toContain("text.revise");
            }
            return result;
          };
          return rpc;
        ...
        onTurnAccepted: (thread, turn, operation) => {
          binding = gateway.bind(operation);
      ...
      const opened = await driver.openSession({
        ...
        context: {
          text: "Use the drawloom MCP text.revise tool for this test. Tool discovery/search is allowed if necessary to find it. Do not use filesystem, shell or unrelated tools. A denial must not be retried.",
        },
        tools: gateway.exposure,
      });
      if (opened.status !== "ok") {
        await mcp.close();
        const finished = await finishDisposableCodexThread(opened.failure, {
        ...
        if (finished.cleanup.kind === "failed")
          throw new AggregateError(
            [Error(opened.failure.message), Error(finished.cleanup.reason)],
            "Native review failed and its disposable thread cleanup also failed",
          );
      ...
      const pump = (async () => {
        for await (const event of session.signals()) {
          if (event.kind === "approval.requested") {
            approvals++;
            observed.push("human-request");
            expect(calls).toBe(0);
            if (mode === "delegated")
              throw Error("Delegated review unexpectedly requested human approval");
            if (mode === "revoked") grant = false;
            const choice = event.request.options.find(
              (option) => option.label === (mode === "deny" ? "Deny" : "Approve once"),
            );
          ...
          if (event.kind === "provider.observation" && event.name === "approval-review")
            observed.push(event.summary ?? "review");
      ...
      try {
        expect(
          (
            await session.execute({
              operationId: "edit",
              reviewer: mode === "delegated" ? "delegated" : "human",
              text: 'Discover the drawloom MCP tool text.revise if needed, then call it exactly once with {"text":"Revised synthetic passage"}. This edits only a disposable in-memory test value and is authorised. No unrelated tools. If denied, stop without retrying. Then briefly report the result.',
            })
          ).status,
        ).toBe("ok");
        await Promise.race([
          ...
          pump.then(() => {
            throw Error("Stream closed before completion");
          }),
          new Promise<never>((_, reject) => {
            timer = setTimeout(() => reject(Error("Native review did not finish")), 150000);
          }),
        ]);
        console.log(JSON.stringify({ mode, calls, approvals, observed }));
        expect(observed.at(-1)).toBe("operation.completed");
        if (mode === "approve" || mode === "delegated") {
          expect(calls).toBe(1);
          expect(text).toBe("Revised synthetic passage");
        } else {
          expect(calls).toBe(0);
          expect(text).toBe("Original synthetic passage");
        }
        if (mode === "delegated") {
          expect(approvals).toBe(0);
          const reviewed = observed.findIndex((value) =>
            value.startsWith("Automatic review approved"),
          );
          expect(reviewed).toBeGreaterThanOrEqual(0);
          expect(reviewed).toBeLessThan(observed.indexOf("handler"));
        ...
        if (writerClosed) {
          const finished = await finishDisposableCodexThread(
            { mode, calls, approvals, observed },
            {
              threadId: ownedThreadId,
              connect: async () => createStdioTransport({ ...codexCommand(), cwd: root }),
            },
          ...
          if (ownedThreadId && finished.cleanup.kind !== "archived")
            cleanupFailure ??= Error(
              finished.cleanup.kind === "failed"
                ? finished.cleanup.reason
                : "Owned native thread was not archived",
            );
        } else if (ownedThreadId) {
          cleanupOutcome = {
            kind: "failed",
            threadId: ownedThreadId,
            reason: "Native review writer closure was not confirmed; archive was not attempted",
          };
          cleanupFailure ??= Error(
            "Native review writer closure was not confirmed; archive was not attempted",
          );
        }
        console.log(
          JSON.stringify({
      ...
      if (primaryFailure && cleanupFailure)
        throw new AggregateError(
          ...
          "Native review and disposable-thread cleanup both failed",
        );
