...
import {
  createTestDesktopApplication as createDesktopApplication,
  addTestProject,
} from "./test-project.fixture.js";
...
test("legacy conversations default to human review without changing identity or provider", () => {
  ...
  ).toMatchObject({ id: "old", provider: "codex", reviewer: "human" });
});
test("native MCP configuration reviews every mutating or unclassified invocation", () => {
  const config = composition.mcpReviewConfiguration({
    id: "exposure",
    tools: [
      {
        name: "text.read",
  ...
  expect(config).toEqual({
    ...
    tools: {
      "text.read": { approval_mode: "approve" },
      "text.edit": { approval_mode: "prompt" },
      "text.unknown": { approval_mode: "prompt" },
    },
  });
});
test("conversation review mode persists separately and unsupported delegated selection does not change it", async () => {
  ...
  try {
    ...
    await app.command({
      ...
      projectId: selected.selectedProjectId!,
    });
    await expect(
      app.command({ kind: "set_reviewer", conversationId: "local", reviewer: "delegated" }),
    ).rejects.toThrow("review");
    expect((await app.snapshot()).conversations.find((c) => c.id === "local")?.reviewer).toBe(
      "human",
    );
...
test("accepted execution locks reviewer selection before delayed started delivery", async () => {
  ...
  try {
    ...
    const accepted = await app.command({
      kind: "send",
      conversationId,
      text: "Independent synthetic fixture",
      attachmentKeys: [],
      contextArtifactIds: [],
    });
    expect(accepted.activeOperation).toBeTruthy();
    await expect(
      app.command({ kind: "set_reviewer", conversationId, reviewer: "human" }),
    ).rejects.toThrow("idle");
    deliver();
    finish();
    for (let attempt = 0; attempt < 100 && (await app.snapshot()).activeOperation; attempt++)
...
test("a missing project directory blocks new work but not stopping an existing operation", async () => {
  ...
  try {
    ...
    working = state.projects[0]!.directory;
    const started = await app.command({
      kind: "send",
      conversationId: state.selectedId,
      text: "Synthetic delayed task",
      attachmentKeys: [],
      contextArtifactIds: [],
    });
    ...
    await app?.close();
    replacement.mockRestore();
    if (working) await rename(working + "-offline", working).catch(() => {});
    await rm(root, { recursive: true, force: true });
  }
});
