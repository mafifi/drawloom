...
test("one result cannot multiply the inline capture allowance across content blocks", async () => {
  ...
  const collector = createResourceContent({
    assets: assetsWith({
      put: async (bytes, mediaType) => {
        ...
        return { key: `asset-${writes}`, size: bytes.length, mediaType };
      },
      read: async () => new Uint8Array(),
    }),
    existing: async () => undefined,
  ...
  const result = await collector.capture({
    ...
    content: Array.from({ length: 3 }, (_, i) => ({
      type: "resource",
      resource: { uri: `doc://${i}`, mimeType: "text/plain", text: "x".repeat(8 * 1024 * 1024) },
    })),
  });
...
test("capture storage failure is explicit and never advertises a saved result", async () => {
  ...
  await expect(
    collector.capture({
      ...
      source: "test",
      content: [
        { type: "resource", resource: { uri: "doc://a", text: "hello", mimeType: "text/plain" } },
      ],
    }),
...
test("provider-owned opaque receipts make returned links readable without revealing their native mapping", async () => {
  ...
  const entry = await collector.capture({
    id: "receipt",
    source: "codex:docs",
    content: [{ type: "resource_link", uri: "doc://returned", name: "Returned" }],
    resourceSelections: { "doc://returned": { id: "opaque-id", revision: "opaque-revision" } },
  });
  expect(entry.resources?.[0]).toMatchObject({
    status: "readable",
    retrieval: { id: "opaque-id", revision: "opaque-revision" },
  });
});
...
test("standard resources retain provenance and capture inline bytes once, without resolving links", async () => {
  ...
  const collector = createResourceContent({
    assets: assetsWith({
      put: async (bytes, mediaType) => {
        writes++;
        return { key: `key-${writes}`, size: bytes.length, mediaType };
      },
      read: async () => new Uint8Array(),
    }),
  ...
  const input = {
    ...
    content: [
      {
        type: "resource",
        resource: { uri: "document://a", mimeType: "text/plain", text: "A source" },
      },
...
test("arbitrary structured output is not a file or skill; invalid links never become readable", async () => {
  ...
  const entry = await collector.capture({
    ...
    source: "x",
    content: [
      { type: "text", text: '{"skill":"trusted","file":"/etc/passwd"}' },
