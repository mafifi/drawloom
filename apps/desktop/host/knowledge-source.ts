...
export function createInstalledGitKnowledgeFeed(options: {
  ...
  const candidates = [...options.presentation].filter(
    ([, value]) =>
      value.available && (value.name === "git.changes" || value.name === "git.acknowledge"),
  );
  const pairs = candidates.flatMap(([changesName, changes]) =>
    changes.name !== "git.changes"
      ? []
      : candidates
          .filter(
            ([, acknowledgement]) =>
              acknowledgement.name === "git.acknowledge" &&
              acknowledgement.ownerId === changes.ownerId &&
              acknowledgement.origin === changes.origin,
          )
  ...
  const sourceId = `git:${createHash("sha256")
    .update(JSON.stringify([options.projectId, pair.identity.ownerId, pair.identity.origin]))
    .digest("hex")
    .slice(0, 48)}`;
  async function invoke(tool: ToolDefinition, value: unknown) {
    ...
    const output = await tool.execute(input, {
      ...
      operationId: `knowledge-source:${sourceId}`,
  ...
  return {
    ...
    async acknowledge(token) {
      await invoke(acknowledge, { token });
    },
  };
}
