...
export function mediaPolicy(raw: unknown): string[] {
  ...
  for (const [key, value] of Object.entries(policy.csp ?? {}))
    if (key !== "resourceDomains" && (!Array.isArray(value) || value.length))
      throw Error("MCP connection, frame and base permissions are unsupported");
  const domains = z
    .array(ResourceOriginSchema)
    .max(32)
    .parse(policy.csp?.resourceDomains ?? []);
