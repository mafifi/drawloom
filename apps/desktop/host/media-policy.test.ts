...
test("a plugin media result declares one shared origin, survives restart and never stores signed URLs", async () => {
  ...
  await policy.capture("package:motion", [
    {
      type: "resource_link",
      name: "Candidate",
      uri: "https://media.example/video.mp4?signature=PRIVATE",
      mimeType: "video/mp4",
    },
  ]);
  const snapshot = policy.snapshot();
  expect(snapshot.sources).toEqual([
    { origin: "https://media.example", sources: ["package:motion"] },
  ]);
  ...
  await policy.capture("package:motion", [
    {
      type: "resource_link",
      name: "New candidate",
      uri: "https://media.example/other.mp4",
      mimeType: "video/mp4",
    },
  ]);
...
test("only explicit media declarations register sources; invalid URLs and prose do not", async () => {
  const policy = await createMediaPolicy(
    createNodeJsonStore(await mkdtemp(join(tmpdir(), "drawloom-media-validation-"))),
  );
  await policy.capture("package:editor", [
    { type: "text", text: "https://prose.example/a.png" },
    {
      type: "resource_link",
      name: "HTML",
      uri: "https://active.example/a.html",
      mimeType: "text/html",
    },
    {
      type: "resource_link",
      name: "Credentials",
      uri: "https://user:secret@credentials.example/a.png",
      mimeType: "image/png",
    },
    {
      type: "resource_link",
  ...
  await policy.capture("package:music", [
    {
      type: "resource",
      resource: { uri: "https://music.example/a.wav", mimeType: "audio/wav", text: "" },
    },
  ]);
  expect(policy.snapshot().sources).toEqual([
    { origin: "https://music.example", sources: ["package:music"] },
  ]);
});
...
test("concurrent declarations retain both sources and failed persistence does not publish permission", async () => {
  ...
  await Promise.all([
    policy.declare("one", ["https://one.example"]),
    policy.declare("two", ["https://two.example"]),
  ]);
  expect(policy.snapshot().sources.map((s) => s.origin)).toEqual([
    "https://one.example",
    "https://two.example",
  ]);
  const failed = await createMediaPolicy({
    get: async () => undefined,
    set: async () => {
...
test("cumulative declarations do not hit the bounded single-delivery limit", async () => {
  ...
  const policy = await createMediaPolicy(store);
  await policy.declare(
    "batch",
    Array.from({ length: 256 }, (_, i) => `https://media-${i}.example`),
  );
  await policy.capture("later", [
    {
      type: "resource_link",
      name: "Later",
      uri: "https://later.example/a.png",
      mimeType: "image/png",
    },
  ]);
