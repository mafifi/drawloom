...
function localOrchestrator(registry: Registry) {
  ...
  const orchestrator: Orchestrator = {
    ...
    async get(runId) {
      ...
      if (!value) throw Error("run unavailable");
...
test("trusted installed evaluation executes through attached authority-wrapped handlers and standard MCP App tools", async () => {
  ...
  const assessment: EvaluationAssessmentProvider = {
    async assess(scorer, args, context) {
      observedOwners.push(authority.current());
  ...
  try {
    ...
    const workflowUrl = pathToFileURL(
      join(process.cwd(), "packages/evaluation/evaluation-orchestration/dist/index.js"),
    ).href;
    await writeFile(
      join(packageRoot, "plugin.json"),
      JSON.stringify({
        $schema: "https://agent-plugins.org/schemas/1.0.0/plugin.schema.json",
        name: "installed-evaluation",
        extensions: {
          "org.drawloom": {
            version: 1,
            ...
            workbenches: [
              {
                id: "evaluation",
                title: "Evaluation",
                openingTool: { server: "evaluation", tool: "open" },
              },
            ],
          },
        },
    ...
    const loaded = await loadInstalledPackages({
      ...
      host: { store: createNodeJsonStore(join(root, "state")), assets: unusedAssets() },
      async prepareWorkflows(_installation, inventory): Promise<InstalledWorkflowRegistration> {
        const registry = (
          await import(
            pathToFileURL(join(inventory.root, inventory.drawloom!.workflows!.entrypoint)).href
          )
        ).default as Registry;
    ...
    try {
      ...
      const started = await app!.callTool({
        name: "start",
        arguments: { requestId: "installed-request" },
      });
      ...
      const page = (
        await app!.callTool({ name: "results", arguments: { runId: start.evaluationRunId } })
      ).structuredContent as { items: Array<{ findingCount: number; status: string }> };
      ...
      expect(observedOwners).toEqual([
        {
          ...
          stepId: expect.stringContaining("scorer"),
