...
test("real provider-length identities retain evidence on local disk", async () => {
  ...
  try {
    const authority = createWorkflowAuthority(),
      runId = "a".repeat(64) + "/" + crypto.randomUUID();
    const ownerScope = createWorkflowToolScope({
      authority,
      projectId: "project",
      installationId: "owner",
      workbenchIds: ["test"],
      grants: new Map([["test", new Set(["edit"])]]),
      refreshGrants: async () => {},
      store: createNodeJsonStore(root),
    });
    const handler = authority.wrap(
      { projectId: "project", installationId: "owner" },
      [
        {
          id: "task",
          version: "1",
    ...
    await expect(
      handler[0]!.run(
        {},
        {
          runId,
          stepId: runId + "/D1-D2",
          attemptId: runId + "/D1-D2/attempt/1",
          attempt: 1,
          taskVersion: "1",
...
test("detached tasks require all declaring grants, refresh each call and retain scoped evidence without a conversation", async () => {
  ...
  const invoke = () =>
    gateway.invoke(gateway.bind("run"), "edit", {}, new AbortController().signal);
  const handlers = authority.wrap(
    { projectId: "project", installationId: "installation" },
    [{ id: "task", version: "1", run: invoke }],
    scope.refresh,
  );
  const context = {
    runId: "run",
  ...
  expect(
    ((await handlers[0]!.run({}, context)) as { outcome: { status: string } }).outcome.status,
  ).toBe("failed");
  expect(calls).toBe(0);
  ...
  expect([...data.keys()]).toEqual([
    "tool-evidence:" + workflowEvidenceKey("installation", "run", "step", "attempt"),
  ]);
  expect(JSON.stringify([...data.values()])).toContain('"operationId":"run"');
  await invoke();
  expect(calls).toBe(1);
  const other = authority.wrap(
    { projectId: "other", installationId: "installation" },
    [{ id: "task", version: "1", run: invoke }],
    async () => {},
  );
  await other[0]!.run({}, context);
  expect(calls).toBe(1);
  const revoked = authority.wrap(
    { projectId: "project", installationId: "installation" },
    [
      {
        id: "task",
  ...
  await revoked[0]!.run({}, context);
  expect(calls).toBe(1);
  grants.set("b", new Set(["edit"]));
  const abort = new AbortController();
  const cancelled = authority.wrap(
    { projectId: "project", installationId: "installation" },
    [
      {
        id: "task",
  ...
  expect(await cancelled[0]!.run({}, { ...context, signal: abort.signal })).toMatchObject({
