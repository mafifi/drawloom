...
test("a producer declaration and later media result are shared with another workbench and remote viewer", async () => {
  ...
  for (const id of ["producer", "consumer"]) {
    ...
    await installs.configure(installation, {
      ...
      configuration: {
        id,
        ...(id === "producer" ? { origin: "https://initial.example" } : {}),
        url: "https://returned.example/video.mp4?signature=PRIVATE",
      },
    });
  }
  ...
  try {
    ...
    expect((await app.viewPresentation(consumer)).resourceDomains).toContain(
      "https://initial.example",
    );
    const before = initial.mediaPolicy.revision;
    const producer = await app.command({
      kind: "create_conversation",
      workbenchId: "producer",
      provider: "synthetic",
    });
    await app.viewRequest({
      conversationId: producer.selectedId,
      viewId: "producer.view",
      request: { name: "result", arguments: {} },
    });
    const changed = await app.snapshot();
    expect(changed.mediaPolicy.revision).not.toBe(before);
    ...
    const params = new URLSearchParams({
      conversationId: producer.selectedId,
      entryId: entry.id,
      resourceId: entry.resources![0]!.id,
    });
    ...
    expect(preview.headers.get("content-security-policy")).toContain(
      "media-src https://returned.example",
    );
    ...
    expect(
      (await fetch(host.origin + "/api/remote-media?" + params, { headers: { cookie } })).status,
    ).toBe(404);
    await app.command({ kind: "select_conversation", conversationId: consumer.conversationId });
    await app.viewSession({ ...consumer, action: "open" });
    const shared = await fetch(
      host.origin + "/api/views/consumer.view?conversationId=" + consumer.conversationId,
      { headers: { cookie } },
    );
    ...
    expect(csp.split(";").find((p) => p.includes("style-src"))).not.toContain(
      "https://returned.example",
    );
  } finally {
    await host.close();
  }
  ...
  try {
    expect((await reopened.snapshot()).mediaPolicy.sources.map((s) => s.origin)).toEqual([
      "https://initial.example",
      "https://returned.example",
    ]);
  } finally {
