...
const context = (runId: string): TaskContext => ({
  runId,
  stepId: "render",
  attemptId: runId + ":1",
  attempt: 1,
...
test("task authority is installation/project scoped, concurrent and expires after the handler", async () => {
  ...
  const handler: RegisteredTaskHandler = {
    id: "render",
    version: "1",
    async run() {
  ...
  expect(
    await Promise.all([a[0]!.run({}, context("first")), b[0]!.run({}, context("second"))]),
    ...
    [
      {
        ...
        recover() {
          expect(authority.current()?.recovering).toBe(true);
