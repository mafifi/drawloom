...
test("state feed sends a baseline, no unchanged body and only changed sections", () => {
  ...
  expect(
    feed.read({ selectedId: "a", active: "op", operator: { title: "one" } }, first!.token),
  ).toBeUndefined();
  const delta = feed.read({ selectedId: "a", operator: { title: "two" } }, first!.token);
  expect(delta).toMatchObject({
    kind: "patch",
    sections: { operator: { title: "two" } },
    removed: ["active"],
  });
  expect(delta?.sections).not.toHaveProperty("selectedId");
  expect(
    feed.read({ selectedId: "a", operator: { title: "two" } }, "other-generation:2")?.kind,
  ).toBe("snapshot");
});
