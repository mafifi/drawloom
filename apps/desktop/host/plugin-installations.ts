...
import {
  ...
  ElicitationDisabledServersSchema,
...
export const InstallationSchema = z
  .strictObject({
    ...
    name: z.string().min(1),
    enabled: z.boolean(),
    trustedBackend: z.boolean(),
    servers: z.array(z.string()),
    configuration: z.record(z.string(), JsonValueSchema).default({}),
  })
  .refine(
    (value) => value.elicitationDisabledServers.every((name) => value.servers.includes(name)),
...
export async function createInstallationStore(store: JsonStore) {
  ...
  return {
    ...
    async inspect(root: string): Promise<PackageInventory> {
      return inspectPackage(root);
    },
    async add(root: string) {
      ...
      await change((next) => {
        ...
        next.installations.push({
          ...
          servers: inventory.servers.filter((s) => s.config.type !== "sse").map((s) => s.name),
          configuration: {},
        });
      });
      return id;
    ...
    async configure(
      id: string,
      input: Pick<Installation, "enabled" | "trustedBackend" | "servers" | "configuration"> & {
        approvedResourceOrigins?: string[] | undefined;
        elicitationDisabledServers?: string[] | undefined;
      },
    ) {
      ...
      if (
        parsed.servers.some(
          (name) => !inventory.servers.some((s) => s.name === name && s.config.type !== "sse"),
        )
      )
        throw Error("Unknown or unsupported server");
      await change((next) => {
        next.installations = next.installations.map((i) => (i.id === id ? parsed : i));
      });
    },
    pendingRestart(id: string) {
      return (
        JSON.stringify(state.installations.find((i) => i.id === id)) !==
        JSON.stringify(startup.find((i) => i.id === id))
      );
    },
  };
}
