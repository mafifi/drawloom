...
test("reader failures never copy arbitrary provider text into public status", async () => {
  ...
  try {
    ...
    await writer.synchronize({
      namespace: "test",
      read: async () => {
        throw new HistoryReadError("unavailable", "SECRET native payload");
      },
    });
  ...
  try {
    ...
    await writer.synchronize({
      ...
      read: async (context, options) => {
        expect(options.direction).toBe("latest");
        calls++;
        const previous = await context.checkpoint("scan");
        return {
          entries: [],
          checkpoints: [{ key: "scan", value: calls }],
          hasOlder: true,
          hasMore: previous === undefined,
        };
      },
    });
    expect(calls).toBe(2);
...
test("a non-progressing latest continuation stops with a safe error", async () => {
  ...
  try {
    ...
    await writer.synchronize({
      ...
      read: async () => {
        calls++;
        return { entries: [], checkpoints: [], hasOlder: false, hasMore: true };
      },
    });
    expect(calls).toBe(1);
    expect((await store.status("conversation")).sync).toBe("error");
    await writer.close();
  } finally {
    await store.close();
    await rm(root, { recursive: true, force: true });
...
test("storage failure stays a history error, never invokes execution or advances checkpoints", async () => {
  ...
  try {
    let commits = 0;
    const writer = createHistoryCoordinator(
      {
        ...store,
        commit: async () => {
          commits++;
...
test("text and artifact delivery merge in either order", async () => {
  ...
  try {
    ...
    await writer.writeAsset("one", "operation", asset);
    await text("one");
    await text("two");
    await writer.writeAsset("two", "operation", asset);
