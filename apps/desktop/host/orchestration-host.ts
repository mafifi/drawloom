...
import {
  createOrchestrationPresentation,
...
type Entry = {
  title: string;
  registration?: LocalTemporalRegistration;
  readiness: OrchestrationReadiness;
};
const key = (projectId: string, installationId: string) =>
  JSON.stringify([projectId, installationId]);
const projectUnavailable: OrchestrationReadiness = {
  status: "unavailable",
  code: "project_unavailable",
  message:
    "The owning project or installed plugin is unavailable. Saved work has not been removed.",
};
function unavailable(error: unknown): OrchestrationReadiness {
  const message = error instanceof Error ? error.message : "";
  if (/ENOENT|not found|executable|version|spawn/i.test(message))
    return {
  ...
  if (/bundle|changed|unfinished/i.test(message))
    return {
      status: "unavailable",
      code: "workflow_code_changed",
      message:
        "Workflow code changed while saved work still needs it. Restore the installed package before recovery.",
    };
  return {
    status: "unavailable",
    code: "local_runtime_unavailable",
    message:
      "Local workflow startup failed. Check the runtime installation and saved orchestration data; nothing has been reset.",
  };
}
...
export function createOrchestrationHost(options: {
  ...
  async function guardInstallation(installationId: string) {
    if ((await persisted()) && (await (await manager()).hasUnfinishedInstallation(installationId)))
      throw new WorkflowControlError(
        "This plugin has unfinished workflows. Finish or cancel them before changing its configuration.",
      );
  }
  async function persisted() {
    if (pending) return true;
    try {
      await access(join(options.dataDirectory, "orchestration"));
      return true;
    } catch (error) {
  ...
  const presentation = createOrchestrationPresentation(async (raw) => {
    ...
    if (closed || !entry?.registration || entry.readiness.status !== "ready")
      throw new WorkflowControlError(entry?.readiness.message ?? "Workflow owner unavailable");
  ...
  return {
    ...
    async prepare(
      ...
      wrap: (handlers: readonly RegisteredTaskHandler[]) => readonly RegisteredTaskHandler[],
    ): Promise<InstalledWorkflowRegistration> {
      if (closed || !installation.trustedBackend || !inventory.drawloom?.workflows)
        throw Error("Trusted workflow package required");
      const entry: Entry = {
        title: inventory.name,
        readiness: {
          status: "unavailable",
          code: "starting",
          message: "Preparing local workflows.",
        },
      };
      ...
      try {
        entry.registration = await serialize(installation.id, async () => {
          ...
          const checked = await executableExtensionPath(
            inventory.root,
            inventory.drawloom!.workflows!.entrypoint,
          );
          const registration = await (await manager()).prepare({
            projectId,
            installationId: installation.id,
            packageDirectory: checked.root,
            entrypoint: inventory.drawloom!.workflows!.entrypoint,
          });
          return {
            ...registration,
            orchestrator: {
              ...registration.orchestrator,
              start: (identity, workflow, input) =>
                serialize(installation.id, async () => {
                  requireCurrent(installation.id);
        ...
        entry.readiness = unavailable(error);
      }
      return {
        capabilities: {
          ...(entry.registration ? { orchestration: entry.registration.orchestrator } : {}),
          orchestrationReadiness: async () =>
            entry.registration && entry.readiness.status === "ready"
              ? entry.registration.readiness()
              : entry.readiness,
        },
        async attach(handlers) {
          if (!entry.registration) return;
          try {
            await entry.registration.attach(wrap(handlers));
            entry.readiness = entry.registration.readiness();
          } catch (error) {
            entry.readiness = unavailable(error);
            throw error;
          }
        ...
        async close() {
          ...
          entry.readiness = {
            status: "unavailable",
            code: "stopped",
            message: "Local execution is stopped.",
          };
        },
      };
    },
    async owners(projectId: string) {
      ...
      try {
        ...
        if (![...entries.keys()].some((id) => (JSON.parse(id) as string[])[0] === projectId))
          throw error;
        missing = true;
      }
      return WorkflowOwnersSchema.parse(
