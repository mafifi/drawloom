...
import {
  MediaPolicySnapshotSchema,
  type MediaPolicySnapshot,
} from "../src/lib/media-policy-protocol.js";
...
export function remoteMediaUrl(value: string, mediaType: string | undefined): URL | undefined {
  ...
  try {
    ...
    if (
      ...
      url.password ||
      value.length > 4096 ||
      !ResourceOriginSchema.safeParse(url.origin).success
    )
      return;
    return url;
  } catch {
...
export async function createMediaPolicy(store: JsonStore, configured: readonly string[] = []) {
  ...
  const snapshot = (): MediaPolicySnapshot => ({
    revision: createHash("sha256")
      .update(JSON.stringify(sources.map((s) => s.origin)))
      .digest("hex"),
  ...
  async function declare(source: string, values: readonly string[]) {
    ...
    const next = queue.then(async () => {
      ...
      for (const origin of origins) {
        ...
        owners.add(owner);
        merged.set(origin, owners);
      }
      const updated = [...merged]
        .sort(([a], [b]) => a.localeCompare(b))
        .map(([origin, owners]) => ({ origin, sources: [...owners].sort() }));
  ...
  return {
    ...
    allows(value: string) {
      try {
        const url = new URL(value);
        return !url.username && !url.password && sources.some((s) => s.origin === url.origin);
      } catch {
        return false;
      }
    },
