...
test("10,000-entry public history uses bounded pages, no-body unchanged polls and single-record updates", async () => {
  ...
  try {
    for (let start = 0; start < 10000; start += 200) {
      const entries = Array.from({ length: 200 }, (_, offset) => {
        const i = start + offset,
          text =
            `${i} ${i % 50 === 0 ? "searchable-marker " : ""}` +
            "Public synthetic history. ".repeat(170);
      ...
      await store.commit(id, { expectedRevision: (await store.status(id)).revision, entries });
      sample();
    }
    ...
    do {
      ...
      searchCount += result.items.length;
      searchCursor = result.cursor;
    } while (searchCursor);
    expect(searchCount).toBe(200);
    await store.close();
    store = createSqliteConversationHistory(join(root, "history.sqlite"));
    expect((await store.search({ query: "9999", conversationIds: [id] })).items[0]?.entryId).toBe(
      "public-9999",
    );
    const indexed = (await store.get(id, "public-9950"))!;
    await store.commit(id, {
      expectedRevision: (await store.status(id)).revision,
      entries: [{ ...indexed, text: "Restart-safe unique-search-update" }],
    });
    expect(
      (await store.search({ query: "searchable-marker", conversationIds: [id], limit: 100 })).items,
    ).toHaveLength(100);
    expect(
    ...
    const get = (path: string, extra: Record<string, string> = {}) =>
      fetch(server!.origin + path, { headers: { cookie, ...extra } });
    ...
    const initialBody = await initial.text(),
      page = HistoryPageSchema.parse(JSON.parse(initialBody));
    const initialMs = performance.now() - start;
    sample();
    expect(page.entries.length).toBe(50);
    expect(page.entries[0]?.id).toBe("public-9950");
    const noChanges = await get(
      "/api/history/changes?" +
        new URLSearchParams({ conversationId: id, after: page.changeCursor }),
      { "If-None-Match": initial.headers.get("etag")! },
    );
    expect(noChanges.status).toBe(204);
    expect((await noChanges.arrayBuffer()).byteLength).toBe(0);
    const olderStart = performance.now();
    const olderResponse = await get(
      "/api/history?" + new URLSearchParams({ conversationId: id, before: page.olderCursor! }),
    );
    const olderBody = await olderResponse.text(),
      older = HistoryPageSchema.parse(JSON.parse(olderBody));
    const olderMs = performance.now() - olderStart;
    expect(older.entries.length).toBe(50);
    expect(older.entries[0]?.id).toBe("public-9900");
    const previous = page.entries.at(-1)!;
    await store.commit(id, {
      expectedRevision: (await store.status(id)).revision,
      entries: [{ ...previous, text: "One revised message" }],
    });
    ...
    const changedBody = await changedResponse.text(),
      changed = HistoryChangesSchema.parse(JSON.parse(changedBody));
    expect(changed.entries).toHaveLength(1);
    expect(changed.entries[0]?.id).toBe(previous.id);
    ...
    console.log(
      "HISTORY_MEASUREMENT " +
        JSON.stringify({
          entries: 10000,
          sourceBytes,
          initialEntries: page.entries.length,
          initialBytes: Buffer.byteLength(initialBody),
          initialMs,
          olderEntries: older.entries.length,
          olderBytes: Buffer.byteLength(olderBody),
          olderMs,
          unchangedApplicationBytes: 0,
          unchangedHistoryBytes: 0,
          oneChangeBytes: Buffer.byteLength(changedBody),
          sampledPeakRssBytes: peakRss,
          providerCalls: 0,
          scenario: "synthetic SQLite and authenticated loopback HTTP, not a live model",
        }),
    );
  } finally {
    await store.close();
