...
export function createPackageResources(clients: ReadonlyMap<string, Client>) {
  ...
  const receipts = new Map<
    string,
    { source: string; uri: string; revision: string; client: Client }
  >();
  ...
  function snapshot(): Page {
    ...
    for (const [source, client] of clients) {
      ...
      if (saved?.client === client) {
        result.entries.push(...saved.page.entries);
        result.categories.push(...saved.page.categories);
  ...
  async function discover(refresh = false, wait = true): Promise<Page> {
    ...
    if (!refresh && [...clients].every(([source, client]) => cache.get(source)?.client === client))
      return snapshot();
      ...
      for (const [source, client] of clients) {
        if (!generations.has(client)) {
          ...
          client.setNotificationHandler(ResourceListChangedNotificationSchema, () => {
            ...
            for (const [id, receipt] of receipts)
              if (receipt.source === source) receipts.delete(id);
          });
        }
        const generation = generations.get(client);
        const cached = cache.get(source);
        if (!refresh && cached?.client === client) {
          result.entries.push(...cached.page.entries);
          result.categories.push(...cached.page.categories);
          continue;
        }
        ...
        try {
          ...
          for (let index = 0; index < 100; index++) {
            const response = await client.listResources(cursor ? { cursor } : {}, {
              timeout: 15_000,
            });
            for (const resource of response.resources) {
              if (page.entries.length >= 2000) throw Error("Resource catalogue limit");
              const id = `package-resource:${createHash("sha256")
                .update(JSON.stringify([source, resource.uri]))
                .digest("hex")}`;
              if (ids.has(id)) throw Error("Duplicate resource identity");
              ids.add(id);
          ...
          if (clients.get(source) !== client || generations.get(client) !== generation)
            throw Error("Resource connection changed");
          ...
          page.categories.push({
            ...
            status: "error",
            message:
              "Package resource discovery failed. Refresh to retry; tools and other servers remain independent.",
          });
          if (clients.get(source) === client && generations.get(client) === generation)
            cache.set(source, { client, page });
        }
        result.entries.push(...page.entries);
        result.categories.push(...page.categories);
      }
      return result;
    })().finally(() => {
      pending = undefined;
  ...
  return {
    ...
    async read(selection: { id: string; revision: string }) {
      ...
      if (
        ...
        clients.get(receipt.source) !== receipt.client
      )
      ...
      if (
        receipts.get(selection.id) !== receipt ||
        clients.get(receipt.source) !== receipt.client ||
        generations.get(receipt.client) !== generation
      )
        throw Error("Resource unavailable");
      return {
        ...result,
        contents: result.contents.filter((content) => content.uri === receipt.uri),
      };
    },
  };
}
