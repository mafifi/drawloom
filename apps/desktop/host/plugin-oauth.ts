...
export interface PluginOAuthStatus {
  state: PluginOAuthState;
  credentialMode: "os" | "session";
  issuer?: string;
  code?: string;
}
...
const ClientInformation = z.union([
  OAuthClientInformationFullSchema,
  OAuthClientInformationSchema.strict(),
]);
export const PreconfiguredClientSchema = z.strictObject({
  issuer: z.string().url(),
...
function secureUrl(value: string): URL {
  ...
  if (url.protocol !== "https:" || url.username || url.password || url.hash)
    throw Error("Unsafe OAuth endpoint");
  return url;
}
...
export function createPluginOAuthManager(options: PluginOAuthManagerOptions): PluginOAuthManager {
  ...
  const manager: PluginOAuthManager = {
    connection(input) {
      if (!input.installationId || !input.serverName)
        throw Error("Missing OAuth connection identity");
      const resource = new URL(
        PackageRemoteConfigSchema.parse({ type: "streamable-http", url: input.serverUrl }).url,
      ).href;
      ...
      let state: PluginOAuthState = "disconnected",
      ...
      const status = (): PluginOAuthStatus => ({
        state,
        credentialMode: options.credentials.mode,
        ...(discovery ? { issuer: discovery.authorizationServerUrl } : {}),
        ...(code ? { code } : {}),
      });
      const setState = (next: PluginOAuthState, reason?: string) => {
        state = next;
        code = reason;
        return status();
      };
      const assertCurrent = (version: number) => {
        if (version !== epoch) throw Error("OAuth operation cancelled");
      };
      const cancelPending = () => {
        if (pending) callbacks.delete(pending.state);
        pending = undefined;
        epoch++;
      };
      const metadata = () => ({
        ...
        client_name: "Drawloom",
        client_uri: "https://drawloom.org/",
      ...
      const network: FetchLike = async (inputUrl, init) => {
        ...
        if (
          !(method === "GET" && url.origin === new URL(resource).origin && url.protocol === "http:")
        )
          secureUrl(url.href);
        ...
        if (method !== "GET" && !(method === "POST" && (tokenRequest || registrationRequest)))
          throw Error("Unbound OAuth endpoint");
        if (method === "GET" && (new Headers(init?.headers).has("authorization") || init?.body))
          throw Error("Credentials forbidden in discovery");
        ...
        if (response.status >= 300 && response.status < 400) {
          ...
          throw Error("OAuth redirect rejected");
        }
        ...
        if (tokenRequest && response.ok) {
          ...
          if (payload && typeof payload === "object") {
            if ("resource" in payload && payload.resource !== resource)
              throw Error("Token resource mismatch");
            if ("iss" in payload && payload.iss !== discovery?.authorizationServerUrl)
              throw Error("Token issuer mismatch");
          }
        }
      ...
      async function ensureLoaded() {
        loading ??= (async () => {
          ...
          if (registration)
            preconfigured = PreconfiguredClientSchema.parse(JSON.parse(registration));
          ...
          if (
            ...
            throw Error("Resource metadata mismatch");
          ...
          if (!serverMetadata || serverMetadata.issuer !== issuer)
            throw Error("Issuer metadata mismatch");
          issuerParameterRequired =
            z
              .object({ authorization_response_iss_parameter_supported: z.boolean().optional() })
              .parse(serverMetadata).authorization_response_iss_parameter_supported === true ||
            resourceMetadata.authorization_servers.length > 1;
          secureUrl(serverMetadata.authorization_endpoint);
          secureUrl(serverMetadata.token_endpoint);
          if (serverMetadata.registration_endpoint) secureUrl(serverMetadata.registration_endpoint);
          if (
            !serverMetadata.response_types_supported.includes("code") ||
            !serverMetadata.code_challenge_methods_supported?.includes("S256")
          )
            throw Error("S256 authorization code required");
          discovery = {
          ...
          credentialKey = createHash("sha256")
            .update(JSON.stringify([identity, issuer]))
            .digest("hex");
          const saved = await options.credentials.get(credentialKey);
          if (saved) {
            const parsed = StoredCredentials.parse(JSON.parse(saved));
            if (parsed.issuer !== issuer || parsed.resource !== resource)
              throw Error("Stored credential binding mismatch");
            stored = parsed;
            if (!paused) setState("authorized");
      ...
      async function save(
        ...
        if (
          ...
          validated.token_type.toLowerCase() !== "bearer" ||
          (validated.expires_in !== undefined &&
            (!Number.isFinite(validated.expires_in) || validated.expires_in < 0))
        )
          throw Error("Invalid OAuth tokens");
        const record: Stored = {
          version: 1,
          issuer: discovery!.authorizationServerUrl,
          resource,
          client: ClientInformation.parse(client),
          tokens: validated,
          ...(validated.expires_in === undefined
            ? {}
        ...
        writes = writes
          ...
          .then(async () => {
            ...
            if (version !== epoch) {
              ...
              throw Error("OAuth operation cancelled");
            }
            stored = record;
          });
        await writes;
      ...
      function flowProvider(flow: Pending): OAuthClientProvider {
        let client = preconfigured
          ? ClientInformation.parse(preconfigured.information)
          : stored?.client;
        if (preconfigured?.issuer !== undefined && preconfigured.issuer !== flow.issuer)
          throw Error("Preconfigured client issuer mismatch");
        return {
          redirectUrl: flow.redirect,
          clientMetadataUrl,
          get clientMetadata() {
            return metadata();
          },
          ...
          saveClientInformation(value) {
            ...
            if (
              ...
              throw Error("Client registration unavailable");
          ...
          saveTokens(tokens) {
            assertCurrent(flow.epoch);
            if (!client) throw Error("Missing client identity");
            return save(tokens, client, flow.epoch);
          },
          ...
          codeVerifier() {
            assertCurrent(flow.epoch);
            if (!flow.verifier) throw Error("Missing PKCE verifier");
          ...
          redirectToAuthorization(url) {
            ...
            if (
              url.origin + url.pathname !==
                new URL(discovery!.authorizationServerMetadata!.authorization_endpoint).origin +
                  new URL(discovery!.authorizationServerMetadata!.authorization_endpoint)
                    .pathname ||
          ...
          validateResourceURL(_url, discovered) {
            if (discovered !== resource) return Promise.reject(Error("Resource mismatch"));
            return Promise.resolve(new URL(resource));
          },
          invalidateCredentials() {
            throw new UnauthorizedError("Explicit OAuth reconnect required");
          },
        };
      }
      ...
      const connection: PluginOAuthConnection = {
        ...
        provider: {
          ...
          async tokens() {
            ...
            try {
              ...
              if (!paused && version === epoch) setState("failed", "oauth_discovery_failed");
              return undefined;
            }
          },
          saveTokens() {
            throw new UnauthorizedError("Use explicit OAuth login");
          },
          ...
          discoveryState() {
            if (!paused) setState("auth-required");
            throw new UnauthorizedError("Use explicit OAuth login");
          },
          invalidateCredentials() {
            throw new UnauthorizedError("Use explicit OAuth reconnect");
          },
        },
        async login() {
          cancelPending();
          ...
          try {
            ...
            pending = flow;
            interactiveProvider = flowProvider(flow);
            const result = await auth(interactiveProvider, {
              serverUrl: resource,
              fetchFn: network,
            });
            assertCurrent(version);
            if (result !== "REDIRECT" || !flow.authorizationUrl)
              throw Error("Authorization redirect missing");
            callbacks.set(flow.state, connection);
            setState("awaiting-approval");
            return { ...status(), authorizationUrl: flow.authorizationUrl };
          } catch {
            if (version === epoch) {
              cancelPending();
              setState("failed", "oauth_login_failed");
            }
            return status();
          }
        },
        async callback(url) {
          ...
          if (
            ...
            values.get("state") !== flow.state ||
            callbacks.get(flow.state) !== connection
          )
            throw Error("Invalid OAuth callback state");
          callbacks.delete(flow.state);
          pending = undefined;
          if (
            flow.expiresAt < Date.now() ||
            url.origin + url.pathname !==
              new URL(flow.redirect).origin + new URL(flow.redirect).pathname ||
            url.hash ||
            values.getAll("iss").length > 1 ||
            ...
            setState("failed", "oauth_callback_invalid");
            throw Error("Invalid OAuth callback binding");
          }
          if (values.has("error")) {
            paused = true;
            delete flow.verifier;
            return setState(
              values.get("error") === "access_denied" ? "denied" : "failed",
              "oauth_authorization_denied",
            );
          }
          if (!values.get("code") || !interactiveProvider) {
            setState("failed", "oauth_callback_invalid");
            throw Error("Missing authorization code");
          }
          const callbackProvider = interactiveProvider;
          try {
            const result = await auth(callbackProvider, {
              serverUrl: resource,
            ...
            if (flow.epoch === epoch) setState("failed", "oauth_exchange_failed");
            return status();
          } finally {
            delete flow.verifier;
            if (interactiveProvider === callbackProvider) interactiveProvider = undefined;
          }
        },
        cancel() {
          cancelPending();
        ...
        async disconnect() {
          ...
          try {
            ...
            return setState("failed", "oauth_credential_delete_failed");
          }
        },
        refresh() {
          refreshing ??= (async () => {
            ...
            try {
              ...
              await save(next, stored.client, version);
              return setState("authorized");
            } catch {
              if (version === epoch) {
                paused = true;
                ...
                if (credentialKey) {
                  try {
                    ...
                    /* Status remains explicit failure. */
                  }
                }
