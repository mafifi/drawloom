...
test("evaluation is supplied only when declared, with availability independent of orchestration", async () => {
  ...
  let compositions = 0;
  const evaluation: EvaluationComposer = {
    compose() {
      compositions++;
      throw Error("Not called by discovery");
    },
  };
  try {
    await writeBackend(
      root,
      `export default context => ({ contributions: { skills: [{
        ...
        try {
          const inventory: PackageInventory = {
            ...
            drawloom: {
              ...
              ...(declared ? { optional: [{ kind: "capability", id: "evaluation" }] } : {}),
            },
          };
          const result = await loader.activate(inventory, {
            installationId: "sample",
            dataDirectory: join(root, "data"),
            trusted: true,
            configuration: {},
            available: [{ kind: "capability", id: "evaluation" }],
            capabilities: supplied ? { evaluation } : {},
          });
          expect(result.status).toBe("ready");
          if (result.status === "ready") {
            const report = result.backend.contributions?.skills?.[0]!;
            expect(report.title).toBe(declared && supplied ? "evaluation" : "");
            expect(JSON.parse(report.instructions)).toEqual(
              declared ? [{ kind: "capability", id: "evaluation", available: supplied }] : [],
            );
          }
...
test("required evaluation and backend trust are checked before importing the module", async () => {
  ...
  try {
    ...
    expect((await loader.activate(inventory, { ...options, trusted: false })).status).toBe(
      "untrusted",
    );
      ...
      try {
        ...
        const result = await loader.activate(inventory, {
          ...
          capabilities: {
            ...
            orchestrationReadiness: async () => ({
              status: enabled ? "ready" : "configuration_required",
        ...
        if (result.status === "ready") {
          ...
          expect(report.title).toBe(
            enabled ? "orchestration,orchestrationReadiness" : "orchestrationReadiness",
          );
          expect(JSON.parse(report.description!)).toEqual([
            { kind: "capability", id: "orchestration", available: enabled },
          ]);
          expect(JSON.parse(report.instructions).status).toBe(
            enabled ? "ready" : "configuration_required",
          );
        }
      } finally {
        await loader.close();
...
test("one installation has a distinct fixed activation in each project", async () => {
  ...
  try {
    ...
    const a = await loader.activate(inventory, {
      ...options,
      project: { id: "a", directory: join(root, "a") },
    });
    const b = await loader.activate(inventory, {
      ...options,
      project: { id: "b", directory: join(root, "b") },
    });
    ...
    expect(
      await loader.activate(inventory, {
        ...options,
        project: { id: "a", directory: join(root, "a") },
      }),
    ).toBe(a);
...
test("optional dependencies do not block activation and backend sees only declared availability", async () => {
  ...
  try {
    ...
    const result = await loader.activate(inventory as PackageInventory, {
      installationId: "sample",
      dataDirectory: join(root, "data"),
      configuration: {},
      capabilities: {},
      available: [
        { kind: "skill", id: "package:editor:skill:edit" },
        { kind: "tool", id: "not-declared" },
      ],
      trusted: true,
    });
    expect(result.status).toBe("ready");
    if (result.status === "ready")
      expect(result.backend.contributions?.skills).toEqual([
        {
          id: "package:media:media:inspect",
          title: "missing",
          description: "tool",
          instructions: "test",
        },
        {
          id: "package:editor:skill:edit",
          title: "available",
          description: "skill",
          instructions: "test",
        },
...
test("one synchronous dispose failure still awaits every backend cleanup exactly once", async () => {
  ...
  try {
    ...
    for (const installationId of ["first", "second"]) {
      const result = await loader.activate(inventory, {
        installationId,
        dataDirectory: join(root, installationId),
        configuration: {},
        capabilities: {},
        available: [],
        trusted: true,
