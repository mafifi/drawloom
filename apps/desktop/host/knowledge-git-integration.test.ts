...
test("an installed standard Git package keeps revisions and withdrawals current in managed SQLite across restart", async () => {
  ...
  try {
    ...
    await writeFile(
      ...
      "Committed integration phrase: copper lantern protocol.\n",
    );
    git(repository, ["add", "notes.txt"]);
    git(repository, ["commit", "-m", "public fixture"]);
    ...
    const mcp = JSON.parse(await readFile(join(standard, "mcp.json"), "utf8")) as {
    ...
    const open = () => {
      const service = createManagedLocalKnowledgeClient({
    ...
    expect(
      (
        await knowledge!.ingest({
          ...
          record: {
            ref: claimRef,
            body: "Derived claim from the installed Git source.",
            status: "active",
            freshness: "current",
            confidence: { value: "test" },
            provenance: { producer: { type: "integration", id: "fixture" }, inputs: [firstRef] },
          },
          links: [{ from: claimRef, to: firstRef, relation: "support" }],
        })
      ).kind,
    ).toBe("accepted");
    ...
    await writeFile(
      join(repository, "notes.txt"),
      "Updated integration phrase: emerald observatory policy.\n",
    );
    git(repository, ["add", "notes.txt"]);
    git(repository, ["commit", "-m", "public revision"]);
    const revisedCommit = git(repository, ["rev-parse", "HEAD"]);
    await app.knowledgeCommand({ action: "source", enabled: true });
    const old = await search("copper lantern protocol");
    expect(old.kind).toBe("ok");
    if (old.kind === "ok") expect(old.items).toHaveLength(0);
    const revised = await search("emerald observatory policy");
    expect(revised.kind).toBe("ok");
    if (revised.kind !== "ok") throw Error("expected revised source");
    expect(revised.items).toHaveLength(1);
    expect(revised.items[0]!.record.ref).toMatchObject({
      type: "source",
      origin: firstRef.origin,
      id: firstRef.id,
      revision: revisedCommit,
    });
    ...
    expect(historic.kind === "ok" && historic.record?.body).toContain(
      "Committed integration phrase: copper lantern protocol.",
    );
    const staleClaim = await knowledge!.get(claimRef);
    if (
      staleClaim.kind !== "ok" ||
      staleClaim.record?.ref.type !== "claim" ||
      !("freshness" in staleClaim.record)
    )
      throw Error("expected retained claim");
    ...
    if (withdrawn.kind === "ok") expect(withdrawn.items).toHaveLength(0);
    ...
    if (restarted.kind !== "ok") throw Error("expected restarted lexical source");
