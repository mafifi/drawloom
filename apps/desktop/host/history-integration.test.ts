...
test("failed history writes do not turn successful synthetic work into failed execution", async () => {
  ...
  try {
    ...
    await app.command({
      kind: "send",
      conversationId: id,
      text: "Provider success survives storage failure",
      attachmentKeys: [],
      contextArtifactIds: [],
    });
    for (
...
test("cached Codex history opens without connecting, preserving project assets and native mappings", async () => {
  ...
  const asset = await app.importAsset(
    new TextEncoder().encode("Public attachment"),
    "text/plain",
    "Reference",
  );
  await app.close();
  ...
  try {
    ...
    expect(await json.get("codex:" + id)).toEqual({
      threadId: "private-native-fixture",
      materialized: true,
    });
