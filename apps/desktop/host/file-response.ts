...
export async function fileResponse(
  ...
  const span = trace.getTracer("drawloom.desktop").startSpan("host.file.deliver", {
    attributes: { "drawloom.file.cached": options.immutable, "drawloom.outcome": "ok" },
  });
  let bytes = 0,
    closed = false;
  const close = async () => {
    ...
    try {
      ...
      span.setAttribute("drawloom.file.bytes", bytes);
      span.end();
    }
  };
  ...
  headers.set(
    "Cache-Control",
    options.immutable ? "private, max-age=31536000, immutable" : "no-store",
  );
  headers.set("Accept-Ranges", "bytes");
  let start = 0,
    endExclusive = reader.size,
    status = 200;
  ...
  if (range) {
    ...
    if (match && (match[1] || match[2])) {
      if (!match[1]) {
        ...
        start = Math.max(0, reader.size - suffix);
        if (!Number.isSafeInteger(suffix) || suffix <= 0) start = reader.size;
      } else {
        start = Number(match[1]);
        endExclusive = match[2] ? Math.min(Number(match[2]) + 1, reader.size) : reader.size;
      }
    ...
    status = 206;
    headers.set("Content-Range", `bytes ${start}-${endExclusive - 1}/${reader.size}`);
  }
  headers.set("Content-Length", String(endExclusive - start));
  if (request.method === "HEAD" || endExclusive === start) {
    await close();
