...
test("10,000 native fixture items cross real stdio once and survive a warm restart without payload rereads", async () => {
  ...
  try {
    ...
    const cachedBefore = payloadCalls + metadataCalls,
    ...
    let counted = 0,
      ...
      ordered = true;
    do {
      const page = await store.page("session", {
        limit: 200,
        ...(cursor ? { before: cursor } : {}),
      });
      for (const entry of page.entries.toReversed()) {
        ordered &&= entry.text.startsWith(`${counted} `);
        counted++;
      }
      cursor = page.olderCursor;
    } while (cursor);
    expect(counted).toBe(10000);
    expect(ordered).toBe(true);
    await writer.synchronize(session.history);
    expect(payloadCalls).toBe(10000);
    expect(responseBytes).toBeGreaterThan(40_000_000);
    console.log(
      "NATIVE_HISTORY_MEASUREMENT " +
        JSON.stringify({
          entries: 10000,
          payloadCalls,
          metadataCalls,
          nativeResponseJsonBytes: responseBytes,
          initialPayloadCalls: 50,
          initialResponseJsonBytes: initialBytes,
          initialMs,
          warmRestartPayloadCalls: 0,
          cachedPageProviderCalls: 0,
          cachedPageMs,
          sampledParentPeakRssBytes: peakRss,
          scenario:
            "public synthetic child process, real stdio, Codex adapter and SQLite; not live Codex performance",
        }),
    );
  } finally {
    await session?.close();
