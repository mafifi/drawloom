...
test("registered contributions are discoverable without executing a tool or disclosing instructions", async () => {
  ...
  try {
    ...
    expect(catalog.entries.some((e) => e.kind === "plugin" && e.name === "synthetic.text")).toBe(
      true,
    );
    expect(catalog.entries.some((e) => e.kind === "tool" && e.name === "text.word_count")).toBe(
      true,
    );
    ...
    await expect(
      app.command({
        kind: "send",
        conversationId: id,
        text: "No forged selection",
        attachmentKeys: [],
        contextArtifactIds: [],
        selections: [{ id: "forged", revision: "forged" }],
      }),
    ).rejects.toThrow();
    expect((await app.historyPage(id)).entries).toEqual([]);
  } finally {
