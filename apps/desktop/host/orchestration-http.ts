...
export function createOrchestrationHttp(
  app: Pick<
    Awaited<ReturnType<typeof createDesktopApplication>>,
    | "workflowOwners"
    | "workflowRuns"
    | "workflowSteps"
    | "workflowCommand"
    | "knowledgeActivityOwner"
    | "knowledgeActivityRuns"
    | "knowledgeActivityRun"
    | "knowledgeActivitySteps"
  >,
) {
  ...
  return async (request: Request, url: URL): Promise<{ body: unknown; status: number }> => {
    ...
    if (url.pathname.startsWith("/api/knowledge-activity/")) {
      if (request.method !== "GET")
        return { body: { error: "Knowledge maintenance activity is read-only." }, status: 405 };
      ...
      if (url.pathname.endsWith("/runs"))
        return {
          body: await app.knowledgeActivityRuns(KnowledgeActivityReadSchema.parse(query)),
          status: 200,
        };
      const input = KnowledgeActivityDetailSchema.parse(query);
      return {
        body: await (url.pathname.endsWith("/steps")
          ? app.knowledgeActivitySteps(input)
          : app.knowledgeActivityRun(input)),
        status: 200,
      };
    }
    if (request.method === "GET") {
      ...
      return {
        body: await (url.pathname.endsWith("/steps")
          ? app.workflowSteps(scope)
          : app.workflowRuns(scope)),
        status: 200,
      };
    }
    ...
    try {
      for (;;) {
        ...
        if (size > 65536) {
          await reader.cancel();
          return {
            body: { error: "Workflow input is too large. Use file references instead." },
            status: 413,
          };
        }
        chunks.push(value);
    ...
    try {
      return { body: await next, status: 200 };
    } finally {
      if (commands.get(key) === settled) commands.delete(key);
    }
  };
}
