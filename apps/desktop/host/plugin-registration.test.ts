  ...
  try {
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        information: { client_id: "test", client_secret: "fixture" },
      }),
    );
    expect(await readClientRegistration(file)).toMatchObject({
      information: { client_id: "test" },
    });
    ...
    await writeFile(
      ...
      JSON.stringify({
        issuer: "https://issuer.example",
        information: { client_secret: "fixture" },
      }),
    );
    await expect(readClientRegistration(file)).rejects.toThrow("registration");
  } finally {
