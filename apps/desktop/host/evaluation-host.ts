...
import type {InstalledEvaluationRegistration, InstalledWorkflowRegistration} from './plugin-packages.js';
...
export async function createInstalledEvaluation(options:{
  ...
  const store=createSqliteEvaluationStore({dataDirectory:options.dataDirectory,scope:options.scope});
  try {
    ...
    const evaluation=createEvaluationComposer({store,assessment:options.assessment,
      ...(orchestration?{orchestrator:orchestration}:{}),
      readiness:async()=>{
        if (!orchestration) return {status:'unavailable',reason:'Orchestration is not configured.'};
        try {
          const state=await readiness?.();
          return !state || state.status==='ready' ? {status:'ready'} : {status:'unavailable',reason:state.message??'Orchestration is unavailable.'};
        } catch {return {status:'unavailable',reason:'Orchestration readiness could not be checked.'};}
      },
      clock:{now:()=>Date.now()},
    ...
    try {await store.close();} catch { /* Preserve the original setup failure. */ }
    throw error;
  }
}
