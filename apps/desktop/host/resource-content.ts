...
export function createResourceContent(options: {
  ...
  return {
    async capture(input: {
      ...
      save?: boolean;
      resourceSelections?: Record<string, NonNullable<ResourceReference["retrieval"]>>;
    }): Promise<Entry> {
      ...
      for (const [index, block] of content.entries()) {
        ...
        let asset = uri?.startsWith("asset://")
          ? options.knownAsset(uri.slice("asset://".length))
          : undefined;
        const data =
          block.type === "image" || block.type === "audio"
            ...
            : block.type === "resource" && "blob" in block.resource
              ...
              : undefined;
        const text =
          block.type === "resource" && "text" in block.resource ? block.resource.text : undefined;
        if (data !== undefined || text !== undefined) {
          if (
            browserImportTypes.has(mimeType ?? "text/plain") &&
            (text === undefined || text.length <= 16 * 1024 * 1024) &&
            (data === undefined ||
              (data.length <= 24 * 1024 * 1024 && /^[A-Za-z0-9+/]*={0,2}$/.test(data)))
          ) {
            const bytes =
              data !== undefined ? Buffer.from(data, "base64") : new TextEncoder().encode(text);
            ...
            if (bytes.length > 0 && capturedBytes + bytes.length <= 16 * 1024 * 1024) {
              asset = await options.assets.put(bytes, mimeType ?? "text/plain");
              capturedBytes += bytes.length;
            }
          }
        }
        resources.push(
          ResourceReferenceSchema.parse({
            id: createHash("sha256")
              .update(input.id + ":" + index)
              .digest("hex"),
            source: input.source,
            title,
            ...(uri ? { uri: uri.slice(0, 4096) } : {}),
            ...(mimeType ? { mimeType } : {}),
            ...(asset ? { asset } : {}),
            ...(uri && input.resourceSelections?.[uri]
              ? { retrieval: input.resourceSelections[uri] }
              : {}),
            status: asset
              ? "ready"
              : uri && (input.readable?.(uri) || input.resourceSelections?.[uri])
                ? "readable"
                : "unavailable",
          }),
        );
