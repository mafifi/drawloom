...
const snapshot: RunSnapshot = {
  ...
  steps: [
    { stepId: "inspect", attempts: 1, status: "completed", result: { secret: "private-result" } },
  ],
...
test("presentation validates scope before provider access and strips task content", async () => {
  ...
  const provider: Orchestrator = {
    ...
    getSteps: async () => ({ steps: snapshot.steps }),
    result: async () => ({}),
    respond: async (run, request) => {
      if (request !== "review") throw Error("Input request unavailable");
  ...
  await expect(presentation.list({ projectId: "b", installationId: "i" })).rejects.toThrow(
    "Owner unavailable",
  );
  ...
  ).rejects.toThrow(WorkflowControlError);
