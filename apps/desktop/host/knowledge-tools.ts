...
import {
  ...
  type IntakeInput, type IntakeResult, type ToolOutcomeProjector, ToolOutcomeObservationSchema, IntakeInputSchema,
  EvidenceToolResultSchema, type EvidenceReadReceipts, type EvidenceResult,
...
export function createKnowledgePlugin(service: Pick<KnowledgeService, "search" | "evidence" | "ingest">, receipts?: EvidenceReadReceipts): PluginInstaller {
  return { config: {}, plugin: definePlugin({
    ...
    contribute: () => ({ tools: [
      ...
      defineTool({ name: "knowledge.evidence", description: "Read a bounded, paginated evidence graph for an exact local knowledge record revision.",
        annotations: { readOnlyHint: true }, input: EvidenceRequestSchema, output: EvidenceToolResultSchema,
        execute: async (request, execution) => {
          const result = EvidenceResultSchema.parse(await service.evidence(request));
          return receipts?.select(execution.operationId, execution.invocationId, request, result) ?? result;
...
export function createEvidenceReadReceipts(): EvidenceReadReceipts {
  ...
  const bound = <V>(map: Map<string, V>) => { while (map.size > 128) map.delete(map.keys().next().value!); };
  return {
    select(executionId, invocationId, rawRequest, rawResult) {
      ...
      if (result.kind !== "ok") { delivered.delete(key); return result; }
      const resultHash = digest(result);
      if (delivered.get(key) === resultHash) return { kind: "already_delivered", records: result.records.map(record => record.ref), ...(result.cursor ? { cursor: result.cursor } : {}) };
      pending.set(invocationId, { executionId, key, resultHash }); bound(pending);
      return result;
    },
    confirmDelivered(executionId, invocationId, raw) {
      ...
      if (!prior || prior.executionId !== executionId || !result.success || result.data.kind !== "ok" || digest(result.data) !== prior.resultHash) return false;
      delivered.set(prior.key, prior.resultHash); bound(delivered); pending.delete(invocationId); return true;
    },
    invalidate(executionId) {
      for (const [id, entry] of pending) if (entry.executionId === executionId) pending.delete(id);
      for (const key of delivered.keys()) if (key.startsWith(`${executionId}:`)) delivered.delete(key);
    },
  };
}
...
const CaptureEntrySchema = z.strictObject({
  invocationId: z.string().min(1).max(256), tool: z.string().min(1),
  operationId: z.string().optional(), projector: z.string().optional(),
  intake: IntakeInputSchema.optional(),
  failure: z.enum(["disabled", "denied", "unavailable", "projection_failed", "unfinished"]).optional(),
});
...
export async function createKnowledgeOutcomeCapture(options: {
  store: JsonStore; conversationId: string; projectId: string;
  ...
  const persist = async (next: typeof pending) => {
    await options.store.set(key, JsonValueSchema.parse(schema.parse(next))); pending = next;
  };
  const replace = (entry: z.infer<typeof CaptureEntrySchema>) => persist(pending.map(prior => prior.invocationId === entry.invocationId ? entry : prior));
  async function finish(raw: ToolResult) {
    ...
    if (!entry) return;
    if (entry.projector && !await options.enabled()) { await replace({ ...entry, failure: "disabled" }); return; }
    if (!entry.intake) {
      const registeredName = options.registeredName?.(entry.tool) ?? entry.tool;
      let intake = knowledgeObservation({ toolName: entry.tool, registeredName, invocationId: entry.invocationId, outcome: result.outcome,
        ...(options.producerOrigin?.(entry.tool) ? { producerOrigin: options.producerOrigin(entry.tool)! } : {}) });
      if (intake && entry.projector && result.evidence === "recorded" && result.outcome.status === "ok") {
        try {
          ...
          if (projected) {
            ...
            const revision = createHash("sha256").update(JSON.stringify([entry.invocationId, entry.projector, observation, options.projectId, options.conversationId])).digest("hex");
            intake = IntakeInputSchema.parse({ operation: "upsert", expectedRevision: null, links: [], record: {
              ref: { type: "observation", origin: "host-tool-outcome", id: entry.invocationId, revision },
              body: observation.body, status: "active", confidence: { value: "observed", contentCaptured: true,
                operationStatus: "completed", projectId: options.projectId, conversationId: options.conversationId,
                ...(entry.operationId ? { executionId: entry.operationId } : {}) },
              provenance: { producer: { type: "drawloom-tool-projector", id: entry.projector }, inputs: [] },
            } });
          }
        } catch { await replace({ ...entry, failure: "projection_failed" }); return; }
      }
      if (!intake) { await persist(pending.filter(item => item.invocationId !== entry!.invocationId)); return; }
      entry = { ...entry, intake };
      await replace(entry);
    }
    ...
    try {
      ...
      if (result.kind === "accepted" || result.kind === "duplicate") await persist(pending.filter(item => item.invocationId !== entry!.invocationId));
      else await replace({ ...entry, failure: result.kind === "denied" ? "denied" : "unavailable" });
    } catch { await replace({ ...entry, failure: "unavailable" }); }
  }
  return {
    pending: () => structuredClone(pending),
    started: (record: Extract<ToolEvidence, { kind: "started" }>) => serial(async () => {
      ...
      if (!knowledgeObservation({ toolName: record.tool, registeredName, invocationId: record.invocationId, outcome: { status: "ok" } })) return;
      const projector = await options.enabled() ? options.projectors.get(record.tool) : undefined;
      await persist([...pending, CaptureEntrySchema.parse({ invocationId: record.invocationId, tool: record.tool,
        ...(record.operationId ? { operationId: record.operationId } : {}), ...(projector ? { projector: projector.id } : {}), failure: "unfinished" })]);
    }),
    finished: (result: ToolResult) => serial(() => finish(result)),
    recover: () => serial(async () => {
      for (const entry of [...pending]) {
        const result = options.evidence.activity().find(result => result.invocationId === entry.invocationId);
