...
export function packageToolName(installation: string, server: string, tool: string) {
  return (
    "pkg_" +
    createHash("sha256")
      .update(JSON.stringify([installation, server, tool]))
      .digest("hex")
      .slice(0, 48)
  );
}
export async function projectPackageServer(
  installation: string,
  server: string,
  client: Client,
  ...
  if (!client.getServerCapabilities()?.tools)
    return { tools: [] as ToolDefinition[], inventory, diagnostics };
  ...
  for (const tool of inventory) {
    ...
    try {
        ...
        async execute(value, context) {
          ...
          const result = CallToolResultSchema.parse(
            await (invoke
              ...
              : current().callTool(params, undefined, { signal: context.signal, timeout: 60_000 })),
          );
      ...
      diagnostics.push(`unsupported-tool-schema:${tool.name}`);
    }
  }
