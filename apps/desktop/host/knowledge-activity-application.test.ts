...
test("authenticated Activity uses the composed Nightloom registration without a selected project and rejects command/scope forgery", async () => {
  ...
  try {
    ...
    for (const suffix of [
      "runs?projectId=forged",
      "runs?installationId=forged",
      "runs?capabilityId=forged",
      "runs?limit=101",
      "owner?projectId=forged",
      "run?runId=unknown",
      "steps?runId=unknown",
    ]) {
    ...
    for (const path of ["owner", "runs", "run", "steps"])
      for (const method of ["POST", "PUT", "DELETE"])
        expect((await fetch(base + path, { method, headers, body: "{}" })).status).toBe(405);
    for (const path of ["run", "steps"])
      expect(
        (await fetch(base + path + "?runId=synthetic-nightloom-run&limit=1", { headers })).status,
      ).toBe(200);
    expect(
      (
        await fetch(base + "steps?runId=synthetic-nightloom-run&cursor=later-steps&limit=1", {
          headers,
        })
      ).status,
    ).toBe(200);
    ...
    expect(
      (await fetch(plugin + "?projectId=forged&installationId=knowledge-maintenance", { headers }))
        .status,
    ).toBe(400);
    expect(
      (
        await fetch(plugin, {
          ...
          body: JSON.stringify({
            action: "cancel",
            projectId: "forged",
            installationId: "knowledge-maintenance",
