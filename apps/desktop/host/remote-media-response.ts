...
export function remoteMediaResponse(
  media: { url: string; mediaType: string; title: string },
  hostOrigin: string,
  ...
  const origins = [
    ...new Set([url.origin, ...sharedOrigins.map((origin) => ResourceOriginSchema.parse(origin))]),
  ]
    .filter((origin) => origin !== hostOrigin)
    .join(" ");
  const nonce = randomBytes(24).toString("base64");
  const tag = media.mediaType.startsWith("image/")
    ? "img"
