...
test("restore completes eligible failed intake without opening a synthetic execution", async () => {
  ...
  try {
    ...
    app = await open();
    await app.restore();
    const found = await app.knowledgeCommand({
      action: "search",
      request: { query: "counted", mode: "lexical", limit: 10, maxBytes: 4096 },
    });
    expect(JSON.stringify(found)).toContain("counted 2 words");
    expect(found).toMatchObject({
      kind: "ok",
      items: [{ record: { body: "The text inspection counted 2 words." } }],
    });
    expect(await app.knowledgeCommand({ action: "status" })).toMatchObject({
...
test("recovering one conversation keeps another conversation's learning warning pending", async () => {
        ...
        local: {
          ...
          ingest: (input, operation) => {
            ...
            const conversationId =
              ...
              typeof confidence.conversationId === "string"
  ...
  try {
    ...
    await app.close();
    recoverable.add(conversations[0]!);
    app = await open();
    await app.restore();
    expect(await app.knowledgeCommand({ action: "status" })).toMatchObject({
      capture: { state: "pending", pendingObservations: 1 },
    ...
    recoverable.add(conversations[1]!);
    await app.restore();
    expect(await app.knowledgeCommand({ action: "status" })).toMatchObject({
      capture: { state: "idle", pendingObservations: 0 },
    });
...
test("an unreadable capture queue keeps the aggregate pending count unknown", async () => {
  ...
  try {
    ...
    await app.close();
    await rename(firstDirectory, unavailableDirectory);
    app = await open();
    await app.restore();
    ...
    expect((pending as { capture: { message: string } }).capture.message).toContain(
      "Tool observations are waiting",
    );

    await app.close();
    await rename(unavailableDirectory, firstDirectory);
    denied = false;
    app = await open();
    await app.restore();
    expect(await app.knowledgeCommand({ action: "status" })).toMatchObject({
    ...
    expect(found).toMatchObject({ kind: "ok", items: [{}, {}, {}] });
    for (const [conversationId, activities] of [
      [conversations[0]!, 2],
      [conversations[1]!, 1],
    ] as const) {
      await app.command({ kind: "select_conversation", conversationId });
...
test("enabled product word count captures selected counts without retaining supplied text", async () => {
  ...
  try {
    ...
    await app.command({
      kind: "send",
      conversationId: id,
      text: "PRIVATE WORDS HERE",
      attachmentKeys: [],
      contextArtifactIds: [],
    });
    let found: unknown;
    for (let i = 0; i < 100; i++) {
      found = await app.knowledgeCommand({
        action: "search",
        request: { query: "counted", mode: "lexical", limit: 10, maxBytes: 4096 },
      });
...
test("two project conversations capture into the real shared SQLite service and remain readable after host restart", async () => {
  ...
  try {
    ...
    const expanded = await app.knowledgeCommand({
      action: "evidence",
      request: {
        root: references[0],
        direction: "forward",
...
test("Nightloom keeps SQLite evidence cursor dimensions fixed and classifies aggregate overflow explicitly", async () => {
  ...
  const maintenance: KnowledgeMaintenance = {
    async status() {
      return { kind: "ok", pendingUnits: 0, checkpoint: "unused" };
    },
    async pending() {
  ...
  try {
    expect(
      (
        await client.ingest({
          ...
          record: record(secondRef, "z".repeat(40_000)),
          links: [],
        })
      ).kind,
    ).toBe("accepted");
    expect(
      (
        await client.ingest({
          operation: "upsert",
          expectedRevision: null,
          record: record(firstRef, "y".repeat(40_000)),
          links: [{ from: firstRef, to: secondRef, relation: "support" }],
        })
      ).kind,
    ).toBe("accepted");
    expect(
      (
        await client.ingest({
          operation: "upsert",
          expectedRevision: null,
          record: record(rootRef, "root"),
          links: [{ from: rootRef, to: firstRef, relation: "support" }],
        })
      ).kind,
    ).toBe("accepted");
    const assess = createNightloomTaskHandlers({
      ...
      assessment,
      subject,
      fingerprint: () => "real-sqlite-pagination",
      receipts: {
    ...
    const context: TaskContext = {
      taskVersion: "1",
      runId: "host/real-sqlite",
      stepId: "host/real-sqlite/assess",
      attemptId: "host/real-sqlite/assess/1",
      attempt: 1,
