            ...
            spans: [
              {
                ...
                kind: 3,
                attributes: [{ key: "http.route", value: { stringValue: "/api/discovery" } }],
                events: [{ name: "SECRET" }],
              },
            ],
          },
...
test("UI relay forwards bounded standard traces with fixed resource and no user content", async () => {
  ...
  try {
    ...
    expect(
      (
        await relay.handle(
          new Request("http://localhost/", { method: "POST", body: "x".repeat(65_537) }),
        )
      ).status,
    ).toBe(413);
  } finally {
...
test("disabled UI relay is closed and invalid endpoints cannot become an export proxy", async () => {
  expect(
    (await createTelemetryRelay("disabled").handle(new Request("http://localhost/"))).status,
  ).toBe(404);
  expect(() => createTelemetryRelay("export", "https://example.com")).toThrow();
  const value = payload();
  value.resourceSpans[0]!.scopeSpans[0]!.spans[0]!.name = "SECRET";
  expect(
    (
      await createTelemetryRelay("recording").handle(
        new Request("http://localhost/", { method: "POST", body: JSON.stringify(value) }),
      )
    ).status,
  ).toBe(400);
});
    ...
    expect(
      (
        await fetch(url, {
          method: "POST",
          body,
          headers: { cookie, "Content-Type": "application/json", origin: "http://evil.invalid" },
        })
      ).status,
    ).toBe(403);
    expect(
...
test("a stalled body and overloaded UI relay are bounded", async () => {
  ...
  const hung = new Request("http://localhost/", {
    method: "POST",
  ...
  expect(
    (await relay.handle(new Request("http://localhost/", { method: "POST", body: "{}" }))).status,
  ).toBe(429);
