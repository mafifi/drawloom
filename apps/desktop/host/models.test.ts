  ...
  try {
    ...
    await expect(
      app.command({
        kind: "set_model",
        conversationId: "native",
        selection: { model: "small", effort: "high" },
      }),
    ).rejects.toThrow();
    await expect(
      app.command({ kind: "set_model", conversationId: "local", selection: { model: "small" } }),
    ).rejects.toThrow();
    await app.close();
    app = await createTestDesktopApplication(root);
    const after = (await app.snapshot()).conversations;
    expect(after.map((c) => c.id)).toEqual(before.map((c) => c.id));
    expect(after.find((c) => c.id === "native")).toMatchObject({
      title: "Existing",
      provider: "codex",
      modelSelection: { model: "small", effort: "low" },
    });
    await app.command({ kind: "set_model", conversationId: "native" });
    expect(
      (await app.snapshot()).conversations.find((c) => c.id === "native")?.modelSelection,
    ).toBeUndefined();
  } finally {
    await app.close();
