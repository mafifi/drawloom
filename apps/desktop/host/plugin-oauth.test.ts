...
test("OAuth advertises the canonical client identity without a registration bridge", async () => {
  ...
  try {
    const connection = createPluginOAuthManager({
    ...
    expect(new URL(login.authorizationUrl!).searchParams.get("client_id")).toBe(
      "https://drawloom.org/oauth/client.json",
    );
...
test("preconfigured client persists only in credential storage and is bound to its connection", async () => {
  ...
  try {
    ...
    await first.configureClient({
      issuer,
      information: { client_id: "private-client", client_secret: "private-secret" },
    });
    ...
    expect(new URL(other.authorizationUrl!).searchParams.get("client_id")).not.toBe(
      "private-client",
    );
    await connection.disconnect();
    const disconnected = await createPluginOAuthManager(options).connection(identity).login();
    expect(new URL(disconnected.authorizationUrl!).searchParams.get("client_id")).not.toBe(
      "private-client",
    );
    const changed = createPluginOAuthManager(options).connection(identity);
    await changed.configureClient({
      issuer: "https://other-issuer.example",
      information: { client_id: "wrong-client" },
    });
    ...
    async fetch(request) {
      ...
      if (url.pathname.startsWith("/.well-known/oauth-protected-resource")) {
        ...
        return Response.json({
          ...
          authorization_servers: [config.advertisedIssuer],
        });
      }
      if (url.pathname === "/.well-known/oauth-authorization-server")
        return Response.json({
          ...
          token_endpoint: config.badEndpoint ? "http://insecure.example/token" : issuer + "/token",
          ...
          client_id_metadata_document_supported: config.cimd,
        });
      ...
      if (url.pathname === "/authorize") {
        ...
        if (config.denied) redirect.searchParams.set("error", "access_denied");
          ...
          codes.set(code, {
            ...
            redirect: redirect.origin + redirect.pathname,
      ...
      if (url.pathname === "/token") {
        effects.token++;
        if (config.redirectToken)
          return new Response(null, {
            status: 307,
            headers: { location: "https://intruder.example/token" },
          });
        const form = new URLSearchParams(await request.text());
        if (config.requireClientPost) {
          const valid =
            !request.headers.has("authorization") &&
            form.get("client_secret") === "registered-secret";
        ...
        return Response.json({
          access_token: "secret-access-" + effects.token,
          refresh_token: "secret-refresh-" + effects.token,
          token_type: "Bearer",
          expires_in: 3600,
          ...(config.wrongTokenResource ? { resource: "https://intruder.example" } : {}),
        });
      }
...
async function approve(
  connection: ReturnType<ReturnType<typeof createPluginOAuthManager>["connection"]>,
  remote: Awaited<ReturnType<typeof service>>,
) {
  const login = await connection.login();
  expect(login.state).toBe("awaiting-approval");
...
test("SDK login binds PKCE/resource/issuer, persists credentials and never exposes secrets", async () => {
  const remote = await service(),
    credentials = persistedStore();
  try {
    const manager = createPluginOAuthManager({
      credentials,
      redirectUrl: () => callback,
      fetch: remote.fetch,
    });
    ...
    const restarted = createPluginOAuthManager({
      credentials,
      redirectUrl: callback,
      fetch: remote.fetch,
    }).connection({ installationId: "one", serverName: "remote", serverUrl: resource });
...
test("callbacks reject cross-instance, wrong issuer, replay and cancellation", async () => {
  ...
  try {
    ...
    const other = manager.connection({
      installationId: "two",
      serverName: "remote",
      serverUrl: resource,
    });
    ...
    const wrongIssuer = new URL(returned);
    wrongIssuer.searchParams.set("iss", "https://intruder.example");
    ...
    const cancelled = await approve(first, remote);
    first.cancel();
...
test("denial and insecure or substituted metadata never exchange tokens", async () => {
  ...
  try {
    ...
    const denied = make();
    expect((await denied.callback(await approve(denied, remote))).state).toBe("denied");
    remote.config.resource = "https://intruder.example";
    expect((await make().login()).state).toBe("failed");
    remote.config.resource = resource;
    remote.config.issuer = "https://intruder.example";
    expect((await make().login()).state).toBe("failed");
    remote.config.issuer = issuer;
    remote.config.badEndpoint = true;
    expect((await make().login()).state).toBe("failed");
...
test("DCR, session restart and disconnect keep ownership separate", async () => {
  ...
  try {
    ...
    const restart = createPluginOAuthManager({
      credentials: createSessionCredentialStore(),
      redirectUrl: callback,
      fetch: remote.fetch,
    }).connection({ installationId: "one", serverName: "remote", serverUrl: resource });
...
test("restart disconnect removes credentials before a connection has loaded its tokens", async () => {
  ...
  try {
    ...
    const first = make();
    await first.callback(await approve(first, remote));
...
test("issuer identifiers retain their exact advertised trailing slash", async () => {
  ...
  try {
    remote.config.issuer = issuer + "/";
    remote.config.advertisedIssuer = issuer + "/";
    const connection = createPluginOAuthManager({
      credentials: createSessionCredentialStore(),
      redirectUrl: callback,
      fetch: remote.fetch,
...
test("expired callbacks, substituted token resources, and token redirects never authorize", async () => {
  ...
  try {
    ...
    const wrong = make();
    expect((await wrong.callback(await approve(wrong, remote))).state).toBe("failed");
    expect(await wrong.provider.tokens()).toBeUndefined();
    remote.config.wrongTokenResource = false;
    remote.config.redirectToken = true;
    const redirected = make();
    expect((await redirected.callback(await approve(redirected, remote))).state).toBe("failed");
  ...
  try {
    ...
    const make = (boundIssuer: string) =>
      createPluginOAuthManager({
        ...
        serverName: "remote",
        serverUrl: resource,
        preconfiguredClient: { issuer: boundIssuer, information: { client_id: "preconfigured" } },
      });
    expect((await make("https://intruder.example").login()).state).toBe("failed");
    const correct = make(issuer);
    expect((await correct.callback(await approve(correct, remote))).state).toBe("authorized");
    expect(remote.effects.registration).toBe(0);
  } finally {
    remote.close();
  }
...
test("late cancelled exchange cannot clear a newer login callback", async () => {
  ...
  try {
    const fetch: FetchLike = async (url, init) => {
      const response = await remote.fetch(url, init);
      if (new URL(url).pathname === "/token" && firstToken) {
        firstToken = false;
        arrived();
        await blocked;
      }
...
test("loopback HTTP MCP resources can use HTTPS authorization endpoints", async () => {
  const localResource = "http://127.0.0.1:43219/mcp",
    remote = await service(localResource);
  try {
    const connection = createPluginOAuthManager({
      credentials: createSessionCredentialStore(),
      redirectUrl: callback,
      fetch: remote.fetch,
    }).connection({ installationId: "one", serverName: "local", serverUrl: localResource });
    expect((await connection.callback(await approve(connection, remote))).state).toBe("authorized");
  } finally {
    remote.close();
  }
});
test.each(["credential read", "refresh"] as const)(
  "cancellation during %s prevents an in-flight tokens read from returning credentials",
  async (boundary) => {
    const remote = await service(),
      saved = persistedStore();
    let release!: () => void, arrived!: () => void;
    const blocked = new Promise<void>((resolve) => {
        release = resolve;
      }),
    ...
    try {
      const original = createPluginOAuthManager({
        credentials: saved,
        redirectUrl: callback,
        fetch: remote.fetch,
      }).connection({ installationId: "one", serverName: "remote", serverUrl: resource });
      ...
      const fetch: FetchLike = async (url, init) => {
        const response = await remote.fetch(url, init);
        if (boundary === "refresh" && new URL(url).pathname === "/token") {
          arrived();
          await blocked;
        }
        return response;
...
test.each(["preconfigured", "registered"] as const)(
  "%s client POST authentication survives refresh and restart",
  async (registration) => {
    const remote = await service(),
      credentials = persistedStore();
    try {
      remote.config.cimd = false;
      remote.config.requireClientPost = true;
      const connection = createPluginOAuthManager({
        credentials,
        redirectUrl: callback,
        fetch: remote.fetch,
      ...
      expect((await connection.callback(await approve(connection, remote))).state).toBe(
        "authorized",
      );
