...
test("nonblocking package discovery exposes loading then caches success without repeated reads", async () => {
  ...
  const client = {
    ...
    async listResources() {
      ...
      return { resources: [{ uri: "doc://one", name: "One" }] };
    },
  } as unknown as Client;
  const resources = createPackageResources(new Map([["source", client]]));
  const first = await Promise.race([
    resources.discover(false, false),
    new Promise<never>((_, reject) =>
      setTimeout(() => reject(Error("Package discovery blocked presentation")), 100),
    ),
  ]);
  expect(first).toEqual({ entries: [], categories: [{ kind: "resource", status: "loading" }] });
  await resources.discover(true, false);
  expect(calls).toBe(1);
  release();
  await resources.discover();
  expect((await resources.discover(false, false)).entries[0]?.name).toBe("One");
  expect(calls).toBe(1);
});
...
test("resource-list invalidation rejects old receipts and an in-flight stale listing", async () => {
  ...
  const loading = resources.discover();
  notify();
  release!();
...
test("a package resource receipt returns only the selected URI", async () => {
  const client = {
    ...
    async readResource() {
      return {
        contents: [
          { uri: "doc://one", text: "Selected" },
          { uri: "doc://other", text: "Not selected" },
        ],
      };
    },
  } as unknown as Client;
  const resources = createPackageResources(new Map([["source", client]]));
  const entry = (await resources.discover()).entries[0]!;
  expect(await resources.read(entry)).toEqual({
    contents: [{ uri: "doc://one", text: "Selected" }],
  });
});
  ...
  test(`a pending resource read rejects after ${invalidate}`, async () => {
    ...
    const client = {
      ...
      async readResource() {
        ...
        return { contents: [{ uri: "doc://one", text: "Stale" }] };
      },
    } as unknown as Client;
    const clients = new Map([["source", client]]),
      resources = createPackageResources(clients);
    const entry = (await resources.discover()).entries[0]!;
