...
test("the installed source adapter invokes only the fixed feed and ACK tools without repository arguments", async () => {
  ...
  const presentation = new Map([
    [
      ...
      {
        name: "git.changes",
        origin: "Git source / configured",
        ownerId: "drawloom:plugin:package:one",
        available: true,
      },
    ],
    [
  ...
  const feed = createInstalledGitKnowledgeFeed({
    projectId: "project-one",
    tools: [changes, acknowledge],
    presentation,
  });
  const batch = await feed.changes();
  await feed.acknowledge(batch.token);
  expect(calls).toEqual([
    { name: "changes", input: {} },
    { name: "ack", input: { token: "token-1" } },
  ]);
...
test("ambiguous or cross-installation feed tools fail closed", () => {
  ...
  const presentation = new Map([
    ["one", { name: "git.changes", origin: "one", ownerId: "owner-one", available: true }],
  ]);
  expect(() =>
    createInstalledGitKnowledgeFeed({ projectId: "project", tools: [tool], presentation }),
  ).toThrow("Installed Git knowledge source unavailable");
});
