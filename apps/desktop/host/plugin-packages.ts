...
export type InstalledPackageStatus = {
  id: string;
  name: string;
  status: "disabled" | "ready" | "partial" | "failed";
  codes: string[];
  servers: ActivePackage["statuses"];
};
...
export async function loadInstalledPackages(options: {
  ...
  ) => Promise<InstalledWorkflowRegistration>;
  prepareEvaluation?: (
    installation: Installation,
    inventory: PackageInventory,
  ...
  const slots = new Map<
    string,
    { client: Client; connection: ActivePackageServer; declaration: string; source: string }
  >();
  ...
  for (const installation of options.installations) {
    const status: InstalledPackageStatus = {
      id: installation.id,
      name: installation.name,
      status: installation.enabled ? "ready" : "disabled",
      codes: [],
      servers: [],
    };
    ...
    try {
      const inventory = await observed(
        "host.package.inspect",
        { "drawloom.plugin.id": installation.id },
        () => inspectPackage(installation.root),
      );
      await options.mediaPolicy?.declare(
        `package:${inventory.name}:${installation.id}`,
        installation.approvedResourceOrigins ?? [],
      );
      const authProviderFor = options.authProviderFor?.(installation);
      const connections = await observed(
        "host.package.connect",
        { "drawloom.plugin.id": installation.id },
        async () => {
      ...
      status.servers = connections.statuses;
      ...
      for (const [serverName, connection] of connections.servers) {
        try {
          ...
          const projection = await projectPackageServer(
            installation.id,
            serverName,
            connection.client,
            () => {
              const slot = slots.get(key);
              if (!slot) throw Error("Package connection unavailable");
              return slot.client;
            },
            (params, context) => {
              const slot = slots.get(key);
              if (!slot) throw Error("Package connection unavailable");
              return slot.connection.callTool(params, context);
            },
          );
          tools.push(...projection.tools);
          status.codes.push(...projection.diagnostics);
          for (const tool of projection.inventory) {
            const alias = packageToolName(installation.id, serverName, tool.name);
            toolPresentation.set(alias, {
              name: tool.title ?? tool.name,
              description: tool.description ?? "",
              origin: `${inventory.name} / ${serverName}`,
              ownerId: `drawloom:plugin:package:${installation.id}`,
              appOnly: isToolVisibilityAppOnly(tool),
              available: projection.tools.some((t) => t.name === alias),
            });
          }
          projection.tools.forEach((tool) => toolIds.add(tool.name));
          const source = `package:${installation.id}:${createHash("sha256")
            .update(JSON.stringify(inventory.servers.find((s) => s.name === serverName)))
            .digest("hex")}`;
          slots.set(key, {
            client: connection.client,
            connection,
            declaration: JSON.stringify(projection.inventory),
            source,
          });
          if (connection.client.getServerCapabilities()?.resources)
            resourceClients.set(source, connection.client);
          ...
          for (const tool of projection.inventory) {
            ...
            dependencies.push({
              ...
              requirement: { kind: "tool", id },
              alias,
            });
          }
        } catch {
          status.codes.push(`tools:${serverName}:discovery-failed`);
        }
      }
      const skills = [];
      for (const skill of inventory.skills) {
        try {
          skills.push({
            id: `package:${installation.id}:skill:${skill.name}`,
            title: skill.name,
          ...
          dependencies.push({
            packageName: inventory.name,
            requirement: { kind: "skill", id: `package:${inventory.name}:skill:${skill.name}` },
            alias: `package:${installation.id}:skill:${skill.name}`,
          });
        } catch {
          status.codes.push(`skill:${skill.name}:unavailable`);
        }
      }
      ...
      installs.push({
        config: {},
        plugin: {
          id: `package:${installation.id}`,
          version: inventory.version ?? "unversioned",
          requires: [],
      ...
      status.codes.push("package-unavailable");
    }
  }
  ...
  for (const dependency of dependencies) {
    if (
      prepared.filter((p) => p.inventory.name === dependency.packageName).length !== 1 ||
      dependencies.filter(
        (d) =>
          d.requirement.kind === dependency.requirement.kind &&
          d.requirement.id === dependency.requirement.id,
      ).length !== 1
    )
      continue;
    available.push(dependency.requirement);
    if (dependency.alias)
      (dependency.requirement.kind === "tool" ? toolAliases : skillAliases).set(
  ...
  for (const { installation, inventory, connections, status } of prepared) {
    ...
    const gateway = options.toolsFor?.(
      installation,
      standardTools,
      inventory.drawloom?.workbenches?.map((w) => w.id) ?? [],
    );
    ...
    const tools: ToolGateway | undefined = gateway && {
      exposure: {
        ...gateway.exposure,
        tools: gateway.exposure.tools.map((tool) => {
          const name = identities.get(tool.name) ?? tool.name;
          aliases.set(name, tool.name);
          return { ...tool, name };
        }),
      },
      bind: (operationId) => gateway.bind(operationId),
      revoke: (binding) => gateway.revoke(binding),
      invoke: async (binding, name, args, signal) => {
        const alias = aliases.get(name);
        if (!alias) throw Error("Tool unavailable to this backend");
    ...
    const prerequisites = (inventory.drawloom?.requires ?? []).filter(
      (r) => r.kind !== "capability" || (r.id !== "orchestration" && r.id !== "evaluation"),
    );
    let workflow: InstalledWorkflowRegistration | undefined;
    if (
      installation.trustedBackend &&
      inventory.drawloom?.backend &&
      inventory.drawloom.workflows &&
      prerequisites.every((r) => present.has(`${r.kind}:${r.id}`)) &&
      options.prepareWorkflows
    ) {
      try {
    ...
    if (
      ...
      try {
        ...
        status.codes.push("evaluation:unavailable");
      }
    }
    const releaseEvaluation = async () => {
      ...
      try {
        ...
        status.codes.push("evaluation:cleanup-failed");
      }
    };
    const requirementsResolved = (inventory.drawloom?.requires ?? []).every(
      (r) => present.has(`${r.kind}:${r.id}`) && (r.kind !== "tool" || !tools || aliases.has(r.id)),
    );
    if (!requirementsResolved) {
      status.codes.push("extension:unavailable");
      status.status = "partial";
      await releaseEvaluation();
      continue;
    }
    const result = await observed(
      "host.package.activate",
      { "drawloom.plugin.id": installation.id },
      async () => {
        const result = await backends.activate(inventory, {
          trusted: installation.trustedBackend,
          available,
          ...(options.project ? { project: options.project } : {}),
          installationId: installation.id,
          dataDirectory: join(options.root, "plugins", installation.id),
          ...
          capabilities: {
            host: {
              ...
              store: {
                get: (key) =>
                  options.host.store.get(JSON.stringify(["plugin", installation.id, key])),
                set: (key, value) =>
                  options.host.store.set(JSON.stringify(["plugin", installation.id, key]), value),
              },
            },
            ...(tools ? { tools } : {}),
    ...
    if (result.status === "ready") {
      ...
      if (workflow) {
        try {
          await workflow.attach(backend.taskHandlers ?? []);
        } catch {
          status.codes.push("orchestration:handlers-unavailable");
        }
      ...
      if (backend.contributions || backend.controllers) {
        try {
          ...
          const contribution = {
            ...
            ...(raw.workbenches
              ? {
                  workbenches: raw.workbenches.map((workbench) => ({
                    ...workbench,
                    tools: workbench.tools.map((id) => toolAliases.get(id) ?? id),
                    skills: workbench.skills.map((id) => skillAliases.get(id) ?? id),
                  })),
                }
              : {}),
          };
          const preparedInstalls = [
            {
              config: {},
              plugin: {
                id: `package:${installation.id}:backend`,
                version: inventory.version ?? "unversioned",
                requires: [],
                prepare: () => () => contribution,
              },
          ...
          const validated = createPluginRegistry(
            [...(options.builtins ?? []), ...installs, ...preparedInstalls],
            ["agent", "host"],
          );
          for (const [id, controller] of backend.controllers ?? []) {
            if (
              !contribution.workbenches?.some((w) => w.id === id) ||
              typeof controller.dispatch !== "function"
            )
              throw Error("Invalid controller");
            OperatorSnapshotSchema.parse(await controller.snapshot());
          }
          const duplicate = [...(backend.controllers?.keys() ?? [])].some((id) =>
            controllers.has(id),
          );
          if (duplicate) status.codes.push("backend:duplicate-contribution");
            ...
            ownedViews = validated.views.filter(
              (view) =>
                ownedPlugins.has(view.pluginId) &&
                validated.contributions.some(
                  (c) =>
                    c.pluginId === view.pluginId &&
                    c.kind === "workbench" &&
                    c.contributionId === view.workbenchId,
                ),
            );
            for (const [id, controller] of backend.controllers ?? [])
          ...
          status.codes.push("backend:invalid-contribution");
        }
      }
      for (const server of result.backend.servers ?? []) {
        const client = new Client(
          { name: "drawloom-package-backend", version: "0.0.0" },
          {
            capabilities: {
              extensions: { "io.modelcontextprotocol/ui": { mimeTypes: [RESOURCE_MIME_TYPE] } },
            },
        ...
        try {
          await client.connect(server.transport, { timeout: 15_000 });
          backendClients.push(client);
          connections.servers.set(server.name, {
            client,
            callTool: async (params, context) =>
              CallToolResultSchema.parse(
                await client.callTool(params, undefined, { signal: context.signal }),
              ),
            close: () => Promise.resolve(),
          });
        } catch {
          await client.close();
          status.codes.push(`backend-server:${server.name}:failed`);
        }
      }
    } else {
    ...
    for (const placement of inventory.drawloom?.workbenches ?? []) {
      ...
      if (
        !view ||
        mcpApps.has(placement.id) ||
        inventory.drawloom!.workbenches!.filter((p) => p.id === placement.id).length !== 1
      ) {
        status.codes.push(`workbench:${placement.id}:view-unavailable`);
        continue;
      }
      const connection = connections.servers.get(placement.openingTool.server);
      if (!connection) {
        status.codes.push(`workbench:${placement.id}:server-unavailable`);
        continue;
      }
      try {
        const projection = await projectPackageServer(
          installation.id,
          placement.openingTool.server,
          connection.client,
        );
    ...
    if (status.codes.length || status.servers.some((s) => s.status !== "connected"))
      status.status = "partial";
  }
  return {
    installs,
    ...
    activeServer(installationId: string, name: string) {
      ...
      const server = pkg?.installation.servers.includes(name)
        ? pkg.inventory.servers.find((s) => s.name === name)
        : undefined;
      return server ? structuredClone(server) : undefined;
    },
    transportFor: (installationId: string, name: string) =>
      prepared
        .find((p) => p.installation.id === installationId)
        ?.inventory.servers.find((s) => s.name === name)?.config.type,
    async disconnect(installationId: string, serverName: string) {
      ...
      if (state) {
        ...
        state.code = "disconnected";
      }
    },
    async reconnect(installationId: string, serverName: string) {
      const pkg = prepared.find((p) => p.installation.id === installationId);
      if (!pkg || !pkg.installation.servers.includes(serverName))
        throw Error("Installation server unavailable");
      if (viewConnections.has(JSON.stringify([installationId, serverName])))
      ...
      const opened = await activatePackage(pkg.inventory, {
        dataRoot: join(options.root, "plugins"),
        installationId,
        ...
        elicitationDisabledServers: (pkg.installation.elicitationDisabledServers ?? []).filter(
      ...
      if (!slot || slot.declaration !== JSON.stringify(projection.inventory)) {
        await opened.close();
        return { restartRequired: true };
      }
      ...
      if (connection.client.getServerCapabilities()?.resources)
        resourceClients.set(slot.source, connection.client);
      active.push(opened);
    ...
    async readResource(source: string, uri: string) {
      ...
      const client = resourceClients.get(source);
      if (!client) throw Error("Package resource unavailable");
