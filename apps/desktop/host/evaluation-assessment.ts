...
const modelSchema=z.string().min(1).max(256).refine(value=>value.trim()===value && !/[\r\n\t]/.test(value));
...
export async function createDesktopAssessment(options:{
  ...
  const {createBraintrustAssessmentProvider,createAgentRubricScorer}=await import('@drawloom/braintrust-assessment');
  ...
  const owner=createHash('sha256').update(JSON.stringify([scope.installationId,scope.projectId])).digest('hex');
  const driver=createCodexDriver({
    workingDirectory:options.workingDirectory,
    store:createNodeJsonStore(join(options.dataDirectory,'evaluation-agents',owner)),
    archiveOnClose:true,
    connect:options.connect??(async()=>observedRpc(createStdioTransport({...command,cwd:options.workingDirectory}))),
  });
  return createBraintrustAssessmentProvider({scorers:[createAgentRubricScorer({driver,configuredModel:options.model!})]});
}
