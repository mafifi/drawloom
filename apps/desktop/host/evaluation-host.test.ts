...
import {EvaluationJsonSchema, type EvaluationAssessmentProvider, type EvaluationDefinition, type EvaluationScorer} from '@drawloom/evaluation';
...
const assessment: EvaluationAssessmentProvider = {async assess(){throw Error('Saved reads must not assess');}};
const definition: EvaluationDefinition = {schemaVersion:1,id:'saved',revision:'1',name:'Saved check',mode:'assess_existing',
  scorers:[{id:'exact',revision:'1'}],cases:[{id:'one',revision:'1',input:'input',suppliedOutput:'saved',references:[]}]};
const scorer:EvaluationScorer={id:'exact',revision:'1',input:EvaluationJsonSchema,output:EvaluationJsonSchema,async score(){throw Error('No assessment');}};
...
test('installed evaluation reads only its saved scope without an engine and closes its store', async()=>{
  ...
  try {
    ...
    await seeded.saveDefinition(definition);await seeded.close();
    const first=await createInstalledEvaluation({dataDirectory:root,scope,assessment});
    const second=await createInstalledEvaluation({dataDirectory:root,scope:{...scope,projectId:'second'},assessment});
    const other=await createInstalledEvaluation({dataDirectory:root,scope:{...scope,installationId:'other'},assessment});
    ...
    } finally {await first.close();await second.close();await other.close();}
