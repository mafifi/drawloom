...
test("configuration is serialized with starts and blocks old configuration until restart", async () => {
  ...
  const manager: ReturnType<typeof createLocalTemporalManager> = {
    ...
    listOwners: async () => [],
    hasUnfinishedInstallation: async () => unfinished,
    close: async () => {},
  };
  ...
  const inventory: PackageInventory = {
    ...
    drawloom: {
      ...
      workflows: { entrypoint: "./org.drawloom/workflow.mjs" },
    },
  };
  try {
    ...
    await started;
    const change = host.changeInstallation(installation.id, async () => {
      applied = true;
    });
    ...
    unfinished = false;
    await host.changeInstallation(installation.id, async () => {
      applied = true;
    });
    expect(applied).toBe(true);
    await expect(
      registration.capabilities.orchestration!.start("two", workflow, {}),
    ).rejects.toThrow("restart");
    expect(starts).toBe(1);
    await host.changeInstallation(
      installation.id,
      async () => {},
      () => false,
    );
    await expect(
      registration.capabilities.orchestration!.start("three", workflow, {}),
    ).resolves.toBe("r");
  } finally {
    release();
    await host.close();
...
test("ordinary startup is lazy; saved owners restore unopened projects and protect installation changes", async () => {
  ...
  const manager: ReturnType<typeof createLocalTemporalManager> = {
    ...
    hasUnfinishedInstallation: async (id) => id === "i",
    close: async () => {
      closed++;
    },
  };
  ...
  try {
    ...
    expect((await host.owners("b"))[0]?.readiness).toMatchObject({
      status: "unavailable",
...
test("missing prerequisites expose actionable readiness without disabling ordinary backend use", async () => {
  ...
  const manager: ReturnType<typeof createLocalTemporalManager> = {
    prepare: async () => {
      throw Error("spawn temporal ENOENT");
    },
    prepareHost: async () => {
      throw Error("not used");
    },
  ...
  try {
    ...
    expect(await registration.capabilities.orchestrationReadiness!()).toMatchObject({
      status: "configuration_required",
      code: "local_runtime_required",
    });
    await registration.attach([]);
    expect(await host.owners("a")).toHaveLength(1);
    await expect(host.list({ projectId: "a", installationId: installed.id })).rejects.toThrow(
...
test("saved owners remain visible when their project is unavailable; changed bundles remain blocked", async () => {
  ...
  const manager: ReturnType<typeof createLocalTemporalManager> = {
    prepare: async () => {
      throw Error("Workflow bundle changed with unfinished runs");
    },
    prepareHost: async () => {
      throw Error("not used");
    },
    ...
    hasUnfinishedInstallation: async () => true,
    close: async () => {},
  };
  const host = createOrchestrationHost({
    dataDirectory: root,
    manager: async () => manager,
    ensureProject: async () => {
      throw Error("Project directory missing");
    },
  });
  try {
    await host.restore();
    expect((await host.owners("offline"))[0]?.readiness).toMatchObject({
      status: "unavailable",
      code: "project_unavailable",
    });
    await expect(host.list({ projectId: "offline", installationId: "i" })).rejects.toThrow();
    await expect(host.changeInstallation("i", async () => {})).rejects.toThrow(
      "unfinished workflows",
    );
    await writeWorkflow(root, "workflow.mjs");
    const registration = await host.prepare(
      "online",
      {
        id: "i",
        root,
      ...
      },
      {
        ...
        servers: [],
        diagnostics: [],
        drawloom: { version: 1, workflows: { entrypoint: "./org.drawloom/workflow.mjs" } },
      },
      (handlers) => handlers,
    );
    expect(await registration.capabilities.orchestrationReadiness!()).toMatchObject({
...
test("failed manager restoration reports readiness instead of disabling ordinary startup", async () => {
  ...
  const host = createOrchestrationHost({
    dataDirectory: root,
    manager: async () => {
      throw Error("private internal startup detail");
    },
    ensureProject: async () => {},
  });
  try {
    expect(await host.restore()).toMatchObject({
      status: "unavailable",
