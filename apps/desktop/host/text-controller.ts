...
export async function createTextController(store: JsonStore) {
  let state: OperatorSnapshot = OperatorSnapshotSchema.parse(
    (await store.get("text-presentation")) ?? {
      artifacts: [],
      candidates: [],
      reviews: [],
      readiness: "ready",
      summary: "Local synthetic text inspection. No model calls.",
      configuration: [{ key: "title", label: "Artifact title", value: "Introduction" }],
      grants: [{ toolName: "text.word_count", allowed: false }],
    },
  );
  // Historical text drafts remain editable; unrelated histories gain no guessed lineage.
  for (const artifact of state.artifacts)
    if (artifact.content.kind === "text") artifact.editable = true;
  for (const candidate of state.candidates)
    if (
      candidate.artifactIds.some((id) =>
        state.artifacts.some((a) => a.id === id && a.content.kind === "text"),
      )
    )
      candidate.comparisonKey ??= candidate.id;
  async function save(next: OperatorSnapshot) {
    await store.set("text-presentation", JsonValueSchema.parse(next));
    state = next;
  }
  ...
  const controller: OperatorController = {
    ...
    dispatch(raw): Promise<OperatorResult> {
      return mutate(async () => {
        const p = OperatorCommandSchema.safeParse(raw);
        const reject = (
          code: "invalid_command" | "not_found",
          message: string,
        ): OperatorResult => ({ status: "rejected", code, message });
        ...
        if (command.kind === "revise_document") {
          ...
          if (
            !candidate?.artifactIds.includes(command.artifactId) ||
            artifact?.content.kind !== "text"
          )
            return reject("not_found", "Document revision unavailable");
          ...
          next.candidates.push({
            id: crypto.randomUUID(),
            comparisonKey: candidate.comparisonKey ?? candidate.id,
            label: `Revision ${next.candidates.length + 1}`,
            artifactIds: [id],
            status: "draft",
          });
          candidate.comparisonKey ??= candidate.id;
          ...
          if (
            command.key !== "title" ||
            !z.string().min(1).max(120).safeParse(command.value).success
          )
            return reject("invalid_command", "Artifact title must be 1–120 characters");
  ...
  return {
    ...controller,
    observeArtifact(raw: import("@drawloom/workbench").ArtifactIntake) {
      return mutate(async () => {
        const input = ArtifactIntakeSchema.parse(raw);
        if (
          state.artifacts.some(
            (a) =>
              a.operationId === input.operationId &&
              a.content.kind === "asset" &&
              a.content.asset.key === input.asset.key,
          )
        )
          return;
    ...
    addText(text: string) {
      return mutate(async () => {
        ...
        next.artifacts.push({
          id: artifactId,
          editable: true,
          title: String(next.configuration[0]!.value),
          content: { kind: "text", text },
        });
        next.candidates.push({
          id: crypto.randomUUID(),
          comparisonKey: artifactId,
