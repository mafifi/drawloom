  ...
  return {
    read(value: object, since?: string): StateUpdate | undefined {
      ...
      const changes = current.filter(
        ([key, data]) => sections.get(key)?.serialized !== JSON.stringify(data),
      );
      const removed = [...sections]
        .filter(([key, section]) => section.serialized !== undefined && !keys.has(key))
        .map(([key]) => key);
      if (changes.length || removed.length) {
        revision++;
        for (const [key, data] of changes)
          sections.set(key, { serialized: JSON.stringify(data), revision });
      ...
      return {
        kind: valid ? "patch" : "snapshot",
        token: prefix + revision,
        sections: Object.fromEntries(
          current.filter(([key]) => !valid || sections.get(key)!.revision > position),
        ),
        removed: valid
          ? [...sections]
              .filter(
                ([, section]) => section.serialized === undefined && section.revision > position,
              )
              .map(([key]) => key)
          : [],
      };
