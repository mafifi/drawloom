...
import { createPackageAdministration } from "./package-administration.js";
import { createDiscoveryApplication } from "./discovery-application.js";
...
export async function createDesktopApplication(
  ...
  const packageAdministration = createPackageAdministration({
    selectedRuntime,
    retireRuntimes: (retire) => projectRuntimes.retire(retire),
    installations,
    oauth,
    pluginSettings,
    orchestration,
  ...
  const discoveryApplication = createDiscoveryApplication({
    ...
    observeCache,
    localRevision,
    ...(options.experimentalPluginDiscovery !== undefined
      ? { experimentalPluginDiscovery: options.experimentalPluginDiscovery }
      : {}),
  });
  const { discover } = discoveryApplication;
