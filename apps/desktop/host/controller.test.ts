...
test("overlapping review and native intake retain both committed changes", async () => {
  ...
  block = true;
  const review = controller.dispatch({
    kind: "review_candidate",
    candidateId: candidate.id,
    decision: "accepted",
    summary: "Keep this review",
  });
  await started;
  const intake = controller.observeArtifact({
...
test("concurrent text additions and duplicate intake retain every distinct item once", async () => {
  ...
  const result = await controller.snapshot();
  expect(
    result.artifacts.map((a) => (a.content.kind === "text" ? a.content.text : a.content.asset.key)),
  ).toEqual(["A", "B", "image-key"]);
...
test("text operator conformance and review persistence", async () => {
  ...
  expect((await (await createTextController(store)).snapshot()).reviews[0]?.summary).toBe(
    "Readable",
  );
