...
test("folder chooser requires the authenticated command channel and is only a selection", async () => {
  ...
  try {
    expect(
      (
        await fetch(host.origin + "/api/project-directory", {
          method: "POST",
    ...
    const r = await fetch(host.origin + "/api/project-directory", {
      method: "POST",
      headers: { cookie, origin: host.origin, "Content-Type": "application/json" },
      body: "{}",
    });
    expect(r.status).toBe(200);
    expect(await r.json()).toEqual({ directory: "/chosen/directory" });
    expect(calls).toBe(1);
    expect((await app.snapshot()).projects).toEqual([]);
  } finally {
    await host.close();
