...
import {
  ...
  type PluginBackend,
  type PluginBackendCapabilities,
  type PluginBackendContext,
  type PluginBackendDependency,
  type PluginBackendFactory,
...
export interface BackendActivationOptions
  extends Omit<PluginBackendContext, "packageRoot" | "dependencies"> {
  trusted: boolean;
  available: readonly PluginRequirement[];
}
const method = (value: unknown, key: string) =>
...
export function createBackendLoader() {
  ...
  async function start(
    ...
    options: BackendActivationOptions,
  ): Promise<BackendActivation> {
    ...
    try {
      ({ root, entry } = await executableExtensionPath(
        inventory.root,
        definition.backend.entrypoint,
      ));
    } catch {
      return { status: "failed", code: "backend_path_unavailable" };
    }
    ...
    const capabilities: PluginBackendCapabilities = {
      ...(requested.has("host") && options.capabilities.host
        ? { host: options.capabilities.host }
        : {}),
      ...(requested.has("tools") && options.capabilities.tools
        ? { tools: options.capabilities.tools }
        : {}),
      ...(requested.has("orchestration") && options.capabilities.orchestration
        ? { orchestration: options.capabilities.orchestration }
        : {}),
      ...(requested.has("evaluation") && options.capabilities.evaluation
        ? { evaluation: options.capabilities.evaluation }
        : {}),
      ...(requested.has("orchestration") && options.capabilities.orchestrationReadiness
        ? {
            orchestrationReadiness: async () =>
              OrchestrationReadinessSchema.parse(
                await options.capabilities.orchestrationReadiness!(),
              ),
          }
        : {}),
    };
    ...
    for (const item of declared) {
      ...
      if (item.kind === "tool" || item.kind === "skill")
        dependencies.set(
          `${item.kind}:${item.id}`,
          Object.freeze({ kind: item.kind, id: item.id, available }),
        );
      else if (item.id === "orchestration" || item.id === "evaluation")
        dependencies.set(
          `capability:${item.id}`,
          Object.freeze({ kind: "capability" as const, id: item.id, available }),
        );
    }
    ...
    try {
      ...
      const factory = z
        .object({ default: z.custom<PluginBackendFactory>((v) => typeof v === "function") })
      ...
      if (backend.servers !== undefined && !Array.isArray(backend.servers))
        throw Error("Invalid servers");
      ...
      for (const server of backend.servers ?? []) {
        if (
          ...
          !method(server.transport, "start") ||
          !method(server.transport, "send") ||
          !method(server.transport, "close")
        )
      ...
      if (backend && method(backend, "dispose")) {
        try {
          await backend.dispose();
        } catch {
          /* Preserve activation failure. */
        }
      }
  ...
  return {
    async activate(
      ...
      if (!promise) {
        promise = start(inventory, options);
    ...
    close(): Promise<void> {
      closing ??= (async () => {
        ...
        const closed = await Promise.allSettled(
          results.filter((r) => r.status === "ready").map(async (r) => r.backend.dispose()),
        );
        if (closed.some((r) => r.status === "rejected"))
          throw Error("Plugin backend cleanup failed");
      })();
      return closing;
    },
  };
