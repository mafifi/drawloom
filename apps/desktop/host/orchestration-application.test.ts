...
test("desktop attaches installed task handlers, restores both projects and enforces grants without conversations", async () => {
      ...
      const result =
        message.method === "initialize"
          ? {
              ...
              capabilities: { tools: {} },
              serverInfo: { name: "documents", version: "1" },
            }
  ...
  const manager: ReturnType<typeof createLocalTemporalManager> = {
    async prepare(owner) {
      ...
      savedProjects.add(owner.projectId);
      events.push("prepare:" + owner.projectId);
      return {
        ...
        close: async () => {
          events.push("registration-close");
        },
      };
    },
    prepareHost: async () => {
    ...
    listOwners: async () =>
      [...savedProjects].map((projectId) => ({
        ...
        entrypoint: "./org.drawloom/workflows.mjs",
        bundleFingerprint: "hash",
        owner: projectId,
      })),
    hasUnfinishedInstallation: async () => handlers.size > 0,
    close: async () => {
  ...
  await installed.configure(installationId, {
    enabled: true,
    trustedBackend: true,
    servers: ["remote"],
    configuration: {},
  ...
  try {
    ...
    for (const name of ["first", "second"]) {
      ...
      expect((await app.workflowOwners(snapshot.selectedProjectId!))[0]).toMatchObject({
        installationId,
        readiness: { status: "ready" },
      });
    }
    ...
    expect(await task.run({}, context)).toMatchObject({
    ...
    const concurrent = await Promise.all(
      Array.from({ length: 8 }, (_, i) =>
        task.run({}, { ...context, stepId: "parallel-" + i, attemptId: "parallel-" + i }),
      ),
    );
    expect(
      concurrent.every(
        (result) => (result as { outcome: { status: string } }).outcome.status === "ok",
      ),
    ).toBe(true);
    expect(calls).toBe(9);
    ...
    await expect(
      app.packageAction({
        action: "configure",
        id: installationId,
        settings: { enabled: false, trustedBackend: true, servers: [], configuration: {} },
      }),
