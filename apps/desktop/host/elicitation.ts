import {
  ...
  type ToolElicitationRequest,
  type ToolElicitationResult,
} from "@drawloom/tools";
...
export function createElicitationPresenter(ownerFor: (operationId: string) => string | undefined) {
  const pending = new Map<
    string,
    {
      ...
      finish: (result: ToolElicitationResult) => void;
    }
  >();
  const request: ToolElicitationHandler = (raw, signal) => {
    ...
    if (pending.has(input.requestId))
      return Promise.reject(Error("Duplicate elicitation identity"));
  ...
  return {
    ...
    pending: (conversationId: string) =>
      [...pending.values()]
        ...
        .map((p) => structuredClone(p.request)),
    resolve(conversationId: string, requestId: string, raw: unknown) {
      const entry = pending.get(requestId);
      if (
        !entry ||
        entry.conversationId !== conversationId ||
        ownerFor(entry.request.operationId) !== conversationId
      )
      ...
      if (result.action === "accept") {
        ...
        if (
          ...
          !result.content ||
          !new AjvJsonSchemaValidator().getValidator({ ...schema, type: "object" })(result.content)
            .valid
        )
