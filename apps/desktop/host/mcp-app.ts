...
export async function connectMcpApp(
  connection: { transport: Transport; toolName: string },
  uri: string,
): Promise<ConnectedMcpApp> {
  const client = new Client(
    { name: "drawloom-ui-host", version: "0.0.0" },
    {
      capabilities: {
...
export async function connectMcpAppClient(
  client: Client,
  toolName: string,
  uri: string,
  ownsClient = false,
  ...
  try {
    ...
    const html = resource.contents.find(
      (content) =>
        content.uri === uri && content.mimeType === RESOURCE_MIME_TYPE && "text" in content,
    );
    ...
    return {
      ...
      async listResources(cursor) {
        ...
        if (page.resources.length > 2000 || cursors.size >= 100)
          throw Error("Resource catalogue exceeds host limit");
        if (page.nextCursor && (page.nextCursor === cursor || cursors.has(page.nextCursor)))
          throw Error("Invalid resource pagination");
        if (page.nextCursor) cursors.add(page.nextCursor);
        const resources = page.resources
          .filter((r) => r.uri !== uri && r.mimeType !== RESOURCE_MIME_TYPE)
          .map((r) => ({
            uri: r.uri,
            name: r.name,
      ...
      async readResource(value, previouslyAdvertised = false) {
        ...
        if (!resourceSupport || (!readable.has(value) && !previouslyAdvertised))
      ...
      async callTool(raw) {
        ...
        const result = await observed("mcp.request", { "rpc.method": "tools/call" }, async () => {
          ...
          const {
            traceparent: _ignored,
            tracestate: _state,
            baggage: _baggage,
            ...metadata
          } = params._meta ?? {};
          const value = CallToolResultSchema.parse(
