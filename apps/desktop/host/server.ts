...
export function serveDesktop(
  ...
  webRoot: string,
  port = 0,
  telemetry?: ReturnType<typeof createTelemetryRelay>,
  options: { pickDirectory?: (signal: AbortSignal) => Promise<string | undefined> } = {},
) {
  ...
  let viewFiles:
    | { token: string; mountId: string; conversationId: string; viewId: string }
    | undefined;
  async function workingResponse(
    request: Request,
  ...
  const secure = {
    ...
    "Content-Security-Policy":
      "default-src 'self'; script-src 'self'; style-src 'self' 'unsafe-inline'; img-src 'self' blob:; media-src 'self' blob:; connect-src 'self'; frame-src 'self'; object-src 'none'; base-uri 'none'; frame-ancestors 'none'",
  };
