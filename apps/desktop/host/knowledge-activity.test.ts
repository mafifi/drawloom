...
const run: RunSnapshot = {
  ...
  steps: [
    {
      stepId: "run/assess-50",
      status: "completed",
      attempts: 1,
      result: { secret: "raw-evidence" },
    },
  ],
  pendingInputs: [],
  childRunIds: [],
...
test("fixed host activity is bounded, content-free and read-only, including held terminal runs", async () => {
  ...
  const provider: Orchestrator = {
    ...
    respond: async () => {
      throw Error("must not respond");
    },
    list: async (input) => {
      calls.push(input);
      return { runs: [run], cursor: "next" };
    },
    get: async (id) => {
      if (id !== "run") throw Error("secret unknown run");
      return run;
    },
    getSteps: async (id, input) => {
      calls.push({ id, ...input });
      return { steps: run.steps, cursor: "next-step" };
    },
    result: async () => ({ kind: "deferred", processed: 0, remaining: true }),
  };
  const activity = createKnowledgeActivity(() => ({
    orchestrator: provider,
    readiness: () => ({ status: "ready" }),
  }));
  expect(await activity.owner()).toMatchObject({
  ...
  for (const forged of [
    ...
    await expect(activity.list(forged)).rejects.toThrow();
  ...
  expect(await activity.detail({ runId: "run" })).toMatchObject({
    displayStatus: "Needs attention",
  });
  expect(await activity.steps({ runId: "run", limit: 1, cursor: "steps" })).toEqual({
    steps: [{ stepId: "run/assess-50", status: "completed", attempts: 1 }],
    cursor: "next-step",
  });
  await expect(activity.detail({ runId: "forged" })).rejects.toThrow(
    "Knowledge maintenance run is unavailable",
  );
  await expect(activity.steps({ runId: "forged" })).rejects.toThrow(
    "Knowledge maintenance run is unavailable",
  );
