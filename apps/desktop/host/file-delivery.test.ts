...
test("a stalled upload cannot block navigation and cancellation leaves no registered partial asset", async () => {
  ...
  const project = await app.command({ kind: "add_project", directory: join(root, "working") });
  ...
  const body = new ReadableStream<Uint8Array>({
    async start(stream) {
      stream.enqueue(new Uint8Array(65536));
      await gate;
      try {
        stream.close();
      } catch {}
  ...
  try {
    ...
    const upload = fetch(
      ...
      {
        method: "POST",
        headers: { cookie, origin: host.origin, "Content-Type": "application/octet-stream" },
        body,
    ...
    const navigation = await fetch(host.origin + "/api/command", {
      ...
      body: JSON.stringify({ kind: "select_project", projectId: project.selectedProjectId }),
...
test("authenticated working files stream ranges without importing, and reject directory escapes", async () => {
  ...
  const file = await open(join(working, "large.mp4"), "w");
  await file.truncate(32 * 1024 * 1024);
  await file.write(new Uint8Array([1, 2, 3, 4]), 0, 4, 100);
  await file.close();
  const app = await createDesktopApplication(join(root, "data"));
  ...
  try {
    ...
    const url =
      host.origin +
      "/api/files?" +
      new URLSearchParams({ conversationId: state.selectedId, path: "large.mp4" });
    expect((await fetch(url)).status).toBe(401);
    const r = await fetch(url, { headers: { cookie, range: "bytes=100-103" } });
    expect(r.status).toBe(206);
    expect(r.headers.get("content-range")).toBe("bytes 100-103/33554432");
    expect(new Uint8Array(await r.arrayBuffer())).toEqual(new Uint8Array([1, 2, 3, 4]));
    ...
    for (const path of ["../outside.txt", join(root, "outside.txt"), "escape.txt"]) {
      const denied = await fetch(
        host.origin +
          "/api/files?" +
          new URLSearchParams({ conversationId: state.selectedId, path }),
        { headers: { cookie } },
      );
...
test("17 MiB browser media imports stream, preserve identity and serve a suffix range", async () => {
  const root = await mkdtemp(join(tmpdir(), "drawloom-upload-"));
  await mkdir(join(root, "working"));
  const app = await createDesktopApplication(join(root, "data"));
  ...
  try {
    const boot = await fetch(host.url, { redirect: "manual" });
    const cookie = boot.headers.get("set-cookie")!.split(";")[0]!;
    const chunk = new Uint8Array(1024 * 1024).fill(4);
    const body = new Blob(Array.from({ length: 17 }, () => chunk));
    const r = await fetch(
      host.origin +
        "/api/import?" +
        new URLSearchParams({
    ...
    const asset = (await r.json()) as { key: string; size: number };
    expect({ status: r.status, asset }).toMatchObject({
      status: 200,
      asset: { size: 17 * 1024 * 1024 },
    });
    const tail = await fetch(host.origin + "/api/assets/" + asset.key, {
      headers: { cookie, range: "bytes=-5" },
    });
    expect(tail.status).toBe(206);
    expect(new Uint8Array(await tail.arrayBuffer())).toEqual(new Uint8Array(5).fill(4));
  } finally {
    await host.close();
    await rm(root, { recursive: true, force: true });
  }
