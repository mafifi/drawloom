...
test("failed display capture retries retained evidence, not execution, and restart keeps the same identity", async () => {
  const result: ToolResult = {
    ...
    outcome: {
      ...
      value: {},
      text: "Done",
      content: [{ type: "resource_link", uri: "doc://one", name: "One" }],
    },
  };
  let attempts = 0,
    fail = true;
...
test("cached history recovers missed gateway resources from durable evidence without a provider", async () => {
    ...
    result: {
      ...
      outcome: {
        ...
        content: [
          {
            type: "resource",
            resource: { uri: "doc://retained", text: "Durable reference", mimeType: "text/plain" },
          },
        ],
      },
    },
  ...
  app = await createDesktopApplication(root);
  app.assets.put = async () => {
    throw Error("Must use cached capture");
  };
  try {
