...
export function projectRun(run: RunSnapshot) {
  ...
  return WorkflowRunSchema.parse({
    ...summary,
    steps: run.steps.map(({ result: _result, ...step }) => step),
  });
}

/** Internal desktop reads: owner resolution remains the host's responsibility. */
export function createOrchestrationPresentation(
  resolve: (scope: { projectId: string; installationId: string }) => Promise<Orchestrator>,
) {
  return {
    ...
    async steps(raw: unknown) {
      ...
      const provider = await resolve(input);
      const page = await provider.getSteps(input.runId, {
        limit: input.limit,
        ...(input.cursor ? { cursor: input.cursor } : {}),
      });
    ...
    async command(raw: unknown) {
      ...
      const run = await provider.get(input.runId).catch(() => {
        throw new WorkflowControlError(
          "The command was accepted, but updated run details are unavailable. Refresh to check its state.",
        );
      });
