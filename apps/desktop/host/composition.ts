...
export const toolOutcomeProjectors: ReadonlyMap<string, ToolOutcomeProjector> = new Map([
  [
    "text.word_count",
    {
      id: "synthetic.text:word-count@1",
...
export function mcpReviewConfiguration(exposure: ToolExposure) {
  return {
    ...
    tools: Object.fromEntries(
      ToolExposureSchema.parse(exposure).tools.map((tool) => [
        tool.name,
        { approval_mode: tool.annotations?.readOnlyHint === true ? "approve" : "prompt" },
      ]),
    ),
  };
}
  ...
  contribute: () => ({
    tools: [
      defineTool({
        ...
        execute: ({ text }) => ({ count: text.trim() ? text.trim().split(/\s+/u).length : 0 }),
      }),
    ],
    skills: [
      {
        id: "text.clear-writing",
        title: "Clear writing",
        description: "Help make a draft shorter and easier to read.",
        instructions: "Use short sentences and plain language. Preserve the author’s meaning.",
      },
    ],
    workbenches: [
      {
        id: "text",
        title: "Text studio",
        description: "Local synthetic text inspection",
