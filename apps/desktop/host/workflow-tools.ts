...
export const workflowEvidenceKey = (
  ...
  "workflow:" +
  createHash("sha256")
    .update(JSON.stringify([installationId, runId, stepId, attemptId]))
    .digest("hex");
...
export function createWorkflowToolScope(options: {
  ...
  function owner(operationId?: string) {
    ...
    return current &&
      ...
      current.installationId === options.installationId &&
      current.runId === operationId
      ? current
      : undefined;
  }
  return {
    ...
    async record(record: ToolEvidence) {
      ...
      if (!sink) {
        sink = createDesktopEvidence(options.store, key);
        evidence.set(key, sink);
      }
