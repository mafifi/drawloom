...
export async function desktopModels() {
  ...
  try {
    return await Promise.race([
      (async()=>{await rpc.request('initialize',{clientInfo:{name:'drawloom-models',version:'0.0.0'},capabilities:{}});rpc.notify('initialized');return readCodexModels(rpc);})(),
      new Promise<never>((_,reject)=>{timer=setTimeout(()=>reject(Error('Model discovery timed out')),8000);}),
    ]);
