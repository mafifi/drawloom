...
const attribute = z.discriminatedUnion("key", [
  ...
  z.object({
    key: z.literal("http.response.status_code"),
    value: z.object({
      intValue: z
        .union([z.number().int(), z.string().regex(/^\d{3}$/)])
        .transform(Number)
...
const envelope = z.object({
  resourceSpans: z
    .array(
      z.object({
        scopeSpans: z.array(z.object({ spans: z.array(span).min(1).max(32) })).length(1),
      }),
    )
    .length(1),
});
export function createTelemetryRelay(mode: "disabled" | "recording" | "export", endpoint?: string) {
  ...
  if (endpoint) {
    ...
    if (
      ...
      throw Error("Invalid local telemetry endpoint");
  ...
  const drop = (count = 1) => {
    dropped += count;
    metrics.getMeter("drawloom.desktop").createCounter("telemetry.dropped").add(count);
  };
  return {
    ...
    async handle(request: Request): Promise<Response> {
      ...
      try {
        ...
        const spans = input.resourceSpans[0]!.scopeSpans[0]!.spans.map((s) => ({
          ...s,
          kind: 3,
          flags: 1,
        }));
        const output = {
          resourceSpans: [
            {
              resource: {
                attributes: [{ key: "service.name", value: { stringValue: "drawloom.ui" } }],
              },
              scopeSpans: [{ scope: { name: "drawloom.ui" }, spans }],
            },
          ],
        };
