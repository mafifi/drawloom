    ...
    connect: async () => ({
      ...
      async request(method, raw) {
        ...
        if (method === "thread/turns/list") {
          ...
          const turns = [
            ...(later
              ? Array.from({ length: 250 }, (_, i) => ({ id: `new-${i}`, status: "completed" }))
              : []),
            { id: "old", status: later ? "completed" : "inProgress" },
          ];
  ...
  try {
    ...
    do {
      ...
      for (const entry of page.entries)
        if (entry.id !== old.id)
          newEntriesFollowOld &&=
            entry.position[0] > old.position[0] ||
            (entry.position[0] === old.position[0] && entry.position[1] > old.position[1]);
