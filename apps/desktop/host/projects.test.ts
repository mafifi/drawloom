...
test("directory binding rejects internal state, parents, roots and escaping replacements", async () => {
  ...
  try {
    const internal = join(root, "private");
...
test("aliases bind to the same canonical directory; missing directories fail without creating them", async () => {
  ...
  try {
    ...
    expect(await bindProjectDirectory(join(root, "alias"), join(root, "private"))).toEqual(
      await bindProjectDirectory(join(root, "working"), join(root, "private")),
    );
    await expect(
      bindProjectDirectory(join(root, "absent"), join(root, "private")),
    ).rejects.toBeInstanceOf(ProjectDirectoryError);
    await expect(bindProjectDirectory(join(root, "absent"), join(root, "private"))).rejects.toThrow(
      "unavailable",
    );
  } finally {
    await rm(root, { recursive: true, force: true });
