...
test("standard server projection preserves wire names, app visibility, validation and grants", async () => {
  ...
  server.registerTool("edit", { inputSchema: { text: z.string() } }, async ({ text }) => {
    ...
    return {
      content: [
        { type: "text", text },
        { type: "resource_link", uri: "reference://passage/1", name: "Passage" },
      ],
    };
  });
  server.registerTool(
    ...
    async () => {
      throw Error("App-only must not be exposed");
    },
  );
  ...
  try {
    ...
    expect(
      (await gateway.invoke(binding, first.tools[0]!.name, { text: "hello" }, signal)).outcome
        .status,
    ).toBe("failed");
    expect(calls).toBe(0);
    allowed = true;
    expect(
      (await gateway.invoke(binding, first.tools[0]!.name, { text: 7 }, signal)).outcome.status,
    ).toBe("failed");
    expect(calls).toBe(0);
    const result = await gateway.invoke(binding, first.tools[0]!.name, { text: "hello" }, signal);
    expect(result.outcome.status).toBe("ok");
    if (result.outcome.status === "ok")
      expect(result.outcome.content?.[1]?.type).toBe("resource_link");
    expect(calls).toBe(1);
  } finally {
    await client.close();
    await server.close();
