...
test("returned and listed resources stay source-bound and cached across restart", async () => {
  ...
  try {
    await expect(
      app.openListedResource(conversationId, viewId, "file:///etc/passwd"),
    ).rejects.toThrow();
    const returned = await app.viewRequest({
      conversationId,
      viewId,
      request: { name: "example.reference", arguments: {} },
    });
    ...
    expect(new TextDecoder().decode(await app.assets.read(ready.asset!.key))).toBe(
      "Use simple words.",
    );
    await app.close();
    app = await openExample(root);
    ...
    expect(
      (await app.openListedResource(conversationId, viewId, "document://example/guide")).id,
    ).toBe(listed.id);
    ...
    const scoped = createNodeJsonStore(
      join(root, "projects", (await app.snapshot()).selectedProjectId!, "state"),
    );
...
test("a returning project directory activates its installed workbenches after offline display", async () => {
  ...
  app = await openExample(root);
  try {
    const offline = await app.snapshot();
    expect(offline.projects[0]!.available).toBe(false);
    expect(offline.workbenches.some((w) => w.id === "example")).toBe(false);
    await rename(working + "-offline", working);
    const online = await app.snapshot();
    expect(online.projects[0]!.available).toBe(true);
...
test("a stale app-resource read is rejected before contacting the source", async () => {
  ...
  try {
    ...
    await app.viewRequest({
      conversationId: original.selectedId,
      viewId: "example.view",
      request: { name: "example.reference", arguments: {} },
    });
    ...
    await expect(
      app.readResource(original.selectedId, entry.id, entry.resources![0]!.id),
    ).rejects.toThrow();
    ...
    expect(
      await store.get(JSON.stringify(["plugin", installation.id, "example.reads"])),
    ).toBeUndefined();
  } finally {
    await app.close();
  }
...
test("MCP view data stays with its active owner and persists without an OperatorController", async () => {
  ...
  try {
    expect(
      (await app.viewRequest({ ...target, request: { name: "example.open", arguments: {} } }))
        .structuredContent,
    ).toEqual({ choice: "first" });
    expect(
      (
        await app.viewRequest({
          ...target,
          request: { name: "example.inspect", arguments: { choice: "second" } },
        })
      ).structuredContent,
    ).toEqual({ choice: "second" });
    await expect(
      app.viewRequest({ ...target, viewId: "other", request: { name: "example.open" } }),
    ).rejects.toThrow();
    await expect(
      app.viewRequest({ ...target, request: { name: "review_candidate" } }),
    ).rejects.toThrow();
    await app.command({ kind: "select_conversation", conversationId: previous });
    await expect(
      app.viewRequest({ ...target, request: { name: "example.open" } }),
    ).rejects.toThrow();
  ...
  try {
    const reply = await reopened.viewRequest({
      ...target,
      request: { name: "example.open", arguments: {} },
    });
    expect(reply.structuredContent).toEqual({ choice: "second" });
  } finally {
...
test("MCP HTML and calls require authenticated parent channel and retain sandbox restrictions", async () => {
  ...
  try {
    ...
    expect(((await reply.json()) as { structuredContent: unknown }).structuredContent).toEqual({
      choice: "first",
    });
  } finally {
    await server.close();
...
test("context updates do not start an agent and stale views cannot send conversation requests", async () => {
  ...
  const target = {
    ...routing,
    mountId: (await app.viewSession({ ...routing, action: "open" })).mountId,
  };
  try {
    const before = await app.snapshot();
    expect(
      await app.viewInteraction({
        ...target,
        request: {
          method: "ui/update-model-context",
          params: { content: [{ type: "text", text: "A selected revision" }] },
        },
      }),
    ).toEqual({});
    ...
    expect(
      await app.viewInteraction({
        ...
        request: {
          method: "ui/message",
          params: { role: "user", content: [{ type: "text", text: "Revise it" }] },
        },
      }),
    ...
    await expect(
      app.viewInteraction({
        ...target,
        request: { method: "ui/update-model-context", params: {} },
      }),
    ).rejects.toThrow();
    expect(
      await app.viewInteraction({
    ...
    await expect(
      app.viewInteraction({
        ...
        request: {
          method: "ui/message",
          params: { role: "user", content: [{ type: "text", text: "Wrong conversation" }] },
        },
      }),
    ).rejects.toThrow();
    expect((await app.historyPage(before.selectedId)).entries).toEqual([]);
...
test("opaque MCP frame receives a project-only media URL revoked on navigation", async () => {
  ...
  try {
    ...
    const response = await fetch(
      host.origin + "/api/views/example.view?conversationId=" + state.selectedId,
      { headers: { cookie } },
    );
    ...
    expect(await file.text()).toBe("public");
    expect((await fetch(base + "..%2Foutside.txt", { headers: { origin: "null" } })).status).toBe(
      403,
    );
