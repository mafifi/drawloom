...
test("controllerless installed evaluation tasks run while protected tools remain denied", async () => {
  ...
  const remote = Bun.serve({ hostname: "127.0.0.1", port: 0, async fetch(request) {
    ...
    const result = message.method === "initialize"
      ? { protocolVersion: "2025-03-26", capabilities: { tools: {} }, serverInfo: { name: "remote", version: "1" } }
  ...
  const manager: ReturnType<typeof createLocalTemporalManager> = {
    async prepare(owner) {
      return {
        ...
        orchestrator: {
          start: async () => "unused", get: async () => { throw Error("unused"); },
          getSteps: async () => ({ steps: [] }), list: async () => ({ runs: [] }),
          respond: async () => {}, cancel: async () => {}, result: async () => ({}),
        },
        attach: async values => { handlers.set(owner.projectId, values); },
        close: async () => {},
      };
    },
    prepareHost: async () => { throw Error("unused"); },
    listHostOwners: async () => [], listOwners: async () => [],
    hasUnfinishedInstallation: async () => false, close: async () => {},
  };
  try {
    await writeFile(join(packageRoot, "plugin.json"), JSON.stringify({
      ...
      extensions: { "io.github.mafifi.drawloom": {
        ...
        workbenches: [{ id: "evaluation", title: "Evaluation", openingTool: { server: "remote", tool: "inspect" } }],
      } },
    }));
    ...
    await installations.configure(installationId, { enabled: true, trustedBackend: true, servers: ["remote"], configuration: {} });
    const app = await createDesktopApplication(root, {
      evaluation: { assessment: { assess: (scorer, args, context) => scorer.score(args, context) } },
    ...
    try {
      ...
      const context = { runId: "evaluation-run", stepId: "plan", attemptId: "attempt", attempt: 1, taskVersion: "1", signal: new AbortController().signal };
      const plan = projectHandlers!.find(handler => handler.id === "evaluation.plan");
      expect(await plan!.run({}, context)).toEqual({ status: "planned" });
      const protectedTask = projectHandlers!.find(handler => handler.id === "evaluation.protected");
      expect(await protectedTask!.run({}, { ...context, stepId: "protected", attemptId: "protected" }))
