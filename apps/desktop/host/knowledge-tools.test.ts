...
test("evidence bodies are omitted only after exact same-execution delivery and every hit reauthorizes", async () => {
            ...
            records: [
              {
                ...
                provenance: { producer: { type: "test", id: "manual" }, inputs: [] },
              },
            ],
            links: [],
            bytes: 100,
  ...
  const tool = createKnowledgePlugin(evidenceService, receipts)
    .plugin.prepare({})()
    .tools!.find((tool) => tool.name === "knowledge.evidence")!;
  const request = {
    root: ref,
    direction: "forward",
    maxDepth: 2,
  ...
  let invocation = 0;
  const read = (operationId = "execution", input = request) =>
    tool.execute(tool.parseInput(input), {
      operationId,
  ...
  expect(reads).toBe(3);
  denied = true;
  expect(await read()).toEqual({ kind: "denied" });
  denied = false;
  body = "Changed content";
  expect(await read()).toMatchObject({ kind: "ok" });
  expect(await read("another-execution")).toMatchObject({ kind: "ok" });
  expect(await read("execution", { ...request, direction: "reverse" })).toMatchObject({
    kind: "ok",
  });
...
test("knowledge retrieval tools expose bounded contract inputs without subject or provider controls", async () => {
  ...
  expect(calls).toEqual([
    { query: "global knowledge", mode: "best_available", limit: 5, maxBytes: 4096 },
  ]);
  expect(() =>
    search.parseInput({
      query: "x",
...
test("knowledge contribution creates only a host-owned record for the invocation", async () => {
  ...
  expect(contribute.annotations).toMatchObject({
    readOnlyHint: false,
    destructiveHint: false,
    openWorldHint: false,
  });
  const input = contribute.parseInput({ body: "A bounded deliberate note.", kind: "claim" });
  await contribute.execute(input, {
    invocationId: "contribution-1",
  ...
  expect(calls.at(-1)).toMatchObject({
    ...
    record: {
      ...
      body: "A bounded deliberate note.",
      status: "active",
      freshness: "current",
      provenance: {
...
test("automatic observations cover safe registered outcomes and exclude internal or unexecuted calls", () => {
  const projected = knowledgeObservation({
    toolName: "text.word_count",
  ...
  expect(projected).toMatchObject({
    ...
    expectedRevision: null,
    record: {
      ref: { type: "observation", origin: "host-tool", id: "invocation-1" },
      status: "active",
    },
  });
  ...
  expect(
    knowledgeObservation({
      toolName: "package-tool-1",
      registeredName: "files.inspect",
      producerOrigin: "package:installed:server",
      invocationId: "invocation-2",
      outcome: { status: "failed", code: "handler_failed", execution: "completed" },
    }),
  ).toMatchObject({
    record: {
      body: "A registered tool operation failed after execution began.",
      confidence: { operationStatus: "failed", contentCaptured: false },
    },
  });
  expect(
    knowledgeObservation({
  ...
  expect(
    knowledgeObservation({
      toolName: "package-tool-3",
      invocationId: "invocation-5",
      outcome: { status: "failed", code: "cancelled", execution: "unknown" },
    }),
  ).toBeUndefined();
  expect(
    knowledgeObservation({
      toolName: "package-tool-4",
      invocationId: "invocation-6",
      outcome: { status: "failed", code: "handler_failed", execution: "unknown" },
    }),
  ).toMatchObject({
    record: {
      body: "A registered tool operation ended with an unknown execution outcome.",
      confidence: { operationStatus: "unknown" },
    },
  });
});
