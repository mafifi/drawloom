...
export function createDesktopAssets(root: string) {
  ...
  async function putStream(
    ...
    if (rootInfo.isSymbolicLink() || !rootInfo.isDirectory())
      throw Error("Asset storage unavailable");
  ...
  return {
    ...
    async imageInput(asset: Asset) {
      ...
      if (!["image/png", "image/jpeg", "image/webp", "image/gif"].includes(asset.mediaType))
        throw Error("Codex accepts image attachments; other files remain viewable artifacts");
      const bytes = await store.read(asset.key);
      if (bytes.length !== asset.size || !/^[a-f0-9]{64}$/.test(asset.key))
    ...
    async captureImage(result: string) {
      ...
      if (!/^[A-Za-z0-9+/]+={0,2}$/.test(base64) || base64.length > 24 * 1024 * 1024)
        throw Error("Invalid image result");
      const bytes = Buffer.from(base64, "base64");
      if (bytes.length > nativeImageByteLimit) throw Error("Invalid image result");
      if (!bytes.subarray(0, 8).equals(Buffer.from([137, 80, 78, 71, 13, 10, 26, 10])))
        throw Error("Invalid PNG result");
