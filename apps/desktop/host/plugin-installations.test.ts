  ...
  try {
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        mcpServers: {
          render: { type: "stdio", command: "must-not-execute" },
          consent: { type: "stdio", command: "must-not-execute" },
        },
      }),
    );
    const store = createNodeJsonStore(join(root, "state"));
    ...
    for (const names of [["unknown"], ["render", "render"]]) {
      await expect(
        current.configure(id, { ...settings, elicitationDisabledServers: names }),
      ).rejects.toThrow();
    }
    await expect(
      current.configure(id, {
        ...settings,
        servers: ["consent"],
        elicitationDisabledServers: ["render"],
      }),
    ).rejects.toThrow();
  ...
  try {
    ...
    await writeFile(
      ...
      JSON.stringify({
        $schema: "https://agent-plugins.org/schemas/1.0.0/mcp.schema.json",
        mcpServers: {
          one: { type: "stdio", command: "must-not-execute" },
          two: { type: "stdio", command: "must-not-execute" },
        },
      }),
    );
    ...
    try {
      ...
      await app.installations.configure(id, {
        enabled: false,
        trustedBackend: false,
        servers: ["one"],
        configuration: {},
