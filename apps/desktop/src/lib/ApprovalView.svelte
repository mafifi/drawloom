<script lang="ts">
  ...
  import { Alert, Button, Collapsible, StatefulButton } from "@drawloom/ui";
  ...
  <Alert.Description>
    ...
    {#if p.details}
      <Collapsible.Root>
        ...
        <Collapsible.Content><pre>{p.details}</pre></Collapsible.Content>
      </Collapsible.Root>
    {/if}
    <p role="status" aria-live="polite">{p.message}</p>
    <div class="flex flex-wrap gap-2">
      {#each p.options as option (option.optionId)}
        <StatefulButton variant="outline" disabled={p.disabled} pending={option.pending} onclick={() => a.choose(option.optionId)}>{option.label}</StatefulButton>
      {/each}
