<script lang="ts">
  ...
  const options=$derived.by(()=>{
    ...
    if(vm.pickerKind!=='skill' && vm.state?.goal?.supported && !vm.state.goal.snapshot && (!vm.pickerQuery || 'create goal'.includes(vm.pickerQuery.toLowerCase()))) result.push({id:'action:goal',title:'Create goal',description:'Set an objective to keep pursuing',group:vm.pickerKind==='action'?'Actions':'Add',kind:'action'});
    if(vm.pickerKind!=='skill' && vm.state?.modes.includes('plan') && (!vm.pickerQuery || 'plan mode'.includes(vm.pickerQuery.toLowerCase()))) result.push({id:'action:plan',title:'Plan mode',description:'Discuss an approach before implementation',group:'Actions',kind:'action',disabled:!!vm.state.activeOperation});
    ...
    const order=['Add','Actions','Plugins','Skills','Files','Conversations',''];
...
<MentionPicker open={vm.pickerOpen} {anchor} {input} {options} activeId={vm.pickerActiveId} label={vm.pickerKind==='skill'?'Skills':vm.pickerKind==='action'?'Actions and skills':'Add context'} status={vm.cataloguePending?'Loading…':vm.catalogueError?'Discovery unavailable. Refresh in Plugins.':''} onOpenChange={open=>vm.pickerOpen=open} onActiveChange={id=>vm.pickerActiveId=id} onSelect={select}/>
