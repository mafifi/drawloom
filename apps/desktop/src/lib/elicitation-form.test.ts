...
test("standard form fields retain scalar types, multiselect values and omitted optional fields", () => {
  const params: ToolElicitationRequest["params"] = {
    message: "Stationery options",
    requestedSchema: {
      type: "object",
      properties: {
        paper: { type: "string", oneOf: [{ const: "plain", title: "Plain paper" }] },
        copies: { type: "integer" },
        recycled: { type: "boolean" },
        colours: { type: "array", items: { type: "string", enum: ["blue", "red"] } },
        note: { type: "string" },
      },
  ...
  form.set("paper", "plain");
  form.set("copies", "2");
  form.append("colours", "blue");
  form.append("colours", "red");
  form.set("note", "");
  expect(elicitationContent(params, form)).toEqual({
    paper: "plain",
    copies: 2,
    recycled: false,
    colours: ["blue", "red"],
