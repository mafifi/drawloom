...
test("sharing a conversation selects context without navigation or execution", async () => {
  ...
  expect(h.commands).toEqual([]);
  await h.vm.send();
  expect(h.commands.find((c) => c.kind === "send")).toMatchObject({
    conversationContextIds: ["conversation-b"],
  });
  expect(h.vm.conversationContextIds).toEqual([]);
});
test("settings returns to the prior destination without losing a draft", async () => {
  const h = await harness();
  h.vm.primaryView = "activity";
  h.vm.draft = "Keep this draft";
  h.vm.primaryView = "settings";
  h.vm.settingsSection = "media";
  h.vm.closeSettings();
  expect(String(h.vm.primaryView)).toBe("activity");
  expect(h.vm.draft).toBe("Keep this draft");
});
...
test("opening shared working material explicitly leaves the installed app view", async () => {
  ...
  const resource = {
    ...
    asset: { key: "image-key", mediaType: "image/png", size: 1 },
  };
...
test("workspace resources resolve current history identity and never retain removed media URLs", async () => {
  ...
  const first = {
    ...
    uri: "https://media.example/first.mp4",
    mimeType: "video/mp4",
  };
  h.setHistory([
    {
  ...
  await h.vm.loadLatest();
  expect(h.vm.workspaceResource?.resource).toMatchObject({
    id: "resource",
    status: "unavailable",
...
test("a successful resource read override is bound to the captured history revision", async () => {
  ...
  const changed = {
    ...
    uri: "https://media.example/changed.txt",
    mimeType: "text/plain",
  };
  h.setHistory([
    {
...
test("attachment labels recover from existing artifact metadata after reconnect", async () => {
  const initial = snapshot();
  initial.operator.artifacts.push({
    id: "imported",
    title: "research-notes.txt",
    content: { kind: "asset", asset: { key: "research-key", size: 24, mediaType: "text/plain" } },
  });
  const h = await harness(initial);
...
test("project actions retain exact identities and missing directories block only new execution", async () => {
  const initial = snapshot();
  initial.projects[0]!.available = false;
...
test("workbench readiness never crosses projects that share the same workbench id", async () => {
  ...
  const otherProjectSelected = structuredClone(initial);
  otherProjectSelected.selectedProjectId = "project-b";
  h.setState(otherProjectSelected);
  await h.vm.start();
  h.vm.stopPolling();
...
test("cancelled or unavailable native selection leaves path entry available", async () => {
  ...
  globalThis.fetch = (async (url, init) =>
    url === "/api/project-directory" ? result : previous(url, init)) as typeof fetch;
...
test("legacy history stays readable and project assignment binds the displayed conversation", async () => {
  const initial = snapshot();
  delete initial.conversations[0]!.projectId;
  const h = await harness(initial);
  expect(h.vm.canSend).toBe(false);
  expect(h.historyRequests.length).toBeGreaterThan(0);
...
test("working-file presentation binds to the conversation project, not the selected project", async () => {
  ...
  const reference = {
    ...
    source: "native",
    status: "unavailable" as const,
    uri: "file:///work/a/clip.mp4",
    mimeType: "video/mp4",
  };
  expect(h.vm.workingFile(reference)?.url).toBe(
    "/api/files?conversationId=conversation-a&path=clip.mp4",
  );
  expect(h.vm.workingFile({ ...reference, uri: "file:///work/b/clip.mp4" })).toBeUndefined();
  const unavailable = snapshot();
  unavailable.projects[0]!.available = false;
  h.setState(unavailable);
  await h.vm.select("conversation-a");
...
test("provider sign-in uses a catalogue identity and discards a late URL after navigation", async () => {
  ...
  let release!: (value: Response) => void;
  globalThis.fetch = (async (url, init) => {
    if (url === "/api/discovery/authenticate") {
  ...
  await h.vm.authenticateIntegration("invented");
  expect(requests).toHaveLength(0);
  const pending = h.vm.authenticateIntegration("codex:integration:a");
  expect(requests).toEqual([
    { conversationId: "conversation-a", id: "codex:integration:a", revision: "r1" },
  ]);
  h.setState({
    ...snapshot(),
    selectedId: "conversation-b",
    conversations: [
      ...snapshot().conversations,
      { ...snapshot().conversations[0]!, id: "conversation-b" },
    ],
  });
  await h.vm.select("conversation-b");
  release(Response.json({ authorizationUrl: "https://auth.example/authorize" }));
  await pending;
...
function snapshot(): DesktopSnapshot {
  return {
    ...
    projects: [{ id: "project-a", name: "Project A", directory: "/work/a", available: true }],
      ...
      artifacts: [
        {
          ...
          editable: true,
          title: "A",
          content: { kind: "text", text: "Original A" },
        },
      ],
      candidates: [
        {
          id: "candidate-a",
          comparisonKey: "document-a",
          label: "A",
...
async function harness(initial = snapshot()) {
  ...
  let catalogue: DesktopCatalogue = {
    entries: [],
    categories: [],
    experimentalPluginDiscovery: false,
  };
  ...
  globalThis.fetch = (async (url: string | URL | Request, options?: RequestInit) => {
    if (String(url).startsWith("/api/discovery?")) {
      discoveryRequests.push(String(url));
      return discoveryResponse ? discoveryResponse() : Response.json(catalogue);
    }
    if (String(url).startsWith("/api/history")) {
      ...
      return Response.json({
        ...
        hasOlder: false,
        changeCursor: "c1",
        status: { revision: 0, sync: "idle", hasOlder: false },
      });
    }
    if (url === "/api/resource/read") {
      resourceRequests.push(JSON.parse(String(options?.body)));
      return Response.json({
        id: "r1",
        source: "source-a",
        title: "Document",
        status: "ready",
        asset: { key: "text-asset", mediaType: "text/plain", size: 1 },
      });
    }
    if (String(url).startsWith("/api/resources")) {
      resourceRequests.push(String(url));
      return Response.json({ resources: [{ uri: "doc://one", name: "One" }], nextCursor: "next" });
    }
    if (url === "/api/resource/open") {
      resourceRequests.push(JSON.parse(String(options?.body)));
      return Response.json({
        id: "opened",
        position: [1, 0],
    ...
    if (url === "/api/discovery/resource/read") {
      resourceRequests.push(JSON.parse(String(options?.body)));
      return Response.json({
        id: "native-opened",
        position: [2, 0],
    ...
    if (String(url).startsWith("/api/import")) {
      ...
      return importResponse
        ? importResponse()
        : Response.json({ key: "new-image", mediaType: "image/png", size: 1 });
    }
    const state = stateResponse ? await stateResponse() : Response.json(current);
    return Response.json({
      kind: "snapshot",
      token: "host:1",
      sections: await state.json(),
      removed: [],
    });
  }) as typeof fetch;
  const vm = createDesktopViewModel();
  ...
  return {
    ...
    historyRequests,
    resourceRequests,
    setHistory(value: HistoryEntry[]) {
      historyEntries = value;
    },
    setCatalogue(value: DesktopCatalogue) {
      catalogue = value;
    },
    deferDiscovery() {
      ...
      return (response = Response.json(catalogue)) => resolve(response);
    },
    setState(value: DesktopSnapshot) {
      current = value;
    },
    deferCommand() {
      let resolve!: (response: Response) => void;
      ...
      return (response = Response.json(current)) => resolve(response);
    },
    deferImport() {
      let resolve!: (response: Response) => void;
      const promise = new Promise<Response>((r) => {
      ...
      return (response = Response.json({ key: "new-image", mediaType: "image/png", size: 1 })) =>
        resolve(response);
    },
    deferState() {
      ...
      const request = new Promise<void>((r) => {
        requested = r;
      });
      stateResponse = () => {
        requested();
...
test("package form submission binds to the displayed request and exposes pending feedback", async () => {
  ...
  next.elicitations = [
    {
      requestId: "paper-form",
      source: "package:one:stationery",
      operationId: "op",
      invocationId: "call",
      params: {
...
test("a pending command rejects overlapping navigation without clearing its feedback", async () => {
  const h = await harness();
  const resolve = h.deferCommand();
  h.vm.draft = "Keep this operation";
  const sending = h.vm.send();
...
test("editing A is not retargeted when polling discovers document B", async () => {
  const h = await harness();
  h.vm.editing = true;
  h.vm.editText = "Edited A";
  const next = snapshot();
  next.operator.artifacts.push({
    id: "artifact-b",
    title: "B",
    content: { kind: "text", text: "Original B" },
  });
  next.operator.candidates.push({
    id: "candidate-b",
    label: "B",
...
test("conversation navigation cancels the previous document editing session", async () => {
  const h = await harness();
  h.vm.editing = true;
  h.vm.editText = "Do not apply to B";
  const next = snapshot();
...
test("accepted send preserves later draft, attachment and context edits", async () => {
  const h = await harness();
  h.vm.draft = "Submitted draft";
  h.vm.toggleContext("artifact-a");
  const release = h.deferCommand();
  const sending = h.vm.send();
  h.vm.draft = "Next unsent correction";
  h.vm.toggleContext("artifact-b");
  await h.vm.importFiles([
    new File([new Uint8Array([1])], "next.png", { type: "image/png" }),
...
test("reloading untouched conversation B never restores conversation A draft", async () => {
  const h = await harness();
  h.vm.draft = "Draft belonging to A";
  const next = snapshot();
  next.selectedId = "conversation-b";
  ...
  await h.vm.select("conversation-b");
  expect(h.vm.draft).toBe("");
  const reloaded = createDesktopViewModel();
  await reloaded.start();
  reloaded.stopPolling();
  expect(reloaded.draft).toBe("");
  h.setState(snapshot());
  await reloaded.select("conversation-a");
...
test("unowned legacy draft is retained without assigning it to the selected conversation", async () => {
  ...
  expect(reloaded.draft).toBe("");
  expect(h.stored.has("drawloom-composer:conversation-a")).toBe(false);
  reloaded.draft = "New scoped draft";
  await reloaded.send();
  expect(h.stored.get("drawloom-composer")).toBe("Unscoped older draft");
});
...
test("reload restores explicit selections and uploaded attachment references without persisting file bytes", async () => {
  ...
  await h.vm.refreshCatalogue(true);
  h.vm.selectDiscovery("native:skill:a");
  h.vm.toggleContext("artifact-a");
  h.vm.toggleResource("entry-a", {
    id: "r1",
    source: "source-a",
    title: "Document",
    status: "ready",
  ...
  await h.vm.importFiles([
    new File(["never-persist-these-bytes"], "reference.png", { type: "image/png" }),
  ]);
  const reloaded = createDesktopViewModel();
  await reloaded.start();
  reloaded.stopPolling();
  ...
  expect(saved).not.toContain("never-persist-these-bytes");
  expect(saved).not.toContain(btoa("never-persist-these-bytes"));
  reloaded.draft = "Use references";
  await reloaded.send();
  const afterSend = createDesktopViewModel();
  await afterSend.start();
  afterSend.stopPolling();
  expect(afterSend.selectedDiscoveries).toEqual([]);
  expect(afterSend.selectedResources).toEqual([]);
  expect(afterSend.attachments).toEqual([]);
});
...
test("restored unavailable selections stay removable and never leak into another conversation", async () => {
  ...
  await reloaded.start();
  reloaded.stopPolling();
  await reloaded.refreshCatalogue(true);
  expect(reloaded.selectedDiscoveries[0]?.unavailable).toBe(true);
  const next = snapshot();
  next.selectedId = "conversation-b";
  h.setState(next);
  await reloaded.select("conversation-b");
  const bReload = createDesktopViewModel();
  await bReload.start();
  bReload.stopPolling();
  expect(bReload.selectedDiscoveries).toEqual([]);
  h.setState(snapshot());
  await bReload.select("conversation-a");
  expect(bReload.selectedDiscoveries[0]?.id).toBe("native:skill:a");
  bReload.removeDiscovery("native:skill:a");
  const removed = createDesktopViewModel();
  await removed.start();
  removed.stopPolling();
  expect(removed.selectedDiscoveries).toEqual([]);
});

test("reload of an interrupted upload retains a removable failure instead of pretending its local File survived", async () => {
  const h = await harness();
  const release = h.deferImport();
  const uploading = h.vm.importFiles([new File(["a"], "pending.png", { type: "image/png" })]);
  const reloaded = createDesktopViewModel();
  await reloaded.start();
  reloaded.stopPolling();
  expect(reloaded.attachments[0]?.status).toBe("failed");
  expect(reloaded.attachmentKeys).toEqual([]);
  expect(reloaded.canRetryAttachment(reloaded.attachments[0]!.id)).toBe(false);
  reloaded.removeAttachment(reloaded.attachments[0]!.id);
...
test("invalid saved reference data cannot replace the scoped text draft or become a selection", async () => {
  const h = await harness();
  h.vm.draft = "Keep text";
  h.stored.set(
    "drawloom-composer-references:conversation-a",
    '{"version":1,"attachments":[{"asset":{"key":"bad"}}]}',
  );
  const reloaded = createDesktopViewModel();
  await reloaded.start();
  reloaded.stopPolling();
  expect(reloaded.draft).toBe("Keep text");
  expect(reloaded.attachments).toEqual([]);
  expect(reloaded.selectedDiscoveries).toEqual([]);
...
test("group navigation discovers unattached documents and comparison excludes unrelated outputs", async () => {
  ...
  next.operator.artifacts.push({
    id: "overview",
    title: "Overview",
    content: { kind: "text", text: "Read only" },
  });
  next.operator.candidates.push(
    {
      id: "revision-a",
    ...
    { id: "unrelated", comparisonKey: "document-b", label: "B", artifactIds: [], status: "draft" },
  );
  next.operator.groups = [
    {
      id: "writing",
      title: "Writing",
      artifactIds: ["overview"],
      candidateIds: ["candidate-a", "revision-a", "unrelated"],
    },
  ];
  h.setState(next);
  await h.vm.start();
  h.vm.stopPolling();
  h.vm.groupId = "writing";
  h.vm.candidateId = "candidate-a";
  expect(h.vm.comparableCandidates.map((c) => c.id)).toEqual(["revision-a"]);
  h.vm.artifactId = "overview";
  expect(h.vm.artifact?.id).toBe("overview");
  expect(h.vm.candidate).toBeUndefined();
  h.vm.editing = true;
  expect(h.vm.editing).toBe(false);
});
...
test("unsupported synthetic workbench keeps draft and sends no request", async () => {
  const h = await harness();
  const next = snapshot();
  next.conversations[0]!.workbenchId = "media";
  h.setState(next);
  await h.vm.start();
  h.vm.stopPolling();
  h.vm.draft = "Keep this";
  expect(h.vm.canSend).toBe(false);
  await h.vm.send();
  expect(h.commands).toEqual([]);
  expect(h.vm.draft).toBe("Keep this");
});

test("review notes remain local to the inspected candidate", async () => {
  const h = await harness();
  h.vm.reviewSummary = "Exact recovery acknowledgement";
  h.vm.candidateId = "another-candidate";
  expect(h.vm.reviewSummary).toBe("");
  h.vm.reviewSummary = "Document notes";
  h.vm.groupId = "another-group";
  expect(h.vm.reviewSummary).toBe("");
  h.vm.reviewSummary = "Notes for A";
  const next = snapshot();
  next.operator.candidates.push({
    id: "newest",
    label: "New result",
...
const skillCatalogue: DesktopCatalogue = {
  entries: [
    {
      id: "native:skill:a",
      origin: "codex",
...
test("large catalogue exposes bounded pages but searches every entry and resets expanded pages", async () => {
  ...
  h.setCatalogue({
    ...skillCatalogue,
    entries: Array.from({ length: 4301 }, (_, index) => ({
      ...skillCatalogue.entries[0]!,
      id: "app:" + index,
      name: index === 4300 ? "Last searchable integration" : "Integration " + index,
      kind: "app" as const,
    })),
  });
  await h.vm.refreshCatalogue(true);
  expect(h.vm.filteredCatalogue).toHaveLength(100);
  expect(h.vm.catalogueMatchCount).toBe(4301);
  h.vm.showMoreCatalogue();
  expect(h.vm.filteredCatalogue).toHaveLength(200);
  h.vm.catalogueQuery = "Last searchable";
  ...
  h.vm.showMorePicker();
  expect(h.vm.pickerEntries).toHaveLength(200);
  h.vm.pickerQuery = "Last searchable";
  expect(h.vm.pickerEntries.map((item) => item.id)).toEqual(["app:4300"]);
  h.vm.pickerQuery = "";
  expect(h.vm.pickerEntries).toHaveLength(100);
  h.vm.showMoreCatalogue();
  h.vm.showMorePicker();
...
test("resource results and contribution details have explicit bounded expansion", async () => {
  ...
  await h.vm.refreshCatalogue(true);
  h.vm.openPicker("context");
  expect(h.vm.nativeResources).toHaveLength(100);
  expect(h.vm.nativeResourceMatchCount).toBe(225);
  h.vm.showMoreNativeResources();
  expect(h.vm.nativeResources).toHaveLength(200);
  h.vm.pickerQuery = "Resource 224";
  expect(h.vm.nativeResources.map((item) => item.id)).toEqual(["resource:224"]);
  expect(h.vm.contributionOwner("owner")?.name).toBe("Write");
  expect(h.vm.contributionsFor("owner")).toHaveLength(100);
  expect(h.vm.contributionCount("owner")).toBe(225);
  h.vm.showMoreContributions("owner");
  expect(h.vm.contributionsFor("owner")).toHaveLength(200);
  await h.vm.refreshCatalogue(true);
  expect(h.vm.contributionsFor("owner")).toHaveLength(100);
});
...
test("picker sends distinct origin identities once and consumes the typed token", async () => {
  ...
  h.setCatalogue(skillCatalogue);
  await h.vm.refreshCatalogue(true);
  h.vm.draft = "Please $Wri";
  h.vm.openPicker("skill", { start: 7, end: 11 });
  h.vm.selectDiscovery("native:skill:a");
  h.vm.selectDiscovery("native:skill:a");
  h.vm.selectDiscovery("registry:skill:a");
  expect(h.vm.draft).toBe("Please ");
  h.vm.draft = "Please write";
  await h.vm.send();
  expect(h.commands.at(-1)).toMatchObject({
    selections: [
      { id: "native:skill:a", revision: "r1" },
      { id: "registry:skill:a", revision: "r1" },
    ],
  });
...
test("keyboard discovery follows one rendered selectable identity list", async () => {
  ...
  await h.vm.refreshCatalogue(true);
  h.vm.draft = "Please $";
  h.vm.openPicker("skill", { start: 7, end: 8 });
  h.vm.movePicker(1);
...
test("keyboard context navigation can choose a rendered document", async () => {
  ...
  await h.vm.refreshCatalogue(true);
  h.vm.draft = "Read @A";
  h.vm.openPicker("context", { start: 5, end: 7 });
  h.vm.movePicker(1);
...
test("refresh preserves stale selection identity and draft when a same-name replacement appears", async () => {
  ...
  await h.vm.refreshCatalogue(true);
  h.vm.selectDiscovery("native:skill:a");
  h.vm.draft = "Keep draft";
  h.setCatalogue({
    ...skillCatalogue,
    entries: [{ ...skillCatalogue.entries[0]!, id: "native:skill:b", revision: "r2" }],
  });
  await h.vm.refreshCatalogue(true);
  expect(h.vm.selectedDiscoveries[0]).toMatchObject({ id: "native:skill:a", unavailable: true });
  expect(h.vm.draft).toBe("Keep draft");
  h.vm.removeDiscovery("native:skill:a");
  expect(h.vm.selectedDiscoveries).toEqual([]);
});

test("catalogue discards a response from the previous conversation", async () => {
  const h = await harness();
  const release = h.deferDiscovery();
  const pending = h.vm.refreshCatalogue(true);
  const next = snapshot();
  next.selectedId = "conversation-b";
  h.setState(next);
  await h.vm.select("conversation-b");
  release(Response.json(skillCatalogue));
  await pending;
...
test("concurrent catalogue reads coalesce and loading updates preserve expanded pages and selections", async () => {
  ...
  h.setCatalogue({
    ...skillCatalogue,
    entries: Array.from({ length: 220 }, (_, n) => ({
      ...skillCatalogue.entries[0]!,
      id: "skill:" + n,
    })),
    categories: [{ kind: "app", status: "loading" }],
  });
  await h.vm.refreshCatalogue(true);
  h.vm.showMoreCatalogue();
  h.vm.selectDiscovery("skill:0");
  const before = h.discoveryRequests.length,
    release = h.deferDiscovery();
  const read = h.vm.refreshCatalogue(false, undefined, true);
  await h.vm.refreshCatalogue(true);
  expect(h.discoveryRequests.length).toBe(before + 1);
  release();
  await read;
  expect(h.vm.filteredCatalogue).toHaveLength(200);
...
test("explicit native page forwards only its opaque cursor and does not force refresh", async () => {
  const h = await harness();
  h.setCatalogue({ ...skillCatalogue, nextCursor: "opaque+next" });
  await h.vm.refreshCatalogue(true);
  await h.vm.refreshCatalogue(false, h.vm.catalogue!.nextCursor);
  expect(h.discoveryRequests.at(-1)).toBe(
    "/api/discovery?conversationId=conversation-a&cursor=opaque%2Bnext",
  );
});
...
test("temporary discovery failure keeps cached references and draft, while opening the picker reuses cache", async () => {
  ...
  h.vm.selectDiscovery("native:skill:a");
  h.vm.draft = "Keep this";
  const requests = h.discoveryRequests.length;
  h.vm.openPicker("skill");
  expect(h.discoveryRequests).toHaveLength(requests);
  const release = h.deferDiscovery();
  const refresh = h.vm.refreshCatalogue(true);
  release(Response.json({ error: "Discovery offline" }, { status: 503 }));
  await refresh;
  expect(h.vm.catalogueError).toBe("Discovery offline");
  expect(h.vm.cataloguePending).toBe(false);
  expect(h.vm.selectedDiscoveries[0]?.id).toBe("native:skill:a");
  expect(h.vm.draft).toBe("Keep this");
});
...
test("navigation aborts upload and preserves a retryable cancelled reference in its original draft", async () => {
  ...
  const previous = globalThis.fetch;
  globalThis.fetch = (async (url, init) => {
    if (url === "/api/command") expect(h.imports[0]?.signal?.aborted).toBe(true);
    return previous(url, init);
  }) as typeof fetch;
  await h.vm.select("conversation-b");
  ...
  h.setState(snapshot());
  await h.vm.select("conversation-a");
  expect(h.vm.draft).toBe("Draft A");
  expect(h.vm.attachments[0]).toMatchObject({
    name: "a.png",
    status: "failed",
    error: "Import cancelled. Retry to attach this file.",
  });
  const retryRelease = h.deferImport();
  const retrying = h.vm.retryAttachment(h.vm.attachments[0]!.id);
  retryRelease();
  await retrying;
  expect(h.vm.attachments).toHaveLength(1);
  expect(h.vm.attachmentKeys).toEqual(["new-image"]);
});
...
test("successful attachment retry clears its error without navigating away from the composer", async () => {
  ...
  fail(Response.json({ error: "Upload failed" }, { status: 400 }));
  await first;
...
test("native resource opening submits its catalogue identity and displays the captured reference without selecting it", async () => {
  ...
  await h.vm.refreshCatalogue(true);
  await h.vm.openNativeResource("codex:resource:one");
  expect(h.resourceRequests).toEqual([
    { conversationId: "conversation-a", id: "codex:resource:one", revision: "r1" },
  ]);
  expect(h.vm.openedResources[0]?.id).toBe("native-opened");
...
test("resource reads use history identity and only ready text becomes explicit draft context", async () => {
  ...
  await h.vm.loadLatest();
  h.vm.toggleResource("entry-a", resource);
  expect(h.vm.selectedResources).toEqual([]);
  await h.vm.readResource("entry-a", resource);
  expect(h.resourceRequests).toEqual([
    { conversationId: "conversation-a", entryId: "entry-a", resourceId: "r1" },
  ]);
  h.vm.draft = "Read @";
  h.vm.openPicker("context", { start: 5, end: 6 });
  h.vm.toggleResource("entry-a", h.vm.resource("entry-a", resource));
  expect(h.vm.draft).toBe("Read ");
  expect(h.vm.pickerOpen).toBe(false);
  h.vm.draft = "Read document";
  await h.vm.send();
  expect(h.commands.at(-1)).toMatchObject({
    resourceSelections: [{ entryId: "entry-a", resourceId: "r1" }],
  });
  expect(h.vm.selectedResources).toEqual([]);
});
...
test("resource browsing uses its view and cursor, while opening sends only the listed source identity", async () => {
  const h = await harness();
  await h.vm.browseResources("view-a");
  await h.vm.browseResources("view-a", true);
  expect(h.resourceRequests).toEqual([
    "/api/resources?conversationId=conversation-a&viewId=view-a",
    "/api/resources?conversationId=conversation-a&viewId=view-a&cursor=next",
  ]);
  await h.vm.openResource("view-a", "doc://one");
  expect(h.resourceRequests.at(-1)).toEqual({
    conversationId: "conversation-a",
    viewId: "view-a",
    uri: "doc://one",
  });
