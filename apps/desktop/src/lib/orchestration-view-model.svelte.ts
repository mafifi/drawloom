...
export function createOrchestrationViewModel() {
  ...
  let owners = $state<WorkflowOwner[]>([]),
    runs = $state<Array<WorkflowRun | KnowledgeActivityRun>>([]);
  let knowledgeOwner = $state<KnowledgeActivityOwner>(),
    knowledgeSelected = $state(false);
  ...
  let discoveryEpoch = 0,
    discoveryRead = new AbortController();
  const params = () =>
    new URLSearchParams(
      knowledgeSelected ? { limit: "20" } : { projectId, installationId, limit: "20" },
    );
  const route = (path: string) =>
    (knowledgeSelected ? "/api/knowledge-activity/" : "/api/orchestration/") + path;
  function invalidate() {
    epoch++;
    read.abort();
    read = new AbortController();
  ...
  async function readSteps(
    ...
    kind: "first" | "more" | "refresh" = "refresh",
  ) {
    const version = epoch,
      query = params();
    ...
    stepLoading = true;
    stepAction = { runId, kind };
    try {
      const page = WorkflowStepsSchema.parse(
        await body(await fetch(route("steps") + "?" + query, { signal: read.signal })),
      );
      if (version === epoch) stepPage = { runId, ...page, pageCursor: requestedCursor };
    } catch (cause) {
      if (version === epoch)
        error = cause instanceof Error ? cause.message : "Workflow steps unavailable";
    } finally {
      if (version === epoch) {
        stepLoading = false;
        stepAction = undefined;
  ...
  async function refresh(more = false, action: "refresh" | "latest" | "owner" = "refresh") {
    if (
      (!knowledgeSelected && (!projectId || !installationId)) ||
      loading ||
      stepLoading ||
      pendingAction
    )
    ...
    if (requestedCursor) query.set("cursor", requestedCursor);
    loading = true;
    loadingAction = more ? "more" : action;
    error = "";
    try {
      const schema = knowledgeSelected ? KnowledgeActivityPageSchema : WorkflowPageSchema;
      const page = schema.parse(
        await body(await fetch(route("runs") + "?" + query, { signal: read.signal })),
      );
      if (version !== epoch) return;
      // One bounded page, not an ever-growing list of every historical run.
      runs = page.runs;
      cursor = page.cursor;
      pageCursor = requestedCursor;
      if (stepPage && runs.some((run) => run.runId === stepPage?.runId))
        await readSteps(stepPage.runId, stepPage.pageCursor);
      else stepPage = undefined;
    } catch (cause) {
      if (version === epoch)
        error = cause instanceof Error ? cause.message : "Workflows unavailable";
    } finally {
      if (version === epoch) {
        loading = false;
        loadingAction = undefined;
  ...
  async function action(
    ...
    let version = epoch;
    try {
      if (kind === "respond" && (!requestId || !current.pendingInputs.includes(requestId)))
        throw Error("This input request is no longer pending. Refresh the run.");
      if (raw && new TextEncoder().encode(raw).length > 65536)
        throw Error("Workflow input is too large. Use file references instead.");
      const command = WorkflowCommandSchema.parse(
        kind === "cancel"
      ...
      invalidate();
      version = epoch;
      pendingAction = { action: kind, runId, ...(requestId ? { requestId } : {}) };
      error = "";
      const updated = WorkflowRunSchema.parse(
        await body(
          await fetch("/api/orchestration/runs", {
      ...
      if (version === epoch)
        error =
          cause instanceof SyntaxError
            ? "Enter valid JSON for this input request."
            : cause instanceof Error
              ? cause.message
              : "Workflow action unavailable";
    } finally {
      if (version === epoch) pendingAction = undefined;
    }
  }
  const vm = {
    get knowledgeOwner() {
    ...
    get knowledgeSelected() {
      return knowledgeSelected;
    },
    get knowledgeOwnerLoading() {
      return knowledgeOwnerLoading;
    },
    get projectId() {
      return projectId;
    },
    ...
    get loadingAction() {
      return loadingAction ?? (ownersLoading ? ("owners" as const) : undefined);
    },
    get isFirstPage() {
    ...
    async browseSteps(runId: string, more = false) {
      if (loading || stepLoading || pendingAction || !runs.some((run) => run.runId === runId))
        return;
      if (more && (stepPage?.runId !== runId || !stepPage.cursor)) return;
      error = "";
      await readSteps(runId, more ? stepPage?.cursor : undefined, more ? "more" : "first");
    },
    async open(id: string) {
      invalidate();
      invalidateDiscovery();
      knowledgeOwner = undefined;
      ...
      if (!id) return;
      const version = discoveryEpoch;
      ownersLoading = true;
      try {
        const available = WorkflowOwnersSchema.parse(
          await body(
            await fetch("/api/orchestration/owners?" + new URLSearchParams({ projectId: id }), {
        ...
        if (version === discoveryEpoch)
          error = cause instanceof Error ? cause.message : "Orchestration unavailable";
      } finally {
        if (version === discoveryEpoch) ownersLoading = false;
      }
    },
    async openActivity(id: string) {
      const opening = vm.open(id),
        version = discoveryEpoch;
      knowledgeOwnerLoading = true;
      try {
        const owner = KnowledgeActivityOwnerSchema.parse(
          await body(
            await fetch("/api/knowledge-activity/owner?", { signal: discoveryRead.signal }),
          ),
        );
        ...
        if (version === discoveryEpoch)
          error =
            "Knowledge maintenance activity is unavailable. Refresh Activity to check its status.";
      } finally {
        if (version === discoveryEpoch) knowledgeOwnerLoading = false;
      }
      await opening;
    },
    ...
    async openSummary(id: string) {
      ...
      try {
        ...
        const pages = await Promise.all(
          available.map(async (owner) => ({
            owner,
            page: WorkflowPageSchema.parse(
              await body(
                await fetch(
        ...
        if (version === epoch)
          summaryRuns = pages.flatMap(({ owner, page }) =>
            page.runs.map((run) => ({
              installationId: owner.installationId,
        ...
        if (version === epoch)
          error = cause instanceof Error ? cause.message : "Workflow activity unavailable";
      } finally {
        if (version === epoch) {
          loading = false;
          loadingAction = undefined;
    ...
    latest() {
      ...
      pageCursor = undefined;
      return refresh(false, "latest");
    },
    cancel: (runId: string) => action("cancel", runId),
    respond: (runId: string, requestId: string, raw: string) =>
      action("respond", runId, requestId, raw),
    close() {
      invalidate();
      invalidateDiscovery();
      knowledgeOwner = undefined;
