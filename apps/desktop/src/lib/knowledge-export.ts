 ...
 */
export async function serializeKnowledgeExport(
  value: { records: KnowledgeRecord[]; links: KnowledgeLink[] },
  at = new Date(),
): Promise<string> {
  ...
  const frontmatter = [
    "---",
    "type: index",
    `id: knowledge-export-${id}`,
    'title: "Drawloom knowledge page export"',
    "status: draft",
    `created: ${JSON.stringify(date)}`,
    `updated: ${JSON.stringify(date)}`,
    `records: ${JSON.stringify(value.records)}`,
    `links: ${JSON.stringify(value.links)}`,
    "---",
  ];
  const sections = value.records.map((record) => {
    const fence = "`".repeat(
      Math.max(3, ...[...record.body.matchAll(/`+/g)].map((match) => match[0].length + 1)),
    );
    return [
      `## ${JSON.stringify(record.ref.id)}`,
      "",
      `Identity: ${JSON.stringify(record.ref)}`,
      "",
      `${fence}text`,
      record.body,
      fence,
  ...
  return [
    ...frontmatter,
    "",
    "# Drawloom knowledge page export",
    "",
    "This is one explicitly selected page, not necessarily the complete evidence chain. Exact identities, confidence, provenance and directed links are preserved in the document metadata. The SQLite store remains authoritative.",
    "",
    ...sections,
    "",
  ].join("\n");
