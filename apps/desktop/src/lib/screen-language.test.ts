  ...
  const entries = [
    {
      ...
      name: "synthetic.text",
      kind: "plugin",
      description: "Write and revise text",
      origin: "drawloom",
    },
    {
      id: "t",
...
test("display names never change tool identity", () => {
  ...
  ).toBe("Write, revise and inspect documents.");
});
test("unknown owners and same-name plugins retain distinct identities", () => {
  const groups = pluginGroups(
    [
      { id: "a", name: "Search", kind: "plugin", origin: "first" },
      { id: "b", name: "Search", kind: "plugin", origin: "second" },
      { id: "c", name: "Independent skill", kind: "skill", origin: "third", ownerId: "missing" },
    ],
    "",
  );
