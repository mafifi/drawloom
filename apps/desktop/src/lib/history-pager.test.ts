...
const page = (start: number, end: number) => ({
  ...
  olderCursor: String(start),
  hasOlder: start > 0,
  changeCursor: "r1",
  status: { revision: 1, sync: "idle", hasOlder: false },
});
...
test("history pagination bounds the rendered window and ignores a late conversation read", async () => {
  ...
  const pager = createHistoryPager(async (url) => {
    urls.push(url);
    if (url.includes("conversationId=a"))
      return new Promise<Response>((resolve) => {
        finish = resolve;
      });
    return Response.json(
      page(url.includes("before=") ? 9500 : 9950, url.includes("before=") ? 9700 : 10000),
    );
  });
  const a = pager.open("a");
  await pager.open("b");
  finish(Response.json(page(0, 1)));
  await a;
...
test("unchanged changes have no body and stable edits replace rather than duplicate", async () => {
  ...
  const pager = createHistoryPager(async (url) => {
    ...
    return Response.json({
      entries: [{ ...entry(3), text: "edited" }],
      cursor: "r2",
      hasMore: false,
      status: { revision: 2, sync: "idle", hasOlder: false },
    });
  });
  await pager.open("a");
  await pager.poll();
  ...
  const pager = createHistoryPager(async (url) => {
    if (url.includes("/changes"))
      return Response.json({
        ...
        cursor: "r2",
        hasMore: false,
        status: { revision: 2, sync: "idle", hasOlder: true },
      });
    reads++;
    return Response.json(reads === 1 ? page(0, 0) : page(50, 100));
  });
  await pager.open("a");
  await pager.poll();
...
test("a stable search hit opens a bounded around window and polling never replaces it with newest", async () => {
  ...
  const pager = createHistoryPager(async (url) => {
    ...
    if (url.includes("/around"))
      return Response.json({ ...page(25, 74), anchorIndex: 25, hasNewer: true });
...
test("a failed around read reports failure without pretending the exact hit opened", async () => {
  const pager = createHistoryPager(async () =>
