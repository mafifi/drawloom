...
test('projects use shared sidebar controls and a compact main-view directory form', async () => {
  ...
  expect(primary).toContain('<Projects presentation={projectPresentation} actions={projectActions}');
...
test("primary destinations are main-content views while artifact details remain independently docked", async () => {
  ...
  expect(page).toContain("detailsPanePresentation(vm, vm.primaryView === 'conversation', drawer.current)");
...
test('workspace presentation keeps one pane mounted across ordinary layout changes', async () => {
  ...
  expect(page).toContain("class:workspace-pane-hidden={vm.primaryView !== 'conversation' || !vm.detailsOpen}");
  expect(page).toContain('class:workspace-pane-full={drawer.current || vm.detailsExpanded}');
  ...
  expect(details).toContain('class:workspace-content-hidden={!presentation.detailsOpen || !showPluginView}');
});
