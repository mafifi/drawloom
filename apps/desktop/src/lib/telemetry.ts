...
export function initializeUiTelemetry(): Promise<void> {
  return (initialized ??= (async () => {
    try {
      ...
      const provider = new BasicTracerProvider({
        spanProcessors: [
          new BatchSpanProcessor(
            new OTLPTraceExporter({ url: "/api/telemetry/v1/traces", timeoutMillis: 1000 }),
            {
              maxQueueSize: 128,
      ...
      /* Telemetry setup cannot prevent opening a cached workspace. */
    }
  })());
}
...
export async function telemetryFetch(
  ...
  const span = tracer.startSpan("ui.request", {
