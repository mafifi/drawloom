...
export async function closePluginViewBridge(bridge: AppBridge): Promise<void> {
  try {
    await bridge.teardownResource({}, { timeout: 250 });
  } catch {
    /* An unresponsive or not-yet-initialized app must not block closing. */
  } finally {
    await bridge.close();
  }
}
...
export function createPluginViewBridge(options: {
  ...
  const bridge = new AppBridge(
    null,
    { name: "Drawloom", version: "0.0.0" },
    {
      serverTools: {},
      ...(options.message ? { message: { text: {} } } : {}),
      ...(options.updateContext ? { updateModelContext: { text: {} } } : {}),
    },
    {
      hostContext: {
        theme: options.theme,
  ...
  if (options.updateContext)
    bridge.onupdatemodelcontext = (params) => options.updateContext!(params);
  return bridge;
}
