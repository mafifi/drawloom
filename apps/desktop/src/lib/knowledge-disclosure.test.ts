...
test("disclosure only claims use after confirmed delivery and labels reference-only evidence", () => {
  const summary = {
    ...
    references: [
      {
        ref: { type: "claim" as const, origin: "public", id: "large", revision: "2" },
  ...
  const result = presentKnowledgeDisclosure({
    ...summary,
...
test("empty and disabled preparation are quiet; failure does not claim nothing was relevant", () => {
  for (const kind of ["empty", "disabled", "denied"] as const)
    expect(presentKnowledgeDisclosure({ kind, references: [] }).notice).toBe("");
  for (const kind of ["timeout", "unavailable", "cancelled"] as const)
    expect(presentKnowledgeDisclosure({ kind, references: [] }).notice).toContain("unavailable");
});
