...
export const ElicitationDisabledServersSchema = z
  .array(z.string().min(1).max(100))
  .max(100)
  .refine((names) => new Set(names).size === names.length, "Choose each parallel server once");
export const ResourceOriginSchema = z
  .string()
  .max(2048)
  .refine((value) => {
    try {
...
export const PackageSettingsSchema = z
  .strictObject({
    ...
    servers: z.array(z.string()).max(100),
    configuration: z.record(z.string(), JsonValueSchema).optional(),
  })
  .refine(
    (settings) =>
      settings.elicitationDisabledServers?.every((name) => settings.servers.includes(name)) ?? true,
...
export const InstalledPackagesSchema = z.array(
  z.strictObject({
    ...
    diagnostics: z.array(z.string()),
    availableServers: z
      .array(z.strictObject({ name: z.string(), transport: z.string() }))
      .default([]),
    connections: z.array(
      z.strictObject({
        name: z.string(),
...
export const PackageOAuthActionSchema = z.discriminatedUnion("action", [
  z.strictObject({
    action: z.enum(["status", "connect", "cancel", "reconnect", "disconnect"]),
    id: z.string().uuid(),
    server: z.string().min(1),
  }),
  z.strictObject({
    action: z.literal("configure-client"),
    id: z.string().uuid(),
    server: z.string().min(1),
...
export const PackageOAuthStatusSchema = z.strictObject({
  state: z.string(),
  credentialMode: z.enum(["os", "session"]),
