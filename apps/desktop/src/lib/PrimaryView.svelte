...
<main class="primary-view">
  <header class="primary-view-header">
    <Sidebar.Trigger class="primary-navigation-trigger" aria-label={vm.primaryView === 'settings' ? 'Settings navigation' : 'Toggle navigation'} />
    <h1 class="min-w-0 flex-1">{vm.primaryView === 'plugins' ? 'Plugins' : vm.primaryView === 'projects' ? 'Projects' : vm.primaryView === 'project' ? (vm.selectedProject?.name ?? 'Project') : vm.primaryView === 'workbench' ? (vm.selectedWorkbench?.title ?? 'Workbench') : vm.primaryView === 'activity' ? 'Activity' : vm.primaryView === 'archived' ? 'Archived conversations' : vm.primaryView === 'knowledge' ? 'Knowledge' : vm.selectedSettingsPage ? pluginSettingsTitle(vm.selectedSettingsPage) : (settingsSections.find(s => s.id === vm.settingsSection)?.title ?? 'Settings')}</h1>
  ...
  {:else}
    <section class="primary-view-content settings-content">
      ...
      {#if vm.conversation?.provider==='codex'}
        <div class="settings-row"><div><h3>Conversation model</h3><p class="text-sm text-muted-foreground">Applies to the next turn in {vm.conversation.title}.</p></div>
      ...
      <section class="settings-group">
      <h2>Plugins</h2><div class="settings-surface"><div class="settings-row"><p class="text-muted-foreground">Inspect installed plugins and configure their connections.</p><Button variant="outline" class="w-fit" onclick={() => vm.primaryView = 'plugins'}>Open Plugins</Button>
      </div></div></section>
      {/if}
    </section>
  {/if}
