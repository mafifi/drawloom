...
Bun.plugin({
  name: "conversation-navigation-view-model-tests",
  setup(build) {
    build.onLoad(
      { filter: /conversation-navigation-view-model\.svelte\.ts$/ },
      async ({ path }) => ({
...
const { createConversationNavigationViewModel } = await import(
  "./conversation-navigation-view-model.svelte.js"
);
