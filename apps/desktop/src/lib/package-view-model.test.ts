...
const installation = {
  ...
  servers: ["old"],
  availableServers: [{ name: "new", transport: "stdio" }],
...
test("server choices retain removed selections and configure sends the exact new set", async () => {
  ...
  globalThis.fetch = (async (_url, input) => {
    if (input?.body) submitted = JSON.parse(String(input.body));
  ...
  expect(submitted).toEqual({
    action: "configure",
    id: installation.id,
    settings: { enabled: true, trustedBackend: false, servers: ["new"] },
  });
});
...
test("parallel connection choices submit only explicitly selected servers", async () => {
  ...
  globalThis.fetch = (async (_url, input) => {
    if (input?.body) submitted.push(JSON.parse(String(input.body)));
    return Response.json([installation]);
  }) as typeof fetch;
  const vm = createPackageViewModel();
  await vm.refresh();
  ...
  expect(submitted).toEqual([
    {
      ...
      settings: {
        ...
        elicitationDisabledServers: ["new"],
      },
    },
  ]);
  ...
  expect(submitted).toEqual([
    {
      ...
      settings: {
        ...
        trustedBackend: false,
        servers: ["new"],
        approvedResourceOrigins: ["https://media.example", "http://localhost:8080"],
      },
    },
  ]);
  for (const origin of [
...
test("cancel prevents an older authorization installation refresh from replacing newer state", async () => {
  ...
  globalThis.fetch = (async (url, input) => {
    if (url === "/api/packages/oauth") {
      ...
      return Response.json({
        state: action === "cancel" ? "cancelled" : "awaiting-approval",
  ...
  release(Response.json([installation]));
  await connecting;
  expect(vm.installations[0]?.status).toBe("disconnected");
...
test("a late failed connect cannot publish an error after cancellation", async () => {
  ...
  globalThis.fetch = (async (url, input) => {
    ...
    if (JSON.parse(String(input?.body)).action === "connect")
      return new Promise<Response>((resolve) => {
        release = resolve;
      });
  ...
  const connecting = vm.authenticate(installation.id, "remote", "connect");
  await vm.authenticate(installation.id, "remote", "cancel");
  release(new Response(null, { status: 400 }));
  await connecting;
