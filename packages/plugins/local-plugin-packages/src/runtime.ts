...
import {
  ToolElicitationRequestSchema,
  type ToolContext,
  type ToolElicitationHandler,
} from "@drawloom/tools";
...
export interface ActivatePackageOptions {
  ...
  authProviderFor?: (
    server: PackageServer & { config: PackageRemoteConfig },
...
export interface PackageServerStatus {
  name: string;
  status: "connected" | "failed" | "auth-required";
  code?: string;
}
...
export function packageFetch(
  config: PackageRemoteConfig,
  hostFetch: FetchLike = fetch,
  signal?: AbortSignal,
): FetchLike {
  ...
  return async (input, init) => {
    ...
    const response = await hostFetch(input, {
      ...init,
      headers,
      redirect: "manual",
    ...
    if ((response.status === 401 || response.status === 403) && typeof init?.body === "string") {
      ...
      try {
        ...
        /* Other OAuth request bodies are not MCP. */
      }
      const calls = Array.isArray(message) ? message : [message];
      if (
        calls.some(
          (call) =>
            call && typeof call === "object" && "method" in call && call.method === "tools/call",
        )
      ) {
        await response.body?.cancel();
        // SDK auth can replay requests; an uncertain tool effect must be decided by the caller.
        throw new UnauthorizedError(
          "Tool authentication requires explicit reconnect; call was not retried",
        );
      }
    }
    return response;
...
export async function activatePackage(
  ...
  options: ActivatePackageOptions,
): Promise<ActivePackage> {
  if (!/^[A-Za-z0-9_-]{1,100}$/.test(options.installationId))
  ...
  if (!Number.isFinite(timeout) || timeout < 1 || timeout > 60_000)
    throw Error("Handshake timeout must be between 1 and 60000ms");
  const disabledNames = options.elicitationDisabledServers ?? [];
  if (
    !Array.isArray(disabledNames) ||
    disabledNames.length > 100 ||
    new Set(disabledNames).size !== disabledNames.length ||
    disabledNames.some((name) => typeof name !== "string" || name.length < 1 || name.length > 100)
  )
  ...
  for (const name of disabledNames) {
    ...
    if (!selectedNames.has(name) || !selected)
      throw Error("Elicitation-disabled server must be selected and prepared");
    const config = PackageServerConfigSchema.parse(selected.config);
    if (config.type === "sse")
      throw Error("Elicitation-disabled server must use a supported transport");
  }
  const elicitationDisabled = new Set(disabledNames);
  const servers = new Map<string, ActivePackageServer>(),
    statuses: PackageServerStatus[] = [];
  await Promise.all(
    [...new Set(options.selectedServers)].map(async (name) => {
      const allowsElicitation = Boolean(options.elicitation) && !elicitationDisabled.has(name);
      const { elicitation: _unsupportedCapability, ...capabilities } =
        options.clientCapabilities ?? {};
      const client = new Client(
        { name: "drawloom-plugin-packages", version: "0.0.0" },
        {
          capabilities: {
            ...capabilities,
            ...(allowsElicitation ? { elicitation: { form: {} } } : {}),
          },
        },
      );
      ...
      let current:
        | { context?: ToolContext; signal: AbortSignal; traceContext: Context }
        | undefined;
      ...
      client.request = async (...args) => {
        ...
        const execute = () =>
          otelContext.with(parent, () =>
            trace.getTracer("drawloom.mcp").startActiveSpan(
              "mcp.request",
              {
                kind: SpanKind.CLIENT,
                attributes: {
                  "drawloom.plugin.id": options.installationId,
                  "rpc.method": telemetryMethods.has(args[0].method) ? args[0].method : "other",
                },
              },
              async (span) => {
                ...
                try {
                  ...
                  const message = carrier.traceparent
                    ? {
                        ...args[0],
                        params: {
                          ...args[0].params,
                          _meta: { ...args[0].params?._meta, traceparent: carrier.traceparent },
                        },
                      }
                    : args[0];
        ...
        try {
          return await Promise.race([
            ...
            new Promise<never>((_, reject) => {
              cancel = () => {
                if (!started) reject(queuedSignal.reason);
              };
              if (queuedSignal.aborted) cancel();
              else queuedSignal.addEventListener("abort", cancel, { once: true });
            }),
          ]);
        } finally {
          if (cancel) queuedSignal.removeEventListener("abort", cancel);
        }
      };
      client.onclose = () => {
        abort.abort();
      ...
      if (allowsElicitation)
        client.setRequestHandler(ElicitRequestSchema, async (request, extra) => {
          ...
          const owner = current;
          if (!owner?.context || owner.signal.aborted)
            throw new McpError(
              ErrorCode.InvalidRequest,
              "No bound tool invocation for elicitation",
            );
          const invocation = owner.context;
          return otelContext.with(owner.traceContext, () =>
            trace
              .getTracer("drawloom.mcp")
              .startActiveSpan("mcp.elicitation.wait", async (span) => {
                const signal = AbortSignal.any([owner.signal, extra.signal]);
                const envelope = ToolElicitationRequestSchema.parse({
                  requestId: crypto.randomUUID(),
                  source: `package:${options.installationId}:${name}`,
                ...
                try {
                  const result = ElicitResultSchema.parse(
                    await Promise.race([
                      ...
                      new Promise<{ action: "cancel" }>((resolve) => {
                  ...
                  if (
                    ...
                    (!result.content ||
                      !new AjvJsonSchemaValidator().getValidator({ ...schema, type: "object" })(
                        result.content,
                      ).valid)
                  )
                    throw new McpError(
                      ErrorCode.InvalidParams,
                      "Invalid elicitation response content",
                    );
                  span.setAttribute(
                    "drawloom.outcome",
                    result.action === "accept"
      ...
      try {
        ...
        if (config.type === "stdio") {
          ...
          const cwd = await containedPath(
            cwdSpec.startsWith("${PLUGIN_DATA}") ? data : root,
            expand(cwdSpec),
          );
          if (!(await stat(cwd)).isDirectory()) throw Error("Expected working directory");
          const command = config.command.startsWith("./")
            ? await containedPath(root, config.command)
            : config.command;
          ...
          transport = new StreamableHTTPClientTransport(new URL(config.url), {
            ...
            fetch: packageFetch(config, options.fetch, abort.signal),
            reconnectionOptions: {
              maxRetries: 0,
            ...
            contextStore.run(context, async () =>
              CallToolResultSchema.parse(
                await client.callTool(params, undefined, {
                  ...
                  timeout: options.elicitation ? 300_000 : 60_000,
                }),
              ),
            ),
          close() {
            return (serverClosing ??= (async () => {
              abort.abort();
              await client.close();
            })());
          },
        ...
        statuses.push({
          ...
          code: authRequired ? "authentication-required" : "connection-failed",
        });
      } finally {
