...
import {
  ...
  PackageSkillMetadataSchema,
  PackageMcpConfigSchema,
...
export async function inspectPackage(inputRoot: string): Promise<PackageInventory> {
  ...
  const report = (component: string, code: string) =>
    inventory.diagnostics.push({ component, code });
  for (const key of Object.keys(raw as object)) {
    if (!Object.hasOwn(PackageManifestSchema.shape, key))
      report("manifest", `ignored-field:${key}`);
  }
  if (manifest.extensions !== undefined) {
    if (
      ...
      report("manifest", "ignored-extensions");
  ...
  if (Object.hasOwn(inventory.extensions, DRAWLOOM_EXTENSION)) {
    const parsed = DrawloomPackageExtensionSchema.safeParse(
      inventory.extensions[DRAWLOOM_EXTENSION],
    );
    if (parsed.success) {
      try {
        if (parsed.data.backend) await extensionEntrypoint(root, parsed.data.backend.entrypoint);
        if (parsed.data.workflows)
          await extensionEntrypoint(root, parsed.data.workflows.entrypoint);
        inventory.drawloom = parsed.data;
      } catch {
        report(`extension:${DRAWLOOM_EXTENSION}`, "invalid");
      }
    ...
    try {
      ...
      for (const name of (await readdir(directory)).sort()) {
        ...
        try {
          ...
          if (!(await stat(child)).isDirectory() || !(await exists(join(child, "SKILL.md"))))
            continue;
    ...
    try {
      ...
      for (const [name, value] of Object.entries(mcp.mcpServers)) {
        try {
          ...
          if (config.type === "sse") {
            report(`server:${name}`, "unsupported-transport");
            continue;
          }
          if (config.type === "stdio") {
            ...
            if (config.cwd && !config.cwd.startsWith("${PLUGIN_DATA}")) {
              ...
              if (!(await stat(await containedPath(root, cwd))).isDirectory())
                throw Error("Expected cwd directory");
            }
          }
          inventory.servers.push({ name, config });
        } catch {
          report(`server:${name}`, "invalid");
        }
      }
    } catch {
      report("mcp", "invalid");
