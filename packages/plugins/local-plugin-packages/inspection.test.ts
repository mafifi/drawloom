...
test("shared package inventory conformance", () =>
  packageInspectionConformance(inspector, fixture));
test("resolved symlink escapes cannot supply components or supporting files", async () => {
  ...
  try {
    ...
    await expect(
      inspector.readSupportingFile(inventory, "valid", "../../../plugin.json"),
    ).rejects.toThrow();
  } finally {
    await local.dispose();
  ...
  try {
    await local.write(
      ...
      JSON.stringify({
        ...
        name: "local",
        extensions: { "org.drawloom": { version: 1, backend: { entrypoint: "../escape.js" } } },
      }),
    );
    ...
    expect(inventory.diagnostics.map((d) => d.component)).toEqual(
      expect.arrayContaining(["extension:org.drawloom", "skills", "server:bad"]),
    );
  } finally {
    await local.dispose();
  }
...
test("inspection recognizes extension metadata without importing backend or fetching schemas", async () => {
  ...
  try {
    await local.write(
      ...
      'throw new Error("Must not execute on inspection");',
    );
    await local.write(
      "org.drawloom/workflows.mjs",
      'throw new Error("Workflow module must not execute on inspection");',
    );
    await local.write(
      "plugin.json",
      JSON.stringify({
...
test("namespace directories and executable files cannot be symlinks even when they resolve inside the package", async () => {
  for (const linked of ["namespace", "backend", "workflows"] as const) {
    ...
    try {
      ...
      if (linked === "namespace")
        await symlink(join(local.root, "real"), join(local.root, "org.drawloom"));
      else {
        await local.write("org.drawloom/placeholder", "");
        await symlink(
          join(local.root, "real", `${linked}.mjs`),
          join(local.root, "org.drawloom", `${linked}.mjs`),
        );
        await local.write(
          `org.drawloom/${linked === "backend" ? "workflows" : "backend"}.mjs`,
          "export default {}",
        );
      }
      await local.write(
      ...
      expect(inventory.diagnostics).toContainEqual({
        component: "extension:org.drawloom",
        code: "invalid",
      });
    } finally {
      await local.dispose();
