...
export async function fixture(
  ...
  const active = await activatePackage(await inspectPackage(root), {
    dataRoot: join(root, "data"),
    installationId: "one",
    selectedServers: ["stationery"],
    ...(handler ? { elicitation: handler } : {}),
    ...(elicitationDisabledServers ? { elicitationDisabledServers } : {}),
  });
  return {
    root,
    active,
    server: active.servers.get("stationery")!,
    async close() {
      await active.close();
...
const context = (id: string, signal = new AbortController().signal) => ({
...
test("package-owned invocation retains accept, decline and cancel with source-bound standard forms", async () => {
  ...
  try {
    ...
    expect(requests.map((r) => [r.invocationId, r.operationId, r.source])).toEqual([
      ["accept", "operation-accept", "package:one:stationery"],
      ["decline", "operation-decline", "package:one:stationery"],
      ["cancel", "operation-cancel", "package:one:stationery"],
    ]);
...
test("package form validates accepted data and rejects URL and unbound client requests", async () => {
  ...
  const f = await fixture(async () => {
    presentations++;
    return { action: "accept", content: { paper: "invalid" } };
  });
  try {
    ...
    const url = await f.server.callTool(
      { name: "choose", arguments: { url: true } },
      context("url"),
    );
    expect(url.content[0]).toMatchObject({ type: "text", text: expect.stringContaining("URL") });
    const unbound = CallToolResultSchema.parse(
      await f.server.client.callTool({ name: "choose", arguments: {} }),
    );
    expect(unbound.content[0]).toMatchObject({
      type: "text",
      text: expect.stringContaining("bound"),
    });
...
test("concurrent calls serialize by connection and retain their original invocation owners", async () => {
  const firstShown = Promise.withResolvers<void>(),
    release = Promise.withResolvers<ToolElicitationResult>();
  const owners: string[] = [];
  const f = await fixture(async (request) => {
  ...
  try {
    ...
    expect(results.map((result) => result.content[0])).toEqual([
      { type: "text", text: '{"action":"accept","content":{"paper":"plain"}}' },
      { type: "text", text: '{"action":"decline"}' },
    ]);
  } finally {
    release.resolve({ action: "cancel" });
    await f.close();
  }
});
...
test("disconnect invalidates pending form and prevents subsequent calls", async () => {
  ...
  const f = await fixture(async (_request, current) => {
    signal = current;
    shown.resolve();
    return new Promise(() => {});
  });
  try {
    const pending = f.server.callTool({ name: "choose", arguments: {} }, context("one")).then(
      () => undefined,
      (error) => error,
    );
    await shown.promise;
    ...
    await expect(
      f.server.callTool({ name: "choose", arguments: {} }, context("two")),
    ).rejects.toThrow();
  } finally {
    await f.close();
  }
...
test("queued cancellation sends zero call and active cancellation retires connection and pending form", async () => {
  ...
  let presentations = 0,
    formSignal: AbortSignal | undefined;
  const f = await fixture(async (_request, signal) => {
    presentations++;
    formSignal = signal;
    shown.resolve();
  ...
  try {
    ...
    const first = f.server.callTool(
      { name: "choose", arguments: {} },
      context("first", firstAbort.signal),
    );
    const failedFirst = first.then(
      () => undefined,
      (error) => error,
    );
    await shown.promise;
    const second = f.server.callTool(
      { name: "choose", arguments: {} },
      context("second", queuedAbort.signal),
    );
    const failedSecond = second.then(
      () => undefined,
      (error) => error,
    );
    queuedAbort.abort();
    const queuedResult = await Promise.race([
      failedSecond,
      new Promise((resolve) => setTimeout(() => resolve("still queued"), 100)),
    ]);
    firstAbort.abort();
    ...
    await expect(
      f.server.callTool({ name: "choose", arguments: {} }, context("third")),
    ).rejects.toThrow();
    expect(presentations).toBe(1);
  } finally {
    await f.close();
...
test("explicit elicitation opt-out overlaps requests without presenting or advertising forms", async () => {
  ...
  try {
    ...
    const results = await Promise.all([
      f.server.callTool(
        { name: "choose", arguments: { parallel: true, id: "first", delay: 80 } },
        context("first"),
      ),
      f.server.callTool(
        { name: "choose", arguments: { parallel: true, id: "second", delay: 20 } },
        context("second"),
      ),
    ]);
    expect(
      results.map(
        (result) =>
          JSON.parse(result.content[0]!.type === "text" ? result.content[0]!.text : "").active,
      ),
    ).toEqual([1, 2]);
    const unsolicited = await f.server.callTool({ name: "choose", arguments: {} }, context("form"));
    expect(unsolicited.content[0]).toMatchObject({
      type: "text",
      text: expect.stringContaining("Method not found"),
    });
...
test("invalid elicitation opt-out names fail before any selected server executes", async () => {
  ...
  try {
    await expect(
      activatePackage(await inspectPackage(root), {
        dataRoot: join(root, "data"),
        installationId: "one",
        selectedServers: ["stationery"],
        elicitationDisabledServers: ["missing"],
      }),
    ).rejects.toThrow();
    await expect(
      activatePackage(await inspectPackage(root), {
...
test("parallel cancellation never duplicates tool work", async () => {
  ...
  try {
    ...
    const pending = f.server
      .callTool(
        { name: "choose", arguments: { parallel: true, id: "cancelled", delay: 200 } },
    ...
    expect(lines.filter((line) => line.event === "start" && line.id === "cancelled")).toHaveLength(
      1,
    );
    await expect(
      f.server.callTool(
        { name: "choose", arguments: { parallel: true, id: "after" } },
        context("after"),
      ),
    ).rejects.toThrow();
  } finally {
...
test("parallel requests retain distinct telemetry parents and propagate their own request spans", async () => {
  ...
  try {
    ...
    const invoke = (id: string, delay: number) =>
      trace.getTracer("test").startActiveSpan(`parent-${id}`, async (parent) => {
        parentIds.set(id, parent.spanContext().spanId);
        try {
          return await server.callTool(
            { name: "choose", arguments: { parallel: true, id, delay } },
            context(id),
          );
        } finally {
          parent.end();
    ...
    const requestSpans = spans
      .getFinishedSpans()
      .filter(
        (span) => span.name === "mcp.request" && span.attributes["rpc.method"] === "tools/call",
      );
    expect(requestSpans).toHaveLength(2);
    const payloads = results.map(
      (result) =>
        JSON.parse(result.content[0]!.type === "text" ? result.content[0]!.text : "") as {
          id: string;
          traceparent: string;
        },
    );
