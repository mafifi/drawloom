...
test("unchanged acknowledged polls never hide a later commit", async () => {
  ...
  try {
    ...
    const unchanged = await source.changes();
    await source.acknowledge(unchanged.token);
...
test("acknowledgement tokens cannot cross configured sources", async () => {
  ...
  try {
    ...
    const one = new GitKnowledgeSource({
      repository: fixture.repo,
      paths: ["one.txt"],
      dataDirectory: fixture.data,
    });
    const two = new GitKnowledgeSource({
      repository: fixture.repo,
      paths: ["two.txt"],
      dataDirectory: join(fixture.root, "other"),
    });
    const first = await one.changes();
    const second = await two.changes();
...
test("bounds the complete MCP result and applies the file page limit", async () => {
  ...
  try {
    ...
    const source = new GitKnowledgeSource({
      ...
      dataDirectory: fixture.data,
      limits: { maxFiles: 1, maxBatchBytes: 1024 },
    });
    const page = await source.changes();
    expect(page.updates).toHaveLength(1);
    expect(Buffer.byteLength(JSON.stringify(gitBatchResult(page)), "utf8")).toBeLessThanOrEqual(
      1024,
    );
...
test("replays a stable committed page until acknowledged and preserves revisions", async () => {
  ...
  try {
    ...
    await source.acknowledge(first.token);
    await source.acknowledge(first.token);
    const restarted = new GitKnowledgeSource({
      repository: fixture.repo,
      paths: ["notes.txt"],
      dataDirectory: fixture.data,
    });
    ...
    expect(withdrawn.updates[0]).toMatchObject({
      revision: withdrawnRevision,
      previous: changedRevision,
      state: "withdrawn",
    });
  } finally {
    rmSync(fixture.root, { recursive: true, force: true });
...
test("ignores working-tree changes and rejects unsafe configured committed files", async () => {
  ...
  try {
    ...
    writeFileSync(
      join(fixture.repo, ".git/hooks/post-checkout"),
      "#!/bin/sh\ntouch '" + hookMarker + "'\n",
      { mode: 0o755 },
    );
    const source = new GitKnowledgeSource({
    ...
    const initial = await source.changes();
    await source.acknowledge(initial.token);
    ...
    const linked = new GitKnowledgeSource({
      repository: fixture.repo,
      paths: ["linked.txt"],
      dataDirectory: join(fixture.root, "other-data"),
    });
...
test("a generic MCP client can poll and acknowledge the packed server outside this checkout", async () => {
  ...
  try {
    ...
    cpSync(
      join(process.cwd(), "packages/plugins/git-knowledge-source/dist/server.mjs"),
      join(packed, "server.mjs"),
    );
    const transport = new StdioClientTransport({
      command: "node",
      args: [join(packed, "server.mjs")],
      env: {
        GIT_SOURCE_REPOSITORY: fixture.repo,
    ...
    const changed = GitBatchSchema.parse(
      (await client.callTool({ name: "git.changes", arguments: {} })).structuredContent,
    );
    ...
    const withdrawn = GitBatchSchema.parse(
      (await client.callTool({ name: "git.changes", arguments: {} })).structuredContent,
    );
  ...
  try {
    ...
    const launch = () =>
      new StdioClientTransport({
        ...
        env: {
          GIT_SOURCE_REPOSITORY: fixture.repo,
          GIT_SOURCE_PATHS: JSON.stringify(["one.txt", "two.txt"]),
          PLUGIN_DATA: fixture.data,
        },
        stderr: "pipe",
      });
    ...
    const first = GitBatchSchema.parse(
      (await firstClient.callTool({ name: "git.changes", arguments: {} })).structuredContent,
    );
    ...
    received.set(`${firstUpdate.id}:${firstUpdate.revision}`, {
      id: firstUpdate.id,
    ...
    const replayed = GitBatchSchema.parse(
      (await restartedClient.callTool({ name: "git.changes", arguments: {} })).structuredContent,
    );
    ...
    await restartedClient.callTool({
      name: "git.acknowledge",
      arguments: { token: replayed.token },
    });
    ...
    const final = GitBatchSchema.parse(
      (await restartedClient.callTool({ name: "git.changes", arguments: {} })).structuredContent,
    );
    expect(final.updates).toEqual(
...
test("reports force-push reconciliation and never treats corrupt private state as empty", async () => {
  ...
  try {
    ...
    await source.acknowledge(second.token);
...
test("rejects binary and oversized committed content rather than truncating it", async () => {
  ...
  try {
    ...
    git(fixture.repo, ["add", "binary.txt"]);
    git(fixture.repo, ["commit", "-m", "public binary fixture"]);
    const binary = new GitKnowledgeSource({
      repository: fixture.repo,
      paths: ["binary.txt"],
      dataDirectory: fixture.data,
    });
    ...
    const large = new GitKnowledgeSource({
      repository: fixture.repo,
      paths: ["large.txt"],
      dataDirectory: join(fixture.root, "other-data"),
...
test("holds a response-bounded page stable and advances only after its acknowledgement", async () => {
  ...
  try {
    ...
    git(fixture.repo, ["add", "two.txt"]);
    git(fixture.repo, ["commit", "-m", "second public file"]);
    const source = new GitKnowledgeSource({
      repository: fixture.repo,
      paths: ["one.txt", "two.txt"],
      dataDirectory: fixture.data,
