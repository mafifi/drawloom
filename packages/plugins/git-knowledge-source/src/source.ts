...
export const GitBatchSchema = z.strictObject({
  token: z.string().min(1).max(256),
  updates: z.array(GitUpdateSchema).max(200),
});
...
export function gitBatchResult(batch: GitBatch) {
  return {
    content: [{ type: "text" as const, text: `${batch.updates.length} committed source updates.` }],
...
function sourceError(code: GitKnowledgeSourceError["code"], message: string): never {
  throw new GitKnowledgeSourceError(code, message);
}
function token(deliveryId: string, offset: number): string {
  return `${deliveryId}:${offset}`;
}
...
export class GitKnowledgeSource {
  ...
  constructor(options: GitKnowledgeSourceOptions) {
    try {
      ...
      if (new Set(paths).size !== paths.length)
        sourceError("invalid_path", "Configured paths must be unique");
    ...
    const result = this.#queue.then(async () => {
      ...
      const lock = await open(path, "wx", 0o600).catch((error: NodeJS.ErrnoException) => {
        if (error.code === "EEXIST")
          sourceError(
            "writer_locked",
            "Source writer is active or was interrupted. Confirm it has stopped before removing its writer.lock; retained state is unchanged.",
          );
        throw error;
      });
      try {
  ...
  async #binding(): Promise<string> {
    return JSON.stringify({
      repository: await realpath(this.repository),
      paths: this.paths,
      limits: this.limits,
    });
  }
  ...
  async #load(): Promise<SourceState> {
    ...
    try {
      ...
      if (state.binding !== binding)
        sourceError("configuration_changed", "Git source launch configuration changed");
      return state;
    } catch (error) {
      if ((error as NodeJS.ErrnoException).code === "ENOENT")
        return { version: 1, binding, head: null, files: {}, acknowledgedTokens: [] };
      if (error instanceof GitKnowledgeSourceError) throw error;
      sourceError(
        "state_corrupt",
        "Git source state is corrupt; recovery requires operator action",
      );
    }
  }
  ...
  async #head(): Promise<string> {
    ...
    if (!/^[0-9a-f]{40,64}$/.test(value))
      sourceError("unsupported_file", "Repository has no valid committed HEAD");
    return value;
  }

  async #isAncestor(older: string, newer: string): Promise<boolean> {
    try {
      await this.#git(["merge-base", "--is-ancestor", older, newer]);
      return true;
    } catch {
      return false;
    }
  ...
  async #blobAt(head: string, path: string): Promise<string | null> {
    ...
    if (!match)
      sourceError("unsupported_file", `Configured path is not an ordinary committed file: ${path}`);
    return match[2]!;
  }
  ...
  async #content(blob: string, path: string): Promise<string> {
    ...
    if (!Number.isSafeInteger(size) || size < 0)
      sourceError("unsupported_file", `Cannot inspect committed file: ${path}`);
    if (size > this.limits.maxFileBytes)
      sourceError("file_too_large", `Configured committed file exceeds maxFileBytes: ${path}`);
    const bytes = Buffer.from(await this.#git(["cat-file", "blob", blob], true));
    if (bytes.includes(0))
      sourceError("unsupported_file", `Configured committed file is binary: ${path}`);
    try {
      return new TextDecoder("utf-8", { fatal: true }).decode(bytes);
    } catch {
      sourceError("unsupported_file", `Configured committed file is not UTF-8 text: ${path}`);
    }
  }
  ...
  async #buildPending(
    state: SourceState,
    head: string,
  ): Promise<NonNullable<SourceState["pending"]>> {
    ...
    for (const path of this.paths) {
      ...
      const update = GitUpdateSchema.parse({
        id: path,
        revision: head,
        previous: previous?.revision ?? null,
        kind: "source",
        state: blob ? "active" : "withdrawn",
        text,
      });
      if (Buffer.byteLength(JSON.stringify(update), "utf8") > this.limits.maxBatchBytes)
        sourceError("batch_too_large", `One configured update exceeds maxBatchBytes: ${path}`);
      updates.push(update);
  ...
  #page(pending: NonNullable<SourceState["pending"]>): GitBatch {
    ...
    for (const update of pending.updates.slice(pending.offset)) {
      ...
      const candidate = {
        token: token(pending.deliveryId, pending.offset),
        updates: [...updates, update],
      };
      // Reserve space for the JSON-RPC envelope in addition to the complete result.
      if (
        Buffer.byteLength(JSON.stringify(gitBatchResult(candidate)), "utf8") + 128 >
        this.limits.maxBatchBytes
      ) {
        if (!updates.length)
          sourceError("batch_too_large", "One source update exceeds the response byte limit");
        break;
      }
      updates.push(update);
    }
  ...
  changes(): Promise<GitBatch> {
    return this.#serial(async () => {
      ...
      if (state.head && !(await this.#isAncestor(state.head, head)))
        sourceError(
          "reconciliation_required",
          "Configured repository history changed; reconcile the delivery checkpoint explicitly",
        );
  ...
  acknowledge(batchToken: string): Promise<void> {
    return this.#serial(async () => {
      ...
      if (!pending || batchToken !== token(pending.deliveryId, pending.offset))
        sourceError("unknown_ack", "Unknown, stale, or cross-configuration acknowledgement");
