...
server.registerTool(
  ...
  {
    description:
      "Inspect configured committed UTF-8 text files. It never reads the worktree or executes repository code.",
    inputSchema: {},
    annotations: {
      readOnlyHint: true,
...
server.registerTool(
  ...
  {
    description:
      "Acknowledge one durably received Git changes batch. This only advances the plugin delivery checkpoint.",
  ...
  async ({ token }) => {
    try {
      ...
      return {
        isError: true,
        content: [{ type: "text", text: "git.acknowledge failed: " + code }],
      };
    }
  },
);
