...
import {
  ...
  ContextReferenceMaterialSchema,
  ContextPreparationSummarySchema,
...
export const DiscoveryKindSchema = z.enum([
  "skill",
  "app",
  "plugin",
  "tool",
...
export const DiscoverySnapshotSchema = z.strictObject({
  ...
  categories: z.array(
    z.strictObject({
      kind: DiscoveryKindSchema,
      status: z.enum(["loading", "available", "unsupported", "error"]),
...
export interface AgentDiscovery {
  ...
  list(options?: {
    ...
    wait?: boolean;
    cursor?: string;
  }): Promise<AgentResult<DiscoverySnapshot>>;
  invalidate(): void;
...
export const AgentModelSchema = z.strictObject({
  ...
  efforts: z.array(z.string().min(1).max(64)).max(32),
...
export const AgentOperationFailureSchema = z.strictObject({
  code: z.enum(["provider_unavailable", "provider_rejected", "invalid_provider_response"]),
...
export const AgentOperationUsageSchema = z
  .strictObject({
  ...
  .superRefine((value, context) => {
    ...
    if (
      ...
      context.addIssue({
        ...
        message: "Cached input is included in input",
      });
    if (
      ...
      context.addIssue({
        ...
        message: "Reasoning tokens are included in output",
      });
  });
...
export interface AgentSession {
  ...
  execute(input: AgentOperationInput): Promise<AgentResult<{ operationId: string }>>;
