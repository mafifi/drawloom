...
test("returned-only native resource receipts survive restart and cannot authorise other sources", async () => {
  ...
  const rpc: RpcTransport = {
    async request(method, params) {
      ...
      return {
        contents: [{ uri: "document://unlisted/one", text: "Reference", mimeType: "text/plain" }],
      };
    },
    notify() {},
    respond() {},
  ...
  const receipts = await first.rememberReturnedResources("source-one", [
    { type: "resource_link", uri: "document://unlisted/one", name: "Reference" },
  ]);
  ...
  expect(await reopened.discovery.readResource!(receipt)).toMatchObject({
    status: "ok",
    value: [{ type: "resource", resource: { text: "Reference" } }],
  });
  expect(calls).toEqual([
    {
      method: "mcpServer/resource/read",
      params: { threadId: "thread", server: "source-one", uri: "document://unlisted/one" },
    },
  ]);
  expect((await reopened.discovery.readResource!({ ...receipt, revision: "forged" })).status).toBe(
    "rejected",
  );
...
test("concurrent completions returning the same native link receive the same durable receipt", async () => {
  ...
  const rpc: RpcTransport = {
    async request() {
      return { contents: [{ uri: "doc://same", text: "Same" }] };
    },
    notify() {},
    respond() {},
    subscribe() {
  ...
  const [first, second] = await Promise.all([
    catalog.rememberReturnedResources("source", content),
    catalog.rememberReturnedResources("source", content),
  ]);
...
test("failed native content capture leaves coverage recoverable; settled content is not captured again", async () => {
              ...
              item: {
                ...
                result: {
                  content: [
                    {
                      type: "resource",
                      resource: { uri: "doc://a", text: "A", mimeType: "text/plain" },
                    },
                  ],
                },
              },
        ...
        resources: [
          {
            ...
            title: "A",
            status: "ready",
            asset: { key: "cached", mediaType: "text/plain", size: 1 },
          },
        ],
      };
    },
  ...
  const context = {
    get: async (id: string) => entries.get(id),
    checkpoint: async (key: string) => checkpoints.get(key),
  };
  ...
  batch.entries.forEach((e) => entries.set(e.id, e));
  batch.checkpoints.forEach((c) => checkpoints.set(c.key, c.value));
    ...
    const event = {
      ...
      params: {
        ...
        item: {
          ...
          status: "completed",
          result: {
            content: [{ type: "resource_link", uri: "document://a", name: "A" }],
            _meta: { secret: "never visible" },
          },
        },
      },
    };
