...
const server = z.object({
  name: id,
  pluginId: id.nullable(),
  authStatus: z.enum(["unknown", "unsupported", "notLoggedIn", "bearerToken", "oAuth"]).optional(),
  tools: z.record(z.string(), z.object({ name: id, description: z.string().optional() })),
  resources: z
    .array(
      z.object({
        uri: id,
...
const rejected = (): AgentResult<never> => ({
  ...
  failure: {
    code: "invalid_state",
    message: "Discovery changed or the selection is unavailable. Refresh and select again.",
  },
});
...
export function createCodexDiscovery(
    ...
    const run = Promise.resolve()
      .then(async () => {
        const add = async (
          ...
          if (current !== generation || isClosed()) throw Error("Discovery changed");
        ...
        try {
          ...
          part.status = {
            kind,
            status: z.object({ code: z.literal(-32601) }).safeParse(error).success
              ? "unsupported"
              : "error",
            message: "Provider discovery is unavailable for this category. Refresh to retry.",
          };
  ...
  function loadApps(forceRefetch: boolean, cursor: string | null = null) {
    ...
    load("app", async (_part, add) => {
      const result = z.object({ data: z.array(app).max(1000), nextCursor: id.nullable() }).parse(
      ...
      if (result.nextCursor && (appCursors.has(result.nextCursor) || appCursors.size >= 100))
      ...
      for (const item of result.data) {
        ...
        await add(
          ...
          available ? { type: "mention", name: item.name, path: `app://${item.id}` } : undefined,
        );
      }
      if (current !== generation || isClosed() || dirtyKinds.has("app")) return;
      nextCursor = undefined;
          ...
          for (const item of group.skills)
            await add(
              item.path,
              {
                ...
                ...(item.pluginId ? { ownerId: await identity("plugin", item.pluginId) } : {}),
              },
              item.enabled ? { type: "skill", name: item.name, path: item.path } : undefined,
            );
        }
      },
      refresh,
        ...
        for (const item of result.data) {
          if (item.authStatus === "notLoggedIn" || item.authStatus === "oAuth") {
            const key = await add(item.name, {
              ...
              scope: "session",
              availability: item.authStatus === "oAuth" ? "available" : "unavailable",
              selectable: false,
    ...
    if (experimentalPlugins)
      load(
        ...
        async (_part, add, forceRefetch) => {
          const result = z
            .object({
              marketplaces: z.array(z.object({ name: id, plugins: z.array(plugin) })),
  ...
  const discovery: AgentDiscovery = {
    ...
    async authenticate(selection) {
      const current = generation;
      const name =
        cached?.revision === selection.revision ? authentication.get(selection.id) : undefined;
      ...
      try {
        ...
        if (current !== generation || cached?.revision !== selection.revision || isClosed())
          return rejected();
        ...
        return {
          ...
          failure: {
            ...
            message: "Native sign-in is unavailable. Use the provider’s own integration settings.",
          },
        };
      }
    },
    async readResource(selection) {
      const current = generation;
      let target =
        cached?.revision === selection.revision ? resources.get(selection.id) : undefined;
      ...
      if (!target && store && /^codex:returned-resource:[a-f0-9]{64}$/.test(selection.id)) {
        const receipt = z
          ...
          .safeParse(await store.get(`codex-resource:${threadId}:${selection.id}`));
      ...
      try {
        const result = z.object({ contents: z.array(z.unknown()) }).parse(
        ...
        if (
          ...
          isClosed() ||
          (listed &&
            (cached?.revision !== selection.revision || resources.get(selection.id) !== target))
        )
          return rejected();
        return {
          status: "ok",
          value: content.filter(
            (block) => block.type === "resource" && block.resource.uri === target.uri,
          ),
        };
      } catch (error) {
        return {
          ...
          failure: {
            code: "provider_unavailable",
            message: "This provider resource could not be read. No tool was invoked.",
          },
        };
      }
    },
    ...
    async rememberReturnedResources(
      ...
      for (const block of content.slice(0, 256)) {
        ...
        const revision = saved.success
          ? saved.data.revision
          : await identity("resource-receipt", JSON.stringify([1, threadId, source, block.uri]));
        if (!saved.success)
