    ...
    for (const item of next.data)
      result.push(
        AgentModelSchema.parse({
          ...
          ...(item.defaultReasoningEffort ? { defaultEffort: item.defaultReasoningEffort } : {}),
        }),
      );
    if (!next.nextCursor)
      return result.filter((model, index) => result.findIndex((m) => m.id === model.id) === index);
    if (cursors.has(next.nextCursor)) throw Error("Repeated model cursor");
    cursors.add(next.nextCursor);
    cursor = next.nextCursor;
  }
  throw Error("Model catalogue exceeds page limit");
}
export function permitsModel(
  models: readonly AgentModel[],
  selection: AgentModelSelection,
): boolean {
  const model = models.find((m) => m.id === selection.model);
  return Boolean(model && (!selection.effort || model.efforts.includes(selection.effort)));
}
