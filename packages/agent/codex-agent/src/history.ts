...
const turnsPage = z.object({
  data: z.array(z.record(z.string(), z.unknown())).max(50),
  nextCursor: z.string().nullable(),
});
const itemsPage = z.object({
  data: z.array(z.record(z.string(), z.unknown())).max(1),
  nextCursor: z.string().nullable(),
});
...
const backfillSchema = z.object({
  nextTurnCursor: z.string().nullable(),
  pending: z.array(savedTurn).max(50),
  activeItemCursor: z.string().nullable(),
  activeItemIds: z.array(z.string()).max(8).default([]),
  activeCursors: z.array(z.string()).max(8).default([]),
  mode: z.enum(["latest", "older"]).default("older"),
  anchor: z.tuple([z.number().int().safe(), z.number().int().safe()]).nullable().default(null),
...
export function createCodexHistoryReader(
  ...
  const itemKey = async (turnId: string, signature: string, epoch: number, itemId: string) =>
    `item-seen:${(await nativeMessageId(`${turnId}\0${signature}\0${epoch}\0${itemId}`)).slice(8)}`;
  const nativePage = async (
    method: string,
    params: unknown,
    schema: typeof turnsPage | typeof itemsPage,
  ) => {
    let value: unknown;
    try {
      ...
      if (code === -32601)
        throw new HistoryReadError(
          "unsupported",
          "Native conversation history is unsupported by this provider.",
        );
  ...
  async function displayItem(
  ...
  ): Promise<NativeDisplay | undefined> {
    ...
    if (item.type === "userMessage") {
      ...
      const texts = content.data
        .filter((value) => value.type === "text")
        .map((value) => z.string().safeParse(value.text));
      if (texts.some((value) => !value.success))
        throw new HistoryReadError(
          "error",
          "Native conversation history returned invalid item data.",
        );
    ...
    if (item.type === "mcpToolCall" && captureToolContent) {
      ...
      try {
        ...
        throw new HistoryReadError("error", "Historical resource capture failed.");
      }
    }
    return undefined;
  }

  async function positionItems(
    itemsDescending: NativeDisplay[],
    context: HistoryReadContext,
    direction: "latest" | "older",
    bounds: { min: number; max: number },
    newerPosition?: readonly [number, number],
  ) {
    ...
    if (anchorIndex >= 0) {
      ...
      for (let index = 0; index < chronological.length; index++) {
        ...
        if (existing) {
          ...
          const updated = entry(
            existing.state === "complete" && item.state === "partial"
              ? { ...item, state: "complete" }
              : item,
            existing.position,
          );
          if (
            ...
            JSON.stringify(updated.assets) !== JSON.stringify(existing.assets) ||
            JSON.stringify(updated.resources) !== JSON.stringify(existing.resources) ||
            JSON.stringify(updated.preparation) !== JSON.stringify(existing.preparation)
          )
            output.push(updated);
          continue;
        }
  ...
  const read = async (
    ...
    const savedLatestContinuation = backfillSchema.safeParse(
      await context.checkpoint("latest-continuation"),
    );
    ...
    const savedUnfinishedScan = z
      .string()
      .nullable()
      .safeParse(await context.checkpoint("unfinished-scan"));
    ...
    const state = useLatestContinuation
      ? savedLatestContinuation.data
      : savedBackfill.success
        ? savedBackfill.data
        : undefined;
    const unfinished = new Map(
      (savedUnfinished.success ? savedUnfinished.data : []).map((turn) => [turn.id, turn]),
    );
    const coveredHead = new Map(
      (savedHead.success ? savedHead.data : []).map((turn) => [turn.id, JSON.stringify(turn)]),
    );
    ...
    if (options.direction === "older" && state) {
      ...
      nextTurnCursor = state.nextTurnCursor;
      itemCursor = state.activeItemCursor;
      ...
      const head = await nativePage(
        "thread/turns/list",
        { threadId, limit: 50, sortDirection: "desc", itemsView: "notLoaded" },
        turnsPage,
      );
      pending = head.data.map((value) => savedTurn.parse(value));
      nextTurnCursor = head.nextCursor;
      itemCursor = null;
      activeItemIds = [];
      activeCursors = [];
      ...
      let scanCursor =
        savedUnfinishedScan.success && savedUnfinishedScan.data
          ? savedUnfinishedScan.data
          : head.nextCursor;
      const scanSeen = new Set<string>();
      for (
        let scans = 0;
        scans < 3 && scanCursor && [...unfinished.keys()].some((id) => !present.has(id));
        scans++
      ) {
        if (scanSeen.has(scanCursor))
          throw new HistoryReadError("error", "Invalid history cursor.");
        scanSeen.add(scanCursor);
        const requested = scanCursor;
        let scanned;
        try {
          scanned = await nativePage(
            "thread/turns/list",
            {
              threadId,
              limit: 50,
        ...
        for (const value of scanned.data) {
          ...
          if (unfinished.has(turn.id) && !present.has(turn.id)) {
            discoveredUnfinished.push(turn);
            present.add(turn.id);
          }
        }
        const unfinishedScope = `unfinished\0${JSON.stringify([...unfinished.values()])}`;
        if (
          scanned.nextCursor === scanCursor ||
          (scanned.nextCursor &&
            (await context.checkpoint(
              await cursorKey("turn", scanned.nextCursor, unfinishedScope),
            )) === true)
        )
          throw new HistoryReadError("error", "Invalid history cursor.");
    ...
    let newerPosition: readonly [number, number] | undefined =
      options.direction === "older" ? (state?.anchor ?? undefined) : undefined;
    let turnsVisited = 0;
    let itemRequests = 0;
    ...
    while (turnsVisited < 200 && itemRequests < 200) {
      if (!pending.length) {
        if (discoveredUnfinished.length) {
          pending = discoveredUnfinished.splice(0, 50);
          itemCursor = null;
          continue;
        }
        ...
        try {
          ...
          recoveryEpoch++;
          traversalId = (await nativeMessageId(`${traversalId}\0recovery\0${recoveryEpoch}`)).slice(
            8,
          );
          pageValue = await nativePage(
            "thread/turns/list",
        ...
        if (!pageValue.data.length && !pageValue.nextCursor) {
          nextTurnCursor = null;
          break;
        }
        ...
        if (
          !turnReconciling &&
          (pageValue.nextCursor === requestedTurnCursor ||
            (await context.checkpoint(
              await cursorKey("turn", pageValue.nextCursor ?? "", turnScope),
            )) === true)
        )
          throw new HistoryReadError("error", "Invalid history cursor.");
        checkpoints.push({
          key: await cursorKey("turn", requestedTurnCursor, turnScope),
          value: true,
        });
        pending = pageValue.data.map((value) => savedTurn.parse(value));
        nextTurnCursor = pageValue.nextCursor;
        itemCursor = null;
      }
      ...
      const sameCoverage =
        savedCoverage.success && JSON.stringify(savedCoverage.data) === signature;
      const sameHead =
        options.direction === "latest" && terminal && coveredHead.get(turn.id) === signature;
      ...
      try {
        ...
        items = await nativePage(
          "thread/items/list",
          { threadId, turnId: turn.id, limit: 1, sortDirection: "desc" },
          itemsPage,
        );
      }
      itemRequests++;
      ...
      for (const raw of items.data) {
        let itemId: string;
        try {
          itemId = z.string().min(1).parse(z.record(z.string(), z.unknown()).parse(raw.item).id);
        } catch {
          throw new HistoryReadError(
            "error",
            "Native conversation history returned invalid item data.",
        ...
        if ((await context.checkpoint(seenItemKey)) === true) {
          if (itemCursor && options.direction === "older" && !reconciling)
            throw new HistoryReadError("error", "Invalid history cursor.");
        ...
        const item = await displayItem(turn.id, turn.status, raw, context);
        if (item) nativeItems.push(item);
      }
      const positioned = await positionItems(
        nativeItems,
        context,
        traversalMode,
        bounds,
      ...
      if (items.nextCursor) {
        ...
        if (
          !reconciling &&
          (items.nextCursor === itemCursor ||
            activeCursors.includes(items.nextCursor) ||
            (await context.checkpoint(await cursorKey("item", items.nextCursor, itemScope))) ===
              true)
        )
          throw new HistoryReadError("error", "Invalid history cursor.");
        if (itemCursor)
          checkpoints.push({ key: await cursorKey("item", itemCursor, itemScope), value: true });
        const scopedCursor = `${itemScope}\0${items.nextCursor}`;
        if (seenCursors.has(scopedCursor))
          throw new HistoryReadError("error", "Invalid history cursor.");
        seenCursors.add(scopedCursor);
        activeCursors = [...activeCursors.slice(-7), items.nextCursor];
        itemCursor = items.nextCursor;
        if (options.direction === "latest" && terminal) nextHeadCoverage.push(turn);
        if (entries.length >= limit) break;
      } else {
        if (terminal) {
          checkpoints.push({ key, value: turn });
          nextHeadCoverage.push(turn);
        }
        pending.shift();
        itemCursor = null;
    ...
    const newBackfill = {
      ...
      activeItemCursor: itemCursor,
      activeItemIds,
      activeCursors,
      mode: traversalMode,
      anchor: newerPosition ?? null,
      recoveryEpoch,
      traversalId,
    };
    const latestNeedsContinuation =
      options.direction === "latest" &&
      (itemCursor !== null ||
        pending.length > 0 ||
        (createdEntries > 0 && nextTurnCursor !== null));
    if (options.direction === "latest" && latestNeedsContinuation && savedBackfill.success)
      checkpoints.push({ key: "latest-continuation", value: newBackfill });
    else if (options.direction === "latest" && latestNeedsContinuation)
      checkpoints.push({ key: "backfill", value: newBackfill });
    else if (options.direction === "older" && useLatestContinuation)
      checkpoints.push({
        key: "latest-continuation",
        value: newBackfill.pending.length || newBackfill.nextTurnCursor ? newBackfill : null,
      });
    else if (options.direction === "older")
      checkpoints.push({ key: "backfill", value: newBackfill });
    else if (!savedBackfill.success) checkpoints.push({ key: "backfill", value: newBackfill });
    const latestStillOlder =
      options.direction === "latest"
        ? latestNeedsContinuation || savedLatestContinuation.success
        : useLatestContinuation &&
          (newBackfill.pending.length > 0 || newBackfill.nextTurnCursor !== null);
    const baseStillOlder =
      options.direction === "latest" && savedBackfill.success
    ...
    if (options.direction === "latest")
      checkpoints.push({ key: "latest-covered", value: nextHeadCoverage.slice(0, 50) });
    if (unfinished.size > 50)
      throw new HistoryReadError(
        "error",
        "Native conversation history has too much unfinished work for one bounded sync.",
      );
    checkpoints.push({ key: "unfinished", value: [...unfinished.values()].slice(0, 50) });
    if (options.direction === "latest")
      checkpoints.push({
        key: "unfinished-scan",
        value: unfinished.size ? nextUnfinishedScan : null,
      });
    checkpoints.push({ key: "bounds", value: bounds });
    const hasMore =
      options.direction === "latest" && unfinished.size > 0 && nextUnfinishedScan !== null;
  ...
  return {
    ...
    async read(context, options) {
      try {
        return await read(context, options);
      } catch (cause) {
        if (cause instanceof HistoryReadError) throw cause;
        if (cause instanceof InvalidNativeCursor)
          throw new HistoryReadError("error", "Invalid history cursor.");
        throw new HistoryReadError("error", "Native conversation history returned invalid data.");
      }
    },
