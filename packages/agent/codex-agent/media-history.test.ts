...
function memoryContext() {
  ...
  const context: HistoryReadContext = {
    checkpoint: async (key) => checkpoints.get(key),
...
test("native display identity is the Web Crypto SHA-256 digest", async () => {
  expect(await nativeMessageId("native-user")).toBe(
    "message-68e26fb5bcec8902c508c9ee278907db6b7547ad22dbd85ae36f8ab97fe8c24f",
  );
});
...
test("native chronology inserts an unseen user before its already-stored live assistant", async () => {
  ...
  const request = async (method: string, raw: unknown) => {
    ...
    if (method === "thread/turns/list")
      return { data: [{ id: "turn-a", status: "completed" }], nextCursor: null };
    const items = [
      {
        turnId: "turn-a",
        item: {
  ...
  const batch = await reader.read(state.context, { direction: "latest", limit: 50 });
  expect(batch.entries[0]).toMatchObject({
    id: `operation-a:${await nativeMessageId("user-native")}`,
    role: "user",
...
test("settled turn coverage makes a warm latest read issue zero item payload requests", async () => {
  ...
  const request = async (method: string) => {
    ...
    itemRequests++;
    return {
      data: [{ turnId: "turn-a", item: { id: "a", type: "agentMessage", text: "done" } }],
      nextCursor: null,
    };
  };
  const state = memoryContext();
...
test("a warm read stops at the committed newest turn instead of scanning cached metadata", async () => {
  ...
  const request = async (method: string, raw: unknown) => {
        ...
        data: [
          {
            ...
            item: {
              id: `item-${params.turnId}`,
              type: "agentMessage",
              text: String(params.turnId),
            },
          },
        ],
        nextCursor: null,
    ...
    turnRequests++;
    const offset = Number(params.cursor ?? 0);
    const turns = Array.from({ length: 2_100 }, (_, index) => ({
      id: `turn-${2_099 - index}`,
      status: "completed",
    }));
    return {
      data: turns.slice(offset, offset + 50),
      nextCursor: offset + 50 < turns.length ? String(offset + 50) : null,
    };
  };
  const state = memoryContext();
  const reader = createCodexHistoryReader(request, "thread", {});
...
test("failed commit replay returns identical entries from the last committed checkpoint", async () => {
  const request = async (method: string) =>
    method === "thread/turns/list"
      ? { data: [{ id: "turn-a", status: "completed" }], nextCursor: null }
      : {
          data: [{ turnId: "turn-a", item: { id: "a", type: "agentMessage", text: "done" } }],
          nextCursor: null,
        };
...
test("captured historical images are reused after their batch commits", async () => {
    ...
    method === "thread/turns/list"
      ...
      : {
          data: [
            {
              turnId: "turn-a",
              item: { id: "image", type: "imageGeneration", status: "completed", result: "bytes" },
            },
          ],
          nextCursor: null,
        };
  ...
  const reader = createCodexHistoryReader(
    ...
    async () => {
      captures++;
      return { key: "asset", mediaType: "image/png", size: 5 };
    },
  );
  const first = await reader.read(state.context, { direction: "latest", limit: 50 });
  state.commit(first);
  ...
  expect(first.entries[0]?.assets).toEqual([{ key: "asset", mediaType: "image/png", size: 5 }]);
});

test("cursor loops and cross-turn item correlation are rejected", async () => {
  const loop = createCodexHistoryReader(
    async (method) =>
      method === "thread/turns/list"
  ...
  await expect(
    loop.read(memoryContext().context, { direction: "latest", limit: 50 }),
  ).rejects.toThrow("Invalid history cursor");
  const crossed = createCodexHistoryReader(
    async (method) =>
      method === "thread/turns/list"
        ? { data: [{ id: "turn" }], nextCursor: null }
        : {
            data: [{ turnId: "other", item: { id: "a", type: "agentMessage", text: "x" } }],
            nextCursor: null,
          },
    "thread",
    {},
  ...
  await expect(
    crossed.read(memoryContext().context, { direction: "latest", limit: 50 }),
  ).rejects.toThrow("Invalid history correlation");
});
...
test("one huge turn backfills beyond two thousand entries without a ceiling", async () => {
  ...
  const request = async (method: string, raw: unknown) => {
    if (method === "thread/turns/list")
      return { data: [{ id: "huge", status: "completed" }], nextCursor: null };
      ...
      data: [
        {
          turnId: "huge",
          item: {
            id: `item-${chronological}`,
            type: "agentMessage",
            text: `message-${chronological}`,
          },
        },
      ],
  ...
  const state = memoryContext();
  const reader = createCodexHistoryReader(request, "thread", {});
  let batch = await reader.read(state.context, { direction: "latest", limit: 50 });
  state.commit(batch);
  while (batch.hasOlder) {
    batch = await reader.read(state.context, { direction: "older", limit: 200 });
    state.commit(batch);
  }
...
test("unfinished coverage restarts independently and appends newly arrived native content", async () => {
  ...
  const request = async (method: string, raw: unknown) => {
    if (method === "thread/turns/list")
      return { data: [{ id: "active", status: "inProgress" }], nextCursor: null };
    const offset = Number((raw as { cursor?: string }).cursor ?? 0);
    const items = Array.from({ length: itemCount }, (_, index) => ({
      turnId: "active",
      item: { id: `item-${index}`, type: "agentMessage", text: `message-${index}` },
    })).reverse();
    return {
      data: items.slice(offset, offset + 1),
      nextCursor: offset + 1 < items.length ? String(offset + 1) : null,
  ...
  const partial = await reader.read(state.context, { direction: "latest", limit: 50 });
  state.commit(partial);
  itemCount = 2;
  reader = createCodexHistoryReader(request, "thread", {});
  const restarted = await reader.read(state.context, { direction: "latest", limit: 50 });
  state.commit(restarted);
  expect(partial.entries[0]?.state).toBe("partial");
  expect(restarted.entries.map((entry) => entry.text)).toEqual(["message-1"]);
  expect(
    restarted.entries[0]!.position[0] > partial.entries[0]!.position[0] ||
      restarted.entries[0]!.position[1] > partial.entries[0]!.position[1],
  ).toBe(true);
});
...
test("a completed reread updates a partial record without moving it", async () => {
  ...
  const request = async (method: string) =>
    method === "thread/turns/list"
      ...
      : {
          data: [{ turnId: "turn", item: { id: "item", type: "agentMessage", text: "answer" } }],
          nextCursor: null,
        };
  const state = memoryContext();
  const reader = createCodexHistoryReader(request, "thread", {});
  ...
  expect(complete.entries[0]).toMatchObject({
    id: partial.entries[0]!.id,
    position: partial.entries[0]!.position,
    state: "complete",
  });
});
...
test("new live turns remain after older backfill even when adapter bounds lag live storage", async () => {
  const request = async (method: string, raw: unknown) => {
    ...
    return turnId === "future"
      ? {
          data: [
            { turnId, item: { id: "future-assistant", type: "agentMessage", text: "future" } },
          ],
          nextCursor: null,
        }
      : {
          data: [{ turnId, item: { id: "old-assistant", type: "agentMessage", text: "old" } }],
          nextCursor: null,
        };
  };
  const state = memoryContext();
  const futureId = `operation-future:${await nativeMessageId("future-assistant")}`;
  state.entries.set(futureId, {
    id: futureId,
    position: [900, 0],
    role: "assistant",
    origin: { kind: "assistant" as const },
  ...
  expect(
    oldPosition && (oldPosition[0] < 900 || (oldPosition[0] === 900 && oldPosition[1] < 0)),
  ).toBe(true);
});
...
test("an expired backfill cursor reconciles from a bounded safe page", async () => {
  ...
  state.commit({
    ...
    checkpoints: [
      {
        key: "backfill",
        value: { nextTurnCursor: "expired", pending: [], activeItemCursor: null },
      },
    ],
    hasOlder: true,
  });
  const request = async (method: string, raw: unknown) => {
    ...
    return {
      data: [{ turnId: "turn", item: { id: "a", type: "agentMessage", text: "safe" } }],
      nextCursor: null,
    };
  };
  const batch = await createCodexHistoryReader(request, "thread", {}).read(state.context, {
    direction: "older",
    limit: 50,
  });
...
test("a new head turn cannot shift an active huge-turn backfill cursor", async () => {
  ...
  const request = async (method: string, raw: unknown) => {
    ...
    if (method === "thread/turns/list")
      return {
        data: [
          ...(newHead ? [{ id: "new", status: "completed" }] : []),
          { id: "huge", status: "completed" },
        ],
        nextCursor: null,
      };
      ...
      data: [
        {
          turnId: params.turnId,
          item: {
            id: `${params.turnId}-${offset}`,
            type: "agentMessage",
            text: `${params.turnId}-${offset}`,
          },
        },
      ],
      nextCursor: params.turnId === "huge" && offset < 2 ? String(offset + 1) : null,
    };
  };
  const state = memoryContext();
  const reader = createCodexHistoryReader(request, "thread", {});
...
test("an exhausted limit boundary advances to the next turn and skips settled coverage", async () => {
  ...
  const request = async (method: string, raw: unknown) => {
    ...
    const turnId = (raw as { turnId: string }).turnId;
    calls.set(turnId, (calls.get(turnId) ?? 0) + 1);
    return {
      data: [{ turnId, item: { id: turnId, type: "agentMessage", text: turnId } }],
      nextCursor: null,
    };
  };
  const state = memoryContext();
...
test("oversized native metadata and item pages are rejected", async () => {
  const tooManyTurns = createCodexHistoryReader(
    async () => ({
      data: Array.from({ length: 51 }, (_, id) => ({ id: String(id) })),
      nextCursor: null,
    }),
    "thread",
    {},
  ...
  await expect(
    tooManyTurns.read(memoryContext().context, { direction: "latest", limit: 50 }),
  ).rejects.toThrow("invalid data");
  const tooManyItems = createCodexHistoryReader(
    async (method) =>
      method === "thread/turns/list"
        ? { data: [{ id: "turn" }], nextCursor: null }
        : {
  ...
  await expect(
    tooManyItems.read(memoryContext().context, { direction: "latest", limit: 50 }),
  ).rejects.toThrow("invalid data");
});
...
test("a partially imported completed huge turn has zero unchanged warm item requests", async () => {
  ...
  const request = async (method: string, raw: unknown) => {
    ...
    const offset = Number((raw as { cursor?: string }).cursor ?? 0);
    return {
      data: [{ turnId: "huge", item: { id: `item-${offset}`, type: "agentMessage", text: "x" } }],
...
test("latest sync preserves an existing older pending continuation", async () => {
  ...
  const request = async (method: string, raw: unknown) => {
    ...
    if (method === "thread/turns/list") {
      if (params.cursor === "older-page")
        return { data: [{ id: "old", status: "completed" }], nextCursor: null };
      return newestOnly
        ? { data: [{ id: "new", status: "completed" }], nextCursor: null }
        : { data: [{ id: "head", status: "completed" }], nextCursor: "older-page" };
    }
    return {
      data: [
        {
          turnId: params.turnId,
          item: { id: params.turnId, type: "agentMessage", text: params.turnId },
        },
      ],
      nextCursor: null,
    };
  ...
  expect(
    (await reader.read(state.context, { direction: "older", limit: 50 })).entries.map(
      (entry) => entry.text,
    ),
  ).toEqual(["old"]);
});
...
test("latest sync discovers unfinished work beyond the newest fifty turns", async () => {
  ...
  const turns = Array.from({ length: 51 }, (_, index) => ({
    id: index === 50 ? "old" : `new-${index}`,
    status: index === 50 ? oldStatus : "completed",
  }));
  const request = async (method: string, raw: unknown) => {
    const params = raw as { cursor?: string; turnId?: string };
    if (method === "thread/turns/list") {
  ...
  let batch = await reader.read(state.context, { direction: "latest", limit: 50 });
  state.commit(batch);
  batch = await reader.read(state.context, { direction: "older", limit: 50 });
  state.commit(batch);
  oldStatus = "completed";
  turns[50] = { id: "old", status: oldStatus };
  const warm = await reader.read(state.context, { direction: "latest", limit: 50 });
...
test("unfinished metadata discovery advances across bounded latest reads", async () => {
  const turns = Array.from({ length: 251 }, (_, index) => ({
    id: index === 250 ? "old" : `new-${index}`,
    status: "completed",
  }));
  const request = async (method: string, raw: unknown) => {
    const params = raw as { cursor?: string; turnId?: string };
  ...
  state.commit({
    entries: [],
    checkpoints: [{ key: "unfinished", value: [{ id: "old", status: "inProgress" }] }],
    hasOlder: true,
  });
  const reader = createCodexHistoryReader(request, "thread", {});
  const first = await reader.read(state.context, { direction: "latest", limit: 50 });
...
test("same requested item cursor and repeated item identity reject cross-batch loops", async () => {
  const request = async (method: string, raw: unknown) => {
    ...
    return {
      data: [{ turnId: "turn", item: { id: "same", type: "agentMessage", text: "same" } }],
      nextCursor: cursor ?? "loop",
    };
  };
  const state = memoryContext();
  const reader = createCodexHistoryReader(request, "thread", {});
  const first = await reader.read(state.context, { direction: "latest", limit: 1 });
  state.commit(first);
  await expect(reader.read(state.context, { direction: "older", limit: 1 })).rejects.toThrow(
    "Invalid history cursor",
  );
});

test("empty terminal turns finish safely and malformed items use HistoryReadError", async () => {
  const empty = createCodexHistoryReader(
    async (method) =>
      method === "thread/turns/list"
        ? { data: [{ id: "turn", status: "completed" }], nextCursor: null }
  ...
  expect(
    await empty.read(memoryContext().context, { direction: "latest", limit: 50 }),
  ).toMatchObject({ entries: [], hasOlder: false });
  const malformed = createCodexHistoryReader(
    async (method) =>
      method === "thread/turns/list"
        ? { data: [{ id: "turn" }], nextCursor: null }
        : { data: [{ turnId: "turn", item: { type: "agentMessage" } }], nextCursor: null },
    "thread",
    {},
  );
  await expect(
    malformed.read(memoryContext().context, { direction: "latest", limit: 50 }),
  ).rejects.toMatchObject({ name: "HistoryReadError", status: "error" });
});
...
test("ten-thousand-entry tens-of-megabytes fixture stays bounded and warm payload-free", async () => {
  ...
  const request = async (method: string, raw: unknown) => {
    if (method === "thread/turns/list") {
      metadataRequests++;
      return { data: [{ id: "large", status: "completed" }], nextCursor: null };
    }
    payloadRequests++;
    const offset = Number((raw as { cursor?: string }).cursor ?? 0);
    return {
      data: [{ turnId: "large", item: { id: `item-${offset}`, type: "agentMessage", text: body } }],
      nextCursor: offset + 1 < total ? String(offset + 1) : null,
    };
  };
  const state = memoryContext();
...
test("expired turn cursor fallback with no turns returns an empty bounded batch", async () => {
    ...
    checkpoints: [
      {
        key: "backfill",
        value: {
          nextTurnCursor: "expired",
          pending: [],
          activeItemCursor: null,
          activeItemIds: [],
          activeCursors: [],
        },
      },
  ...
  const reader = createCodexHistoryReader(
    async (method, raw) => {
      if (method === "thread/turns/list" && (raw as { cursor?: string }).cursor)
        throw { code: -32602 };
  ...
  await expect(
    reader.read(state.context, { direction: "older", limit: 50 }),
  ).resolves.toMatchObject({ entries: [], hasOlder: false });
});
  ...
  for (const item of malformed) {
    const reader = createCodexHistoryReader(
      async (method) =>
        method === "thread/turns/list"
          ? { data: [{ id: "turn", status: "completed" }], nextCursor: null }
          : { data: [{ turnId: "turn", item }], nextCursor: null },
      "thread",
      {},
      async () => ({ key: "asset", mediaType: "image/png", size: 1 }),
    );
    await expect(
      reader.read(memoryContext().context, { direction: "latest", limit: 50 }),
    ).rejects.toMatchObject({ name: "HistoryReadError", status: "error" });
  }
});
...
test("a new latest partial continuation stacks ahead of retained older traversal", async () => {
  ...
  const request = async (method: string, raw: unknown) => {
    ...
    if (method === "thread/turns/list") {
      if (params.cursor === "older-page")
        return { data: [{ id: "old", status: "completed" }], nextCursor: null };
      return {
        data: [{ id: newHuge ? "new" : "head", status: "completed" }],
        nextCursor: newHuge ? null : "older-page",
      };
    }
    const turn = params.turnId!;
    payloads.set(turn, (payloads.get(turn) ?? 0) + 1);
    const offset = Number(params.cursor ?? 0);
    return {
      data: [
        {
          turnId: turn,
          item: { id: `${turn}-${offset}`, type: "agentMessage", text: `${turn}-${offset}` },
        },
      ],
      nextCursor: turn === "new" && offset < 1 ? String(offset + 1) : null,
    };
  };
  const state = memoryContext();
  const reader = createCodexHistoryReader(request, "thread", {});
  ...
  newHuge = true;
  state.commit(await reader.read(state.context, { direction: "latest", limit: 1 }));
  let older = await reader.read(state.context, { direction: "older", limit: 1 });
  state.commit(older);
  while (older.hasOlder) {
    older = await reader.read(state.context, { direction: "older", limit: 50 });
    state.commit(older);
  }
  ...
  const reader = createCodexHistoryReader(
    async (method) =>
      method === "thread/turns/list"
        ...
        : (payloads++,
          {
            data: [{ turnId: "turn", item: { id: "item", type: "agentMessage", text: "x" } }],
            nextCursor: null,
          }),
    "thread",
    {},
...
test("a cursor cycle longer than eight is rejected across committed batches", async () => {
  ...
  const reader = createCodexHistoryReader(
    async (method, raw) => {
      ...
      return {
        data: [{ turnId: "turn", item: { id: `item-${cursor}`, type: "agentMessage", text: "x" } }],
        nextCursor: String((cursor + 1) % 10),
      };
    },
    "thread",
    {},
  );
  let batch = await reader.read(state.context, { direction: "latest", limit: 1 });
  state.commit(batch);
  for (let index = 0; index < 9; index++) {
    batch = await reader.read(state.context, { direction: "older", limit: 1 });
    state.commit(batch);
  }
  await expect(reader.read(state.context, { direction: "older", limit: 1 })).rejects.toThrow(
    "Invalid history cursor",
...
test("a text-only stored image identity is captured once then reused", async () => {
  ...
  const reader = createCodexHistoryReader(
    ...
    async () => {
      captures++;
      return { key: "image", mediaType: "image/png", size: 5 };
    },
  );
  const first = await reader.read(state.context, { direction: "latest", limit: 50 });
  state.commit(first);
...
test("two newly discovered turns remain after cached history in native order", async () => {
  ...
  state.commit({
    entries: [],
    checkpoints: [{ key: "bounds", value: { min: 0, max: 0 } }],
    hasOlder: false,
  });
  const reader = createCodexHistoryReader(
    async (method, raw) => {
      if (method === "thread/turns/list")
        return {
          data: [
            { id: "new2", status: "completed" },
            { id: "new1", status: "completed" },
            { id: "old", status: "completed" },
          ],
          nextCursor: null,
        };
      ...
      return {
        data: [{ turnId, item: { id: itemId, type: "agentMessage", text: turnId } }],
        nextCursor: null,
      };
    },
    "thread",
  ...
  state.commit(await reader.read(state.context, { direction: "latest", limit: 1 }));
  let batch = await reader.read(state.context, { direction: "older", limit: 1 });
  state.commit(batch);
  while (batch.hasOlder) {
    batch = await reader.read(state.context, { direction: "older", limit: 50 });
    state.commit(batch);
...
test("changed completedAt rereads a multi-item turn with reusable native cursors", async () => {
  ...
  const reader = createCodexHistoryReader(
    async (method, raw) => {
      if (method === "thread/turns/list")
        return { data: [{ id: "turn", status: "completed", completedAt }], nextCursor: null };
      payloads++;
      const offset = Number((raw as { cursor?: string }).cursor ?? 0);
      return {
        data: [
          {
            turnId: "turn",
            item: { id: `item-${offset}`, type: "agentMessage", text: String(offset) },
          },
        ],
        nextCursor: offset === 0 ? "1" : null,
      };
...
test("a full new head retains its uncached middle gap without reviving a cached warm head", async () => {
  ...
  const turns = Array.from({ length: 251 }, (_, index) => ({
    id: index === 250 ? "old" : `new-${250 - index}`,
    status: index === 250 ? "completed" : "completed",
  }));
  const request = async (method: string, raw: unknown) => {
    const params = raw as { cursor?: string; turnId?: string };
    if (method === "thread/turns/list") {
...
test("independent unfinished scans may reuse native metadata cursor strings", async () => {
  ...
  const request = async (method: string, raw: unknown) => {
    const params = raw as { cursor?: string; turnId?: string };
    if (method === "thread/turns/list") {
      const all = Array.from({ length: 251 }, (_, index) => ({
        id: index === 250 ? target : `new-${index}`,
        status: "completed",
      }));
      const offset = Number(params.cursor ?? 0);
      return {
        data: all.slice(offset, offset + 50),
  ...
  state.commit({
    entries: [],
    checkpoints: [{ key: "unfinished", value: [{ id: target, status: "inProgress" }] }],
    hasOlder: true,
  });
  state.commit(await reader.read(state.context, { direction: "latest", limit: 50 }));
  state.commit(await reader.read(state.context, { direction: "latest", limit: 50 }));
  ...
  state.commit({
    entries: [],
    checkpoints: [
      { key: "unfinished", value: [{ id: target, status: "inProgress" }] },
      { key: "unfinished-scan", value: null },
    ],
    hasOlder: true,
  });
  state.commit(await reader.read(state.context, { direction: "latest", limit: 50 }));
...
test("expired turn cursor recovery starts a fresh traversal epoch", async () => {
  ...
  const reader = createCodexHistoryReader(request, "thread", {});
  await expect(
    reader.read(state.context, { direction: "older", limit: 200 }),
