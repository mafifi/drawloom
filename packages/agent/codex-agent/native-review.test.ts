...
function fixture(
  version = "Codex Desktop/0.153.4 (Mac OS; arm64) unknown (drawloom; 0.0.0)",
  reviewerEcho: string | undefined = "user",
) {
  ...
  const transport: RpcTransport = {
    async request(method, params) {
      ...
      if (method === "thread/start" || method === "thread/resume")
        return { thread: { id: "native-thread" }, approvalsReviewer: reviewerEcho };
...
function approval(id: number, args = { text: "Revised passage" }): RpcMessage {
  return {
    ...
    params: {
      ...
      serverName: "drawloom",
      mode: "form",
      message: "Allow document edit?",
      requestedSchema: { type: "object", properties: {} },
      _meta: {
        codex_approval_kind: "mcp_tool_call",
        tool_title: "Edit document",
        tool_params: args,
      },
    },
  };
...
test("native reviewer defaults human and delegated selection is passed to the next turn", async () => {
  const f = fixture();
  const { s, next } = await session(f);
  try {
    expect(s.reviewerModes).toEqual(["human", "delegated"]);
    expect((await s.execute({ operationId: "one", text: "work" })).status).toBe("ok");
    await next();
    ...
    await next();
    expect(
      (await s.execute({ operationId: "two", text: "work", reviewer: "delegated" })).status,
    ).toBe("ok");
    await next();
    ...
    expect(
      (await s.steer?.({ operationId: "two", text: "change reviewer", reviewer: "human" }))?.status,
    ).toBe("rejected");
  } finally {
    await s.close();
  }
...
for (const [label, action] of [
  ...
  test(`native MCP ${label} resolves only the exact originating request once`, async () => {
    ...
    const { s, next } = await session(f);
    try {
      await s.execute({ operationId: "one", text: "edit" });
      await next();
      f.emit(approval(11));
      ...
      expect(
        (await s.resolveApproval({ ...response, arguments: {} } as typeof response)).status,
      ).toBe("rejected");
...
test("unmarked elicitation remains input rather than an execution approval", async () => {
  ...
  const { s, next } = await session(f);
  try {
    await s.execute({ operationId: "one", text: "question" });
    await next();
    const message = approval(11);
    ...
    expect(
      (
        await s.resolveApproval({
          approvalId: first.request.approvalId,
          optionId: first.request.options[0]!.optionId,
        })
      ).status,
    ).toBe("rejected");
    expect(f.replies).toHaveLength(0);
...
test("an approval cannot resolve an identically numbered request in a different conversation", async () => {
  const a = fixture(),
    b = fixture();
  const first = await session(a, "a"),
    second = await session(b, "b");
  try {
    for (const entry of [first, second]) {
      await entry.s.execute({ operationId: "one", text: "edit" });
      await entry.next();
    }
    a.emit(approval(11));
    b.emit(approval(11));
    const approvalA = await first.next();
    await second.next();
    if (approvalA.kind !== "approval.requested") throw Error("No approval");
    expect(
      (
        await second.s.resolveApproval({
          approvalId: approvalA.request.approvalId,
          optionId: approvalA.request.options[0]!.optionId,
        })
      ).status,
    ).toBe("rejected");
  ...
  test(`${end} invalidates native approval, including identical IDs in a reopened session`, async () => {
    ...
    if (end === "resolved") {
      f.emit({
        ...
        params: { threadId: "native-thread", requestId: 11 },
      });
      await next();
    }
    ...
    if (end === "restart") {
      await s.close();
      const reopened = await session(f);
      await reopened.s.execute({ operationId: "one", text: "new" });
      await reopened.next();
      const request = approval(11);
      (request.params as Record<string, unknown>).turnId = "turn-2";
      f.emit(request);
      await reopened.next();
      expect((await reopened.s.resolveApproval(response)).status).toBe("rejected");
      await reopened.s.close();
    }
    expect((await s.resolveApproval(response)).status).toBe("rejected");
    expect(f.replies.some((r) => (r.result as { action?: string }).action === "accept")).toBe(
      false,
    );
    await s.close();
  });
...
for (const status of ["approved", "denied", "timedOut", "aborted"] as const) {
  test(`native automatic review reports ${status} through safe existing observations`, async () => {
    ...
    const { s, next } = await session(f);
    try {
      await s.execute({ operationId: "one", text: "edit", reviewer: "delegated" });
      await next();
      const params = {
        threadId: "native-thread",
        turnId: "turn-1",
        ...
        targetItemId: null,
        startedAtMs: 1,
        action: { type: "mcpToolCall", server: "drawloom", toolName: "text.edit" },
      ...
      f.emit({
        ...
        params: {
          ...
          review: { status, rationale: "Scoped to the requested edit" },
        },
      });
      ...
      expect(result).toMatchObject({
        kind: "provider.observation",
        name: "approval-review",
        summary: expect.stringContaining("Scoped to the requested edit"),
      });
  ...
  try {
    ...
    f.emit({
      ...
      params: {
        threadId: "native-thread",
        turnId: "turn-1",
        action: { type: "mcpToolCall", toolName: "text.edit" },
...
for (const [phase, status] of [
  ...
  test(`automatic-review ${phase} rejects mismatched ${status} status`, async () => {
    ...
    try {
      ...
      await next();
      f.emit({
        method: `item/autoApprovalReview/${phase}`,
        params: {
          threadId: "native-thread",
          turnId: "turn-1",
          action: { type: "mcpToolCall", toolName: "text.edit" },
