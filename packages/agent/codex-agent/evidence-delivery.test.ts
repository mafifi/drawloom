...
test("native completed MCP results are correlated to the active execution and compaction invalidates retention", async () => {
  ...
  const driver = createCodexDriver({
    ...
    onContextInvalidated: (operationId) => {
      invalidated.push(operationId);
    },
  });
  ...
  void (async () => {
    for await (const _signal of session.signals()) {
      /* Drain the real session. */
    }
  })();
  expect(await session.execute({ operationId: "execution", text: "read" })).toMatchObject({
    status: "ok",
  });
  const result = {
    content: [{ type: "text", text: "evidence" }],
    _meta: { invocationId: "read-1", operationId: "execution" },
  };
  receive({
  ...
  expect(deliveries).toEqual([
    { operationId: "execution", server: "drawloom", tool: "knowledge.evidence", result },
  ]);
