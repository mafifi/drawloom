...
test("builds a self-contained shared EvaluationWorkbench MCP App and packs exact public fixtures", async () => {
  ...
  expect(await readFile(join(import.meta.dir, "org.drawloom", "workflows.js"), "utf8")).not.toMatch(/from ["']node:/);
  ...
  const extracted = Bun.spawnSync(["tar", "-xzf", archive, "--strip-components=1", "-C", installed], { stdout: "pipe", stderr: "pipe" });
