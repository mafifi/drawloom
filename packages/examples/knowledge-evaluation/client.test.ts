...
test("presentation adapter narrows starts to a fixed definition reference and validates every response", async () => {
  ...
  expect(
    await client.assess({
      ...
      definition: {
        ...
        id: "fixed",
        revision: "r1",
        name: "Browser copy must not cross",
        mode: "assess_existing",
        scorers: [{ id: "invented", revision: "r1" }],
        cases: [
          { id: "invented", revision: "r1", input: null, suppliedOutput: null, references: [] },
        ],
      },
    }),
  ...
  expect(calls.at(-1)).toEqual({
    operation: "assess",
    input: { requestId: "request-1", definition: { id: "fixed", revision: "r1" } },
  });
  responses.set("listDefinitions", {
    items: [
      {
        ref: { id: "invalid space", revision: "r1" },
        name: "Invalid",
        mode: "assess_existing",
        caseCount: 1,
        scorerCount: 1,
