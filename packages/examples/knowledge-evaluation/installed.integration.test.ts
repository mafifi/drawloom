...
import {
  ...
  type InstalledWorkflowRegistration,
...
function localOrchestrator(registry: Registry) {
  ...
  const runs = new Map<
    string,
    { snapshot: RunSnapshot; settled: Promise<unknown>; abort: AbortController }
  >();
  const orchestrator: Orchestrator = {
    async start(identity, workflow, input) {
      ...
      const context: WorkflowContext = {
        ...
        async task(stepId, task, taskInput) {
          ...
          const output = await handler.run(taskInput, {
            taskVersion: task.version,
            runId,
            stepId,
            attemptId: `${runId}/${stepId}/1`,
            attempt: 1,
            signal: abort.signal,
          });
      ...
      const settled = workflow.run(context, input).then(
        (value) => {
          snapshot.status = "completed";
          snapshot.output = value as never;
          return value;
        },
        (cause) => {
    ...
    async get(runId) {
      ...
      if (!value) throw new Error("run unavailable");
      return structuredClone(value.snapshot);
    },
    async getSteps(runId) {
      return { steps: (await orchestrator.get(runId)).steps };
    },
    async list() {
      return { runs: [...runs.values()].map((value) => structuredClone(value.snapshot)) };
    },
    async result(runId) {
      const value = runs.get(runId);
      if (!value) throw new Error("run unavailable");
      return (await value.settled) as never;
    },
    async respond() {
      throw new Error("unsupported");
    },
    async cancel(runId) {
      const value = runs.get(runId);
      if (!value) throw new Error("run unavailable");
      value.snapshot.cancellationRequested = true;
      value.abort.abort();
    },
  };
  return {
    ...
    close: async () => {
      await Promise.allSettled([...runs.values()].map((value) => value.settled));
    },
  };
}
...
test("packed public knowledge consumer executes outside the checkout and preserves project-scoped results and feedback across restart", async () => {
  const root = await mkdtemp(join(tmpdir(), "drawloom-installed-knowledge-evaluation-"));
  temporary.push(root);
  ...
  const fixtureNames = [
    ...
    "local-knowledge-answers-10k.json",
  ] as const;
  const installedSourceHashes = async () =>
    Promise.all(
      fixtureNames.map(async (name) =>
        createHash("sha256")
          .update(await readFile(join(packageRoot, "src", "fixtures", name)))
          .digest("hex"),
      ),
    );
  ...
  const assessment: EvaluationAssessmentProvider = {
    async assess(scorer, args, context) {
      assessmentScopes.push({
        owner: authority.current(),
        hasExpected: args.expected !== undefined,
        outputKind:
          typeof args.output === "object" && args.output !== null
            ? (args.output as { kind?: string }).kind
            : undefined,
      });
  ...
  async function activate(projectId: string) {
    ...
    const loaded = await loadInstalledPackages({
      root,
      project: { id: projectId, directory: root },
      installations: [installation],
      host: { store: createNodeJsonStore(join(root, `host-${projectId}`)), assets: unusedAssets() },
      async prepareWorkflows(_installation, inventory): Promise<InstalledWorkflowRegistration> {
        const registry = (
          await import(
            `${pathToFileURL(join(inventory.root, inventory.drawloom!.workflows!.entrypoint)).href}?project=${projectId}&restart=${Date.now()}`
          )
        ).default as Registry;
        ...
        return {
          capabilities: {
            ...
            orchestrationReadiness: async () => ({ status: "ready" }),
          },
          attach: async (handlers) =>
            local!.attach(
              authority.wrap(
                { projectId, installationId: installation.id },
                handlers,
                async () => {},
              ),
            ),
      ...
      prepareEvaluation: (_installation, _inventory, workflow) =>
        createInstalledEvaluation({
          dataDirectory: join(root, "evaluation-state"),
  ...
  try {
    ...
    const definitions = (
      opened.structuredContent as { items: Array<{ ref: { id: string; revision: string } }> }
    ).items;
    const expected = [
      ["knowledge.current.lexical", 24, 3],
      ["knowledge.current.mlx", 24, 3],
      ["knowledge.historical.lexical", 24, 1],
      ["knowledge.historical.cpu-qwen", 24, 1],
      ["knowledge.historical.cpu-nomic", 24, 1],
      ["knowledge.synthetic.c1-chain-omission", 1, 3],
      ["knowledge.synthetic.x1-stale-only", 1, 3],
    ] as const;
    ...
    for (const [definitionId, caseCount, findingCount] of expected) {
      ...
      const started = await app!.callTool({
        ...
        arguments: {
          operation: "assess",
          input: { requestId: `installed-${definitionId}`, definition: definition.ref },
        },
      });
      expect(started.structuredContent).toMatchObject({ kind: "started" });
      const evaluationRunId = (started.structuredContent as { evaluationRunId: string })
        .evaluationRunId;
      ...
      const results = await app!.callTool({
        name: "evaluation.request",
        arguments: { operation: "listResults", input: { runId: evaluationRunId, limit: 50 } },
      });
      const items = (
        results.structuredContent as {
          items: Array<{ id: string; status: string; findingCount: number }>;
        }
      ).items;
      expect(items).toHaveLength(caseCount);
      expect(
        items.every((item) => item.status === "completed" && item.findingCount === findingCount),
      ).toBe(true);
      resultIds.set(definitionId, items[0]!.id);
    }
    resultId = resultIds.get("knowledge.current.mlx")!;
    const saved = await app!.callTool({
      name: "evaluation.request",
      arguments: {
        operation: "saveFeedback",
        input: {
          schemaVersion: 1,
          id: "installed-feedback",
          resultId,
          attribution: "public integration test",
          rating: "correct",
          correction: "Retained frozen evidence only.",
          createdAtMs: 1,
        },
      },
    });
    ...
    expect(assessmentScopes.every((value) => (value as { hasExpected: boolean }).hasExpected)).toBe(
      true,
    );
    expect(
      assessmentScopes.filter(
        (value) => (value as { outputKind: string }).outputKind === "retrieval",
      ),
    ).toHaveLength(150);
    expect(
      assessmentScopes.filter((value) => (value as { outputKind: string }).outputKind === "answer"),
    ).toHaveLength(72);
    expect(assessmentScopes[0]).toMatchObject({
      owner: { projectId: "project-a", installationId: installation.id, recovering: false },
    });
    const historical = await app!.callTool({
      name: "evaluation.request",
      arguments: { operation: "getResult", input: resultIds.get("knowledge.historical.cpu-qwen") },
    });
    expect(
      (historical.structuredContent as { findings: Array<{ id: string }> }).findings.map(
        (item) => item.id,
      ),
    ).toEqual(["grounded-answer-heuristic"]);
    for (const id of [
      "knowledge.synthetic.c1-chain-omission",
      "knowledge.synthetic.x1-stale-only",
    ] as const) {
      const synthetic = await app!.callTool({
        name: "evaluation.request",
        arguments: { operation: "getResult", input: resultIds.get(id) },
      });
      expect(
        (synthetic.structuredContent as { findings: Array<{ score?: number }> }).findings.some(
          (item) => item.score === 0,
        ),
      ).toBe(true);
    }
  ...
  try {
    ...
    const runs = await app.callTool({
      name: "evaluation.request",
      arguments: { operation: "listRuns", input: { limit: 50 } },
    });
    expect((runs.structuredContent as { items: unknown[] }).items).toHaveLength(7);
    const feedback = await app.callTool({
      name: "evaluation.request",
      arguments: { operation: "listFeedback", input: { resultId: resultId!, limit: 50 } },
    });
    expect(
      (feedback.structuredContent as { items: Array<{ id: string; attribution: string }> }).items,
    ).toEqual([
      expect.objectContaining({ id: "installed-feedback", attribution: "public integration test" }),
    ]);
  } finally {
  ...
  try {
    ...
    const feedback = await app.callTool({
      name: "evaluation.request",
      arguments: { operation: "listFeedback", input: { limit: 50 } },
    });
