    ...
    try {
      ...
      async function activate() {
        ...
        const loaded = await loadInstalledPackages({
          ...
          host: { store: createNodeJsonStore(join(root, "host")), assets: unusedAssets() },
          async prepareWorkflows(_installed, inventory) {
            const registration = await manager.prepare({
              projectId: "project-a",
              installationId: installation.id,
              packageDirectory: inventory.root,
              entrypoint: inventory.drawloom!.workflows!.entrypoint,
            });
            return {
              capabilities: {
                orchestration: registration.orchestrator,
                orchestrationReadiness: async () => registration.readiness(),
              },
              attach: (handlers) =>
                registration.attach(
                  authority.wrap(
                    { projectId: "project-a", installationId: installation.id },
                    handlers,
                    async () => {},
                  ),
                ),
          ...
          prepareEvaluation: (_installed, _inventory, workflow) =>
            createInstalledEvaluation({
              dataDirectory: join(root, "evaluation-state"),
              scope: { installationId: installation.id, projectId: "project-a" },
              assessment,
              ...(workflow ? { workflow } : {}),
            }),
        });
      ...
      const readiness = await firstApp.callTool({
        name: "evaluation.request",
        arguments: { operation: "readiness", input: {} },
      });
      expect(readiness.structuredContent).toEqual({ status: "ready" });
      const started = await firstApp.callTool({
        name: "evaluation.request",
        arguments: {
          operation: "assess",
          input: { requestId: "durable-installed-mlx", definition: definition.ref },
        },
      });
      expect(started.isError, JSON.stringify(started)).not.toBe(true);
      expect(started.structuredContent).toMatchObject({ kind: "started" });
      const start = started.structuredContent as {
        evaluationRunId: unknown;
        orchestrationRunId: unknown;
      };
      ...
      for (let attempt = 0; attempt < 800 && status.kind === "running"; attempt++) {
        ...
        expect(
          typeof (response.structuredContent as { kind?: unknown } | undefined)?.kind,
      ...
      const results = await firstApp.callTool({
        name: "evaluation.request",
      ...
      await firstApp.callTool({
        name: "evaluation.request",
        arguments: {
          operation: "saveFeedback",
          input: {
            schemaVersion: 1,
            id: "durable-installed-feedback",
            resultId,
            attribution: "public real Temporal integration",
            rating: "correct",
            createdAtMs: 1,
          },
        },
      ...
      const restartedDefinition = (
        reopened.structuredContent as { items: Array<{ ref: { id: string; revision: string } }> }
      ).items.find((item) => item.ref.id === "knowledge.current.mlx")!;
      const reconciled = await restartedApp.callTool({
        name: "evaluation.request",
        arguments: {
          operation: "assess",
          input: { requestId: "durable-installed-mlx", definition: restartedDefinition.ref },
        },
      });
      expect(reconciled.structuredContent).toMatchObject({ kind: "reconciled", evaluationRunId });
      expect(scorerCalls).toBe(72);
      const feedback = await restartedApp.callTool({
        name: "evaluation.request",
        arguments: { operation: "listFeedback", input: { resultId, limit: 50 } },
      });
      expect((feedback.structuredContent as { items: Array<{ id: string }> }).items).toEqual([
        expect.objectContaining({ id: "durable-installed-feedback" }),
      ]);
    } finally {
      await active?.loaded.close();
      await active?.manager.close();
