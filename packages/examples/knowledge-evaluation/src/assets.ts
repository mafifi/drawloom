...
const currentReportSchema = z.looseObject({
  kind: z.literal("real_vector_evaluation"), corpus: corpusIdentitySchema, lexical: retrievalRunSchema,
...
export const KnowledgeAnswerOutputSchema = z.strictObject({
  ...
  citations: z.array(z.strictObject({ type: z.literal("source"), origin: z.literal("evaluation-public"), id: z.string().min(1), revision: z.string().min(1) })).max(100),
  abstained: z.boolean(),
});
export const HistoricalScoreSchema = z.strictObject({
  citations: z.strictObject({ supplied: z.number().int().nonnegative(), valid: z.number().int().nonnegative(), current: z.number().int().nonnegative(), expected: z.number().int().nonnegative(), invalid: z.array(z.string()) }),
  surfaceAssertions: z.strictObject({ matched: z.number().int().nonnegative(), total: z.number().int().nonnegative(), checks: z.array(z.strictObject({ label: z.string(), matched: z.boolean() })) }),
...
const historicalResultSchema = z.strictObject({
  mode: z.enum(["lexical", "qwen3-embedding-0.6b", "nomic-embed-text-v1.5"]), questionId: z.string().min(1),
  result: z.strictObject({ kind: z.literal("completed"), output: KnowledgeAnswerOutputSchema, nativeActivity: z.array(z.string()), cached: z.boolean() }),
  score: HistoricalScoreSchema,
});
const embeddedReportSchema = z.looseObject({
  corpus: corpusIdentitySchema, lexical: retrievalRunSchema,
...
const historicalReportSchema = z.looseObject({
  kind: z.literal("answer_quality_evaluation"), answeringModel: z.strictObject({ model: z.string().min(1), effort: z.string().min(1) }),
  retrieval: z.record(z.string(), embeddedReportSchema), results: z.array(historicalResultSchema).length(72), limitations: z.array(z.string()),
});

export type KnowledgeEvaluationAssets = { readonly corpus: Uint8Array; readonly current: Uint8Array; readonly historical: Uint8Array };

function verify(name: string, bytes: Uint8Array, expected: { bytes: number; sha256: string }): void {
  if (bytes.byteLength !== expected.bytes || sha256(bytes) !== expected.sha256) throw new Error(`${name} source identity changed`);
}

function validateQuestions(label: string, questions: z.infer<typeof questionsSchema>): void {
  if (Object.keys(questions).length !== heldOutQuestions.length) throw new Error(`${label} does not contain exactly the frozen questions`);
  for (const question of heldOutQuestions) {
    const reported = questions[question.id];
    if (!reported || reported.category !== question.kind) throw new Error(`${label} is missing frozen question/category ${question.id}`);
  }
}

export async function parseKnowledgeEvaluationAssets(assets: KnowledgeEvaluationAssets, options: { readonly verifySourceBytes?: boolean } = {}) {
  ...
  } catch { throw new Error("Frozen knowledge evaluation JSON is malformed"); }
  const current = currentReportSchema.parse(currentUnknown);
  const historical = historicalReportSchema.parse(historicalUnknown);
  if (corpusVersion !== current.corpus.version || knowledgeEvaluationSourceIdentity.corpus.sha256 !== current.corpus.sha256 || current.corpus.records !== 10_000) {
  ...
  for (const [mode, report] of Object.entries(historical.retrieval)) {
    if (report.corpus.version !== corpusVersion || report.corpus.sha256 !== knowledgeEvaluationSourceIdentity.corpus.sha256 || report.corpus.records !== 10_000) {
  ...
  for (const mode of ["lexical", "qwen3-embedding-0.6b", "nomic-embed-text-v1.5"] as const) {
    ...
    const rawQuestions = mode === "lexical"
    ...
    if (!questions) throw new Error(`Historical retrieval mode ${mode} has no retained vector questions`);
    validateQuestions(`historical ${mode} report`, questions);
  }
  if (new Set(historical.results.map(item => `${item.mode}:${item.questionId}`)).size !== 72) throw new Error("Historical answers do not contain 72 unique mode/question pairs");
  return Object.freeze({ current, historical, corpus: Object.freeze({ version: corpusVersion, documents, questions: heldOutQuestions }) });
}
