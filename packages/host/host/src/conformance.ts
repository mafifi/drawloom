...
export async function assetLibraryConformance(factory: () => Promise<AssetLibrary>): Promise<void> {
  ...
  try {
    await library.putStream(cancelledChunks(), "image/png", { signal: controller.signal });
  } catch {
    cancelled = true;
  }
  ...
  try {
    await library.putStream(chunks([]), "image/png");
  } catch {
    emptyRejected = true;
  }
...
export async function hostConformance(
  ...
  try {
    ...
    check(JSON.stringify(await fixture.store.get("record")) === '{"value":7}', "JSON round trip");
    await fixture.store.set("record", { value: 8 });
    check(JSON.stringify(await fixture.store.get("record")) === '{"value":8}', "replace value");
    const assets = fixture.assets;
    await assets.write("sample.bin", new Uint8Array([0, 4, 255]));
    check((await assets.read("sample.bin")).join(",") === "0,4,255", "asset bytes");
    const reader = await assets.open("sample.bin");
    check(reader.size === 3, "asset reader size");
    check(
      (await collect(reader.stream({ start: 1, endExclusive: 3 }))).join(",") === "4,255",
      "asset exact range",
    );
    for (const options of [
      ...
      try {
        await collect(reader.stream(options));
      } catch {
        invalidRange = true;
      }
    ...
    check(
      stableBytes.byteLength === original.byteLength && stableBytes.every((byte) => byte === 7),
      "open asset identity",
    );
    await stable.close();
    ...
    try {
      await assets.writeStream("atomic.bin", failedWrite(), { maxBytes: 8 });
    } catch {
      writeFailed = true;
    }
    ...
    try {
      await assets.writeStream("atomic.bin", cancelledWrite(), {
        maxBytes: 8,
    ...
    let oversized = false;
    try {
      await assets.writeStream("bounded.bin", chunks([1, 2], [3]), { maxBytes: 2 });
    } catch {
      oversized = true;
    }
    ...
    check(
      JSON.stringify(await fixture.transport.request("echo", { value: "safe" })) ===
        '{"value":"safe"}',
      "RPC correlation",
    );
