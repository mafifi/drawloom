...
const compactStepResult = z.strictObject({
  invocationId: EvaluationIdSchema,
  outcome: z.enum(["succeeded", "failed", "denied", "cancelled", "timed_out", "uncertain"]),
});
...
const planOutput = z
  ...
  .refine(
    (value) =>
      encoder.encode(JSON.stringify(value)).byteLength <= MAX_EVALUATION_PLAN_DISPATCH_BYTES,
    { message: `Evaluation plan exceeds ${MAX_EVALUATION_PLAN_DISPATCH_BYTES} bytes` },
  );
const workflowOutput = z.strictObject({
  evaluationRunId: EvaluationIdSchema,
  resultIds: z.array(EvaluationIdSchema).min(1).max(MAX_EVALUATION_WORKFLOW_STEPS),
});

export const evaluationPlanTask: Task<{ evaluationRunId: string }, z.infer<typeof planOutput>> = {
  id: "evaluation.plan",
  version: "1",
  input: z.strictObject({ evaluationRunId: EvaluationIdSchema }),
  output: planOutput,
  limits: { startToCloseTimeoutMs: 30_000 },
};
export const evaluationTargetTask: Task<
  z.infer<typeof jobPlan>,
  z.infer<typeof compactStepResult>
> = {
  id: "evaluation.target",
  version: "1",
  input: jobPlan,
  output: compactStepResult,
  limits: { startToCloseTimeoutMs: 3_600_000 },
};
const scorerInput = commonStep.extend({
  scorer: reference,
  invocationId: EvaluationIdSchema,
  targetInvocationId: EvaluationIdSchema.optional(),
});
export const evaluationScorerTask: Task<
  z.infer<typeof scorerInput>,
  z.infer<typeof compactStepResult>
> = {
  id: "evaluation.scorer",
  version: "1",
  input: scorerInput,
  output: compactStepResult,
  limits: { startToCloseTimeoutMs: 3_600_000 },
};
const finalizeInput = commonStep.extend({
  resultId: EvaluationIdSchema,
  targetInvocationId: EvaluationIdSchema.optional(),
  scorerInvocationIds: z.array(EvaluationIdSchema).min(1).max(32),
});
const finalizeOutput = z.strictObject({
  resultId: EvaluationIdSchema,
  status: EvaluationResultRecordSchema.shape.status,
});
export const evaluationFinalizeTask: Task<
  z.infer<typeof finalizeInput>,
  z.infer<typeof finalizeOutput>
> = {
...
export const evaluationWorkflow: Workflow<
  ...
  async run(context, input) {
    ...
    const execute = async (job: z.infer<typeof jobPlan>) => {
      ...
      if (job.target && job.targetInvocationId) {
        const target = await context.task(
          `case-${job.caseIndex}-trial-${job.trial}-target`,
          evaluationTargetTask,
          job,
        );
        targetInvocationId = target.invocationId;
      }
      const scorerResults = await Promise.all(
        job.scorers.map((entry, scorerIndex) =>
          context.task(
      ...
      const final = await context.task(
        `case-${job.caseIndex}-trial-${job.trial}-finalize`,
        evaluationFinalizeTask,
        {
          evaluationRunId: job.evaluationRunId,
          definition: job.definition,
          caseId: job.caseId,
          ...
          ...(targetInvocationId ? { targetInvocationId } : {}),
          scorerInvocationIds: scorerResults.map((item) => item.invocationId),
        },
        { maxAttempts: 3 },
      );
    ...
    for (let offset = 0; offset < plan.jobs.length; offset += plan.concurrency) {
      resultIds.push(
        ...(await Promise.all(plan.jobs.slice(offset, offset + plan.concurrency).map(execute))),
      );
    }
...
const toReference = (value: VersionedReference): VersionedReference => ({
  id: value.id,
  revision: value.revision,
});
const selected = <T extends { id: string; revision: string }>(
  ...
  if (!found)
    throw new StepFailure("invalid", `${kind} binding ${publicKey(value)} is unavailable`);
  return found;
};
...
function status(
  target: TargetCheckpoint | undefined,
  scorers: readonly ScorerCheckpoint[],
): EvaluationResultRecord["status"] {
  if (target?.outcome === "uncertain" || scorers.some((item) => item.outcome === "uncertain"))
    return "uncertain";
  if (target && target.outcome !== "succeeded")
    return target.outcome === "failed" ? "failed" : target.outcome;
  ...
  return scorers.some((item) => item.findings.some((finding) => finding.outcome === "scored"))
    ? "completed"
    : "unscored";
}

function boundedUnresolvedEffects(values: readonly string[]): readonly string[] {
  if (values.length <= 100 && values.every((value) => value.length >= 1 && value.length <= 512))
    return values;
  ...
  return [
    ...retained,
    `${omitted} additional unresolved effect${omitted === 1 ? "" : "s"} omitted from this bounded status view`,
  ];
}

function snapshotTarget(value: EvaluationTarget): EvaluationTarget {
  const id = EvaluationIdSchema.parse(value.id),
    revision = z.string().min(1).max(160).parse(value.revision);
...
export function createEvaluationComposer(options: EvaluationComposerOptions): EvaluationComposer {
  ...
  return Object.freeze({
    compose(bindings: EvaluationCompositionBindings) {
      if (composed)
        throw new Error("Evaluation capability is already composed for this activation");
      composed = true;
      const targets = new Map<string, EvaluationTarget>();
      for (const candidate of bindings.targets ?? []) {
        const value = snapshotTarget(candidate),
          identity = key(value);
      ...
      for (const candidate of [...(options.assessment.scorers ?? []), ...bindings.scorers]) {
        const value = snapshotScorer(candidate),
          identity = key(value);
        if (scorers.has(identity)) throw new Error(`Duplicate scorer binding ${publicKey(value)}`);
        scorers.set(identity, value);
      }
      const assess = options.assessment.assess.bind(
        options.assessment,
      ) as EvaluationAssessmentProvider["assess"];
      const identity = async (value: EvaluationJson) =>
        EvaluationIdSchema.parse(await options.identity(EvaluationJsonSchema.parse(value)));
      ...
      const caseFor = async (input: z.infer<typeof commonStep>) => {
        ...
        if (!value || value.revision !== input.caseRevision)
          throw new StepFailure("invalid", "Evaluation case is unavailable");
        return value;
      };
      const buildPlan = async (evaluationRunId: string) => {
        ...
        for (const [caseIndex, item] of definition.cases.entries())
          for (let trial = 0; trial < run.settings.repetitions; trial++) {
            const base = [evaluationRunId, caseIndex, trial] as const;
            const targetInvocationId = definition.target
              ? await identity(["target", ...base])
              : undefined;
            const scorerEntries = await Promise.all(
              definition.scorers.map(async (scorer, scorerIndex) => ({
                scorer: toReference(scorer),
                invocationId: await identity(["scorer", ...base, scorerIndex]),
              })),
            );
            jobs.push(
              jobPlan.parse({
                ...
                trial,
                ...(definition.target
                  ? { target: toReference(definition.target), targetInvocationId }
                  : {}),
                scorers: scorerEntries,
                resultId: await identity(["result", ...base]),
              }),
            );
          }
      ...
      const planHandler = registerTaskHandler(evaluationPlanTask, {
        async run(input, context) {
          await options.store.saveOrchestrationBinding({
        ...
        async recover(input, context) {
          ...
          if (!binding || binding.orchestrationRunId !== context.runId)
      ...
      const targetHandler = registerTaskHandler(evaluationTargetTask, {
        async run(input, context) {
          if (!input.target || !input.targetInvocationId)
            throw new StepFailure("invalid", "Target step is incomplete");
          ...
          if (existing)
            return compactStepResult.parse({
              invocationId: existing.invocationId,
              outcome: existing.outcome,
            });
          const definition = await header(input.definition);
          if (!definition.target || key(definition.target) !== key(input.target))
            throw new StepFailure("invalid", "Target selection changed");
          ...
          try {
            ...
            result = TargetInvocationResultSchema.parse(
              await implementation.invoke(
                {
                  input: parsedInput,
                  ...(definition.target.configuration === undefined
                    ? {}
                    : { configuration: definition.target.configuration }),
            ...
            result = {
              outcome: outcome(error, context.signal),
              error: { code: "target_failed", message: "Target invocation failed" },
              references: [],
            } as const;
          }
          const checkpoint = TargetCheckpointSchema.parse({
            schemaVersion: EVALUATION_SCHEMA_VERSION,
            invocationId: input.targetInvocationId,
            runId: input.evaluationRunId,
            caseId: input.caseId,
            ...
            ...result,
            startedAtMs,
            completedAtMs: options.clock.now(),
          });
        ...
        async recover(input) {
          ...
          const checkpoint = await options.store.getTargetCheckpoint(
            selector(input, input.targetInvocationId),
          );
          return checkpoint
            ? {
                status: "completed",
                output: { invocationId: checkpoint.invocationId, outcome: checkpoint.outcome },
              }
            : { status: "unknown" };
        },
      });
      const scorerHandler = registerTaskHandler(evaluationScorerTask, {
        async run(input, context) {
          ...
          const definition = await header(input.definition);
          const selectedDefinition = definition.scorers.find(
            (value) => key(value) === key(input.scorer),
          );
          ...
          try {
            ...
            if (input.targetInvocationId) {
              const target = await options.store.getTargetCheckpoint(
                selector(input, input.targetInvocationId),
              );
            ...
            if (!result) {
              ...
              const parsedExpected =
                item.expected === undefined || !implementation.expected
                  ? undefined
                  : implementation.expected.parse(item.expected);
              result = ScorerInvocationResultSchema.parse(
                await assess(
                  implementation,
                  {
                    ...
                    ...(selectedDefinition.configuration === undefined
                      ? {}
            ...
            checkpoint = ScorerCheckpointSchema.parse({
              schemaVersion: EVALUATION_SCHEMA_VERSION,
              invocationId: input.invocationId,
              runId: input.evaluationRunId,
              caseId: input.caseId,
            ...
            checkpoint = ScorerCheckpointSchema.parse({
              ...
              scorer: input.scorer,
              outcome: mapped,
              findings: mapped === "failed" ? [errorFinding(input.scorer)] : [],
              startedAtMs,
              completedAtMs: options.clock.now(),
            });
          }
        ...
        async recover(input) {
          const checkpoint = await options.store.getScorerCheckpoint(
            selector(input, input.invocationId),
          );
          return checkpoint
            ? {
                status: "completed",
      ...
      const finalizeHandler = registerTaskHandler(evaluationFinalizeTask, {
        async run(input) {
          ...
          const target = input.targetInvocationId
            ? await options.store.getTargetCheckpoint(selector(input, input.targetInvocationId))
            : undefined;
          if (input.targetInvocationId && !target)
            throw new StepFailure("unknown", "Target checkpoint is unavailable");
          const scorerCheckpoints = await Promise.all(
            input.scorerInvocationIds.map((invocationId) =>
              options.store.getScorerCheckpoint(selector(input, invocationId)),
            ),
          );
          if (scorerCheckpoints.some((value) => !value))
            throw new StepFailure("unknown", "Scorer checkpoint is unavailable");
          ...
          const result = EvaluationResultRecordSchema.parse({
            ...
            caseRevision: input.caseRevision,
            trial: input.trial,
            status: status(target, scorers),
            ...(target ? { targetInvocationId: target.invocationId } : {}),
            scorerInvocationIds: scorers.map((value) => value.invocationId),
            startedAtMs: Math.min(...times.map((value) => value.startedAtMs)),
            completedAtMs: Math.max(...times.map((value) => value.completedAtMs)),
          });
        ...
        async recover(input) {
          ...
          return result
            ? { status: "completed", output: { resultId: result.id, status: result.status } }
            : { status: "retryable" };
        },
      });
      const taskHandlers = Object.freeze([
        planHandler,
        targetHandler,
        scorerHandler,
        finalizeHandler,
      ]) satisfies readonly RegisteredTaskHandler[];

      const available = async () =>
        EvaluationReadinessSchema.parse(
          (await options.readiness?.()) ??
            (options.orchestrator
              ? { status: "ready" as const }
              : { status: "unavailable" as const, reason: "Orchestration is not configured" }),
        );
      const unavailableReason = (readiness: EvaluationReadiness) =>
        readiness.status === "unavailable" ? readiness.reason : undefined;
      const start = async (
        raw: Parameters<EvaluationService["assess"]>[0],
        expectedMode: EvaluationDefinitionHeader["mode"],
      ): Promise<EvaluationStartResult> => {
        const request = EvaluationStartRequestSchema.parse(raw);
        if (request.definition.mode !== expectedMode)
          throw new StepFailure("invalid", `Expected ${expectedMode} definition`);
        ...
        const taskCount =
          1 +
          request.definition.cases.length *
            request.settings.repetitions *
            (request.definition.scorers.length + (request.definition.target ? 1 : 0) + 1);
        if (taskCount > MAX_EVALUATION_WORKFLOW_STEPS)
          throw new StepFailure(
            "invalid",
            `Evaluation exceeds the ${MAX_EVALUATION_WORKFLOW_STEPS}-step workflow bound`,
          );
        const readiness = await available();
        if (readiness.status !== "ready" || !options.orchestrator)
          return EvaluationStartResultSchema.parse({
            kind: "unavailable",
            reason: unavailableReason(readiness) ?? "Orchestration is unavailable",
          });
        const evaluationRunId = await identity(["evaluation-run", request.requestId]);
        const orchestrationIdentity = await identity([
        ...
        const disposition = await options.store.saveRun({
          schemaVersion: EVALUATION_SCHEMA_VERSION,
          id: evaluationRunId,
          requestId: request.requestId,
          definition: definitionRef,
          settings: request.settings,
          createdAtMs: existingRun?.createdAtMs ?? options.clock.now(),
        });
        ...
        if (prior)
          return EvaluationStartResultSchema.parse({
            kind: "reconciled",
            evaluationRunId,
            orchestrationRunId: prior.orchestrationRunId,
          });
        const existingAttempt = await options.store.getStartAttempt(evaluationRunId);
        if (!existingAttempt) {
          try {
            await options.store.saveStartAttempt({
              schemaVersion: EVALUATION_SCHEMA_VERSION,
              evaluationRunId,
              requestedAtMs: options.clock.now(),
            });
          } catch (error) {
            if (
              !(error instanceof EvaluationStoreError) ||
              error.code !== "conflict" ||
              !(await options.store.getStartAttempt(evaluationRunId))
            )
              throw error;
          }
        }
        ...
        try {
          orchestrationRunId = await options.orchestrator.start(
          ...
          return binding
            ? EvaluationStartResultSchema.parse({
                kind: "reconciled",
                evaluationRunId,
                orchestrationRunId: binding.orchestrationRunId,
              })
        ...
        return EvaluationStartResultSchema.parse({
          kind: disposition.kind === "accepted" ? "started" : "reconciled",
          evaluationRunId,
          orchestrationRunId,
        });
      };
      const service: EvaluationService = {
        ...
        async status(evaluationRunId): Promise<EvaluationExecutionStatus> {
          ...
          if (!(await options.store.getRun(runId)))
            throw new StepFailure("invalid", "Evaluation run is unavailable");
          const binding = await options.store.getOrchestrationBinding(runId);
          if (!binding)
            return EvaluationExecutionStatusSchema.parse(
              (await options.store.getStartAttempt(runId))
          ...
          try {
            ...
            if (snapshot.unresolvedEffects.length)
              return EvaluationExecutionStatusSchema.parse({
                kind: "uncertain",
                evaluationRunId: runId,
                orchestrationRunId: binding.orchestrationRunId,
                unresolvedEffects: boundedUnresolvedEffects(snapshot.unresolvedEffects),
              });
            ...
            return EvaluationExecutionStatusSchema.parse({
              kind: "unavailable",
              evaluationRunId: runId,
              reason: "Orchestration status is unavailable",
            });
          }
        },
        async cancel(evaluationRunId): Promise<EvaluationCancelResult> {
          ...
          try {
            ...
            return EvaluationCancelResultSchema.parse({
              kind: "unavailable",
              evaluationRunId: runId,
              reason: "Orchestration cancellation is unavailable",
            });
          }
        },
      };
