...
test("definition modes, case identities, JSON size, and media references are bounded", () => {
  ...
  expect(
    EvaluationDefinitionSchema.safeParse({
      ...
      cases: [
        { id: "case", revision: "r1", input: null },
...
test("whole scorer checkpoints and result checkpoint identities are bounded", () => {
  ...
  expect(
    ScorerCheckpointSchema.safeParse({
      schemaVersion: 1,
      invocationId: "invoke",
      runId: "run",
      caseId: "case",
...
test("evaluation starts materialize bounded defaults and expose uncertainty without inventing a provider run", () => {
  const definition = {
    ...
    cases: [
      { id: "case", revision: "r1", input: "hello", expected: "hello", suppliedOutput: "hello" },
    ],
  } as const;
  ...
  expect(
    EvaluationStartRequestSchema.safeParse({
      requestId: "request",
      definition,
...
test("execution and cancellation outcomes keep saved work distinct from scheduler availability", () => {
  expect(EvaluationExecutionStatusSchema.parse({ kind: "saved", evaluationRunId: "run" })).toEqual({
    kind: "saved",
    evaluationRunId: "run",
  });
  expect(
    EvaluationExecutionStatusSchema.parse({ kind: "start_uncertain", evaluationRunId: "run" }),
  ).toEqual({ kind: "start_uncertain", evaluationRunId: "run" });
  expect(
    EvaluationExecutionStatusSchema.safeParse({
      kind: "uncertain",
      evaluationRunId: "run",
      unresolvedEffects: [],
    }).success,
  ...
  expect(EvaluationCancelResultSchema.parse({ kind: "uncertain", evaluationRunId: "run" })).toEqual(
    { kind: "uncertain", evaluationRunId: "run" },
  );
  expect(
    EvaluationStartAttemptSchema.parse({
      schemaVersion: 1,
      evaluationRunId: "run",
      requestedAtMs: 10,
    }),
  ).toEqual({ schemaVersion: 1, evaluationRunId: "run", requestedAtMs: 10 });
});
