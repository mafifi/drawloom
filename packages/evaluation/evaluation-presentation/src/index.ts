...
export interface EvaluationPresentationClient
  extends Pick<
    EvaluationService,
    | "readiness"
    | "assess"
...
function sameScorers(a: EvaluationDefinitionHeader, b: EvaluationDefinitionHeader): boolean {
  const identity = (value: EvaluationDefinitionHeader["scorers"][number]) =>
    stable({ id: value.id, revision: value.revision, configuration: value.configuration });
...
function compare(current: ResultContext, baseline: ResultContext): EvaluationComparison {
  if (
    current.summary.caseId !== baseline.summary.caseId ||
    current.summary.caseRevision !== baseline.summary.caseRevision
  ) {
    return { kind: "incomparable", reason: "Case identities or revisions do not match." };
  }
  if (
    stable(current.selectedCase.input) !== stable(baseline.selectedCase.input) ||
    stable(current.selectedCase.expected) !== stable(baseline.selectedCase.expected)
  ) {
    return { kind: "incomparable", reason: "Case input or expected material does not match." };
  }
  if (!sameScorers(current.header, baseline.header))
    return { kind: "incomparable", reason: "Criterion revisions or configurations do not match." };
  if (!current.detail || !baseline.detail)
    return {
      kind: "incomparable",
      reason: "Combined details are unavailable. Inspect one scorer checkpoint at a time.",
    };
  const byFinding = (view: EvaluationResultView) =>
    new Map(
      view.scorers.flatMap((checkpoint) =>
        checkpoint.findings.map(
          (finding) =>
            [
              stable([checkpoint.scorer.id, checkpoint.scorer.revision, finding.id]),
              { checkpoint, finding },
            ] as const,
        ),
      ),
    );
  const currentFindings = byFinding(current.detail),
    baselineFindings = byFinding(baseline.detail);
  const keys = [...new Set([...currentFindings.keys(), ...baselineFindings.keys()])].sort();
  return {
    kind: "comparable",
    findings: keys.map((key) => {
      const currentItem = currentFindings.get(key),
        baselineItem = baselineFindings.get(key),
        item = currentItem ?? baselineItem!;
      return {
        ...
        ...(currentItem
          ? {
              current: {
                outcome: currentItem.finding.outcome,
                ...(currentItem.finding.score === undefined
                  ? {}
                  : { score: currentItem.finding.score }),
              },
            }
          : {}),
        ...(baselineItem
          ? {
              baseline: {
                outcome: baselineItem.finding.outcome,
                ...(baselineItem.finding.score === undefined
                  ? {}
                  : { score: baselineItem.finding.score }),
              },
            }
          : {}),
      };
...
export function createEvaluationViewModel(options: {
  ...
  let selectedDefinition: DefinitionSummary | undefined,
    selectedRun: EvaluationRun | undefined,
    selectedResult: ResultSummary | undefined;
  let selectedHeader: EvaluationDefinitionHeader | undefined,
    selectedCase: EvaluationCase | undefined;
  let startReadiness: EvaluationReadiness = {
    status: "unavailable",
    reason: "Start availability is being checked.",
  };
  let execution: EvaluationExecutionStatus | undefined,
    detail: EvaluationResultView | undefined,
    selectedTarget: TargetCheckpoint | undefined,
    selectedScorer: ScorerCheckpoint | undefined;
  let baseline: EvaluationBaselinePresentation | undefined,
    comparison: EvaluationComparison | undefined;
  let baselineResultId = "";
  const drafts = new Map<string, EvaluationFeedbackDraft>(),
    feedbackIntents = new Map<string, EvaluationFeedback>();
  const feedbackPages = new Map<string, readonly EvaluationFeedback[]>(),
    feedbackErrors = new Map<string, string>();
  const feedbackLoadingResults = new Set<string>(),
    feedbackSavingResults = new Set<string>(),
    feedbackReadVersions = new Map<string, number>();
  const startIntents = new Map<string, string>(),
    startsInFlight = new Set<string>();
  ...
  let definitionCursor: string | undefined,
    runCursor: string | undefined,
    resultCursor: string | undefined;
  let definitionPageCursor: string | undefined,
    runPageCursor: string | undefined,
  ...
  const presentation: EvaluationPresentation = {
    ...
    get feedback() {
      return selectedResult ? (feedbackPages.get(selectedResult.id) ?? []) : [];
    },
    get feedbackDraft() {
      return selectedResult ? (drafts.get(selectedResult.id) ?? EMPTY_DRAFT) : EMPTY_DRAFT;
    },
    ...
    get scorerLoading() {
      return scorerLoading;
    },
    get feedbackLoading() {
      return selectedResult ? feedbackLoadingResults.has(selectedResult.id) : false;
    },
    get startPending() {
      return selectedDefinition ? startsInFlight.has(stable(selectedDefinition.ref)) : false;
    },
    get cancelPending() {
      return cancelPending;
    },
    get feedbackPending() {
      return selectedResult ? feedbackSavingResults.has(selectedResult.id) : false;
    },
    get hasMoreDefinitions() {
      return hasMoreDefinitions;
    },
    get hasMoreRuns() {
      return hasMoreRuns;
    },
    ...
    get feedbackError() {
      return selectedResult ? (feedbackErrors.get(selectedResult.id) ?? "") : "";
    },
  };
  const changed = () => {
  ...
  async function readDefinitions(after?: string) {
    const version = ++definitionsVersion,
      active = life;
    ...
    try {
      ...
      definitionCursor = page.cursor;
      definitionPageCursor = after;
      hasMoreDefinitions = page.hasMore;
      if (selectedDefinition)
        selectedDefinition =
          page.items.find((item) => stable(item.ref) === stable(selectedDefinition!.ref)) ??
          selectedDefinition;
    } catch (cause) {
      if (active === life && version === definitionsVersion)
        error = message(cause, "Saved checks are unavailable.");
    } finally {
      if (active === life && version === definitionsVersion) {
  ...
  async function readRuns(after?: string) {
    ...
    try {
      ...
      if (active === life && version === runsVersion)
        error = message(cause, "Saved runs are unavailable.");
    } finally {
      if (active === life && version === runsVersion) {
        runsLoading = false;
        changed();
  ...
  async function readResults(after?: string) {
    ...
    try {
      ...
      resultPageCursor = after;
      hasMoreResults = page.hasMore;
    } catch (cause) {
      if (active === life && version === resultVersion)
        error = message(cause, "Case results are unavailable.");
    } finally {
      if (active === life && version === resultVersion) {
        resultLoading = false;
        changed();
  ...
  async function resultContext(summary: ResultSummary): Promise<ResultContext> {
    ...
    if (!caseValue || caseValue.revision !== summary.caseRevision)
      throw Error("The saved evaluation case is unavailable.");
    return { summary, run, header, selectedCase: caseValue };
  }
  async function readReadiness() {
    const version = ++readinessVersion,
      active = life;
    startReadiness = { status: "unavailable", reason: "Checking evaluation setup." };
    changed();
    try {
      ...
      if (active === life && version === readinessVersion)
        startReadiness = {
          status: "unavailable",
          reason: message(cause, "Start availability could not be read."),
        };
    } finally {
      if (active === life && version === readinessVersion) changed();
    }
  }
  async function reloadFeedbackFor(resultId: string) {
    const version = (feedbackReadVersions.get(resultId) ?? 0) + 1,
      active = life;
    feedbackReadVersions.set(resultId, version);
    feedbackLoadingResults.add(resultId);
    feedbackErrors.delete(resultId);
    changed();
    try {
      const page = await client.listFeedback({ resultId, limit: 50 });
      if (active === life && feedbackReadVersions.get(resultId) === version)
        feedbackPages.set(resultId, page.items);
    } catch (cause) {
      if (active === life && feedbackReadVersions.get(resultId) === version)
        feedbackErrors.set(resultId, message(cause, "Feedback is unavailable."));
    } finally {
      if (active === life && feedbackReadVersions.get(resultId) === version) {
        feedbackLoadingResults.delete(resultId);
        changed();
      }
    }
  }
  async function reloadFeedback() {
    if (selectedResult) await reloadFeedbackFor(selectedResult.id);
  }
  async function selectRun(runId: string) {
    const selection = ++runSelectionVersion,
      active = life;
    ...
    resultPageCursor = undefined;
    execution = undefined;
    cancelPending = false;
    error = "";
    changed();
    const [statusResult] = await Promise.allSettled([client.status(candidate.id), readResults()]);
    if (active !== life || selection !== runSelectionVersion || selectedRun?.id !== candidate.id)
      return;
    execution =
      statusResult.status === "fulfilled"
        ...
        : {
            ...
            reason: "Execution status is unavailable.",
          };
    changed();
  }
  async function selectResult(resultId: string) {
    const selection = ++resultSelectionVersion,
      active = life;
    targetVersion++;
    scorerVersion++;
    baselineVersion++;
    const listed = results.find((item) => item.id === resultId);
    selectedResult = listed;
    selectedHeader = undefined;
    selectedCase = undefined;
    detail = undefined;
    selectedTarget = undefined;
    ...
    try {
      ...
      if (!summary || summary.id !== resultId)
        throw Error("The selected case result is unavailable.");
      ...
      try {
        ...
        if (view && view.result.id !== summary.id)
          throw Error("Evaluation result identity changed.");
      ...
      if (
        active !== life ||
        selection !== resultSelectionVersion ||
        selectedResult?.id !== resultId
      )
        return;
      selectedHeader = context.header;
      ...
      if (active === life && selection === resultSelectionVersion)
        detailError = message(cause, "The selected result is unavailable.");
    } finally {
      if (active === life && selection === resultSelectionVersion) {
        resultLoading = false;
        changed();
  ...
  async function start() {
    ...
    const selection = selectedDefinition,
      selectedKey = stable(selection.ref),
      active = life;
    if (startsInFlight.has(selectedKey)) return;
    if (startReadiness.status !== "ready") {
      error = startReadiness.reason ?? "Starts are unavailable.";
      changed();
      return;
    }
    const requestId = startIntents.get(selectedKey) ?? nextId();
    startIntents.set(selectedKey, requestId);
    startsInFlight.add(selectedKey);
    error = "";
    changed();
    let submitted = false;
    try {
      ...
      if (
        !selected ||
        selected.id !== selection.ref.id ||
        selected.revision !== selection.ref.revision
      )
        throw Error("The selected saved check is unavailable.");
      submitted = true;
      const request = { requestId, definition: selected };
      const result =
        selected.mode === "assess_existing"
          ? await client.assess(request)
          : await client.run(request);
      const stillSelected = () =>
        selectedDefinition?.ref.id === selection.ref.id &&
        selectedDefinition.ref.revision === selection.ref.revision;
      if (result.kind === "unavailable") {
        if (active === life && stillSelected()) {
          startReadiness = { status: "unavailable", reason: result.reason };
          error = result.reason;
        }
        return;
      }
      if (result.kind === "uncertain") {
        if (active === life && stillSelected())
          error =
            "The start outcome is uncertain. Retrying will reconcile the same request identity.";
        return;
      }
      ...
      if (!submitted && startIntents.get(selectedKey) === requestId)
        startIntents.delete(selectedKey);
      if (active === life) error = message(cause, "The evaluation could not be started.");
    } finally {
      startsInFlight.delete(selectedKey);
      changed();
    }
  }
  async function cancel() {
    ...
    const runId = selectedRun.id,
      selection = runSelectionVersion,
      operation = ++cancelVersion,
      active = life;
    cancelPending = true;
    error = "";
    changed();
    ...
    try {
      ...
      if (result.kind === "requested")
        ...
        execution = { kind: "saved", evaluationRunId: runId };
      else if (result.kind === "uncertain")
        error = "Cancellation could not be confirmed. Effects may still settle.";
      else error = result.reason;
    } catch (cause) {
      if (current()) error = message(cause, "Cancellation could not be confirmed.");
    } finally {
      if (current()) {
        cancelPending = false;
        changed();
  ...
  async function selectTarget() {
    ...
    const summary = selectedResult,
      invocationId = selectedResult.targetInvocationId,
      selection = resultSelectionVersion,
    ...
    const current = () =>
      ...
      selection === resultSelectionVersion &&
      operation === targetVersion &&
      selectedResult?.id === summary.id;
    try {
      const checkpoint = await client.getTargetCheckpoint({
        invocationId,
        runId: summary.runId,
      ...
      if (current()) detailError = message(cause, "The retained target checkpoint is unavailable.");
    } finally {
      if (current()) {
        targetLoading = false;
        changed();
  ...
  async function selectScorer(invocationId: string) {
    if (
      !selectedResult ||
      !selectedResult.scorerInvocationIds.includes(invocationId) ||
      scorerLoading
    )
      return;
    const summary = selectedResult,
      selection = resultSelectionVersion,
      operation = ++scorerVersion,
      active = life;
    scorerLoading = true;
    detailError = "";
    changed();
    const current = () =>
      ...
      selectedResult?.id === summary.id;
    try {
      const checkpoint = await client.getScorerCheckpoint({
        invocationId,
        runId: summary.runId,
        caseId: summary.caseId,
        caseRevision: summary.caseRevision,
      ...
      if (current()) detailError = message(cause, "The selected scorer checkpoint is unavailable.");
    } finally {
      if (current()) {
        scorerLoading = false;
        changed();
  ...
  async function selectBaseline(resultId: string) {
    ...
    const currentSummary = selectedResult,
      selection = resultSelectionVersion,
      operation = ++baselineVersion,
      active = life;
    resultLoading = true;
    detailError = "";
    changed();
    const current = () =>
      ...
      selection === resultSelectionVersion &&
      operation === baselineVersion &&
      selectedResult?.id === currentSummary.id &&
      baselineResultId === resultId;
    try {
      const baselineSummary = await client.getResultSummary(resultId);
      if (!baselineSummary || baselineSummary.id !== resultId)
        throw Error("The baseline result is unavailable.");
      ...
      try {
        ...
        if (view && view.result.id !== baselineSummary.id)
          throw Error("Baseline result identity changed.");
        if (view) baselineContext.detail = view;
      } catch {
        /* Exact compatibility remains visible even when combined detail is too large. */
      }
      if (!current()) return;
      baseline = {
        summary: baselineSummary,
        ...(baselineContext.detail ? { detail: baselineContext.detail } : {}),
        header: baselineContext.header,
        selectedCase: baselineContext.selectedCase,
      };
      comparison = compare(currentContext, baselineContext);
    } catch (cause) {
      if (current()) detailError = message(cause, "The baseline could not be compared.");
    } finally {
      if (current()) {
        resultLoading = false;
        changed();
  ...
  function setFeedback(value: Partial<EvaluationFeedbackDraft>) {
    ...
    const resultId = selectedResult.id,
      draft = Object.freeze({ ...(drafts.get(resultId) ?? EMPTY_DRAFT), ...value });
    drafts.set(resultId, draft);
    feedbackIntents.delete(resultId);
    feedbackErrors.delete(resultId);
    changed();
  }
  async function saveFeedback() {
    if (!selectedResult || feedbackSavingResults.has(selectedResult.id)) return;
    const resultId = selectedResult.id,
      draft = drafts.get(resultId) ?? EMPTY_DRAFT;
    if (!draft.attribution.trim()) {
      feedbackErrors.set(resultId, "Feedback attribution is required.");
      changed();
      return;
    }
    if (!draft.rating) {
      feedbackErrors.set(
        resultId,
        "Choose correct, incorrect, or uncertain before saving feedback.",
      );
      changed();
      return;
    }
    ...
    try {
      value ??= EvaluationFeedbackSchema.parse({
        schemaVersion: 1,
        id: nextId(),
        resultId,
      ...
      feedbackErrors.set(resultId, message(cause, "Feedback is invalid."));
      changed();
      return;
    }
    feedbackIntents.set(resultId, value);
    feedbackSavingResults.add(resultId);
    feedbackErrors.delete(resultId);
    changed();
    try {
      await client.saveFeedback(value);
      feedbackIntents.delete(resultId);
      drafts.delete(resultId);
      feedbackErrors.delete(resultId);
      const cached = feedbackPages.get(resultId) ?? [];
      if (!cached.some((item) => item.id === value.id))
        feedbackPages.set(resultId, [value, ...cached]);
      if (opened) await reloadFeedbackFor(resultId);
    } catch (cause) {
      feedbackErrors.set(
        resultId,
        message(cause, "Feedback could not be saved. Your draft is unchanged."),
      );
    } finally {
      feedbackSavingResults.delete(resultId);
      changed();
    }
  }
  async function refreshRun() {
    ...
    try {
      ...
      execution = next;
      await readResults(resultPageCursor);
    } catch (cause) {
      if (active === life && selection === runSelectionVersion)
        error = message(cause, "Evaluation status is unavailable.");
    }
  ...
  const actions: EvaluationActions = {
    selectDefinition(ref) {
      selectedDefinition = definitions.find(
        (item) => item.ref.id === ref.id && item.ref.revision === ref.revision,
      );
      error = "";
      changed();
    },
    setBaselineResultId(resultId) {
      baselineVersion++;
      baselineResultId = resultId;
      comparison = undefined;
      baseline = undefined;
    ...
    reloadFeedback,
    moreDefinitions: () =>
      definitionCursor ? readDefinitions(definitionCursor) : Promise.resolve(),
    async latestDefinitions() {
      await Promise.all([readReadiness(), readDefinitions()]);
    },
  ...
  return {
    ...
    async open() {
      opened = true;
      const active = ++life;
      await Promise.all([readReadiness(), readDefinitions(), readRuns()]);
      if (active === life) changed();
    },
    close() {
      opened = false;
      life++;
      ...
      selectedHeader = undefined;
      selectedCase = undefined;
      startReadiness = { status: "unavailable", reason: "Start availability is being checked." };
      execution = undefined;
      detail = undefined;
      selectedTarget = undefined;
      selectedScorer = undefined;
      ...
      comparison = undefined;
      baselineResultId = "";
      for (const [resultId, version] of feedbackReadVersions)
        feedbackReadVersions.set(resultId, version + 1);
