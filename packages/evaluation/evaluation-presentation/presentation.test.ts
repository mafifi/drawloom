...
const definition: EvaluationDefinition = {
  ...
  cases: [{ id: "case-a", revision: "r1", input: "question", suppliedOutput: "answer", references: [] }],
};
const run = { schemaVersion: 1 as const, id: "run-a", requestId: "request-a", definition: { id: definition.id, revision: definition.revision }, settings: { repetitions: 1, concurrency: 1 }, createdAtMs: 1 };
const baseCase = definition.cases[0]!;
const summary: ResultSummary = { schemaVersion: 1, id: "result-a", runId: run.id, caseId: "case-a", caseRevision: "r1", trial: 0, status: "completed", scorerInvocationIds: ["score-a"], startedAtMs: 2, completedAtMs: 3, findingCount: 1 };
const scorer = (revision = "r1", score = 1): ScorerCheckpoint => ({ schemaVersion: 1, invocationId: revision === "r1" ? "score-a" : "score-b", runId: run.id, caseId: "case-a", caseRevision: "r1", trial: 0, scorer: { id: "criterion", revision }, outcome: "succeeded", findings: [{ id: "finding", name: "Criterion", outcome: "scored", score, references: [] }], startedAtMs: 2, completedAtMs: 3 });
const detail = (result = summary, checkpoint = scorer()): EvaluationResultView => ({ result, scorers: [checkpoint], findings: checkpoint.findings });
function deferred<T>() {
...
function client(overrides: Partial<EvaluationService> = {}) {
  ...
  const service: EvaluationService = {
    ...
    assess: async () => ({ kind: "started", evaluationRunId: run.id, orchestrationRunId: "workflow-a" }),
    run: async () => ({ kind: "started", evaluationRunId: run.id, orchestrationRunId: "workflow-a" }),
    status: async () => ({ kind: "running", evaluationRunId: run.id, orchestrationRunId: "workflow-a", cancellationRequested: false }),
    cancel: async () => ({ kind: "requested", evaluationRunId: run.id, orchestrationRunId: "workflow-a" }),
    getDefinition: async () => definition,
    getDefinitionHeader: async () => definition,
    getCase: async () => baseCase,
    listDefinitions: async () => ({ items: [{ ref: { id: definition.id, revision: definition.revision }, name: definition.name, mode: definition.mode, caseCount: 1, scorerCount: 1 }], hasMore: false }),
    getRun: async () => run,
    ...
    saveFeedback: async value => { feedback.unshift(value); return { kind: "accepted" }; },
    ...overrides,
  };
  return { service, starts: () => starts, countStart() { starts++; } };
}

test("opening and browsing saved pages never starts an evaluation", async () => {
  let starts = 0;
  const f = client({ assess: async () => { starts++; throw Error("must not start"); }, run: async () => { starts++; throw Error("must not start"); } });
  const vm = createEvaluationViewModel({ client: f.service, nextId: () => "request-one", clock: () => 10 });
  await vm.open();
...
test("late result reads cannot replace a newer selection or a closed view", async () => {
  ...
  await vm.open(); await vm.actions.selectRun(run.id);
  const firstRead = vm.actions.selectResult(summary.id);
  const secondRead = vm.actions.selectResult(second.id);
  await Bun.sleep(0);
  responses.get(second.id)!(detail(second, scorer("r2"))); await secondRead;
  responses.get(summary.id)!(detail()); await firstRead;
  expect(vm.presentation.selectedResult?.id).toBe(second.id);
  expect(vm.presentation.detail?.result.id).toBe(second.id);
  const closedRead = vm.actions.selectResult(summary.id); await Bun.sleep(0); vm.close(); responses.get(summary.id)!(detail()); await closedRead;
...
test("rapid start uses one identity and an unavailable response becomes authoritative setup state", async () => {
  ...
  const f = client({ assess: async request => { requests.push(request.requestId); return new Promise(resolve => { release = resolve; }); } });
  const ids = ["request-one", "request-two"];
  const vm = createEvaluationViewModel({ client: f.service, nextId: () => ids.shift()!, clock: () => 10 });
  await vm.open(); vm.actions.selectDefinition({ id: definition.id, revision: definition.revision });
  const first = vm.actions.start(); const duplicate = vm.actions.start();
  await Bun.sleep(0);
  release({ kind: "unavailable", reason: "Orchestration is not configured" }); await Promise.all([first, duplicate]);
  expect(requests).toEqual(["request-one"]);
  await vm.actions.start();
  expect(requests).toEqual(["request-one"]);
  expect(vm.presentation.startReadiness).toEqual({ status: "unavailable", reason: "Orchestration is not configured" });
});

test("cancellation requested stays distinct from cancelled", async () => {
  const f = client();
  const vm = createEvaluationViewModel({ client: f.service, nextId: () => "id", clock: () => 10 });
  await vm.open(); await vm.actions.selectRun(run.id); await vm.actions.cancel();
  expect(vm.presentation.execution).toEqual({ kind: "running", evaluationRunId: run.id, orchestrationRunId: "workflow-a", cancellationRequested: true });
  expect(vm.presentation.copy.executionLabels.cancelled).not.toBe(vm.presentation.copy.executionLabels.cancellationRequested);
});
...
test("feedback validates, preserves drafts on failure, and reload cannot hide a saved record", async () => {
  ...
  const f = client({
    saveFeedback: async value => { saves++; if (saves === 1) throw Error("storage unavailable"); return { kind: "accepted" }; },
    listFeedback: async () => new Promise(resolve => { reload = resolve; }),
  });
  const vm = createEvaluationViewModel({ client: f.service, nextId: () => "feedback-one", clock: () => 10 });
  await vm.open(); await vm.actions.selectRun(run.id);
  const selected = vm.actions.selectResult(summary.id); await Bun.sleep(0); reload({ items: [], hasMore: false }); await selected;
  vm.actions.setFeedback({ attribution: "", rating: "correct", correction: "Keep this correction" });
  await vm.actions.saveFeedback();
  expect(vm.presentation.feedbackError).toContain("attribution");
  vm.actions.setFeedback({ attribution: "Reviewer" }); await vm.actions.saveFeedback();
  ...
  const retry = vm.actions.saveFeedback(); await Bun.sleep(0);
  reload({ items: [{ schemaVersion: 1, id: "feedback-one", resultId: summary.id, attribution: "Reviewer", rating: "correct", correction: "Keep this correction", createdAtMs: 10 }], hasMore: false });
  await retry;
...
test("baseline comparison requires exact criterion configuration", async () => {
  ...
  const baselineRun = { ...run, id: "run-b", definition: { id: "baseline-definition", revision: "r1" } };
  baseline.runId = baselineRun.id;
  const f = client({
    getResultSummary: async id => id === baseline.id ? baseline : summary,
    getResult: async id => id === baseline.id ? detail(baseline, { ...scorer("r1", 0), invocationId: "score-b", runId: baselineRun.id }) : detail(),
    getRun: async id => id === baselineRun.id ? baselineRun : run,
    getDefinitionHeader: async ref => ref.id === baselineRun.definition.id ? { ...definition, id: baselineRun.definition.id, scorers: [{ id: "criterion", revision: "r1", configuration: { rubric: "different" } }] } : definition,
    getCase: async () => baseCase,
  });
  const vm = createEvaluationViewModel({ client: f.service, nextId: () => "id", clock: () => 10 });
  await vm.open(); await vm.actions.selectRun(run.id); await vm.actions.selectResult(summary.id); await vm.actions.selectBaseline(baseline.id);
  expect(vm.presentation.comparison).toEqual({ kind: "incomparable", reason: "Criterion revisions or configurations do not match." });
});

test("different saved outputs and targets compare when bounded case and criterion facts match", async () => {
  const baseline = { ...summary, id: "baseline-compatible", runId: "run-compatible", scorerInvocationIds: ["score-compatible"] };
  const baselineRun = { ...run, id: baseline.runId, definition: { id: "other-output-set", revision: "r3" } };
  const currentHeader = { ...definition, scorers: [{ id: "criterion", revision: "r1", configuration: { rubric: "exact" } }] };
  const baselineHeader = { ...currentHeader, id: baselineRun.definition.id, revision: baselineRun.definition.revision, mode: "experiment" as const, target: { id: "other-target", revision: "r2", configuration: { temperature: 1 } } };
  const currentCase = { ...baseCase, suppliedOutput: "current output" };
  const baselineCase = { ...currentCase, suppliedOutput: "different output" };
  let headerReads = 0, caseReads = 0;
  const f = client({
    getResultSummary: async id => id === baseline.id ? baseline : summary,
    getResult: async id => id === baseline.id ? detail(baseline, { ...scorer("r1", 0), invocationId: "score-compatible", runId: baselineRun.id }) : detail(),
    getRun: async id => id === baselineRun.id ? baselineRun : run,
    getDefinitionHeader: async ref => { headerReads++; return ref.id === baselineRun.definition.id ? baselineHeader : currentHeader; },
    getCase: async ref => { caseReads++; return ref.id === baselineRun.definition.id ? baselineCase : currentCase; },
  });
  const vm = createEvaluationViewModel({ client: f.service, nextId: () => "id", clock: () => 10 });
  await vm.open(); await vm.actions.selectRun(run.id); await vm.actions.selectResult(summary.id); await vm.actions.selectBaseline(baseline.id);
  expect(vm.presentation.comparison?.kind).toBe("comparable");
  expect(vm.presentation.comparison).toMatchObject({ findings: [{ current: { score: 1 }, baseline: { score: 0 } }] });
...
test("oversized combined detail keeps its summary and permits one scorer checkpoint read", async () => {
  const f = client({ getResult: async () => { throw Error("Result detail exceeds its bound"); } });
  const vm = createEvaluationViewModel({ client: f.service, nextId: () => "id", clock: () => 10 });
  await vm.open(); await vm.actions.selectRun(run.id); await vm.actions.selectResult(summary.id);
...
test("same-definition navigation cannot replace the identity of an in-flight or uncertain start", async () => {
  ...
  secondStart.resolve({ kind: "started", evaluationRunId: run.id, orchestrationRunId: "workflow-a" });
  await retry;
});
...
test("scorer loading belongs to its result selection and settles after navigation", async () => {
  ...
  const f = client({
    listResults: async () => ({ items: [summary, second], hasMore: false }),
    getResult: async id => detail(id === second.id ? second : summary, id === second.id ? scorer("r2") : scorer()),
    getScorerCheckpoint: async selector => selector.invocationId === "score-a" ? pending.promise : scorer("r2"),
  });
  const vm = createEvaluationViewModel({ client: f.service, nextId: () => "id", clock: () => 10 });
  await vm.open(); await vm.actions.selectRun(run.id); await vm.actions.selectResult(summary.id);
...
test("feedback saves reconcile with their original result after navigation", async () => {
  ...
  const f = client({
    ...
    saveFeedback: async value => {
      if (value.resultId === summary.id) { await firstSave.promise; saved.push(value); return { kind: "accepted" }; }
      saved.push(value); return { kind: "accepted" };
    },
    listFeedback: async ({ resultId } = {}) => ({ items: saved.filter(item => !resultId || item.resultId === resultId), hasMore: false }),
  });
...
test("late terminal status and old result errors cannot overwrite a newer selection", async () => {
  const secondRun = { ...run, id: "run-b" };
  const secondResult = { ...summary, id: "result-b", runId: secondRun.id, scorerInvocationIds: ["score-b"] };
  ...
  const f = client({
    ...
    listResults: async ({ runId } = {}) => ({ items: runId === secondRun.id ? [secondResult] : [summary], hasMore: false }),
    status: async id => {
      if (id === run.id && ++statusReadsA > 1) return terminalStatus.promise;
      return { kind: "running", evaluationRunId: id, orchestrationRunId: `workflow-${id}`, cancellationRequested: false };
    },
    cancel: async () => ({ kind: "terminal", evaluationRunId: run.id, orchestrationRunId: "workflow-run-a" }),
    getResult: async id => id === summary.id ? oldDetail.promise : detail(secondResult, scorer("r2")),
  ...
  await Bun.sleep(0);
  await vm.actions.selectRun(secondRun.id);
  terminalStatus.resolve({ kind: "completed", evaluationRunId: run.id, orchestrationRunId: "workflow-run-a" });
  await cancelling;
  ...
  await Bun.sleep(0);
  await vm.actions.selectRun(secondRun.id); await vm.actions.selectResult(secondResult.id);
...
test("editing the baseline identity invalidates an older comparison read", async () => {
  ...
  const f = client({ getResultSummary: async id => id === oldBaseline.id ? lookup.promise : summary });
  const vm = createEvaluationViewModel({ client: f.service, nextId: () => "id", clock: () => 10 });
  await vm.open(); await vm.actions.selectRun(run.id); await vm.actions.selectResult(summary.id);
...
test("target checkpoint fallback uses the retained selector when combined detail is unavailable", async () => {
  ...
  const target: TargetCheckpoint = { schemaVersion: 1, invocationId: "target-a", runId: run.id, caseId: "case-a", caseRevision: "r1", trial: 0, target: { id: "model", revision: "r1" }, outcome: "failed", error: { code: "target_failed", message: "Target failed safely" }, references: [{ id: "trace", source: "fixture", uri: "asset://target" }], usage: { totalTokens: 12 }, startedAtMs: 2, completedAtMs: 3 };
  let selector: unknown;
  const f = client({
    listResults: async () => ({ items: [targetSummary], hasMore: false }),
    getResult: async () => { throw Error("Result detail exceeds its bound"); },
    getTargetCheckpoint: async value => { selector = value; return target; },
  });
  const vm = createEvaluationViewModel({ client: f.service, nextId: () => "id", clock: () => 10 });
  await vm.open(); await vm.actions.selectRun(run.id); await vm.actions.selectResult(targetSummary.id);
  await vm.actions.selectTarget();
  expect(selector).toEqual({ invocationId: "target-a", runId: run.id, caseId: "case-a", caseRevision: "r1", trial: 0 });
...
test("authoritative readiness is loaded without starting and blocks unavailable starts", async () => {
  ...
  const f = client({
    ...
    assess: async () => { starts++; return { kind: "started", evaluationRunId: run.id, orchestrationRunId: "workflow" }; },
  });
  const vm = createEvaluationViewModel({ client: f.service, nextId: () => "id", clock: () => 10 });
  await vm.open();
  vm.actions.selectDefinition({ id: definition.id, revision: definition.revision });
  expect(vm.presentation.startReadiness).toEqual({ status: "unavailable", reason: "Configure orchestration." });
  await vm.actions.start();
  expect(starts).toBe(0);
...
test("refreshing saved checks rereads readiness and keeps start blocked until ready", async () => {
  ...
  let readinessReads = 0, starts = 0;
  const f = client({
    readiness: async () => ++readinessReads === 1 ? { status: "unavailable", reason: "Setup is offline." } : refresh.promise,
    assess: async () => { starts++; return { kind: "started", evaluationRunId: run.id, orchestrationRunId: "workflow" }; },
  });
  const vm = createEvaluationViewModel({ client: f.service, nextId: () => "request", clock: () => 10 });
  await vm.open();
  vm.actions.selectDefinition({ id: definition.id, revision: definition.revision });
...
test("same-VM close and reopen preserve drafts and unresolved start identity", async () => {
  const requests: string[] = [];
  let uncertain = true;
  const f = client({ assess: async request => { requests.push(request.requestId); return uncertain ? { kind: "uncertain", evaluationRunId: "uncertain-run" } : { kind: "started", evaluationRunId: run.id, orchestrationRunId: "workflow" }; } });
  const ids = ["request-one", "request-two"];
  const vm = createEvaluationViewModel({ client: f.service, nextId: () => ids.shift()!, clock: () => 10 });
  await vm.open(); await vm.actions.selectRun(run.id); await vm.actions.selectResult(summary.id);
  vm.actions.setFeedback({ attribution: "Reviewer", rating: "uncertain", correction: "KEEP THIS DRAFT" });
  vm.actions.selectDefinition({ id: definition.id, revision: definition.revision });
  await vm.actions.start();
  vm.close();
  await vm.open(); await vm.actions.selectRun(run.id); await vm.actions.selectResult(summary.id);
...
test("reopen does not duplicate in-flight saves or starts and reconciles their completion", async () => {
  ...
  const f = client({
    assess: async () => { starts++; return startResult.promise; },
    saveFeedback: async value => { saves++; submittedFeedback = value; return saveResult.promise; },
    listFeedback: async () => ({ items: submittedFeedback ? [submittedFeedback] : [], hasMore: false }),
  });
  const ids = ["feedback-one", "request-one", "request-two"];
  const vm = createEvaluationViewModel({ client: f.service, nextId: () => ids.shift()!, clock: () => 10 });
  await vm.open(); await vm.actions.selectRun(run.id); await vm.actions.selectResult(summary.id);
  vm.actions.setFeedback({ attribution: "Reviewer", rating: "correct", correction: "Retain until acknowledged" });
