...
if (!process.versions.bun) {
  test("supported Braintrust assessment imports and runs under an OS network-denied clean child", {
    ...
    try {
      ...
      if (!address || typeof address === "string")
        throw new Error("Local probe port is unavailable");
      const child = spawn(
        ...
        [
          ...
          process.execPath,
          "--import",
          join(directory, "fixtures/network-observer.mjs"),
      ...
      assert.equal(
        observed["net.client.socket"],
        1,
        "only the deliberate denied socket probe may be observed",
      );
