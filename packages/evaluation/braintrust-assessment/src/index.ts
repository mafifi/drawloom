...
export const AUTOEVALS_EXACT_MATCH_SCORER = Object.freeze({
  id: "autoevals.exact-match",
  revision: "0.3.0",
});
export const AUTOEVALS_LEVENSHTEIN_SCORER = Object.freeze({
  id: "autoevals.levenshtein",
  revision: "0.3.0",
});
...
const autoevalsExactMatchScorer: EvaluationScorer = Object.freeze({
  ...
  async score({
    output,
    expected,
  }: Parameters<EvaluationScorer["score"]>[0]): Promise<ScorerInvocationResult> {
        ...
        findings: [
          {
            ...
            name: "Exact match",
            outcome: "unscored",
            explanation: "Exact match requires expected material.",
            references: [],
          },
        ],
      };
    ...
    const finding: Finding =
      value.score === null
        ? {
            ...
            name: "Exact match",
            outcome: "unscored",
            explanation: "Autoevals ExactMatch returned no score.",
            references: [],
          }
        : {
            id: "exact-match",
            ...
            outcome: "scored",
            score: value.score,
            explanation: "Autoevals ExactMatch.",
            references: [],
          };
...
const autoevalsLevenshteinScorer: EvaluationScorer<string, string, string> = Object.freeze({
  ...
  async score({
    ...
    EvaluationScorer<string, string, string>["score"]
  >[0]): Promise<ScorerInvocationResult> {
    if (expected === undefined)
      return {
        outcome: "succeeded",
        findings: [
          {
            ...
            outcome: "unscored",
            explanation: "Levenshtein similarity requires expected material.",
            references: [],
          },
        ],
      };
    ...
    const finding: Finding =
      value.score === null
        ? {
            ...
            name: "Levenshtein similarity",
            outcome: "unscored",
            explanation: "Autoevals Levenshtein returned no score.",
            references: [],
          }
        : {
            id: "levenshtein",
            ...
            outcome: "scored",
            score: value.score,
            explanation: "Autoevals Levenshtein.",
            references: [],
          };
    return { outcome: "succeeded", findings: [finding] };
  },
...
export function createBraintrustAssessmentProvider(
  options: BraintrustAssessmentOptions = {},
): EvaluationAssessmentProvider {
  const scorers = Object.freeze([
    autoevalsExactMatchScorer,
    autoevalsLevenshteinScorer,
    ...(options.scorers ?? []),
  ]);
  ...
  for (const scorer of scorers) {
    ...
    if (seen.has(key))
      throw new Error(`Duplicate assessment scorer ${scorer.id}@${scorer.revision}`);
    seen.add(key);
  }
  const provider: EvaluationAssessmentProvider = {
    ...
    async assess(scorer, args, context) {
      ...
      const scorerContext: EvaluationInvocationContext = {
        ...context,
        signal: AbortSignal.any([context.signal]),
      };
      try {
        await Eval<EvaluationJson, EvaluationJson, void, {}>(
          "drawloom-individual-assessment",
          {
            data: [{ input: args.input, metadata: {} }],
            task: () => args.output,
            scores: [
              async () => {
                activeAssessment = Promise.resolve(scorer.score(args, scorerContext)).then(
                  (value) => ScorerInvocationResultSchema.parse(value),
                );
                captured = await activeAssessment;
                const values = captured.findings.map((finding) => ({
                  name: finding.id,
                  score: finding.outcome === "scored" ? finding.score! : null,
                  metadata: { outcome: finding.outcome },
                }));
                return values.length
                  ? values
                  : [
                      {
                        name: "drawloom-assessment",
                        score: null,
                        metadata: { outcome: captured.outcome },
                      },
                    ];
              },
            ],
...
const judgeResponse = z.strictObject({
  score: z.number().finite().min(0).max(1).nullable(),
  explanation: z.string().min(1).max(8192),
});
...
function judgeFailure(
  code: string,
  message: string,
  outcome: ScorerInvocationResult["outcome"] = "failed",
): ScorerInvocationResult {
...
): z.infer<typeof judgeResponse> | undefined {
...
export function createAgentRubricScorer(options: AgentRubricScorerOptions): EvaluationScorer {
  const configuredModel =
    options.configuredModel === undefined
      ? undefined
      : z.string().min(1).max(256).parse(options.configuredModel);
  return Object.freeze({
    ...
    async score(
      ...
      if (!configuration.success)
        return judgeFailure("invalid_rubric", "Agent rubric configuration is invalid");
      if (!configuredModel || !options.driver)
        return judgeFailure(
          "judge_unavailable",
          "Agent rubric requires an explicitly configured model and host agent driver",
          "uncertain",
        );
      const prompt = [
      ...
      if (encoder.encode(prompt).byteLength > MAX_JUDGE_REQUEST_BYTES)
        return judgeFailure("request_too_large", "Agent rubric request exceeds the local bound");
      if (context.signal.aborted)
        return judgeFailure(
          "judge_interrupted",
          "Configured agent judge was cancelled before submission",
          "cancelled",
        );
      const opened = await options.driver.openSession({
        sessionId: `evaluation-${context.invocationId}`,
        context: {
          text: "This is a bounded evaluation-only session. Follow the requested rubric and do not use tools.",
        },
        tools: { id: "evaluation-none", tools: [] },
      });
      if (opened.status !== "ok")
        return judgeFailure(
          "judge_unavailable",
          "Configured agent judge is unavailable",
          opened.failure.code === "provider_unavailable" ? "uncertain" : "failed",
        );
      const session = opened.value;
      const operationId = context.invocationId;
      const abort = () => {
        try {
          void session.interrupt?.(operationId).catch(() => undefined);
        } catch {
          /* best effort only */
        }
      };
      ...
      try {
        if (context.signal.aborted) {
          ...
          if (submitted.status !== "ok") {
            result = judgeFailure(
              "judge_rejected",
              "Configured agent judge rejected the request",
              submitted.failure.code === "provider_unavailable" ? "uncertain" : "failed",
            );
          } else {
            let terminal:
              | Extract<
            ...
            const interruptUnexpectedActivity = async (): Promise<boolean> => {
              ...
              if (!session.interrupt) {
                unexpectedInterruptionFailed = true;
                return false;
              }
              try {
            ...
            for await (const signal of signals) {
              ...
              if (
                ...
                signal.role !== "user" &&
                (signal.phase === undefined || signal.phase === "final")
              )
                response = signal.text;
              if (
                signal.kind === "artifact.available" ||
              ...
              if (
                ...
                !["usage", "reasoning-summary"].includes(signal.name)
              ) {
                if (!(await interruptUnexpectedActivity())) break;
              }
              if (
                ["operation.completed", "operation.interrupted", "operation.failed"].includes(
                  signal.kind,
                )
              ) {
                terminal = signal as typeof terminal;
                break;
              }
            }
            const nativeSettlementUnconfirmed =
              terminal?.kind === "operation.failed" &&
              ["provider_unavailable", "invalid_provider_response"].includes(terminal.failure.code);
            if (unexpectedActivity) {
              result = judgeFailure(
                "unexpected_activity",
                "Agent judge reported unexpected activity",
                !terminal || nativeSettlementUnconfirmed || unexpectedInterruptionFailed
                  ? "uncertain"
                  : "failed",
              );
            } else if (!terminal) {
              result = judgeFailure(
                "judge_unavailable",
                "Configured agent judge ended without a terminal result",
                "uncertain",
              );
            } else if (nativeSettlementUnconfirmed) {
              result = judgeFailure(
                "judge_unavailable",
                "Configured agent judge ended without confirmed native settlement",
                "uncertain",
              );
            } else if (terminal.kind !== "operation.completed") {
              result = judgeFailure(
                terminal.kind === "operation.interrupted" ? "judge_interrupted" : "judge_failed",
                "Configured agent judge did not complete",
        ...
        result = judgeFailure(
          "judge_unavailable",
          "Configured agent judge failed before a terminal result",
          context.signal.aborted ? "cancelled" : "uncertain",
        );
      } finally {
      ...
      for (let attempt = 0; attempt < 2 && !cleaned; attempt++) {
        try {
          cleaned = (await session.close()).status === "ok";
        } catch {
          /* retry cleanup only */
        }
      }
