...
for (const stage of ["startup", "prepare", "start", "task", "recovery", "state", "shutdown"])
  spanNames.add(`drawloom.orchestration.${stage}`);
...
const routes = new Set([
  ...
  "/health",
  "/api/discovery",
  "/api/discovery/authenticate",
  "/api/discovery/resource/read",
  "/api/resource/read",
  "/api/resources",
  "/api/resource/open",
  "/api/history",
...
function attributes(input: Record<string, unknown>, metric = false): Attributes {
  ...
  for (const [key, value] of Object.entries(input)) {
    if (metric && key === "otel.metric.overflow" && value === true) output[key] = true;
    else if (key === "drawloom.outcome" && typeof value === "string" && outcomes.has(value))
      output[key] = value;
    else if (key === "drawloom.operation.name" && typeof value === "string" && spanNames.has(value))
      output[key] = value;
      ...
      !metric &&
      key === "drawloom.file.bytes" &&
      typeof value === "number" &&
      Number.isSafeInteger(value) &&
      value >= 0
    )
      output[key] = value;
    else if (!metric && key === "drawloom.file.cached" && typeof value === "boolean")
      output[key] = value;
    else if (
      !metric &&
      key === "drawloom.record.count" &&
      typeof value === "number" &&
      Number.isSafeInteger(value) &&
      value >= 0 &&
      value <= 1e9
      ...
      output[key] = value;
    else if (key === "rpc.method" && typeof value === "string" && rpcMethods.has(value))
      output[key] = value;
    else if (key === "http.route" && typeof value === "string" && routes.has(value))
      output[key] = value;
    else if (
      !metric &&
      key === "drawloom.attempt" &&
      typeof value === "number" &&
      Number.isInteger(value) &&
      value >= 1 &&
      value <= 100
      ...
      !metric &&
      [
        "drawloom.operation.id",
        "drawloom.invocation.id",
        "drawloom.run.id",
        "drawloom.step.id",
        "drawloom.plugin.id",
      ].includes(key) &&
      typeof value === "string" &&
      /^(?:[a-f0-9]{16,64}|[a-f0-9]{8}(?:-[a-f0-9]{4}){3}-[a-f0-9]{12})$/i.test(value)
    )
      output[key] = value;
  }
  return output;
...
export function initializeObservability(options: ObservabilityOptions): Observability {
  if (!["disabled", "recording", "export"].includes(options.mode))
    throw new Error("Invalid observability mode");
  if (!/^[a-z][a-z0-9.-]{0,63}$/.test(options.serviceName))
    throw new Error("Invalid observability service name");
  const bound = (value: number | undefined, fallback: number, max: number) => {
    const selected = value ?? fallback;
    if (!Number.isInteger(selected) || selected < 1 || selected > max)
      throw new Error("Invalid observability bound");
    return selected;
  };
  const maxQueueSize = bound(options.maxQueueSize, 2048, 65536);
  const maxExportBatchSize = bound(
  ...
  if (options.endpoint !== undefined) {
    ...
    if (
      ...
      throw new Error("Observability requires a loopback HTTP base endpoint");
  }
  if (options.mode === "export" && !endpoint)
    throw new Error("Export requires an explicit endpoint");
  if (options.mode === "export" && options.exporters)
    throw new Error("Custom exporters are only available in recording mode");
  ...
  const counts: ObservabilityDiagnostics = {
    recordedSpans: 0,
    recordedLogs: 0,
    exportFailures: 0,
    deadlineExceeded: 0,
  ...
  if (options.mode === "disabled")
    return { flush: async () => {}, shutdown: async () => {}, diagnostics: () => ({ ...counts }) };
  ...
  const safeSpan = (span: ReadableSpan): ReadableSpan => ({
    name: safeNames.has(span.name) ? span.name : "operation",
    kind: span.kind,
    spanContext: () => ({
    ...
    ...(span.parentSpanContext && isSpanContextValid(span.parentSpanContext)
      ? {
          parentSpanContext: {
            traceId: span.parentSpanContext.traceId,
            spanId: span.parentSpanContext.spanId,
            traceFlags: span.parentSpanContext.traceFlags & 1,
          },
        }
      : {}),
    ...
    events: span.events
      ...
      .map((e) => ({ name: e.name, time: e.time, attributes: attributes(e.attributes ?? {}) })),
    links: span.links
      .filter((link) => isSpanContextValid(link.context))
      .map((link) => ({
        context: {
    ...
    resource,
    instrumentationScope: scopeFor(span.instrumentationScope.name),
  ...
  const safeLog = (log: ReadableLogRecord): ReadableLogRecord => ({
    ...
    ...(log.spanContext && isSpanContextValid(log.spanContext)
      ? {
          spanContext: {
            traceId: log.spanContext.traceId,
            spanId: log.spanContext.spanId,
            traceFlags: log.spanContext.traceFlags & 1,
          },
        }
      : {}),
    ...
    resource,
    instrumentationScope: scopeFor(log.instrumentationScope.name),
  ...
  const exporting = <T>(
    send: (data: T, callback: (result: ExportResult) => void) => void,
    data: T,
    ...
    const finish = (result: ExportResult) => {
      ...
      callback(
        result.code === ExportResultCode.SUCCESS
          ? { code: result.code }
          : { code: result.code, error: new Error("Telemetry export failed") },
      );
    };
  ...
  const sink = {
    export: (_data: unknown, callback: (result: ExportResult) => void) =>
      callback({ code: ExportResultCode.SUCCESS }),
    shutdown: async () => {},
    forceFlush: async () => {},
  };
  const traces: SpanExporter =
    options.exporters?.traces ??
    (options.mode === "export"
  ...
  const logExporter: LogRecordExporter =
    options.exporters?.logs ??
    (options.mode === "export"
      ? new OTLPLogExporter({
          url: new URL("v1/logs", endpoint).href,
          timeoutMillis: exportTimeoutMillis,
        })
      : sink);
  const metricExporter: PushMetricExporter =
    options.exporters?.metrics ??
    (options.mode === "export"
      ? new OTLPMetricExporter({
          url: new URL("v1/metrics", endpoint).href,
          timeoutMillis: exportTimeoutMillis,
        })
      : sink);
    ...
    views: [
      {
        ...
        attributesProcessors: [{ process: (input) => attributes(input, true) }],
      },
    ],
    readers: [
      new PeriodicExportingMetricReader({
        exportIntervalMillis: 60000,
        exportTimeoutMillis: exportTimeoutMillis + 10,
        exporter: {
          export: (data, callback) => {
            ...
            for (const scope of data.scopeMetrics)
              for (const metric of scope.metrics) {
                if (
                  !["@opentelemetry/sdk-trace", "@opentelemetry/sdk-logs"].includes(
                    scope.scope.name,
                  )
                )
                  continue;
                ...
                for (const point of metric.dataPoints)
                  if (
                    point.attributes["error.type"] === "queue_full" &&
                    typeof point.value === "number"
                  )
                    counts.droppedRecords += point.value;
              }
            exporting(
              metricExporter.export.bind(metricExporter),
              {
                resource,
                scopeMetrics: data.scopeMetrics.map((s) => ({
                  scope: scopeFor(s.scope.name),
                  metrics: s.metrics
                    .filter(
                      (m) =>
                        metricNames.has(m.descriptor.name) ||
                        lossMetricNames.has(m.descriptor.name),
                    )
                    .map((m) => {
                      const sanitized: MetricData = {
                        ...structuredClone(m),
                        descriptor: {
                          ...m.descriptor,
                          description: "",
                          unit: m.descriptor.name === "operation.duration" ? "ms" : "1",
                        },
                      };
                      for (const point of sanitized.dataPoints)
                        Object.assign(point, { attributes: attributes(point.attributes, true) });
                      return sanitized;
                    }),
                })),
              },
          ...
          forceFlush: () => bounded(() => metricExporter.forceFlush()),
          shutdown: () => bounded(() => metricExporter.shutdown()),
        },
      }),
    ],
  });
  const batch = new BatchSpanProcessor({
    exporter: {
      export: (data, callback) =>
        exporting(
          traces.export.bind(traces),
          data.filter((s) => isSpanContextValid(s.spanContext())).map(safeSpan),
          callback,
        ),
      shutdown: () => bounded(() => traces.shutdown()),
    },
    maxQueueSize,
    maxExportBatchSize,
    exportTimeoutMillis: exportTimeoutMillis + 10,
  ...
  const logBatch = new BatchLogRecordProcessor({
    exporter: {
      export: (data, callback) =>
        exporting(logExporter.export.bind(logExporter), data.map(safeLog), callback),
      forceFlush: () => bounded(() => logExporter.forceFlush()),
      shutdown: () => bounded(() => logExporter.shutdown()),
    },
    maxQueueSize,
    maxExportBatchSize,
    exportTimeoutMillis: exportTimeoutMillis + 10,
  ...
  const loggerProvider = new LoggerProvider({
    resource,
    logRecordLimits: { attributeCountLimit: 16, attributeValueLengthLimit: 80 },
    processors: [
      {
        onEmit: (log) => {
          counts.recordedLogs++;
        ...
        forceFlush: () => logBatch.forceFlush(),
        shutdown: () => logBatch.shutdown(),
      },
    ],
  });
  active = true;
