...
test("supported orchestration stages retain safe names, state and receipt diagnostics", async () => {
  ...
  const sdk = initializeObservability({
    mode: "recording",
    serviceName: "orchestration-test",
    exporters: { traces: spans },
  });
  const names = ["startup", "prepare", "start", "task", "recovery", "state", "shutdown"].map(
    (name) => `drawloom.orchestration.${name}`,
  );
  try {
    ...
    await sdk.flush();
    assert.deepEqual(
      spans.getFinishedSpans().map((span) => span.name),
      names,
    );
    assert.ok(
      spans
        .getFinishedSpans()
        .every((span) => span.attributes["drawloom.workflow.state"] === "waiting"),
    );
...
test("real SDK preserves concurrent parent and log context and removes accidental content", async () => {
  ...
  const sdk = initializeObservability({
    mode: "recording",
    serviceName: "drawloom-test",
    exporters: { traces: spans, logs: records, metrics: points },
  });
  ...
  try {
    ...
    await Promise.all(
      [1, 2].map(async () =>
        tracer.startActiveSpan("tool.invoke", async (parent) => {
          ...
          await tracer.startActiveSpan("tool.execute", async (child) => {
            ...
            logs.getLogger("PRIVATE_MARKER").emit({
              eventName: "operation.completed",
              body: "PRIVATE_MARKER",
              context: context.active(),
              attributes: { "drawloom.outcome": "ok", secret: "PRIVATE_MARKER" },
            });
            child.end();
          });
          parent.end();
    ...
    tracer
      .startSpan("PRIVATE_MARKER", {
        links: [
          { context: { traceId: "PRIVATE_MARKER", spanId: "PRIVATE_MARKER", traceFlags: 1 } },
        ],
      })
      .end();
    const counter = metrics
      .getMeter("PRIVATE_MARKER")
      .createCounter("operation.count", { description: "PRIVATE_MARKER" });
    ...
    for (const child of finished.filter((s) => s.name === "tool.execute")) {
      assert.ok(
        finished.some(
          (p) =>
            p.spanContext().spanId === child.parentSpanContext?.spanId && p.name === "tool.invoke",
        ),
      );
      assert.ok(
        records
          .getFinishedLogRecords()
          .some((r) => r.spanContext?.spanId === child.spanContext().spanId),
      );
    }
    ...
    assert.equal(
      JSON.stringify([finished, records.getFinishedLogRecords(), points.getMetrics()]).includes(
        "PRIVATE_MARKER",
      ),
      false,
    );
...
test("metric overflow remains distinguishable from an empty dimension set", async () => {
  ...
  const sdk = initializeObservability({
    mode: "recording",
    serviceName: "overflow-test",
    exporters: { metrics: points },
  });
  try {
    const counter = metrics.getMeter("test").createCounter("operation.count");
    counter.add(1);
    for (let status = 100; status < 230; status++)
      counter.add(1, { "http.response.status_code": status });
    await sdk.flush();
    const all = points
      .getMetrics()
      .flatMap((r) => r.scopeMetrics.flatMap((s) => s.metrics.flatMap((m) => m.dataPoints)));
...
test("mutated parent and log correlation contexts cannot carry content to exporters", async () => {
  ...
  const sdk = initializeObservability({
    ...
    exporters: { traces: spans, logs: records },
  });
  try {
    ...
    parent.traceId = "PRIVATE_MARKER";
    parent.spanId = "PRIVATE_MARKER";
    await sdk.flush();
    assert.equal(
      JSON.stringify([spans.getFinishedSpans(), records.getFinishedLogRecords()]).includes(
        "PRIVATE_MARKER",
      ),
      false,
    );
...
test("exporter outage is bounded and structured SDK metrics report queue losses", async () => {
  const sdk = initializeObservability({
    mode: "recording",
    serviceName: "outage-test",
    maxQueueSize: 2,
    maxExportBatchSize: 1,
    exportTimeoutMillis: 15,
    shutdownTimeoutMillis: 60,
    exporters: { traces: { export() {}, shutdown: () => new Promise(() => {}) } },
  });
...
test("opt-in loopback exports all signals under the configured service", async () => {
  ...
  const sdk = initializeObservability({
    mode: "export",
    serviceName: "local-export-test",
    endpoint: `http://127.0.0.1:${address.port}`,
  });
  try {
    trace.getTracer("test").startActiveSpan("tool.execute", (span) => {
      logs.getLogger("test").emit({ eventName: "operation.completed", body: "PRIVATE_MARKER" });
      span.end();
    });
