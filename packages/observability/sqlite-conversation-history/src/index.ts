...
type EntryRow = { id: string; position_0: number; position_1: number; role: "user" | "assistant"; text: string; assets: string; resources: string | null; selections: string | null; preparation: string | null; operation_id: string | null; state: "partial" | "complete" | "interrupted"; changed_sequence: number };
...
function record(row: EntryRow): HistoryEntry {
  try {
    return HistoryEntrySchema.parse({ id: row.id, position: [row.position_0, row.position_1], role: row.role, text: row.text, assets: JSON.parse(row.assets), ...(row.resources ? { resources: JSON.parse(row.resources) } : {}), ...(row.selections ? { selections: JSON.parse(row.selections) } : {}), ...(row.preparation ? { preparation: JSON.parse(row.preparation) } : {}), operationId: row.operation_id ?? undefined, state: row.state });
  } catch (cause) {
  ...
  try {
    ...
    if (version >= 1 && version <= SCHEMA_VERSION) {
      ...
      for (const [name, expected] of Object.entries(schema)) {
        ...
        const versioned = version === 1 ? prior.replace(', resources TEXT, selections TEXT', '') : prior;
        if (!row || normalizeSql(row.sql) !== normalizeSql(versioned)) throw Error('Invalid history schema');
      }
      ...
      if(version>=3)for(const [name,type] of [['history_entries_fts','table'],['history_entries_fts_insert','trigger'],['history_entries_fts_delete','trigger'],['history_entries_fts_update','trigger']] as const){const row=db.query('SELECT 1 FROM sqlite_master WHERE name=? AND type=?').get(name,type);if(!row)throw Error('Invalid history search schema');}
    ...
    if (version > 0 && version < 4) db.transaction(() => {
      ...
      try {
        return db.transaction(() => {
          ...
          for (const value of parsed.entries ?? []) {
            ...
            if (existing && existing.role === value.role && existing.text === value.text && existing.assets === assets && existing.resources === resources && existing.selections === selections && existing.preparation === preparation && (existing.operation_id ?? undefined) === value.operationId && existing.state === value.state) continue;
            sequence += 1;
            ...
            db.query(`INSERT INTO history_entries(conversation_id,id,position_0,position_1,role,text,assets,operation_id,state,changed_sequence,resources,selections,preparation)
              VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(conversation_id,id) DO UPDATE SET role=excluded.role,text=excluded.text,assets=excluded.assets,operation_id=excluded.operation_id,state=excluded.state,changed_sequence=excluded.changed_sequence,resources=excluded.resources,selections=excluded.selections,preparation=excluded.preparation`)
              .run(conversationId, value.id, value.position[0], value.position[1], value.role, value.text, assets, value.operationId ?? null, value.state, sequence, resources, selections, preparation);
          }
