  ...
  try {
    ...
    if (version === 1 || version === SCHEMA_VERSION) {
      ...
      for (const [name, expected] of Object.entries(schema)) {
        ...
        const versioned = version === 1 ? expected.replace(', resources TEXT, selections TEXT', '') : expected;
      ...
      try {
        return db.transaction(() => {
          ...
          for (const value of parsed.entries ?? []) {
            ...
            if (existing && existing.role === value.role && existing.text === value.text && existing.assets === assets && existing.resources === resources && existing.selections === selections && (existing.operation_id ?? undefined) === value.operationId && existing.state === value.state) continue;
            sequence += 1;
            if (!Number.isSafeInteger(sequence)) throw new HistoryStoreError('unavailable', 'History change position is exhausted');
            changed = true;
            db.query(`INSERT INTO history_entries(conversation_id,id,position_0,position_1,role,text,assets,operation_id,state,changed_sequence,resources,selections)
              VALUES (?,?,?,?,?,?,?,?,?,?,?,?) ON CONFLICT(conversation_id,id) DO UPDATE SET role=excluded.role,text=excluded.text,assets=excluded.assets,operation_id=excluded.operation_id,state=excluded.state,changed_sequence=excluded.changed_sequence,resources=excluded.resources,selections=excluded.selections`)
              .run(conversationId, value.id, value.position[0], value.position[1], value.role, value.text, assets, value.operationId ?? null, value.state, sequence, resources, selections);
          }
