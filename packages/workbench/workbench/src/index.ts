...
export const OperatorCommandSchema = z.discriminatedUnion("kind", [
  z.strictObject({
    kind: z.literal("revise_document"),
    candidateId: id,
    artifactId: id,
  ...
  z.strictObject({
    kind: z.literal("review_candidate"),
    candidateId: id,
...
export const OperatorSnapshotSchema = z
  ...
  .superRefine((s, context) => {
    ...
    if (
      artifacts.size !== s.artifacts.length ||
      candidates.size !== s.candidates.length ||
      new Set(s.groups?.map((g) => g.id)).size !== (s.groups?.length ?? 0) ||
      s.candidates.some((c) => c.artifactIds.some((id) => !artifacts.has(id))) ||
      s.groups?.some(
        (g) =>
          g.artifactIds.some((id) => !artifacts.has(id)) ||
          g.candidateIds.some((id) => !candidates.has(id)),
      ) ||
      (s.selectedCandidateId && !candidates.has(s.selectedCandidateId))
    ) {
      context.addIssue({
        code: "custom",
        message: "Presentation identities must be unique and references must resolve",
      });
    }
  });
...
export const OperatorResultSchema = z.discriminatedUnion("status", [
  ...
  z.strictObject({
    ...
    code: z.enum(["invalid_command", "not_found", "not_ready", "conflict"]),
