...
const root = await mkdtemp(join(tmpdir(), "drawloom-client-"));
const manager = createLocalTemporalManager({ dataDirectory: root });
try {
  const registration = await manager.prepare({ projectId: "client", installationId: "synthetic", packageDirectory: resolve("packages/orchestration/temporal-orchestration"), entrypoint: "fixtures/workflows.mjs" });
  await registration.attach(registration.registry.tasks.map((task) => ({ id: task.id, version: task.version, run: (value) => value * 2 })));
  const run = await registration.orchestrator.start("actual-client-request", arithmetic, 7);
