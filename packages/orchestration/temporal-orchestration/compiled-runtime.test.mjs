...
if (!process.versions.bun)
  test("compiled Bun host executes a packaged workflow using only staged Node resources", {
    skip: process.env.DRAWLOOM_TEMPORAL_COMPILED !== "1",
    timeout: 60000,
  }, async () => {
    ...
    try {
      await command("bun", [
        "build",
        "--compile",
        resolve("packages/orchestration/temporal-orchestration/fixtures/compiled-client.mjs"),
        "--outfile",
        executable,
      ]);
      await command("bun", [
      ...
      const output = await new Promise((resolve, reject) => {
        const child = spawn(executable, [join(root, "data"), runtime, pkg], {
          cwd: root,
        ...
        const timeout = setTimeout(() => {
          child.kill("SIGKILL");
          reject(Error("Compiled client timed out"));
        }, 45000);
