...
if (!process.versions.bun) {
  ...
  test("real local Temporal runs unchanged shared conformance in Node", {
    ...
    try {
        ...
        await registration.attach(
          registration.registry.tasks.map((task) => ({
            ...
            async run(input, context) {
              ...
              if (!bridge) {
                bridge = agent.createAgentBridge(
                  context.runId,
                  createSyntheticDriver(() =>
                    context.runId.endsWith("/background")
                      ? new Promise(() => {})
                      : "Synthetic result",
                  ),
                  {
      ...
      const background = await current.orchestrator.start(
        "background",
        workflows.backgroundAgent,
        null,
      );
      await until(
        () => current.orchestrator.get(background),
        (snapshot) => snapshot.pendingInputs.length > 0,
      );
      await current.orchestrator.cancel(background);
      await assert.rejects(current.orchestrator.result(background));
      assert.ok(
        (await current.orchestrator.get(background)).unresolvedEffects.length > 0,
        "cancellation must retain uncertainty for an owned native submission",
      );
    } finally {
      await manager.close();
  ...
  test("full local close/reopen preserves input and completed effects, isolates owners, and refuses changed bundles", {
    skip: process.env.DRAWLOOM_TEMPORAL_TEST !== "1",
    timeout: 120000,
  }, async () => {
    ...
    try {
      ...
      const transformedRun = await registration.orchestrator.start(
        "transformed",
        transformed,
        null,
      );
      const transformedWait = await until(
        () => registration.orchestrator.get(transformedRun),
        (state) => state.pendingInputs.length > 0 || state.status !== "running",
      );
      assert.equal(
        transformedWait.status,
        "running",
        "JSON-preserving transforms must be valid input schemas",
      );
      await registration.orchestrator.respond(
        transformedRun,
        transformedWait.pendingInputs[0],
        "  retained  ",
      );
      ...
      const pending = await until(
        () => registration.orchestrator.get(run),
        (snapshot) => snapshot.pendingInputs.length > 0,
      );
      const other = await manager.prepare({ ...owner, projectId: "other" });
      await other.attach(handlers);
      ...
      await until(
        () => hostRegistration.orchestrator.get(hostRun),
        (state) => state.pendingInputs.length > 0,
      );
      assert.notEqual(
        hostRun.split("/")[0],
        run.split("/")[0],
        "host and plugin run identities must occupy separate owner namespaces",
      );
      assert.equal(
        await hostRegistration.orchestrator.start("nightloom", review, 2),
        hostRun,
        "same host start identity and fingerprint replays safely",
      );
      await assert.rejects(
        hostRegistration.orchestrator.start("nightloom", review, 3),
        /Conflicting start/,
      );
      assert.equal((await manager.listHostOwners()).length, 1);
      assert.equal((await manager.listOwners()).length, 2);
      assert.equal(
        await manager.hasUnfinishedInstallation("knowledge-maintenance"),
        false,
        "host capabilities never become plugin installation guards",
      );
      const page = await registration.orchestrator.list({ limit: 1 });
      assert.ok(page.cursor);
      await assert.rejects(other.orchestrator.list({ cursor: page.cursor }), /cursor/);
      await until(
        () => registration.orchestrator.get(another),
        (state) => state.pendingInputs.length > 0,
      );
      await until(
        () => other.orchestrator.get(second),
        (state) => state.pendingInputs.length > 0,
      );
      ...
      assert.ok(failed.unresolvedEffects.length > 0);
  ...
  test("whole-host process loss reclaims dead ownership and reopens durable input without redispatch", {
    ...
    const child = spawn(
      process.execPath,
      [resolve("packages/orchestration/temporal-orchestration/fixtures/crash-host.mjs"), root],
    ...
    try {
      const saved = await until(
        async () => {
          try {
            return JSON.parse(await readFile(join(root, "ready.json"), "utf8"));
          } catch {
            return null;
          }
        },
      ...
      const registration = await manager.prepare({
        projectId: "crash",
        installationId: "crash",
        packageDirectory: resolve("packages/orchestration/temporal-orchestration"),
        entrypoint: "fixtures/recovery.mjs",
      });
      let dispatches = 0;
      await registration.attach([
        {
      ...
      assert.equal(await readFile(join(root, "effect.txt"), "utf8"), "one write");
    } finally {
      child.kill("SIGKILL");
      await manager?.close();
      await rm(root, { recursive: true, force: true });
  ...
  test("killed effect writer leaves durable intent: absent recovery is unknown and receipt-only recovery never resubmits", {
    skip: process.env.DRAWLOOM_TEMPORAL_TEST !== "1",
    timeout: 15000,
  }, async () => {
    ...
    const child = spawn(
      process.execPath,
      [resolve("packages/orchestration/temporal-orchestration/fixtures/crash-receipt.mjs"), root],
      { stdio: "ignore" },
    );
    const task = { id: "effect", version: "1", input: z.number(), output: z.number() };
    const request = {
      runId: "owner/run",
      stepId: "owner/run/effect",
      task: "effect",
      version: "1",
      input: 3,
      attempt: 2,
    ...
    try {
      await until(
        async () => {
          try {
            return await readFile(join(root, "effect.txt"), "utf8");
          } catch {
            return "";
          }
        },
        (value) => value === "effect submitted",
      );
      await new Promise((resolve) => {
        child.once("exit", resolve);
        child.kill("SIGKILL");
      });
      const withoutRecovery = createReceiptDispatcher(
        root,
        "owner",
        matchTaskHandlers({ workflows: [], tasks: [task] }, [
          {
            id: task.id,
            version: task.version,
            run: () => {
      ...
      const recovered = createReceiptDispatcher(
        root,
        "owner",
        matchTaskHandlers({ workflows: [], tasks: [task] }, [
          {
            ...
            recover: async () => {
              assert.equal(await readFile(join(root, "effect.txt"), "utf8"), "effect submitted");
              return { status: "completed", output: 6 };
            },
          },
        ]),
      );
      assert.equal(await recovered.dispatch(request), 6);
