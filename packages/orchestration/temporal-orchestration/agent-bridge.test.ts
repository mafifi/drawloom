...
test("local shutdown is not native cancellation; explicit run cancellation still reaches owned agents", async () => {
  for (const explicit of [false, true]) {
    ...
    const bridge = createAgentBridge(
      "owner/run",
      createSyntheticDriver(() => new Promise(() => {})),
      {
    ...
    const observedBridge = {
      ...
      requestCancellation: async () => {
        cancellations++;
        return bridge.requestCancellation();
      },
    };
    const tasks = [agentTasks.create, agentTasks.submit];
    const handlers: RegisteredTaskHandler[] = tasks.map((task) => ({
      id: task.id,
      version: task.version,
      run: (input, context) => dispatchAgentTask(observedBridge, task.id, input, context),
    }));
    const dispatcher = createReceiptDispatcher(
      root,
      "owner",
      matchTaskHandlers({ workflows: [], tasks }, handlers),
    );
    const request = (task: string, stepId: string, input: Json) => ({
      task,
      version: "1",
      runId: "owner/run",
      stepId: "owner/run/" + stepId,
      input,
      attempt: 1,
      maxAttempts: 1,
    });
    try {
      ...
      await dispatcher.dispatch(
        request("agent.submit", "submit", {
          sessionId: session,
          operation: { operationId: "operation", text: "synthetic" },
        }),
      );
...
test("owned agent receipt survives lost submission response without repeat and preserves native approval authority", async () => {
  ...
  try {
    ...
    expect(
      (await bridge.submit(session, { operationId: "operation", text: "synthetic" })).status,
    ).toBe("unknown");
    expect(
      (await bridge.submit(session, { operationId: "operation", text: "synthetic" })).status,
    ).toBe("unknown");
    ...
    expect(
      (await createAgentBridge("owned", driver, store, authority).inspect(session, "operation"))
        .status,
    ).toBe("completed");
  } finally {
    await bridge.close();
