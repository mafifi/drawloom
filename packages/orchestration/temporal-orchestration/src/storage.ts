...
export function jsonStore(directory: string) {
  return {
    async get(key: string) {
      const value = await readJson(join(directory, `${digest(key)}.json`));
      return value === undefined ? undefined : z.json().parse(value);
    },
    async set(key: string, value: z.infer<ReturnType<typeof z.json>>) {
      await writeJson(join(directory, `${digest(key)}.json`), value);
    },
  };
}
