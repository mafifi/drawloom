...
const OwnerInput = z.strictObject({
...
export interface LocalTemporalOptions {
  dataDirectory: string;
  temporalPath?: string;
  nodePath?: string /** Composition-owned real Node files for a compiled desktop host. */;
...
const RunRecord = z.strictObject({
  runId: z.string(),
...
export function createLocalTemporalManager(options: LocalTemporalOptions) {
  ...
  async function own(): Promise<void> {
    ...
    lock ??= acquireLock(root).then((release) => {
      unlock = release;
      return release;
    });
    await lock;
  ...
  const server = createServer(async (request, response) => {
    try {
      ...
      if (!owner || !dispatcher || request.method !== "POST") {
      ...
      if (request.url === "/dispatch") output = await dispatcher.dispatch(value);
        ...
        output = dispatcher.cancel(z.strictObject({ runId: z.string() }).parse(value).runId);
      else {
      ...
      if (Buffer.byteLength(body) > 1024 * 1024)
        throw new StepFailure("unknown", "Dispatch result too large");
      response.writeHead(200, { "content-type": "application/json" }).end(body);
    } catch (error) {
      response.writeHead(200, { "content-type": "application/json" }).end(
        JSON.stringify({
          error: "Task dispatch failed",
          code: stepFailureCode(error) ?? (error instanceof z.ZodError ? "invalid" : "unknown"),
        }),
      );
    }
  });
  ...
  async function startService(): Promise<void> {
    ...
    startup ??= observe("startup", async () => {
      ...
      if (nodeVersion.trim() !== `${LOCAL_TEMPORAL_NODE_VERSION}:false`)
        throw new Error(`Local orchestration requires actual Node ${LOCAL_TEMPORAL_NODE_VERSION}`);
      ...
      await writeJson(config, {
        ...
        temporalPath: cli,
        port,
        database: join(root, "temporal.sqlite"),
      });
      ...
      service.once("exit", () => {
        failed ??= new Error("Local Temporal service exited");
      });
      try {
        for (let attempt = 0; attempt < 100; attempt++) {
          if (failed) throw failed;
          try {
            connection = await Connection.connect({ address, connectTimeout: 200 });
            break;
          } catch {
            await pause(100);
          }
  ...
  async function listRecords<T>(directory: string, schema: z.ZodType<T>): Promise<readonly T[]> {
    ...
    return Promise.all(
      paths.map(async (path) => schema.parse(await readJson(join(directory, path)))),
    );
  }
  ...
  const records = async (owner: string) =>
    z.array(RunRecord).parse((await readJson(recordsPath(owner))) ?? []);
    ...
    if (changed)
      await mutate(async () => {
        ...
        for (const entry of latest)
          if (entries.some((observed) => observed.runId === entry.runId && observed.terminal))
            entry.terminal = true;
  ...
  async function hasUnfinishedInstallation(installationId: string): Promise<boolean> {
    for (const owner of await listOwners())
      if (owner.installationId === installationId && (await unfinished(owner.owner))) return true;
    return false;
  }
  async function prepareOwner(
    input: { packageDirectory: string; entrypoint: string },
    owner: string,
    ownerDirectory: string,
    ownerRecord: object,
    hostOwned = false,
  ): Promise<LocalTemporalRegistration> {
    await own();
    ...
    const imported: unknown = await import(
      `${pathToFileURL(entry).href}?drawloom=${compiled.fingerprint}`
    );
    ...
    const check = () => {
      if (
        ...
        throw new Error("Local orchestration is not ready");
    };
    const cursor = (kind: string, value: unknown) => {
      const payload = Buffer.from(
        JSON.stringify({ owner, bundle: compiled.fingerprint, kind, value }),
      ).toString("base64url");
    ...
    const decode = (kind: string, value?: string): number => {
      ...
      if (!payload || !signature || extra || value.length > 8192)
        throw new Error("Invalid owner cursor");
      ...
      const decoded = z
        .strictObject({
          ...
          bundle: z.literal(compiled.fingerprint),
          kind: z.literal(kind),
    ...
    const handle = (runId: string) => {
      check();
      if (!runId.startsWith(`${owner}/`)) throw new Error("Unknown run owner");
      return client!.workflow.getHandle(runId);
    };
    const checkBundle = async (runId: string) => {
      const entries = await records(owner);
      const record = entries.find(
        (entry) => runId === entry.runId || runId.startsWith(`${entry.runId}/child/`),
      );
      if (!record || record.bundleFingerprint !== compiled.fingerprint)
        throw new Error("Unknown run or blocked workflow bundle");
    };
    ...
    const get: Orchestrator["get"] = async (runId) => {
      ...
      const state = snapshot.unresolvedEffects.length
        ? "unknown"
      ...
      if (observedStates.get(runId) !== state) {
        if (observedStates.size >= 1024 && !observedStates.has(runId))
          observedStates.delete(observedStates.keys().next().value!);
        observedStates.set(runId, state);
        const span = trace
          .getTracer("@drawloom/temporal-orchestration")
          .startSpan("drawloom.orchestration.state");
      ...
      if (snapshot.status !== "running")
        await mutate(async () => {
          const entries = await records(owner);
          const entry = entries.find((value) => value.runId === runId);
          if (entry && !entry.terminal) {
            entry.terminal = true;
            await writeJson(recordsPath(owner), entries);
          }
        });
      return snapshot;
    };
    const orchestrator: Orchestrator = {
      async start(identity, workflow, input) {
        ...
        const definition = registry.workflows.find(
          (value) => value.id === workflow.id && value.version === workflow.version,
        );
        ...
        await mutate(async () => {
          const entries = await records(owner);
          const prior = entries.find((entry) => entry.runId === runId);
          if (
            prior &&
            (prior.fingerprint !== fingerprint || prior.bundleFingerprint !== compiled.fingerprint)
          )
        ...
        await trace
          .getTracer("@drawloom/temporal-orchestration")
          .startActiveSpan("drawloom.orchestration.start", async (span) => {
            span.setAttribute("drawloom.run.id", digest(runId));
            try {
              await bounded(() =>
                client!.workflow.start("drawloomWorkflow", {
                  workflowId: runId,
                  taskQueue: `${owner}-${compiled.fingerprint}`,
                  args: [
                    {
                      identity,
              ...
              if (!(error instanceof WorkflowExecutionAlreadyStartedError)) {
                span.setAttribute("drawloom.outcome", "error");
                throw error;
              }
        ...
        if ((await bounded(() => handle(runId).query("startIdentity"))) !== fingerprint)
          throw new Error("Conflicting start");
        return runId;
      },
      get,
      async getSteps(runId, options = {}) {
        const run = handle(runId);
        await checkBundle(runId);
        const offset = decode(`steps:${runId}`, options.cursor),
          size = limit(options.limit);
        const steps = RunSnapshotSchema.shape.steps.parse(
          await bounded(() => run.query("steps", offset, size + 1)),
        );
        return {
          steps: steps.slice(0, size),
          ...(steps.length > size ? { cursor: cursor(`steps:${runId}`, offset + size) } : {}),
        };
      },
      async list(options = {}) {
        check();
        const offset = decode("list", options.cursor),
          size = limit(options.limit);
        const entries = (await records(owner)).filter(
          (entry) => entry.bundleFingerprint === compiled.fingerprint,
        );
        const runs = await Promise.all(
          entries.slice(offset, offset + size).map((entry) => get(entry.runId)),
        );
        return {
          runs,
          ...(entries.length > offset + size ? { cursor: cursor("list", offset + size) } : {}),
        };
      },
    ...
    const registration: LocalTemporalRegistration = {
      ...
      readiness() {
        try {
          ...
          return {
            ...
            message: "Local workflow worker is not ready",
          };
        }
      },
      async attach(handlers) {
        ...
        dispatchers.set(owner, dispatcher);
        ...
        try {
          await new Promise<void>((resolve, reject) => {
            const timer = setTimeout(
              () => reject(new Error("Workflow worker readiness timed out")),
              15000,
            );
            ...
            worker!.once("exit", () => {
              ready = false;
              clearTimeout(timer);
              reject(new Error("Workflow worker exited"));
            });
            worker!.stdout?.on("data", (data: Buffer) => {
              if (data.toString().includes("DRAWLOOM_WORKER_READY")) {
                clearTimeout(timer);
                ready = true;
                resolve();
              }
  ...
  return {
    ...
    prepare(value: PrepareOwner): Promise<LocalTemporalRegistration> {
      ...
      const prior = preparing.get(owner);
      if (prior) return prior;
      const preparingOwner = observe("prepare", () =>
        prepareOwner(input, owner, ownersDirectory, input),
      ).finally(() => preparing.delete(owner));
      preparing.set(owner, preparingOwner);
      return preparingOwner;
    },
    async prepareHost(value: PrepareHostOwner): Promise<LocalTemporalRegistration> {
      ...
      if (prior) return prior;
      const preparingOwner = observe("prepare", () =>
        prepareOwner(input, owner, hostOwnersDirectory, input, true),
      ).finally(() => preparing.delete(owner));
      preparing.set(owner, preparingOwner);
      return preparingOwner;
    },
