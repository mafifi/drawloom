...
const configuration = z
  .strictObject({
    mode: z.enum(["bundle", "worker", "service"]),
    ...
    temporalPath: z.string().optional(),
...
const watchdog = setInterval(() => {
  if (!isAlive(configuration.parent)) void shutdown();
}, 1000);
watchdog.unref();
...
if (configuration.mode === "service") {
  ...
  await writeFile(
    entry,
    `const { executeWorkflow } = require(${JSON.stringify(runtime)}); const registry = require(${JSON.stringify(configuration.entry)}).default; exports.drawloomWorkflow = (start) => executeWorkflow(registry, start);`,
  );
  ...
  const bundle = await bundleWorkflowCode({
    ...
    webpackConfigHook: (config) => {
      ...
      config.resolve.modules = [
        ...
        dirname(
          dirname(
            dirname(createRequire(import.meta.url).resolve("@temporalio/workflow/package.json")),
          ),
        ),
      ];
      config.plugins ??= [];
      config.plugins.push({
        apply(compiler) {
          compiler.hooks.afterCompile.tap("DrawloomCompleteDependencyClosure", (compilation) => {
            if (
              compilation.warnings.some((warning) => /critical dependency/i.test(warning.message))
            )
              throw new Error(
                "Workflow bundle has unresolved dependency warnings; package self-contained deterministic JavaScript",
              );
          });
          compiler.hooks.compilation.tap("DrawloomContainedWorkflow", (compilation) => {
            compilation.hooks.finishModules.tapPromise(
              "DrawloomContainedWorkflow",
              async (modules) => {
                for (const module of modules) {
                  ...
                  if (resource === entry.replace(/\.[^.]+$/, "-autogenerated-entrypoint.cjs"))
                    continue;
  ...
  async function call(path: string, body: unknown, attempt = 1): Promise<unknown> {
    let data;
    try {
      const response = await fetch(`${configuration.bridge}${path}`, {
        method: "POST",
        headers: {
        ...
        signal: AbortSignal.timeout(86_400_000),
      });
      ...
      throw ApplicationFailure.create({
        message: "Dispatch response unavailable; effect outcome unknown",
        type: "unknown",
        nonRetryable: true,
        details: [attempt],
      });
  ...
  const worker = await Worker.create({
    ...
    workflowBundle: { codePath: configuration.bundle! },
