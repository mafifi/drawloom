...
export async function dispatchAgentTask(
  ...
  const cancel = () => {
    if (context.signal.reason !== LOCAL_EXECUTION_PAUSED) void bridge.requestCancellation();
  };
  if (context.signal.aborted) {
    if (context.signal.reason !== LOCAL_EXECUTION_PAUSED) await bridge.requestCancellation();
