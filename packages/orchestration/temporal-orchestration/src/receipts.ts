...
export const DispatchRequest = z.strictObject({
  ...
  stepId: z.string().min(1).max(4096),
...
const Receipt = z.strictObject({
  ...
  code: z.enum(["retryable", "denied", "invalid", "unknown"]).optional(),
});
type Receipt = z.infer<typeof Receipt>;
/** Single writer, protected by the manager's data-root lock. */
export function createReceiptDispatcher(
  directory: string,
  owner: string,
  handlers: readonly MatchedTaskHandler[],
) {
  const active = new Map<
    string,
    { fingerprint: string; controller: AbortController; promise: Promise<Json> }
  >();
  const runCancellation = new Map<string, AbortController>();
  let closed = false;
  const checkOwner = (runId: string) => {
    if (!runId.startsWith(`${owner}/`)) throw new StepFailure("denied", "Unknown run owner");
  };
  async function execute(
    request: z.infer<typeof DispatchRequest>,
    fingerprint: string,
    controller: AbortController,
  ): Promise<Json> {
    const handler = handlers.find(
      (value) => value.task.id === request.task && value.task.version === request.version,
    );
    ...
    if (prior && prior.fingerprint !== fingerprint)
      throw new StepFailure("invalid", "Conflicting step receipt");
    ...
    if (prior && request.attempt < prior.attempt)
      throw new StepFailure("invalid", "Stale task attempt");
    const context: TaskContext = {
      runId: request.runId,
      stepId: request.stepId,
      taskVersion: request.version,
      attempt: request.attempt,
      attemptId: `${request.stepId}/attempt/${request.attempt}`,
    ...
    if (
      ...
      !previousIsUnknown &&
      !(
        prior.code === "retryable" &&
        request.attempt > prior.attempt &&
        request.attempt <= request.maxAttempts
      )
    )
      throw new StepFailure(prior.code ?? "unknown");
    if (!prior && request.attempt !== 1)
      throw new StepFailure("unknown", "Missing prior attempt receipt");
    ...
    try {
      const work = async (): Promise<Json> => {
        if (previousIsUnknown) {
          ...
          const recovered = await observe("recovery", () =>
            handler.recover!(request.input, context),
          );
  ...
  return {
      ...
      return Promise.resolve().then(() => {
        ...
        if (previous) {
          if (previous.fingerprint !== fingerprint)
            throw new StepFailure("invalid", "Conflicting active receipt");
          return previous.promise;
        }
        const controller = new AbortController();
        const promise = trace
          .getTracer("@drawloom/temporal-orchestration")
          .startActiveSpan("drawloom.orchestration.task", async (span) => {
            ...
            try {
              const result = await execute(request, fingerprint, controller);
              span.setAttribute("drawloom.outcome", "ok");
              return result;
            } catch (error) {
              const code = stepFailureCode(error);
              span.setAttribute(
                "drawloom.outcome",
                code === "denied"
                  ? "denied"
                  : code === "retryable" || code === "invalid"
                    ? "error"
                    : "unknown",
              );
              throw error;
    ...
    cancel(runId: string): readonly string[] {
      ...
      for (const [step, value] of active)
        if (step.startsWith(`${runId}/`)) {
    ...
    async close() {
      closed = true;
      for (const run of runCancellation.values()) run.abort(LOCAL_EXECUTION_PAUSED);
      for (const value of active.values()) value.controller.abort(LOCAL_EXECUTION_PAUSED);
