...
test("host owners persist and list outside plugin installation ownership", async () => {
  ...
  try {
    ...
    await writeFile(
      join(orchestration, "owners", "plugin.json"),
      JSON.stringify({
        projectId: "project",
        installationId: "installation",
        packageDirectory: "/plugin",
        entrypoint: "workflow.mjs",
        bundleContext: "/plugin-launch",
        bundleFingerprint: "plugin-bundle",
        owner: "plugin-owner",
      }),
    );
    await writeFile(
      join(orchestration, "host-owners", "host.json"),
      JSON.stringify({
        capabilityId: "knowledge-maintenance",
        packageDirectory: "/host",
        entrypoint: "workflow.mjs",
        bundleContext: "/host-launch",
        bundleFingerprint: "host-bundle",
        owner: "host-owner",
      }),
    );
