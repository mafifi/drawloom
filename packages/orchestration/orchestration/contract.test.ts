...
test("published orchestration boundary rejects non-JSON transforms and mismatched result definitions", async () => {
  ...
  await expect(workflowResult(engine, "run", definition)).rejects.toThrow(
...
test("workflow module definitions reject invalid identities and schema fields", () => {
  ...
  expect(() =>
    defineWorkflowModule({ workflows: [workflow], tasks: [task], future: true }),
  ).toThrow();
});
...
test("workflow modules reject duplicate identities within each definition kind", () => {
  ...
  expect(() =>
    orchestration.defineWorkflowModule({ workflows: [workflow, workflow], tasks: [task] }),
  ).toThrow("Duplicate workflow identity render@1");
  expect(() =>
    orchestration.defineWorkflowModule({ workflows: [workflow], tasks: [task, task] }),
  ).toThrow("Duplicate task identity frame@1");
});
...
test("task execution limits are bounded and legacy tasks receive the documented default", () => {
  ...
  expect(
    orchestration.taskExecutionLimits({ ...task, limits: { startToCloseTimeoutMs: 75_000 } }),
  ).toEqual({ startToCloseTimeoutMs: 75_000 });
  for (const startToCloseTimeoutMs of [0, -1, 1.5, Number.POSITIVE_INFINITY, 86_400_001]) {
    expect(() =>
      orchestration.defineWorkflowModule({
        workflows: [],
        tasks: [{ ...task, limits: { startToCloseTimeoutMs } }],
      }),
    ).toThrow();
  }
});
...
test("task handlers must match every module task identity exactly once", () => {
  ...
  expect(() => orchestration.matchTaskHandlers(module, [handler, handler])).toThrow(
    "Duplicate task handler identity frame@1",
  );
  const wrongVersion = orchestration.registerTaskHandler(
    { ...task, version: "2" },
    { run: (input) => input.length },
  );
  expect(() => orchestration.matchTaskHandlers(module, [wrongVersion])).toThrow(
    "Unexpected task handler frame@2",
  );
});
...
test("matched task handlers validate module-owned inputs, outputs and recovery outcomes", async () => {
  const task = {
    ...
    input: z.string(),
    output: z.number(),
    limits: { startToCloseTimeoutMs: 75_000 },
  };
  const module = orchestration.defineWorkflowModule({ workflows: [], tasks: [task] });
  let calls = 0;
  const context = {
