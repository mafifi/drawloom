...
test("orchestration readiness is a strict bounded desktop-host report", () => {
  ...
  expect(
    desktopHost.OrchestrationReadinessSchema.parse({
      status: "unavailable",
  ...
  expect(desktopHost.OrchestrationReadinessSchema.safeParse({ status: "failed" }).success).toBe(
    false,
  );
  expect(
    desktopHost.OrchestrationReadinessSchema.safeParse({ status: "ready", extra: true }).success,
  ).toBe(false);
  expect(
    desktopHost.OrchestrationReadinessSchema.safeParse({
      status: "unavailable",
...
test("plugin backends return typed task handlers and receive optional orchestration availability", async () => {
  ...
  expect(context.dependencies[0]).toEqual({
    kind: "capability",
    id: "orchestration",
    available: true,
  });
