<script lang="ts" module>
  ...
  export type GoalBarPresentation = {
    ...
    canPause: boolean; canResume: boolean; labels: GoalBarLabels;
  };
  ...
  <Tooltip.Provider delayDuration={250}>
    <div class="flex min-w-0 items-center gap-2">
      ...
      {#if presentation.editing}
        <Input bind:ref={editor} class="h-7 min-w-0 flex-1" aria-label={presentation.labels.objective} value={presentation.draft} oninput={(event) => actions.setDraft(event.currentTarget.value)} onkeydown={(event) => { if (event.key === "Enter" && !event.isComposing) { event.preventDefault(); void save(); } if (event.key === "Escape") { event.preventDefault(); void cancel(); } }} />
        <Tooltip.Root><Tooltip.Trigger>{#snippet child({ props })}<StatefulButton {...props} size="icon-sm" iconOnly pending={presentation.pendingAction === "create" || presentation.pendingAction === "edit"} pendingLabel={presentation.mode === "create" ? presentation.labels.create : presentation.labels.save} disabled={!presentation.draft.trim()} aria-label={presentation.mode === "create" ? presentation.labels.create : presentation.labels.save} onclick={save}><SaveIcon /></StatefulButton>{/snippet}</Tooltip.Trigger><Tooltip.Content>{presentation.mode === "create" ? presentation.labels.create : presentation.labels.save}</Tooltip.Content></Tooltip.Root>
