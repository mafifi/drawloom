<script lang="ts" module>
  export interface MentionOption { id:string; title:string; description?:string; scope?:string; group:string; kind:'skill'|'plugin'|'document'|'attachment'|'browse'; disabled?:boolean; }
</script>
<script lang="ts">
...
<Popover.Root {open} {onOpenChange}>
  <Popover.Portal>
    <Popover.Content customAnchor={anchor} side="top" align="start" sideOffset={8} trapFocus={false}
          ...
          {#each groups as group}<Command.Group heading={group || undefined}>
            {#each options.filter(item=>item.group===group) as item (item.id)}
              <Command.Item id={'mention-'+encodeURIComponent(item.id)} value={item.id} disabled={item.disabled} onSelect={()=>onSelect(item.id)}
                ...
                {@const Icon=item.kind==='skill'?BookOpen:item.kind==='plugin'?Plug:item.kind==='attachment'?Paperclip:item.kind==='browse'?Search:FileText}
