...
<Popover.Root bind:open onOpenChange={next=>{if(next){showingModels=!onEffort;onOpen?.();}}}>
  ...
  <Popover.Portal><Popover.Content side="top" align="end" sideOffset={8} class={`z-50 ${showingModels?'w-80':'w-64'} viewport-menu rounded-2xl border bg-popover p-1 text-popover-foreground shadow-md outline-none`}>
      ...
      <div class="space-y-3 p-3">
        ...
        {#if levels.length>1}
          <Slider.Root type="single" min={0} max={levels.length-1} step={1} bind:value={draftEffort} {disabled} onValueCommit={index=>{const next=levels[index];if(next && next!==effort)onEffort?.(next);}} class="relative flex h-8 w-full touch-none items-center select-none rounded-full bg-accent">
            {#snippet children({tickItems})}<span aria-hidden="true" class="absolute h-full rounded-full" style:width={`calc(calc(var(--spacing-unit)*7) + (100% - calc(var(--spacing-unit)*7)) * ${draftEffort/(levels.length-1)})`} style:background="linear-gradient(to right,var(--effort-fill-start),var(--effort-fill-end))"></span>{#each tickItems as tick}<Slider.Tick index={tick.index} class="absolute size-1 rounded-full bg-disabled-fill" />{/each}<Slider.Thumb index={0} aria-label="Reasoning effort" aria-valuetext={levels[draftEffort]} class="z-10 block size-7 rounded-full bg-foreground shadow-sm outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 focus-visible:ring-offset-popover" />{/snippet}
          </Slider.Root>
