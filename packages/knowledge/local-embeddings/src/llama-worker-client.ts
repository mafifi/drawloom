...
type Spawn = (
  ...
  options: { cwd: string; env: NodeJS.ProcessEnv; stdio: readonly ["ignore", "pipe", "pipe"] },
) => ChildLike;
...
export class LlamaEmbeddingWorker implements EmbeddingWorker {
  ...
  constructor(options: LlamaEmbeddingWorkerOptions) {
    ...
    if (options.ready) this.#ready = options.ready;
      ...
      const setup = createModelSetup({
        root: options.root,
        manifest: knownManifest(options.model),
      });
  ...
  async warmup(options: EmbedOptions = {}): Promise<void> {
    ...
    if (this.#busy)
      throw new LocalEmbeddingsError("worker_busy", "One inference request is already running");
    ...
    if (!Number.isSafeInteger(timeout) || timeout < 1 || timeout > 300_000)
      throw new LocalEmbeddingsError("invalid_request", "Invalid inference deadline");
    ...
    try {
      ...
      if (this.#closed)
        throw new LocalEmbeddingsError("worker_closed", "Inference worker is closed");
    ...
    try {
      ...
      if (!ready)
        throw new LocalEmbeddingsError(
          "model_not_ready",
          "GGUF model and llama.cpp runtime are not ready",
        );
      await this.#abortable(this.#retiring, operation.signal);
      if (this.#closed)
        throw new LocalEmbeddingsError("worker_closed", "Inference worker is closed");
      await this.#ensureStarted(ready, operation.signal);
      const formatted = request.items.map((item) =>
        formatEmbeddingInput(this.#options.model, request.role, item),
      );
      let total = 0;
      for (const content of formatted) {
        const tokenized = TokenizedSchema.safeParse(
          await this.#json(
            "/tokenize",
            { content, add_special: true, parse_special: true },
            operation.signal,
          ),
        );
        if (!tokenized.success)
          throw new LocalEmbeddingsError(
            "invalid_result",
            "llama.cpp returned invalid tokenization",
          );
        if (tokenized.data.tokens.length > ready.manifest.maxTokens)
          throw new LocalEmbeddingsError(
            "input_too_large",
            "Embedding item exceeds its token budget",
          );
        total += tokenized.data.tokens.length;
        if (total > ready.manifest.maxBatchTokens)
          throw new LocalEmbeddingsError(
            "input_too_large",
            "Embedding batch exceeds its token budget",
          );
      }
      ...
      for (const content of formatted) {
        const parsed = EmbeddingsSchema.safeParse(
          await this.#json(
            "/v1/embeddings",
            { input: content, model: this.#options.model, encoding_format: "float" },
            operation.signal,
          ),
        );
        if (
          !parsed.success ||
          parsed.data.model !== this.#options.model ||
      ...
      return validateEmbeddingVectors(vectors, {
        count: request.items.length,
        dimensions: ready.manifest.dimensions,
        normalized: true,
      });
    } catch (cause) {
      ...
      if (
        this.#child &&
        (interrupted ||
          !(cause instanceof LocalEmbeddingsError) ||
          !["input_too_large", "model_not_ready"].includes(cause.code))
      )
        await this.#retire(this.#child);
      if (this.#closed)
        throw new LocalEmbeddingsError("worker_closed", "Inference worker is closed");
      if (timedOut)
        throw new LocalEmbeddingsError("request_timeout", "llama.cpp inference request timed out");
      if (options.signal?.aborted)
        throw new LocalEmbeddingsError(
          "request_cancelled",
          "llama.cpp inference request was cancelled",
        );
  ...
  async #ensureStarted(ready: ReadyModel, signal: AbortSignal): Promise<void> {
    ...
    if (!Number.isSafeInteger(port) || port < 1024 || port > 65_535)
      throw new LocalEmbeddingsError("configuration_mismatch", "Invalid private server port");
    const apiKey = this.#options.apiKey?.() ?? randomBytes(32).toString("base64url");
    if (apiKey.length < 16)
      throw new LocalEmbeddingsError("configuration_mismatch", "Private server key is too short");
    ...
    const child = spawn(binary, args, {
      cwd: ready.runtimeDirectory,
      env: { HOME: this.#options.root, PATH: "/usr/bin:/bin" },
      stdio: ["ignore", "pipe", "pipe"],
    });
    const stopped = new AbortController();
    this.#child = child;
    this.#server = { baseUrl: `http://127.0.0.1:${port}`, apiKey, stopped };
    ...
    for (;;) {
      ...
      if (stopped.signal.aborted)
        throw new LocalEmbeddingsError("worker_failed", "llama.cpp server stopped during startup");
      try {
        const response = await this.#fetch(`${this.#server.baseUrl}/v1/models`, {
  ...
  async #json(path: string, body: unknown, signal: AbortSignal): Promise<unknown> {
    ...
    if (!server) throw new LocalEmbeddingsError("worker_failed", "llama.cpp server is unavailable");
    const response = await this.#fetch(`${server.baseUrl}${path}`, {
      method: "POST",
      headers: this.#headers(),
    ...
    if (!response.ok) {
      await response.body?.cancel().catch(() => undefined);
      throw new LocalEmbeddingsError("worker_failed", "llama.cpp request failed");
    }
    const text = await response.text();
    if (Buffer.byteLength(text) > maxResponseBytes)
      throw new LocalEmbeddingsError(
        "invalid_result",
        "llama.cpp response exceeds its byte budget",
      );
    try {
      return JSON.parse(text);
    } catch {
      throw new LocalEmbeddingsError("invalid_result", "llama.cpp returned malformed JSON");
    }
  }
    ...
    const retiring = new Promise<void>((resolve, reject) => {
      ...
      const timer = setTimeout(() => {
        ...
        escalation = setTimeout(
          () =>
            finish(
              new LocalEmbeddingsError(
                "worker_shutdown_timeout",
                "llama.cpp server did not exit after SIGKILL",
              ),
            ),
          timeout,
        );
