...
test("the supported GGUF identity replaces the old index generation", () => {
  ...
  expect(embeddingConfiguration(modelId).fingerprint).not.toBe(
    "7901d8a98731adf8926fbab80206cd38f59596c361f9ad4b5d1ad74d992dabf3",
  );
  expect(embeddingConfiguration(modelId).fingerprint).not.toBe(
    "c9ca071c3fc2fcf7b37dd7a6ca4300d9819149195931590c4152cef0ca442936",
  );
  ...
  expect(formatEmbeddingInput(modelId, "query", "Where?")).toBe(
...
test.each([
  ...
  "setup makes no download on $platform/$arch when unavailable ($code)",
  async ({ platform, arch, code }) => {
    const root = await mkdtemp(join(tmpdir(), "drawloom-llama-unavailable-"));
    let fetched = 0;
    try {
      const setup = createModelSetup({
        root,
        manifest: KnownModelManifests[modelId],
        llamaRuntime: { platform, arch },
        fetch: async () => {
          fetched++;
...
test("llama worker starts one authenticated loopback embedding-only Metal server and tokenizes before embedding", async () => {
  ...
  const worker = new LlamaEmbeddingWorker({
    ...
    fetch: async (input, init) => {
      ...
      if (url.endsWith("/v1/models"))
        return jsonResponse({
          object: "list",
          data: [{ id: modelId, object: "model", created: 0, owned_by: "llama.cpp" }],
        });
      if (url.endsWith("/tokenize"))
        return jsonResponse({ tokens: body.content === "document" ? [1] : [1, 2, 3] });
      return jsonResponse({
        object: "list",
        model: modelId,
  ...
  try {
    await expect(
      worker.embed({ role: "document", items: ["document", "second"] }),
    ).resolves.toHaveLength(2);
    expect(children).toHaveLength(1);
    expect(starts).toEqual([
      {
        command: "/controlled/runtime/bin/llama-server",
        args: [
          "--model",
          "/controlled/model/Qwen3-Embedding-0.6B-Q8_0.gguf",
          "--alias",
          modelId,
          "--host",
          "127.0.0.1",
        ...
        options: {
          cwd: "/controlled/runtime",
          env: { HOME: "/controlled", PATH: "/usr/bin:/bin" },
    ...
    expect(
      calls
        .filter((call) => new URL(call.url).pathname === "/v1/embeddings")
        .map((call) => call.body.input),
    ).toEqual(["document", "second"]);
    expect(
      calls.every(
        (call) =>
          new Headers(call.init?.headers).get("authorization") === "Bearer private-test-key",
      ),
    ).toBe(true);
  } finally {
    await worker.close();
  ...
  for (const counts of [[2049], [2000, 2000, 2000, 2000, 193]]) {
    ...
    const worker = new LlamaEmbeddingWorker({
      ...
      fetch: async (input) => {
        ...
        if (path === "/tokenize")
          return jsonResponse({ tokens: Array(tokenCounts.shift()!).fill(1) });
        embeddings++;
    ...
    try {
      await expect(
        worker.embed({ role: "document", items: counts.map((_, index) => String(index)) }),
...
test("llama worker rejects wrong indexes and non-normalized, null, or non-finite vectors", async () => {
  for (const data of [
    ...
    const worker = new LlamaEmbeddingWorker({
      ...
      spawn: () => child,
      fetch: async (input) =>
        new URL(String(input)).pathname === "/tokenize"
          ? jsonResponse({ tokens: [1] })
    ...
    await expect(worker.embed({ role: "document", items: ["x"] })).rejects.toMatchObject({
      code: "invalid_result",
    });
...
test("cancellation reaps the uncertain server before a later request starts a replacement", async () => {
  ...
  const first = worker.embed({ role: "document", items: ["first"] }, { signal: controller.signal });
...
test("an already-exited server settles promptly and the next request starts a replacement", async () => {
  ...
  const worker = new LlamaEmbeddingWorker({
    ...
    spawn: () => {
      const child = start === 0 ? new AlreadyExitedChild() : new FakeChild();
      children.push(child);
      if (start++ === 0)
        queueMicrotask(() => {
          (child as AlreadyExitedChild).exited = true;
          child.emit("exit", 1);
        });
      return child;
    },
    fetch: async (input, init) => {
      ...
      return jsonResponse({
        object: "list",
        model: modelId,
        data: [{ index: 0, embedding: unit }],
      });
    },
  });
  const began = performance.now();
  await expect(worker.embed({ role: "document", items: ["first"] })).rejects.toMatchObject({
    code: "worker_failed",
  });
...
test("vector validation rejects non-unit output", () => {
  expect(() =>
    validateEmbeddingVectors([[2, 0]], { count: 1, dimensions: 2, normalized: true }),
  ).toThrow();
  expect(
    validateEmbeddingVectors([[0.6, 0.8]], { count: 1, dimensions: 2, normalized: true }),
  ).toEqual([[0.6, 0.8]]);
});
...
test("obsolete MLX cleanup removes only exact owned directories and refuses a symlinked root", async () => {
  ...
  try {
    ...
    await expect(unsafe.cleanupObsoleteMlxRuntime()).resolves.toEqual({
      kind: "refused",
      code: "unsafe_root",
    });
  } finally {
...
test("obsolete MLX cleanup refuses symlinked ownership intermediates without deleting external data", async () => {
  for (const intermediate of ["runtime", "active"] as const) {
    ...
    try {
      ...
      const setup = createModelSetup({ root, manifest: KnownModelManifests[modelId] });
      await expect(setup.cleanupObsoleteMlxRuntime()).resolves.toEqual({
        kind: "refused",
        code: "unsafe_target",
      });
...
test("runtime artifacts require an executable digest anchored in trusted composition", () => {
  ...
  expect(
    RuntimeArtifactSchema.safeParse({
      ...artifact,
...
test("runtime artifact overrides must be explicitly trusted", () => {
  const manifest = ModelManifestSchema.parse({
    ...
    dtype: "q8",
    runtime: "llama.cpp",
    formatting: { query: "", document: "", pooling: "last_token", layerNorm: false },
    artifacts: [
      {
        path: "model.gguf",
        bytes: 1,
  ...
  const runtime = RuntimeArtifactSchema.parse({
    id: "llama.cpp-darwin-arm64",
    revision: "2f539596c6e9a977e91b6bc6344650422c6bc3b0",
    platform: "darwin",
    arch: "arm64",
  ...
  expect(() =>
    createModelSetup({ root: "/controlled", manifest, runtimeArtifact: runtime as any }),
  ).toThrow();
  expect(() =>
    createModelSetup({
      root: "/controlled",
      manifest,
      runtimeArtifact: { ...runtime, trusted: true },
    }),
  ).not.toThrow();
});
...
test("trusted runtime and model archives are verified and atomically become ready", async () => {
  ...
  try {
    const payload = join(fixture, "drawloom-llama-runtime");
    await mkdir(join(payload, "bin"), { recursive: true });
    await writeFile(join(payload, "bin", "llama-server"), "fixture-binary");
    await chmod(join(payload, "bin", "llama-server"), 0o700);
    await writeFile(join(payload, "LICENSE"), "MIT fixture");
    await writeFile(join(payload, "THIRD_PARTY_NOTICES.txt"), "MIT fixtures");
    ...
    const setup = createModelSetup({
      ...
      manifest,
      runtimeArtifact: { ...runtime, trusted: true },
      llamaRuntime: { platform: "darwin", arch: "arm64" },
      fetch: async (input) =>
        new Response(String(input).includes("runtime") ? archiveBytes : modelBytes),
    });
    await expect(
      setup.install({ consent: true, onProgress: (status) => progress.push(status.kind) }),
    ).resolves.toMatchObject({
      kind: "ready",
      directory: join(root, "active", "fixture"),
      runtimeDirectory: join(root, "runtime", `llama.cpp-${runtime.revision}`),
    });
    expect(progress).toContain("installing_runtime");
    expect(progress.at(-1)).toBe("ready");
    await expect(setup.ready()).resolves.toMatchObject({
      directory: join(root, "active", "fixture"),
    });
