...
export const RecordProvenanceSchema = z.strictObject({
  ...
  inputs: z.array(RecordRefSchema).max(100),
});
export type RecordProvenance = z.infer<typeof RecordProvenanceSchema>;
const ConfidenceSchema = boundedJson(
  z.json(),
  MAX_AUXILIARY_JSON_BYTES,
  "Confidence data is too large",
);
...
export const KnowledgeContributionRequestSchema = boundedJson(
  z.strictObject({
    body: BodySchema,
    kind: z.enum(["observation", "claim"]).default("observation"),
  }),
  MAX_RECORD_BYTES,
  "Knowledge contribution is too large",
);
export type KnowledgeContributionRequest = z.infer<typeof KnowledgeContributionRequestSchema>;
export const SourceRecordSchema = boundedJson(
  z.union([
    z.strictObject({
      ref: RecordRefSchema.extend({ type: z.literal("source") }),
      ...CommonRecord,
  ...
  MAX_RECORD_BYTES,
  "Knowledge record is too large",
);
export const ObservationRecordSchema = boundedJson(
  z.union([
    z.strictObject({
      ref: RecordRefSchema.extend({ type: z.literal("observation") }),
      ...CommonRecord,
      status: z.literal("active"),
    }),
    z.strictObject({
...
export const ClaimRecordSchema = boundedJson(
  z.union([
    z.strictObject({
      ref: RecordRefSchema.extend({ type: z.literal("claim") }),
      ...CommonRecord,
      status: z.literal("active"),
    ...
    z.strictObject({
      ...
      ...CommonRecord,
      status: z.literal("withdrawn"),
      freshness: z.literal("withdrawn"),
    }),
  ]),
  MAX_RECORD_BYTES,
  "Knowledge record is too large",
);
export const KnowledgeRecordSchema = z.union([
  SourceRecordSchema,
...
export const IntakeInputSchema = boundedJson(
  z.union([
    z.strictObject({
      operation: z.literal("upsert"),
      record: ActiveRecordSchema,
      expectedRevision: ExpectedRevisionSchema,
      links: z.array(KnowledgeLinkSchema).max(100).default([]),
    }),
    z.strictObject({
      operation: z.literal("withdraw"),
      record: WithdrawnRecordSchema,
      expectedRevision: Id,
      links: z.array(KnowledgeLinkSchema).max(100).default([]),
    }),
...
export const ToolOutcomeObservationSchema = boundedJson(
  z.strictObject({ body: z.string().min(1).max(8192) }),
  8192,
  "Projected observation is too large",
);
...
const PageWindow = {
  ...
  maxBytes: PageByteLimitSchema,
  cursor: OpaqueCursorSchema.optional(),
};
export const SearchRequestSchema = boundedJson(
  z.strictObject({
    query: z.string().trim().min(1).max(10_000),
    mode: z.enum(["best_available", "lexical"]).default("best_available"),
    ...PageWindow,
  }),
  32 * 1024,
...
export const SearchResultSchema = boundedJson(
  z
    .union([
      z.strictObject({
        kind: z.literal("ok"),
        mode: z.enum(["lexical", "hybrid"]),
        semantic: SemanticReadinessSchema,
        items: z.array(SearchHitSchema).max(100),
        bytes: z.number().int().nonnegative().max(MAX_PAGE_BYTES),
        cursor: OpaqueCursorSchema.optional(),
      }),
      DeniedSchema,
      BoundaryFailureSchema,
    ...
    .superRefine((result, context) => {
      if (result.kind === "ok" && result.mode === "hybrid" && result.semantic.status !== "ready") {
        context.addIssue({
          code: "custom",
          path: ["semantic"],
          message: "Hybrid mode requires a ready semantic index",
        });
      }
    }),
  MAX_PAGE_BYTES,
  "Search result is too large",
);
...
export const RecordReadResultSchema = z.union([
  z.strictObject({ kind: z.literal("ok"), record: KnowledgeRecordSchema.optional() }),
  DeniedSchema,
  BoundaryFailureSchema,
  CursorFailureSchema,
]);
...
export const ExpandRequestSchema = z.strictObject({
  ref: RecordRefSchema,
  direction: TraversalDirectionSchema,
  ...PageWindow,
});
export type ExpandRequest = z.infer<typeof ExpandRequestSchema>;
export const ExpandResultSchema = boundedJson(
  z.union([
    z.strictObject({
      kind: z.literal("ok"),
      items: z.array(KnowledgeLinkSchema).max(100),
      bytes: z.number().int().nonnegative().max(MAX_PAGE_BYTES),
      cursor: OpaqueCursorSchema.optional(),
    }),
    DeniedSchema,
...
export const EvidenceRequestSchema = z.strictObject({
  ...
  maxRecords: PageLimitSchema,
  maxLinks: PageLimitSchema,
  maxBytes: PageByteLimitSchema,
  cursor: OpaqueCursorSchema.optional(),
});
export type EvidenceRequest = z.infer<typeof EvidenceRequestSchema>;
export const EvidenceResultSchema = boundedJson(
  z.union([
    z.strictObject({
      kind: z.literal("ok"),
...
export const EvidenceToolResultSchema = z.union([
  EvidenceResultSchema,
  z.strictObject({
    kind: z.literal("already_delivered"),
    records: z.array(RecordRefSchema).max(100),
    cursor: OpaqueCursorSchema.optional(),
  }),
]);
...
export interface EvidenceReadReceipts {
  select(
    ...
    authorizedResult: EvidenceResult,
  ): EvidenceToolResult;
...
export const EvidencePackageSchema = boundedJson(
  z.strictObject({
    roots: z
      .array(EvidencePackageRootSchema)
      .min(1)
      .max(50)
        ...
        for (const [index, item] of roots.entries()) {
          if (seen.has(item.unitId))
            context.addIssue({
              code: "custom",
              path: [index, "unitId"],
              message: "Evidence package unit identities must be unique",
            });
  ...
  MAX_PAGE_BYTES,
  "Evidence package is too large",
);
export type EvidencePackage = z.infer<typeof EvidencePackageSchema>;
export const KnowledgeExportRequestSchema = z.strictObject({
  refs: z.array(RecordRefSchema).min(1).max(100),
  format: z.literal("okf"),
  maxBytes: PageByteLimitSchema,
});
export type KnowledgeExportRequest = z.infer<typeof KnowledgeExportRequestSchema>;
export const KnowledgeExportResultSchema = boundedJson(
  z.union([
    z.strictObject({
      kind: z.literal("ok"),
      records: z.array(KnowledgeRecordSchema).max(100),
...
export const PendingKnowledgeSchema = boundedJson(
  z.union([
    z.strictObject({
      kind: z.literal("ok"),
      batch: WorkBatchSchema,
      units: z.array(PendingWorkUnitSchema).max(100),
      remaining: z.boolean(),
      bytes: z.number().int().nonnegative().max(MAX_PAGE_BYTES),
    }),
    DeniedSchema,
  ...
  MAX_PAGE_BYTES,
  "Pending knowledge batch is too large",
);
...
const sameLogicalRef = (left: RecordRef, right: RecordRef) =>
  left.type === right.type && left.origin === right.origin && left.id === right.id;
export const ClaimProposalSchema = boundedJson(
  z
    .union([
      z.strictObject({
        record: ClaimRecordSchema,
      ...
      z.strictObject({
        record: ClaimRecordSchema,
        expectedRevision: Id,
        previous: RecordRefSchema.extend({ type: z.literal("claim") }),
        links: z.array(KnowledgeLinkSchema).max(100),
      }),
    ])
    .superRefine((proposal, context) => {
      if (proposal.expectedRevision !== null) {
        if (
          proposal.previous.revision !== proposal.expectedRevision ||
          !sameLogicalRef(proposal.previous, proposal.record.ref)
        ) {
          context.addIssue({
            code: "custom",
            path: ["previous"],
            message: "Edited claim must identify the exact prior logical revision",
          });
        }
        if (!proposal.record.provenance.inputs.some((input) => sameRef(input, proposal.previous))) {
          context.addIssue({
            code: "custom",
            path: ["record", "provenance", "inputs"],
            message: "Edited claim revision must be retained in provenance",
          });
        }
      }
      for (const [index, link] of proposal.links.entries()) {
        if (!sameRef(link.from, proposal.record.ref))
          context.addIssue({
            code: "custom",
            path: ["links", index, "from"],
            message: "Proposal links must originate at the proposed claim revision",
          });
        if (!proposal.record.provenance.inputs.some((input) => sameRef(input, link.to)))
          context.addIssue({
            code: "custom",
            path: ["links", index, "to"],
            message: "Cited evidence must be retained in claim provenance",
          });
      }
    }),
  MAX_PAGE_BYTES,
  "Claim proposal is too large",
);
export type ClaimProposal = z.infer<typeof ClaimProposalSchema>;
export const PublicationInputSchema = boundedJson(
  z.strictObject({ batch: WorkBatchSchema, proposals: z.array(ClaimProposalSchema).max(100) }),
  MAX_PAGE_BYTES,
  "Publication request is too large",
);
...
export const AssessmentRequestSchema = boundedJson(
  AssessmentIdentitySchema.extend({ evidence: EvidencePackageSchema }),
  MAX_PAGE_BYTES,
  "Assessment request is too large",
);
...
export const AssessmentResultSchema = boundedJson(
  z.union([
    z.strictObject({ kind: z.literal("running"), ...AssessmentIdentified }),
    z.strictObject({
      kind: z.literal("completed"),
      ...AssessmentIdentified,
      proposals: z.array(ClaimProposalSchema).max(100),
    }),
  ...
  MAX_PAGE_BYTES,
  "Assessment result is too large",
);
...
export const EmbeddingConfigurationSchema = z.strictObject({
  id: Id,
  fingerprint: Id,
...
export const IndexWorkRequestSchema = z.strictObject({
...
export const IndexWorkResultSchema = boundedJson(
  z.union([
    z.strictObject({
      kind: z.literal("ok"),
      batch: WorkBatchSchema,
      updates: z.array(IndexWorkUpdateSchema).max(100),
      remaining: z.boolean(),
      bytes: z.number().int().nonnegative().max(MAX_PAGE_BYTES),
    }),
    DeniedSchema,
...
export const EmbeddingInputSchema = boundedJson(
  z.strictObject({ id: Id, revision: Id.optional(), text: z.string().min(1).max(100_000) }),
  256 * 1024,
  "Embedding input is too large",
);
export type EmbeddingInput = z.infer<typeof EmbeddingInputSchema>;
export const EmbeddingBatchSchema = boundedJson(
  z.strictObject({
    configuration: EmbeddingConfigurationSchema,
    role: EmbeddingRoleSchema,
    items: z.array(EmbeddingInputSchema).min(1).max(100),
  }),
  MAX_PAGE_BYTES,
  "Embedding batch is too large",
);
export type EmbeddingBatch = z.infer<typeof EmbeddingBatchSchema>;
export const EmbeddedVectorSchema = z.strictObject({
  id: Id,
  revision: Id.optional(),
  vector: z.array(z.number().finite()).min(1).max(16_384),
});
export const EmbeddingResultSchema = boundedJson(
  z.union([
    z.strictObject({
      kind: z.literal("ok"),
      configuration: EmbeddingConfigurationSchema,
      items: z.array(EmbeddedVectorSchema).max(100),
    }),
    DeniedSchema,
    BoundaryFailureSchema,
  ]),
  MAX_PAGE_BYTES,
  "Embedding result is too large",
);
...
export function embeddingResultSchemaFor(batch: EmbeddingBatch) {
  return EmbeddingResultSchema.superRefine((result, context) => {
    ...
    if (
      result.configuration.id !== batch.configuration.id ||
      result.configuration.fingerprint !== batch.configuration.fingerprint ||
      result.configuration.dimensions !== batch.configuration.dimensions
    ) {
      context.addIssue({
        code: "custom",
        path: ["configuration"],
        message: "Embedding configuration does not match the request",
      });
    }
    if (result.items.length !== batch.items.length)
      context.addIssue({
        code: "custom",
        path: ["items"],
        message: "Embedding result count does not match the request",
      });
    for (const [index, item] of result.items.entries()) {
      const input = batch.items[index];
      if (!input || item.id !== input.id || item.revision !== input.revision)
        context.addIssue({
          code: "custom",
          path: ["items", index],
          message: "Embedding result identity or order does not match the request",
        });
      if (item.vector.length !== batch.configuration.dimensions)
        context.addIssue({
          code: "custom",
          path: ["items", index, "vector"],
          message: "Embedding vector dimensions do not match the request",
        });
    }
  });
}
...
export const IndexedEmbeddingSchema = z.strictObject({
  id: Id,
  ref: RecordRefSchema,
  vector: z.array(z.number().finite()).min(1).max(16_384),
});
export type IndexedEmbedding = z.infer<typeof IndexedEmbeddingSchema>;
export const EmbeddingIndexPrepareSchema = z.strictObject({
  configuration: EmbeddingConfigurationSchema,
  generation: z.number().int().positive(),
  expectedActiveGeneration: z.number().int().nonnegative().nullable(),
});
export const EmbeddingIndexPrepareResultSchema = z.union([
  z.strictObject({
    ...
    stageId: Id,
    generation: z.number().int().positive(),
    activeGeneration: z.number().int().nonnegative().nullable(),
  }),
  z.strictObject({ kind: z.literal("conflict") }),
  z.strictObject({ kind: z.literal("configuration_mismatch") }),
  BoundaryFailureSchema,
]);
export const EmbeddingIndexStageSchema = boundedJson(
  z.strictObject({
    stageId: Id,
    entries: z.array(IndexedEmbeddingSchema).max(100),
    removals: z.array(RecordRefSchema).max(100),
  }),
  MAX_PAGE_BYTES,
  "Embedding index update is too large",
);
export const EmbeddingIndexStageResultSchema = z.union([
  z.strictObject({ kind: z.literal("staged"), stageId: Id }),
  z.strictObject({ kind: z.literal("conflict") }),
  z.strictObject({ kind: z.literal("configuration_mismatch") }),
  z.strictObject({ kind: z.literal("dimension_mismatch") }),
  BoundaryFailureSchema,
]);
export const EmbeddingIndexActivateSchema = z.strictObject({
  stageId: Id,
  expectedActiveGeneration: z.number().int().nonnegative().nullable(),
});
export const EmbeddingIndexActivateResultSchema = z.union([
  z.strictObject({ kind: z.literal("activated"), generation: z.number().int().positive() }),
  z.strictObject({ kind: z.literal("conflict") }),
  BoundaryFailureSchema,
]);
export const EmbeddingIndexQuerySchema = z.strictObject({
  configuration: EmbeddingConfigurationSchema,
  vector: z.array(z.number().finite()).min(1).max(16_384),
  limit: PageLimitSchema,
});
        ...
        for (const [index, item] of items.entries()) {
          ...
          if (logical.has(key))
            context.addIssue({
              code: "custom",
              path: [index, "ref"],
              message: "Ranked results must deduplicate record revisions",
            });
          logical.add(key);
        }
      }),
...
export interface KnowledgeEmbeddingIndex {
  prepare(
    input: z.infer<typeof EmbeddingIndexPrepareSchema>,
  ): Promise<z.infer<typeof EmbeddingIndexPrepareResultSchema>>;
  stage(
    input: z.infer<typeof EmbeddingIndexStageSchema>,
  ): Promise<z.infer<typeof EmbeddingIndexStageResultSchema>>;
  activate(
    input: z.infer<typeof EmbeddingIndexActivateSchema>,
