...
import {
  ...
  type KnowledgeAssessmentConformanceFixture,
  type KnowledgeEmbeddingConformanceFixture,
  type KnowledgeIndexWorkConformanceFixture,
  type KnowledgeStorageConformanceFixture,
...
function createMemoryStorageFixture(): KnowledgeStorageConformanceFixture {
  ...
  const current = new Map<
    string,
    { ref: RecordRef; operation: "upsert" | "withdraw" | "delete"; fingerprint: string }
  >();
  ...
  const events: Array<{
    sequence: number;
    update: { ref: RecordRef; operation: "upsert" | "withdraw" | "delete" };
    targets: RecordRef[];
  ...
  const currentRecord = (ref: RecordRef) => {
    ...
    return latest && latest.operation !== "delete" && latest.ref.revision === ref.revision
      ? records.get(revisionKey(ref))
      : undefined;
  };
  const retainedRecord = (ref: RecordRef) =>
    deleted.has(logicalKey(ref)) ? undefined : records.get(revisionKey(ref));
  const affectedClaims = (changed: RecordRef) =>
    links
      .filter((link) => logicalKey(link.to) === logicalKey(changed) && link.from.type === "claim")
      .map((link) => link.from)
      .filter(
        (item, index, all) => all.findIndex((candidate) => sameRef(candidate, item)) === index,
      );
  const staleDependents = (claims: readonly RecordRef[]) => {
    for (const claimRef of claims) {
      const stored = records.get(revisionKey(claimRef));
      if (stored?.ref.type === "claim" && stored.status === "active")
        records.set(revisionKey(claimRef), { ...stored, freshness: "stale" });
    }
  };
  const addEvent = (
    ref: RecordRef,
    operation: "upsert" | "withdraw" | "delete",
    targets = affectedClaims(ref),
  ) => {
    events.push({
      sequence: nextEvent++,
    ...
    async ingest(subject, input) {
      ...
      if (prior?.ref.revision === reference.revision) {
        return prior.fingerprint === fingerprint
          ? { kind: "duplicate", revision: reference.revision }
          : { kind: "conflict" };
      }
      if (
      ...
      if (parsed.operation === "delete") {
        deleted.add(key);
        for (const storedKey of [...records.keys()])
          if (storedKey.startsWith(`${key}\0`)) records.delete(storedKey);
        links = links.filter(
          (link) => logicalKey(link.from) !== key && logicalKey(link.to) !== key,
        );
      } else {
        deleted.delete(key);
      ...
      const items = [...current.values()]
        .flatMap((entry) => {
          ...
          return stored?.status === "active" &&
            stored.body.toLowerCase().includes(parsed.query.toLowerCase())
    ...
    async expand(subject, request) {
      ...
      const candidates = links.filter((link) =>
        parsed.direction === "forward"
          ? sameRef(link.from, parsed.ref)
          : sameRef(link.to, parsed.ref),
      );
    ...
    async evidence(subject, request) {
      ...
      if (parsed.cursor) {
        ...
        if (
          !saved ||
          saved.kind !== "evidence" ||
          saved.request !== signature ||
          saved.subject !== subject.id
        )
      ...
      while (queue.length > 0) {
        ...
        for (const link of links) {
          const matches =
            parsed.direction === "forward"
              ? sameRef(link.from, next.ref)
              : sameRef(link.to, next.ref);
          if (!matches) continue;
          if (
            !traversedLinks.some((candidate) => JSON.stringify(candidate) === JSON.stringify(link))
          )
            traversedLinks.push(link);
          queue.push({
            ref: parsed.direction === "forward" ? link.to : link.from,
            depth: next.depth + 1,
          });
        }
      }
      ...
      if (bytes({ records: pageRecords, links: pageLinks }) > parsed.maxBytes)
      ...
      return {
        ...
        records: pageRecords,
        links: pageLinks,
        bytes: bytes({ records: pageRecords, links: pageLinks }),
        ...(cursor ? { cursor } : {}),
      };
    },
    async export(subject, request) {
      if (!allowed(subject)) return { kind: "denied" };
      const exportRecords = request.refs.flatMap((ref) => {
        const item = currentRecord(ref);
        return item ? [item] : [];
      });
      const exportLinks = links.filter((link) =>
        request.refs.some((ref) => sameRef(ref, link.from) || sameRef(ref, link.to)),
      );
      return {
        kind: "ok",
        records: exportRecords,
        links: exportLinks,
        bytes: bytes({ records: exportRecords, links: exportLinks }),
      };
    },
  };
  ...
  const maintenance: KnowledgeMaintenance = {
    async status(subject) {
      if (!allowed(subject)) return { kind: "denied" };
      const pendingUnits = events.reduce(
        (count, event) => count + (event.targets.length || 1) - event.completed.size,
        0,
      );
      return {
        kind: "ok",
        pendingUnits,
        ...(pendingUnits ? { oldestPendingAtMs: 1_757_678_400_000 } : {}),
        checkpoint: `status-${nextEvent - 1}`,
      };
    },
      ...
      const available = events.flatMap((event) => {
        ...
        return targets.flatMap((target, index) => {
          ...
          const unit: PendingWorkUnit = {
            ...
            update: event.update,
            ...(target?.type === "claim"
              ? { affectedClaim: { ...target, type: "claim" as const } }
              : {}),
          };
    ...
    async publish(subject, input) {
      ...
      if (
        !captured ||
        parsed.batch.checkpoint !== `unit-${captured.units.at(-1)?.unit.id ?? "none"}` ||
        captured.units.some(
          ({ unit }) =>
            current.get(logicalKey(unit.update.ref))?.ref.revision !== unit.update.ref.revision ||
            (unit.affectedClaim !== undefined &&
              current.get(logicalKey(unit.affectedClaim))?.ref.revision !==
                unit.affectedClaim.revision),
        )
      )
        return { kind: "conflict" };
      for (const proposal of parsed.proposals) {
        ...
        if (
          proposal.expectedRevision === null
            ? prior !== undefined
            : prior?.ref.revision !== proposal.expectedRevision
        )
          return { kind: "conflict" };
        if (
          proposal.record.provenance.inputs.some(
            (input) => current.get(logicalKey(input))?.ref.revision !== input.revision,
          )
        )
          return { kind: "conflict" };
      }
      for (const proposal of parsed.proposals) {
        ...
        current.set(logicalKey(proposal.record.ref), {
          ref: proposal.record.ref,
          operation: "upsert",
      ...
      for (const capturedUnit of captured.units) {
        ...
        event.completed.add(
          capturedUnit.unit.affectedClaim ? revisionKey(capturedUnit.unit.affectedClaim) : "source",
        );
      }
      batches.delete(parsed.batch.id);
      const remaining = events.some((event) =>
        (event.targets.length ? event.targets : [undefined]).some(
          (target) => !event.completed.has(target ? revisionKey(target) : "source"),
        ),
      );
...
function assessmentEvidence() {
  ...
  return {
    roots: [{ unitId: "assessment-unit", root: source.ref }],
    records: [source],
    links: [],
    complete: true,
  };
...
function createMemoryAssessmentFixture(): KnowledgeAssessmentConformanceFixture {
  ...
  const requests = new Map<
    string,
    { fingerprint: string; result: AssessmentResult; evidence: typeof evidence }
  >();
  const assessment: KnowledgeAssessment = {
    async assess(subject, input) {
      ...
      if (prior)
        return prior.fingerprint === parsed.payloadFingerprint
          ? prior.result
          : { kind: "conflict" };
      const result = {
          ...
          proposals: [
            {
              record: {
                ref: claimRef,
                body: `Assessment of ${source.body}`,
                status: "active",
                freshness: "current",
                confidence: { value: "provisional" },
                provenance: {
                  producer: { type: "assessment", id: "memory" },
                  inputs: [source.ref],
                },
              },
              expectedRevision: null,
              links: [{ from: claimRef, to: source.ref, relation: "support" }],
            },
          ],
        };
      }
    ...
    async cancel(subject, input) {
      ...
      if (prior.fingerprint !== input.payloadFingerprint) return { kind: "conflict" };
      if (prior.result.kind === "completed")
        return {
          kind: "too_late",
          requestId: input.requestId,
          payloadFingerprint: input.payloadFingerprint,
...
function createMemoryEmbeddingFixture(
  options: {
  ...
  const stages = new Map<
    string,
    {
      ...
      entries: Map<string, IndexedEmbedding>;
    }
  >();
    ...
    async embed(subject, batch) {
      ...
      return {
        ...
        items: parsed.items.map((item, index) => ({
          ...
          vector: Array.from(
            { length: parsed.configuration.dimensions },
            (_, dimension) => (index + dimension + 1) / parsed.configuration.dimensions,
          ),
        })),
      };
    },
    ...
    async ingest(subject, raw) {
      ...
      if (
        input.expectedRevision === null
          ? prior !== undefined
          : prior?.ref.revision !== input.expectedRevision
      )
        return { kind: "conflict" };
  ...
  const embeddingIndex: KnowledgeEmbeddingIndex = {
    async prepare(input) {
      ...
      if (known && !matchesConfiguration(known, parsed.configuration))
        return { kind: "configuration_mismatch" };
      const activeConfiguration = active.get(parsed.configuration.id);
      if (
        (activeConfiguration?.generation ?? null) !== parsed.expectedActiveGeneration ||
        parsed.generation <= (activeConfiguration?.generation ?? 0)
      )
        return { kind: "conflict" };
      ...
      stages.set(stageId, {
        ...
        expected: parsed.expectedActiveGeneration,
        entries: new Map(priorEntries),
      });
      return {
        kind: "ready",
        stageId,
        generation: parsed.generation,
        activeGeneration: activeConfiguration?.generation ?? null,
      };
    },
    async stage(input) {
      stageCalls += 1;
      ...
      if (parsed.entries.some((entry) => entry.vector.length !== stage.configuration.dimensions))
      ...
      for (const removal of parsed.removals)
        for (const [id, entry] of stage.entries)
          if (sameRef(removal, entry.ref)) stage.entries.delete(id);
      for (const entry of parsed.entries) stage.entries.set(entry.id, entry);
      if (options.invalidLaterStageResult && stageCalls >= 3)
    ...
    async activate(input) {
      ...
      if (
        !stage ||
        stage.expected !== parsed.expectedActiveGeneration ||
        (activeConfiguration?.generation ?? null) !== parsed.expectedActiveGeneration
      )
        return { kind: "conflict" };
      if (options.discardOtherConfigurationsOnActivate) active.clear();
      active.set(stage.configuration.id, {
        configuration: stage.configuration,
        generation: stage.generation,
        entries: new Map(stage.entries),
      });
      stages.delete(parsed.stageId);
      if (options.invalidLaterActivationResult && activateCalls >= 2)
        return { kind: "unexpected" } as never;
    ...
    async query(input) {
      ...
      if (parsed.vector.length !== activeConfiguration.configuration.dimensions)
        return { kind: "dimension_mismatch" };
      if (
        options.emptyActiveWhileStaged &&
        [...stages.values()].some((stage) => stage.configuration.id === parsed.configuration.id)
      ) {
        return { kind: "ok", activeGeneration: activeConfiguration.generation, items: [] };
      }
      const seen = new Set<string>();
      const items = [...activeConfiguration.entries.values()]
        .flatMap((entry) => {
          ...
          return [
            {
              ref: entry.ref,
              relevance: entry.vector.reduce(
                (sum, value, index) => sum + value * parsed.vector[index]!,
                0,
              ),
            },
          ];
...
function createMemoryIndexWorkFixture(): KnowledgeIndexWorkConformanceFixture {
  ...
  const events: Array<{
    sequence: number;
    updates: Array<
      | { id: string; operation: "upsert"; record: Extract<KnowledgeRecord, { status: "active" }> }
      | { id: string; operation: "remove"; ref: RecordRef }
    >;
  }> = [];
  ...
  const batches = new Map<
    string,
    {
      ...
      checkpoint: string;
      event: number;
      updates: (typeof events)[number]["updates"];
    }
  >();
  ...
  const intake: KnowledgeIntake = {
    async ingest(subject, raw) {
      ...
      if (prior)
        updates.push({ id: `event-${sequence}-remove`, operation: "remove", ref: prior.ref });
  ...
  const indexWork: KnowledgeIndexWork = {
    async pending(subject, raw) {
      ...
      if (outstanding) {
        ...
        return {
          kind: "ok",
          batch: { id, checkpoint: batch.checkpoint },
          updates: batch.updates,
          remaining: false,
          bytes: bytes(batch.updates),
        };
      }
      ...
      const allUpdates = prior
        ...
        : [...current.values()].flatMap((item, index) =>
            item.status === "active"
              ? [
                  {
                    id: `snapshot-${revisionKey(item.ref)}-${index}`,
      ...
      return {
        ...
        batch: { id, checkpoint },
        updates,
        remaining: allUpdates.length > updates.length,
        bytes: bytes(updates),
      };
    },
    async acknowledge(subject, raw) {
      ...
      if (priorCheckpoint)
        return priorCheckpoint === input.batch.checkpoint
          ? { kind: "acknowledged", checkpoint: priorCheckpoint }
          : { kind: "conflict" };
...
test("embedding conformance rejects an implementation that empties the active generation while staging", async () => {
  await expect(
    knowledgeEmbeddingConformance(createMemoryEmbeddingFixture({ emptyActiveWhileStaged: true })),
  ).rejects.toThrow();
});
test("embedding conformance rejects activation that discards another configuration", async () => {
  await expect(
    knowledgeEmbeddingConformance(
      createMemoryEmbeddingFixture({ discardOtherConfigurationsOnActivate: true }),
    ),
  ).rejects.toThrow();
});
test("embedding conformance rejects a malformed later stage outcome", async () => {
  await expect(
    knowledgeEmbeddingConformance(createMemoryEmbeddingFixture({ invalidLaterStageResult: true })),
  ).rejects.toThrow();
});
test("embedding conformance rejects a malformed later activation outcome", async () => {
  await expect(
    knowledgeEmbeddingConformance(
      createMemoryEmbeddingFixture({ invalidLaterActivationResult: true }),
    ),
  ).rejects.toThrow();
});
