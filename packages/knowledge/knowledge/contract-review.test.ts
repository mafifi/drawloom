...
test("ranked record identities cannot collide through embedded separators", () => {
  ...
  expect(
    contract.EmbeddingIndexQueryResultSchema.safeParse({
      kind: "ok",
      activeGeneration: 1,
      items: [
        { ref, relevance: 1 },
        { ref: { ...ref, origin: "one\0two", id: "three" }, relevance: 0.5 },
      ],
    }).success,
  ).toBe(true);
});
...
function schema(name: string) {
  const value = Reflect.get(contract, name) as
...
test("intake requires an explicit create/update precondition and lifecycle-consistent withdrawal", () => {
  expect(
    contract.IntakeInputSchema.safeParse({ operation: "upsert", record: source }).success,
  ).toBe(false);
  expect(
    contract.IntakeInputSchema.safeParse({
      operation: "upsert",
  ...
  expect(
    contract.IntakeInputSchema.safeParse({
      operation: "withdraw",
      expectedRevision: "r1",
      record: { ...source, ref: { ...source.ref, revision: "r2" } },
    }).success,
  ).toBe(false);
  expect(
    contract.IntakeInputSchema.safeParse({
      operation: "withdraw",
      expectedRevision: "r1",
      record: { ...source, ref: { ...source.ref, revision: "r2" }, status: "withdrawn" },
    }).success,
  ).toBe(true);
});
...
test("agent contribution accepts only new host-attributed observation or claim bodies", () => {
  ...
  expect(contribution.safeParse({ body: "A derived conclusion.", kind: "claim" }).success).toBe(
    true,
  );
...
test("retrieval reports its effective mode separately from semantic readiness", () => {
  ...
  expect(
    result.safeParse({
      kind: "ok",
      mode: "lexical",
      semantic: { status: "rebuilding", configurationId: "embed-v2" },
      items: [{ record: source, relevance: 0.75 }],
      bytes: 128,
    }).success,
  ).toBe(true);
  expect(
...
test("aggregate schemas enforce UTF-8 payload bounds", () => {
  ...
  expect(contract.SourceRecordSchema.safeParse({ ...source, body: multibyteBody }).success).toBe(
    false,
  );
  expect(
    schema("AuthorizationResultSchema").safeParse({ kind: "failure", code: "unavailable" }).success,
  ).toBe(true);
});
...
test("maintenance publication consumes a provider-issued batch instead of choosing a checkpoint", () => {
  ...
  expect(schema("PendingRequestSchema").safeParse({ limit: 10, maxBytes: 65_536 }).success).toBe(
    true,
  );
  expect(
    schema("PendingWorkUnitSchema").safeParse({
      id: "unit-1",
      update: { ref: source.ref, operation: "upsert" },
      affectedClaim: { type: "claim", origin: "assessment", id: "claim", revision: "r1" },
    }).success,
  ).toBe(true);
  expect(
    pending.safeParse({
      kind: "ok",
      batch: { id: "batch-1", checkpoint: "cp-1" },
      units: [{ id: "unit-1", update: { ref: source.ref, operation: "upsert" } }],
      remaining: true,
      bytes: 128,
    }).success,
  ).toBe(true);
  ...
  expect(
    publication.safeParse({ batch: { id: "batch-1", checkpoint: "cp-1" }, proposals: [] }).success,
  ).toBe(true);
  expect(
    publication.safeParse({
      snapshot: { checkpoint: "cp-1", records: [] },
      proposals: [],
      nextCheckpoint: "caller-choice",
    }).success,
  ).toBe(false);
});
...
test("maintenance exposes authorization-bound backlog age and count without draining work", () => {
  expect(
    schema("MaintenanceStatusResultSchema").safeParse({
      kind: "ok",
      pendingUnits: 50,
      oldestPendingAtMs: 1_757_678_400_000,
      checkpoint: "cp-1",
    }).success,
  ).toBe(true);
});
...
test("index work is a configuration-scoped bounded current-revision feed with batch-only acknowledgement", () => {
  ...
  expect(
    schema("IndexWorkRequestSchema").safeParse({ configuration, limit: 10, maxBytes: 65_536 })
      .success,
  ).toBe(true);
  expect(
    schema("IndexWorkResultSchema").safeParse({
      kind: "ok",
      batch: { id: "index-batch-1", checkpoint: "index-cp-1" },
      updates: [{ id: "index-unit-1", operation: "upsert", record: source }],
      remaining: true,
      bytes: 128,
    }).success,
  ).toBe(true);
  expect(
    schema("IndexWorkAcknowledgeInputSchema").safeParse({
      batch: { id: "index-batch-1", checkpoint: "index-cp-1" },
    }).success,
  ).toBe(true);
  expect(
  ...
  expect(
    schema("IndexWorkAcknowledgeResultSchema").safeParse({
      kind: "acknowledged",
      checkpoint: "index-cp-1",
    }).success,
  ).toBe(true);
});
...
test("claim edits pin their exact prior revision in authoritative provenance", () => {
  ...
  expect(
    schema("ClaimProposalSchema").safeParse({ record: next, expectedRevision: "r1", links: [] })
      .success,
  ).toBe(false);
  expect(
    schema("ClaimProposalSchema").safeParse({
      record: next,
      expectedRevision: "r1",
      previous: prior,
      links: [],
...
test("assessment carries stable payload identity through reconciliation and truthful cancellation", () => {
  ...
  expect(
    assessment.safeParse({
      ...identity,
      evidence: { root: source.ref, records: [source], links: [], complete: true },
    }).success,
  ).toBe(false);
  expect(
    assessment.safeParse({
  ...
  expect(
    schema("AssessmentResultSchema").safeParse({ kind: "uncertain", ...identity }).success,
  ).toBe(true);
  expect(schema("AssessmentReconcileRequestSchema").safeParse(identity).success).toBe(true);
  expect(
    schema("AssessmentCancellationResultSchema").safeParse({ kind: "too_late", ...identity })
      .success,
  ).toBe(true);
});
...
test("embedding index stages one immutable configuration and activates it with CAS", () => {
  const configuration = {
    id: "embed",
    fingerprint: "artifact-format-pooling-normalization",
    dimensions: 2,
  };
  expect(schema("EmbeddingConfigurationSchema").safeParse(configuration).success).toBe(true);
  expect(
    schema("EmbeddingIndexPrepareSchema").safeParse({
      configuration,
      generation: 2,
  ...
  expect(
    schema("EmbeddingIndexStageSchema").safeParse({
      stageId: "stage-2",
      entries: [{ id: "guide:passage:1", ref: source.ref, vector: [0.5, 0.5] }],
      removals: [],
    }).success,
  ).toBe(true);
  expect(
    schema("EmbeddingIndexActivateSchema").safeParse({
      stageId: "stage-2",
      expectedActiveGeneration: 1,
    }).success,
  ).toBe(true);
...
test("embedding response validation binds count, order, identity, and dimensions to the request", () => {
  ...
  const batch = contract.EmbeddingBatchSchema.parse({
    configuration,
    role: "document",
    items: [
      { id: "one", text: "first" },
  ...
  expect(
    response.safeParse({
      ...
      configuration,
      items: [
        { id: "two", vector: [0.5, 0.5] },
        { id: "one", vector: [0.5, 0.5] },
      ],
    }).success,
  ).toBe(false);
  expect(
