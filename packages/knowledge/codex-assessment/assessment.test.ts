...
const request: AssessmentRequest = {
  ...
  evidence: {
    roots: [{ unitId: "unit-1", root: source }],
    complete: true,
    records: [
      {
        ref: source,
        body: "The public library closes Monday.",
        status: "active",
        confidence: {},
...
function fixture(
  options: {
    readonly failAt?: FailurePoint | "turn/interrupt";
    readonly hangAt?: string;
    readonly deniedRefs?: readonly string[];
  ...
  const calls: string[] = [];
  const resolverRefs: Array<string | undefined> = [];
  ...
  const transport: RpcTransport = {
    async request(method, params) {
      calls.push(method);
      const handled = await options.handle?.(method, params);
      ...
      if (method === "model/list")
        return {
          data: [
            {
              id: options.model ?? "gpt-5.6-terra",
              model: options.model ?? "gpt-5.6-terra",
              supportedReasoningEfforts: [{ reasoningEffort: options.effort ?? "low" }],
            },
          ],
          nextCursor: null,
        };
      ...
      if (method === "thread/turns/list")
        return {
          data: [{ id: "native-turn", status: interrupted ? "interrupted" : "completed" }],
          nextCursor: null,
        };
                    ...
                    proposals: [
                      {
                        id: "library-hours",
                        previous: null,
                        text: "The library is closed Monday.",
                        confidenceJson: "{}",
                        withdraw: false,
                        supportingIndices: [0],
                        contraryIndices: [],
...
function withHiddenInput(): AssessmentRequest {
  ...
  return {
    ...
    evidence: {
      ...
      records: [
        {
          ...request.evidence.records[0]!,
          provenance: { producer: { type: "test", id: "public" }, inputs: [hidden] },
        },
      ],
      links: [{ from: source, to: hidden, relation: "support" }],
    },
  };
}
...
test("every root in a maintenance batch requires disclosure authority before connecting", async () => {
  ...
  try {
    const batched: AssessmentRequest = {
      ...request,
      evidence: {
        ...request.evidence,
...
test("native intervention preserves uncertainty until native terminal evidence is read", async () => {
  ...
  try {
    ...
    const changed = createCodexAssessment({
      model: "gpt-5.6-sol",
      effort: "low",
      workingDirectory: "/private/tmp",
      timeoutMs: 20,
    ...
    try {
      expect(
        (
          await changed.reconcile(subject, {
            requestId: request.requestId,
            payloadFingerprint: request.payloadFingerprint,
          })
        ).kind,
...
test("an operation shares one deadline across sequential provider requests", async () => {
  ...
  try {
    expect((await f.provider.assess(subject, request)).kind).not.toBe("running");
    expect(f.calls).not.toContain("turn/start");
  } finally {
    await f.provider.close();
  }
...
test("a timed-out never-connected request does not strand connection admission", async () => {
  ...
  const provider = createCodexAssessment({
    ...
    resolveResource: async () => ({ type: "test", id: "local", properties: {} }),
  });
  try {
    await provider.assess(subject, request);
    await provider.assess(subject, { ...request, requestId: "second" });
...
test("late cancellation cannot overwrite a concurrently persisted completed outcome", async () => {
  ...
  const f = fixture({
    handle: async (method) => {
      ...
      if (method === "thread/turns/list" && ++reads === 1)
  ...
  const f = fixture({
    handle: async (method) =>
      method === "thread/turns/list"
        ? { data: [{ id: "native-turn", status: "inProgress" }], nextCursor: null }
        : undefined,
  });
  await f.provider.assess(subject, request);
  f.emit({ id: 7, method: "approval/request", params: {} });
...
test("passes the shared assessment conformance suite", async () => {
  ...
  try {
    await knowledgeAssessmentConformance({
      assessment: f.provider,
...
test("simultaneous identical requests submit at most once", async () => {
  const f = fixture();
  try {
    await Promise.all([f.provider.assess(subject, request), f.provider.assess(subject, request)]);
...
test("lost submission is recovered by its exact persisted native marker", async () => {
  ...
  const f = fixture({
    handle: async (method, params) => {
      if (method === "turn/start") {
        marker = String((params as { input: { text: string }[] }).input[0]!.text).split("\n")[0]!;
        throw Error("lost response");
      }
      if (method === "thread/items/list" && !(params as { cursor?: string }).cursor)
        return {
          data: [
            {
              turnId: "native-turn",
