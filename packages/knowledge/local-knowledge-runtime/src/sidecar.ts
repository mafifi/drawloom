...
import { LocalKnowledgeConfigurationSchema, LocalPreparationSchema, LocalPreparationCancellationSchema } from "./protocol.js";
...
async function dispatch(method: string, params: unknown): Promise<unknown> {
  ...
  if (method === "knowledge.warmup") { Empty.parse(params); return runtime.warmup(); }
  if (method === "knowledge.cancel-preparation") { const { requestId } = LocalPreparationCancellationSchema.parse(params); preparations.get(requestId)?.abort(); return {}; }
  if (method === "knowledge.prepare") {
    const { requestId, ...request } = LocalPreparationSchema.parse(params);
    if (preparations.size >= 16 || preparations.has(requestId)) throw Error("Preparation admission limit");
    const controller = new AbortController(); preparations.set(requestId, controller);
    const timer = setTimeout(() => controller.abort(), 5000);
    try { return await runtime.prepare({ ...request, signal: controller.signal }); }
    finally { clearTimeout(timer); preparations.delete(requestId); }
  }
