...
export function parseStoredLocalKnowledgeConfiguration(
  value: unknown,
): LocalKnowledgeConfiguration {
  if (
    value &&
    typeof value === "object" &&
    !Array.isArray(value) &&
    (value as { embeddingModel?: unknown }).embeddingModel === "qwen3-embedding-0.6b-mlx"
  ) {
    return LocalKnowledgeConfigurationSchema.parse({
