...
function initialState(now: number, settings: NightloomSettings): NightloomCoordinatorState {
  return {
    ...
    settings,
    paused: false,
    budget: { utcDay: utcDay(now), automaticStarts: 0, automaticReservedMilliseconds: 0 },
    nextSequence: 1,
  };
}
...
export function createNightloomCoordinator(input: {
  ...
  async function save(
    prior: NightloomCoordinatorState,
    update: Omit<NightloomCoordinatorState, "revision">,
  ): Promise<NightloomCoordinatorState | undefined> {
  ...
  const normalizeDay = async (
    state: NightloomCoordinatorState,
  ): Promise<NightloomCoordinatorState> => {
    const day = utcDay(input.clock.now());
    if (state.budget.utcDay === day) return state;
    return (
      (await save(state, {
        ...state,
        budget: { utcDay: day, automaticStarts: 0, automaticReservedMilliseconds: 0 },
      })) ?? load()
    );
  };
  async function dispatchReserved(state: NightloomCoordinatorState): Promise<NightloomStartResult> {
    ...
    try {
      const runId = await input.orchestrator.start(state.active.identity, nightloomWorkflow, {
        ...
        maxBytes: state.settings.maxBytes,
        assessmentTimeoutMs: state.settings.assessmentTimeoutMs,
      });
      const saved = await save(state, {
        ...state,
        active: { ...state.active, state: "started", runId },
      });
  ...
  async function clearTerminal(
    state: NightloomCoordinatorState,
  ): Promise<NightloomCoordinatorState | "uncertain" | undefined> {
    ...
    if (
      ...
      NightloomWorkflowResultSchema.parse(await input.orchestrator.result(snapshot.runId)).kind ===
        "deferred"
    )
      return "uncertain";
  ...
  async function start(
    ...
    feed?: { pendingCount: number; oldestPendingAt?: number },
    overrideBudget = false,
  ): Promise<NightloomStartResult> {
    ...
    if (state.active) {
      ...
      try {
        cleared = await clearTerminal(state);
      } catch {
    ...
    if (mode === "automatic") {
      ...
      const due =
        (feed?.pendingCount ?? 0) >= state.settings.pendingThreshold ||
        (feed?.oldestPendingAt !== undefined &&
          now - feed.oldestPendingAt >= state.settings.oldestPendingAgeMs);
      if (!due) return { kind: "idle" };
    }
    const usesBudget = mode === "automatic" || !overrideBudget;
    if (usesBudget) {
      if (
        state.budget.automaticStarts >= state.settings.maxAutomaticStartsPerDay ||
        state.budget.automaticReservedMilliseconds + state.settings.assessmentTimeoutMs >
          state.settings.maxAutomaticMillisecondsPerDay
      )
    ...
    const budget = usesBudget
      ? {
          ...
          automaticReservedMilliseconds:
            state.budget.automaticReservedMilliseconds + state.settings.assessmentTimeoutMs,
        }
      : state.budget;
    const reserved = await save(state, {
      ...state,
      active,
      budget,
    ...
    tick(): Promise<NightloomStartResult> {
      return exclusive(async () => {
        ...
        return start("automatic", {
          pendingCount: status.pendingUnits,
          ...(status.oldestPendingAtMs === undefined
            ? {}
            : { oldestPendingAt: status.oldestPendingAtMs }),
        });
      });
    },
    runNow(overrideBudget = false): Promise<NightloomStartResult> {
      return exclusive(() => start("manual", undefined, overrideBudget));
    },
    pause(): Promise<void> {
    ...
    configure(settings: NightloomSettings): Promise<void> {
      ...
      return exclusive(async () => {
        for (;;) {
          ...
          if (state.active)
            throw Error("Wait for active knowledge maintenance before changing settings");
          if (await save(state, { ...state, settings: next })) return;
        }
      });
    },
    async status(): Promise<NightloomCoordinatorState> {
      return load();
    },
  };
}
...
const PendingTaskInputSchema = z.strictObject({
  limit: z.number().int().min(1).max(100),
  maxBytes: z.number().int().min(1024).max(1_048_576),
});
export const nightloomPendingTask = {
  id: "nightloom.pending",
  version: "1",
  input: PendingTaskInputSchema,
  output: PendingKnowledgeSchema,
};
export const nightloomAssessBatchTask = {
  id: "nightloom.assess-batch",
  version: "2",
  input: AssessBatchInputSchema,
  output: BatchAssessmentSchema,
  limits: { startToCloseTimeoutMs: DEFAULT_NIGHTLOOM_SETTINGS.assessmentTimeoutMs },
};
export const nightloomReleaseTask = {
  id: "nightloom.release",
  version: "1",
  input: WorkBatchReleaseInputSchema,
  output: WorkBatchReleaseResultSchema,
};
export const nightloomPublishTask = {
  id: "nightloom.publish",
  version: "1",
  input: PublicationInputSchema,
  output: PublicationResultSchema,
};

export const NightloomWorkflowInputSchema = z.strictObject({
  batchSize: z.number().int().min(1).max(50),
  maxBytes: z.number().int().min(1024).max(1_048_576),
  assessmentTimeoutMs: z.number().int().min(1).max(300_000).optional(),
});
export const NightloomWorkflowResultSchema = z.union([
  ...
  z.strictObject({
    kind: z.enum(["deferred", "conflict"]),
    processed: z.number().int().nonnegative(),
    remaining: z.literal(true),
  }),
  z.strictObject({
    kind: z.enum(["denied", "unavailable"]),
    processed: z.literal(0),
    remaining: z.boolean(),
  }),
  z.strictObject({
    kind: z.literal("blocked"),
...
export const nightloomWorkflow: Workflow<
  z.infer<typeof NightloomWorkflowInputSchema>,
  NightloomWorkflowResult
> = {
  id: "nightloom.maintenance",
  version: "3",
  input: NightloomWorkflowInputSchema,
  output: NightloomWorkflowResultSchema,
  async run(context, input): Promise<NightloomWorkflowResult> {
    let limit = input.batchSize;
    for (;;) {
      const pending = await context.task(`pending-${limit}`, nightloomPendingTask, {
        limit,
        maxBytes: input.maxBytes,
      });
      if (pending.kind === "denied") return { kind: "denied", processed: 0, remaining: false };
      if (pending.kind !== "ok") {
        if (pending.kind === "failure" && pending.code === "too_large" && limit > 1) {
      ...
      const assessmentInput = {
        ...
        maxLinks: 100,
        maxBytes: input.maxBytes,
        ...(input.assessmentTimeoutMs === undefined
          ? {}
          : { assessmentTimeoutMs: input.assessmentTimeoutMs }),
      };
      let assessed = await context.task(
        `assess-${limit}`,
        nightloomAssessBatchTask,
        assessmentInput,
      );
      for (let poll = 1; assessed.kind === "running"; poll++) {
        await context.sleep(`assessment-wait-${limit}-${poll}`, assessed.waitMs);
        assessed = await context.task(
          `reconcile-${limit}-${poll}`,
          nightloomAssessBatchTask,
          assessmentInput,
        );
      }
      ...
      if (assessed.kind === "blocked") {
        if (assessed.reason === "evidence_too_large" && pending.units.length > 1) {
          const released = await context.task(`release-${limit}`, nightloomReleaseTask, {
            batch: pending.batch,
          });
          if (released.kind === "conflict")
            return { kind: "conflict", processed: 0, remaining: true };
          if (released.kind === "denied") return { kind: "denied", processed: 0, remaining: true };
          if (released.kind !== "released")
            return { kind: "unavailable", processed: 0, remaining: true };
          limit = Math.max(1, Math.floor(pending.units.length / 2));
          continue;
        }
        return { kind: "blocked", processed: 0, remaining: true, reason: assessed.reason };
      }
      const publication = await context.task("publish", nightloomPublishTask, {
        batch: pending.batch,
        proposals: assessed.proposals,
      });
      ...
      return {
        kind: "completed",
        processed: pending.units.length,
...
const ReceiptStateSchema = z.enum([
  "intent",
  "running",
  "uncertain",
  "completed",
...
export const NightloomAssessmentReceiptSchema = z.strictObject({
  ...
  payloadFingerprint: z.string().min(1).max(256),
  state: ReceiptStateSchema,
  proposals: z.array(ClaimProposalSchema).max(100).optional(),
...
async function collectEvidence(
  ...
  const roots = input.units.map((unit) => ({
    unitId: unit.id,
    root: unit.affectedClaim ?? unit.update.ref,
  }));
        ...
        do {
          ...
          if (page.kind !== "ok")
            return {
              kind: "blocked",
              reason:
                page.kind === "failure" && page.code === "too_large"
                  ? "evidence_too_large"
                  : "unavailable",
            };
          const addedRecords = page.records.filter(
            (record) => !records.some((prior) => sameRef(prior.ref, record.ref)),
          );
          const addedLinks = page.links.filter(
            (link) => !links.some((prior) => canonical(prior) === canonical(link)),
          );
          if (
            records.length + addedRecords.length > input.maxRecords ||
            links.length + addedLinks.length > input.maxLinks
          )
            return { kind: "blocked", reason: "evidence_too_large" };
          records.push(...addedRecords);
          links.push(...addedLinks);
          if (transmittedBytes() > input.maxBytes)
            return { kind: "blocked", reason: "evidence_too_large" };
          cursor = page.cursor;
          if (cursor) {
  ...
  if (
    input.units.some(
      (unit) =>
        !(unit.update.operation === "delete" && !unit.affectedClaim) &&
        !retained(unit.affectedClaim ?? unit.update.ref),
    ) ||
    records.some((record) => record.provenance.inputs.some((ref) => !retained(ref))) ||
    links.some((link) => !retained(link.from) || !retained(link.to))
  )
  ...
  return evidence.success && transmittedBytes() <= input.maxBytes
    ? evidence.data
    : { kind: "blocked", reason: "evidence_too_large" };
}
function validateProposals(
  result: Extract<AssessmentResult, { kind: "completed" }>,
  evidence: EvidencePackage,
): ClaimProposal[] | undefined {
  if (result.proposals.length > 100) return undefined;
  const refs = evidence.records.map((record) => record.ref);
  for (const proposal of result.proposals) {
    if (
      !proposal.record.provenance.inputs.every((ref) =>
    ...
    if (
      ...
      !refs.some((candidate) => sameRef(candidate, proposal.previous))
    )
      return undefined;
    if (
      !proposal.links.every(
        (link) =>
          sameRef(link.from, proposal.record.ref) &&
          refs.some((candidate) => sameRef(candidate, link.to)),
      )
    )
      return undefined;
  }
...
export function createNightloomTaskHandlers(input: {
  ...
  async function writeReceipt(
    prior: NightloomAssessmentReceipt,
    update: Omit<NightloomAssessmentReceipt, "revision">,
  ): Promise<NightloomAssessmentReceipt> {
    const next = NightloomAssessmentReceiptSchema.parse({
      ...(prior.deadlineAtMs === undefined ? {} : { deadlineAtMs: prior.deadlineAtMs }),
      ...update,
    ...
    if (!current || current.payloadFingerprint !== prior.payloadFingerprint)
      throw new StepFailure("unknown", "Assessment receipt changed");
    const { revision: _currentRevision, ...currentValue } = current;
    const { revision: _nextRevision, ...nextValue } = next;
    if (canonical(currentValue) !== canonical(nextValue))
      throw new StepFailure("unknown", "Assessment receipt changed");
    return current;
  }
  async function applyAssessment(
    receipt: NightloomAssessmentReceipt,
    evidence: EvidencePackage,
    ...
    if (result.kind === "running" || result.kind === "uncertain") {
      ...
      if (result.kind === "running" && receipt.deadlineAtMs !== undefined)
        return {
          kind: "running",
          waitMs: Math.max(1, Math.min(1000, receipt.deadlineAtMs - Date.now())),
        };
  ...
  async function assessBatch(
    ...
    const payloadFingerprint = z
      .string()
      .min(1)
      .max(256)
      .parse(await input.fingerprint({ batch: request.batch, evidence }));
    let receipt = await input.receipts.load(requestId);
    let createdIntent = false;
    if (receipt && receipt.payloadFingerprint !== payloadFingerprint)
      return { kind: "blocked", reason: "invalid_evidence" };
    if (receipt?.state === "completed")
      return { kind: "completed", proposals: receipt.proposals ?? [] };
    if (!receipt) {
      if (!allowSubmit) throw new StepFailure("unknown", "Missing assessment intent");
      const intent = NightloomAssessmentReceiptSchema.parse({
        revision: 0,
        requestId,
        payloadFingerprint,
        state: "intent",
    ...
    function cancelOwned(): Promise<BatchAssessment> {
      ...
      const cancellation = Promise.resolve().then(async (): Promise<BatchAssessment> => {
        ...
        if (current.state === "completed")
          return { kind: "completed", proposals: current.proposals ?? [] };
        if (current.state === "cancelled") return { kind: "blocked", reason: "cancelled" };
        const held = await writeReceipt(current, {
          requestId,
          payloadFingerprint,
          state: "uncertain",
        });
        ...
        return applyAssessment(
          held,
          retainedEvidence,
          terminal ?? { kind: "uncertain", ...identity },
        );
      });
    ...
    const remaining =
      receipt.deadlineAtMs === undefined
        ? DEFAULT_NIGHTLOOM_SETTINGS.assessmentTimeoutMs
        : receipt.deadlineAtMs - Date.now();
    ...
    if (context.signal.aborted && input.isHostSuspension?.(context.signal.reason))
      throw new StepFailure("unknown", "Assessment host suspended");
  ...
  return Object.freeze([
    registerTaskHandler(nightloomPendingTask, {
      run: (request) => input.maintenance.pending(input.subject, request),
    }),
    registerTaskHandler(nightloomAssessBatchTask, {
      ...
      async recover(request, context) {
        try {
          return { status: "completed", output: await assessBatch(request, context, false) };
        } catch {
          return { status: "unknown" };
        }
      },
    }),
    registerTaskHandler(nightloomReleaseTask, {
      run: (request) => input.maintenance.release(input.subject, request),
    }),
    registerTaskHandler(nightloomPublishTask, {
      run: (request) => input.maintenance.publish(input.subject, request),
    }),
  ]);
}
...
export const nightloomRegistry = defineWorkflowModule({
  ...
  tasks: [
    nightloomPendingTask,
    nightloomAssessBatchTask,
    nightloomReleaseTask,
    nightloomPublishTask,
  ],
});
