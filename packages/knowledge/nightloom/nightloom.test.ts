...
function scriptedOrchestrator() {
  ...
  const engine: Orchestrator = {
    ...
    async result() { return { kind: "completed", processed: 0, remaining: false, checkpoint: "done" }; }, async respond() {},
...
test("assessment shares complete exact roots once and budgets transmitted UTF-8 evidence, not repeated page bytes", async () => {
  ...
  const configured = dependencies({
    async assess(_subject, request) { sent = request.evidence; return { kind: "completed", requestId: request.requestId, payloadFingerprint: request.payloadFingerprint, proposals: [] }; },
    async reconcile() { throw Error("unexpected reconciliation"); }, async cancel() { throw Error("unexpected cancellation"); },
  });
  configured.retrieval = { ...configured.retrieval,
    async evidence() { reads++; return { kind: "ok", records: [source], links: [], bytes: 1000 }; },
    async get() { gets++; return { kind: "ok", record: source }; },
  };
  const assess = createNightloomTaskHandlers(configured).find(handler => handler.id === "nightloom.assess-batch")!;
  expect(await assess.run({ ...assessInput("shared", [unit("one"), unit("two")]), maxBytes: 1024 }, context())).toEqual({ kind: "completed", proposals: [] });
  expect(reads).toBe(1);
  expect(gets).toBe(0);
...
test("assessment counts source update bodies and UTF-8 bytes in its final package", async () => {
  const configured = dependencies({ async assess() { throw Error("oversized evidence must not be sent"); }, async reconcile() { throw Error("unexpected"); }, async cancel() { throw Error("unexpected"); } });
  configured.retrieval = { ...configured.retrieval,
    async evidence() { return { kind: "ok", records: [], links: [], bytes: 0 }; },
    async get() { return { kind: "ok", record: { ...source, body: "界".repeat(400) } }; },
  };
  const assess = createNightloomTaskHandlers(configured).find(handler => handler.id === "nightloom.assess-batch")!;
  expect(await assess.run({ ...assessInput("utf8"), maxBytes: 1024 }, context())).toEqual({ kind: "blocked", reason: "evidence_too_large" });
});
...
for (const ceiling of ["records", "links"] as const) {
  test(`distinct roots with shared evidence fit exactly at the ${ceiling} ceiling`, async () => {
    ...
    const configured = dependencies({
      async assess(_subject, request) { sent = request.evidence; return { kind: "completed", requestId: request.requestId, payloadFingerprint: request.payloadFingerprint, proposals: [] }; },
      async reconcile() { throw Error("unexpected"); }, async cancel() { throw Error("unexpected"); },
    });
    configured.retrieval = { ...configured.retrieval, async evidence(_subject, request) {
      visited.push(request.root.id);
      if (request.root.id === "source" && addUniqueEvidence) return {
        kind: "ok", records: ceiling === "records" ? [{ ...source, ref: { ...source.ref, revision: "r2" } }] : [source],
        links: ceiling === "links" ? [{ from: source.ref, to: parent.ref, relation: "support" }] : [], bytes: 100,
      };
      return { kind: "ok", records: request.root.id === "parent" ? [parent, source] : [source], links: ceiling === "links" ? [link] : [], bytes: 100 };
    } };
    const assess = createNightloomTaskHandlers(configured).find(handler => handler.id === "nightloom.assess-batch")!;
    const units: PendingWorkUnit[] = [{ id: "parent", update: { ref: parent.ref, operation: "upsert" } }, unit("child")];
    expect(await assess.run({ ...assessInput(`exact-${ceiling}`, units), maxRecords: ceiling === "records" ? 2 : 10, maxLinks: ceiling === "links" ? 1 : 10 }, context())).toEqual({ kind: "completed", proposals: [] });
    ...
    expect(await assess.run({ ...assessInput(`overflow-${ceiling}`, units), maxRecords: ceiling === "records" ? 2 : 10, maxLinks: ceiling === "links" ? 1 : 10 }, context())).toEqual({ kind: "blocked", reason: "evidence_too_large" });
  });
}
...
test("nested evidence is not a completed traversal and changed revisions are gathered independently", async () => {
  ...
  const parent = { ...source, ref: { ...source.ref, id: "parent" }, body: "Parent evidence" };
  const visited: string[] = []; let sent: EvidencePackage | undefined;
  const configured = dependencies({
    async assess(_subject, request) { sent = request.evidence; return { kind: "completed", requestId: request.requestId, payloadFingerprint: request.payloadFingerprint, proposals: [] }; },
    async reconcile() { throw Error("unexpected"); }, async cancel() { throw Error("unexpected"); },
  });
  configured.retrieval = { ...configured.retrieval, async evidence(_subject, request) {
    ...
    return { kind: "ok", records: request.root.id === "parent" ? [parent, source] : request.root.revision === "r1" ? [source] : [second], links: [], bytes: 100 };
  } };
  const assess = createNightloomTaskHandlers(configured).find(handler => handler.id === "nightloom.assess-batch")!;
  const units: PendingWorkUnit[] = [
    { id: "parent", update: { ref: parent.ref, operation: "upsert" } }, unit("one"),
    { id: "two", update: { ref: second.ref, operation: "upsert" } },
  ];
  expect(await assess.run(assessInput("different", units), context())).toEqual({ kind: "completed", proposals: [] });
  expect(visited).toEqual(["parent:r1", "source:r1", "source:r2"]);
  expect(sent?.records.map(record => record.body)).toEqual(["Parent evidence", "Evidence", "Updated evidence"]);
});

test("an absent active root cannot become a complete assessment package", async () => {
  let submissions = 0;
  const configured = dependencies({
    async assess(_subject, request) { submissions++; return { kind: "completed", requestId: request.requestId, payloadFingerprint: request.payloadFingerprint, proposals: [] }; },
    async reconcile() { throw Error("unexpected"); }, async cancel() { throw Error("unexpected"); },
  });
  configured.retrieval = { ...configured.retrieval,
    async evidence() { return { kind: "ok", records: [], links: [], bytes: 0 }; },
    async get() { return { kind: "ok" }; },
  };
  const assess = createNightloomTaskHandlers(configured).find(handler => handler.id === "nightloom.assess-batch")!;
  expect(await assess.run(assessInput("absent"), context())).toEqual({ kind: "blocked", reason: "invalid_evidence" });
...
test("unresolved terminal work blocks automatic and manual starts after coordinator restart", async () => {
  ...
  const store = memoryCoordinatorStore();
  const maintenance = backlogMaintenance(() => ({ pendingCount: 50, oldestPendingAt: 0 }));
  let coordinator = createNightloomCoordinator({ orchestrator: base.engine, maintenance, subject, store, readiness: () => ({ status: "ready" }), clock: { now: () => Date.UTC(2026, 8, 12) } });
  const started = await coordinator.tick();
  expect(started.kind).toBe("started");
  ...
  coordinator = createNightloomCoordinator({ orchestrator: base.engine, maintenance, subject, store, readiness: () => ({ status: "ready" }), clock: { now: () => Date.UTC(2026, 8, 12) } });
  ...
  expect(base.runs.size).toBe(1);
  expect((await coordinator.status()).active).toMatchObject({ runId: started.runId, state: "started" });
});
...
test("a preexisting assessment intent is reconciled and never treated as a fresh submission", async () => {
  ...
  const configured = dependencies({
    async assess() { submissions += 1; throw Error("preexisting intent must not submit"); },
    async reconcile(_subject, request) { reconciliations += 1; return { kind: "uncertain", ...request }; },
    async cancel(_subject, request) { return { kind: "too_late", ...request }; },
  }, receipts);
  const assess = createNightloomTaskHandlers(configured).find((handler) => handler.id === "nightloom.assess-batch")!;
  ...
  configured.retrieval = {
    ...
    async evidence(_subject, request) {
      return request.cursor
        ? { kind: "ok", records: [{ ...source, ref: { ...source.ref, id: "additional" } }], links: [], bytes: 100 }
        : { kind: "ok", records: [source], links: [], bytes: 100, cursor: OpaqueCursorSchema.parse("more") };
    },
  };
  const assess = createNightloomTaskHandlers(configured).find((handler) => handler.id === "nightloom.assess-batch")!;
...
test("known-running assessment reconciles in the same workflow and exact batch without resubmission", async () => {
  ...
  const configured = dependencies({
    async assess(_subject, request) { submissions++; const identity = { requestId: request.requestId, payloadFingerprint: request.payloadFingerprint }; identities.push(identity); return { kind: "running", ...identity }; },
    async reconcile(_subject, request) { identities.push(request); reconciliations++; return reconciliations === 1 ? { kind: "running", ...request } : { kind: "completed", ...request, proposals: [] }; },
    async cancel(_subject, request) { throw Error(`unexpected cancellation ${request.requestId}`); },
  });
  ...
  const result = await nightloomWorkflow.run({
    runId: "same-run",
    async task(step, task, input) { steps.push(step); return await handlers.find(handler => handler.id === task.id)!.run(input, context()) as never; },
    async sleep(step) { sleeps.push(step); },
    async child() { throw Error("unexpected child"); }, async input() { throw Error("unexpected input"); },
  ...
  expect(identities).toEqual(Array(3).fill({ requestId: "batch", payloadFingerprint: "evidence-fingerprint" }));
...
test("assessment timeout cancels the exact receipt and does not reset on reconciliation", async () => {
  ...
  const configured = dependencies({
    async assess(_subject, request) { submissions++; return { kind: "running", requestId: request.requestId, payloadFingerprint: request.payloadFingerprint }; },
    async reconcile(_subject, request) { reconciliations++; return { kind: "running", ...request }; },
    async cancel(_subject, request) { cancellations.push(request); return { kind: "cancelled", ...request }; },
  }, receipts);
  const handler = () => createNightloomTaskHandlers(configured).find(value => value.id === "nightloom.assess-batch")!;
  ...
  expect(await handler().recover!({ ...request, assessmentTimeoutMs: 300_000 }, context())).toMatchObject({ status: "completed", output: { kind: "blocked", reason: "cancelled" } });
  ...
  expect(cancellations).toEqual([{ requestId: "batch-deadline", payloadFingerprint: "evidence-fingerprint" }]);
});
...
test("a nonsettling provider is bounded and an unconfirmed cancellation remains held", async () => {
  ...
  let cancellations = 0;
  const configured = dependencies({
    async assess() { return new Promise(() => {}); },
    async reconcile() { throw Error("must not retry"); },
    async cancel(_subject, request) { cancellations++; return { kind: "uncertain", ...request }; },
  }, receipts);
  const handler = createNightloomTaskHandlers(configured).find(value => value.id === "nightloom.assess-batch")!;
  expect(await handler.run({ ...assessInput("hung"), assessmentTimeoutMs: 10 }, context())).toEqual({ kind: "deferred" });
...
test("terminal deferred outcome stays owned across automatic and manual coordinator restart", async () => {
  ...
  const store = memoryCoordinatorStore();
  const options = { orchestrator: base.engine, maintenance: backlogMaintenance(() => ({ pendingCount: 100 })), subject, store,
    readiness: () => ({ status: "ready" as const }), clock: { now: () => 1_789_459_196_404 } };
...
test("cancellation during accepted-assessment pacing interrupts its exact owner without another submission", async () => {
  const controller = new AbortController();
  const receipts = receiptStore();
  let submissions = 0, reconciliations = 0, cancellations = 0;
  const configured = dependencies({
    async assess(_subject, request) { submissions++; return { kind: "running", requestId: request.requestId, payloadFingerprint: request.payloadFingerprint }; },
    async reconcile(_subject, identity) { reconciliations++; return { kind: "running", ...identity }; },
    async cancel(_subject, identity) { cancellations++; expect(identity).toEqual({ requestId: "batch-cancel-wait", payloadFingerprint: "evidence-fingerprint" }); return { kind: "cancelled", ...identity }; },
  }, receipts);
  const handler = createNightloomTaskHandlers(configured).find(value => value.id === "nightloom.assess-batch")!;
  expect(await handler.run(assessInput("cancel-wait"), context(controller.signal))).toMatchObject({ kind: "running" });
  controller.abort();
  while ((await receipts.load("batch-cancel-wait"))?.state !== "cancelled") await new Promise(resolve => setTimeout(resolve, 1));
  expect({ submissions, reconciliations, cancellations }).toEqual({ submissions: 1, reconciliations: 0, cancellations: 1 });
...
test("polling retains one cancellation listener and terminal completion removes it", async () => {
  ...
  const added = spyOn(signal, "addEventListener"), removed = spyOn(signal, "removeEventListener");
  let completed = false;
  const configured = dependencies({
    async assess(_subject, request) { return { kind: "running", requestId: request.requestId, payloadFingerprint: request.payloadFingerprint }; },
    async reconcile(_subject, request) { return completed ? { kind: "completed", ...request, proposals: [] } : { kind: "running", ...request }; },
    async cancel() { throw Error("not cancelled"); },
  });
  const handler = createNightloomTaskHandlers(configured).find(value => value.id === "nightloom.assess-batch")!;
  try {
    for (let poll = 0; poll < 5; poll++) {
      expect(await handler.run(assessInput("listeners"), context(signal))).toMatchObject({ kind: "running" });
      expect(added.mock.calls.length - removed.mock.calls.length).toBe(1);
    }
    completed = true;
    expect(await handler.run(assessInput("listeners"), context(signal))).toMatchObject({ kind: "completed" });
...
test("trusted host suspension releases its listener without cancelling accepted evidence", async () => {
  ...
  const controller = new AbortController();
  const handler = createNightloomTaskHandlers({ ...configured, isHostSuspension: reason => reason === pause }).find(value => value.id === "nightloom.assess-batch")!;
  await handler.run(assessInput("pause"), context(controller.signal));
  const before = await receipts.load("batch-pause");
  controller.abort(pause); await Promise.resolve();
  expect(cancellations).toBe(0); expect(await receipts.load("batch-pause")).toEqual(before);
...
test("abort during active reconciliation joins one exact cancellation and retains its confirmed terminal receipt", async () => {
  ...
  const reconciling = new Promise<void>(resolve => { beginReconcile = resolve; });
  const cancellationHeld = new Promise<void>(resolve => { finishCancel = resolve; });
  const identity = { requestId: "batch-cancel-reconcile", payloadFingerprint: "evidence-fingerprint" };
  const configured = dependencies({
    async assess(_subject, request) { submissions++; return { kind: "running", requestId: request.requestId, payloadFingerprint: request.payloadFingerprint }; },
    async reconcile(_subject, request) { expect(request).toEqual(identity); beginReconcile(); return new Promise<AssessmentResult>(resolve => { finishReconcile = resolve; }); },
    async cancel(_subject, request) { expect(request).toEqual(identity); cancellations++; await cancellationHeld; return { kind: "cancelled", ...request }; },
  }, receipts);
  const handler = createNightloomTaskHandlers(configured).find(value => value.id === "nightloom.assess-batch")!;
  const taskContext = () => context(AbortSignal.any([run.signal, new AbortController().signal]));
  expect(await handler.run(assessInput("cancel-reconcile"), taskContext())).toMatchObject({ kind: "running" });
