...
if (
  process.env.DRAWLOOM_GGUF_EVALUATION !== "1" ||
  !process.env.DRAWLOOM_MODELS_ROOT ||
  !process.argv[2]
)
  throw Error(
    "Requires explicit local GGUF opt-in, existing model root and output path; never downloads",
  );
...
if (!(await createModelSetup({ root: modelRoot, manifest: KnownModelManifests[model] }).ready()))
  throw Error("Existing model is not ready");
...
const provider = createSqliteKnowledge({
  ...
  authorizer,
  resolveResource: () => ({ type: "fixture", id: "public", properties: {} }),
});
const worker = new LlamaEmbeddingWorker({ root: modelRoot, model });
const semantic = createSemanticRetrieval({
  subject,
  configuration,
  embeddings: createKnowledgeEmbeddings({ model, worker, authorizer }),
  retrieval: provider.retrieval,
  work: provider.indexWork,
  index: provider.embeddingIndex,
  authorizeSearch: async () => true,
  revision: async () => {
    const result = await provider.maintenance.status(subject);
    if (result.kind !== "ok") throw Error("status unavailable");
    return result.checkpoint;
  },
});
const guidance = [
  "References may be related but may not answer the request.",
  "Acknowledge missing information; do not infer unsupported facts.",
];
...
async function actualWire(prepared, query) {
  ...
  const rpc = {
    async request(method, params) {
      ...
      if (method === "thread/start")
        return { thread: { id: "synthetic-native" }, approvalsReviewer: "user" };
  ...
  const consume = (async () => {
    for await (const _ of opened.value.signals()) {
      /* attach normal session owner */
    }
  })();
  try {
    ...
    assert.deepEqual(
      delivered.map((item) => item.method),
      ["turn/start", "turn/steer"],
    );
    for (const item of delivered)
      for (const sentence of guidance) assert(item.text.includes(sentence));
    return delivered;
  } finally {
...
let report;
try {
  for (const record of challenge.records)
    assert.equal(
      (
        await provider.intake.ingest(subject, {
          ...
          links: [],
          record: {
            ref: { type: "source", origin: "independent-challenge", id: record.id, revision: "r1" },
            body: record.body,
            status: "active",
            confidence: {},
            provenance: { producer: { type: "fixture", id: challenge.version }, inputs: [] },
          },
        })
      ).kind,
      "accepted",
    );
  for (let i = 0; i < challenge.records.length + 1 && semantic.state !== "ready"; i++)
  ...
  for (const mode of ["lexical", "hybrid"]) {
    for (const example of challenge.cases) {
      const preparer = createKnowledgeContextPreparer({
        subject,
        destination: { type: "test-answerer", id: "synthetic", properties: {} },
        authorizer,
        resolveDisclosureResource: async ({ ref }) => ({
          type: "record",
          id: ref.id,
          properties: {},
        }),
        retrieval: {
          ...
          search: (_subject, request) =>
            semantic.search({
              ...request,
              mode: mode === "lexical" ? "lexical" : "best_available",
            }),
        },
      });
      const prepared = await preparer.prepare({
        request: example.query,
        binding: { executionId: "challenge", conversationId: "synthetic" },
        signal: new AbortController().signal,
        budget: { maxRecords: 8, maxBytes: 12 * 1024 },
      });
      const ids = prepared.references.map((item) => item.ref.id);
      const required =
        example.requiredMode && example.requiredMode !== mode ? [] : (example.require ?? []);
      ...
      cases.push({
        ...
        passed:
          ...
          !unexpected.length &&
          !missingAny &&
          ["ready", "empty"].includes(prepared.kind),
        wire: prepared.kind === "ready" ? await actualWire(prepared, example.query) : [],
      });
    }
  }
  const denied = await createKnowledgeContextPreparer({
    subject,
    destination: { type: "test", id: "synthetic", properties: {} },
    authorizer: { authorize: async () => ({ decision: false }) },
    ...
    request: "Cedar Press ink",
    binding: { executionId: "denied", conversationId: "synthetic" },
    signal: new AbortController().signal,
    budget: { maxRecords: 8, maxBytes: 12 * 1024 },
  });
  assert.deepEqual(denied, { kind: "empty", references: [], bytes: 0 });
  report = {
    challengeSha256: createHash("sha256").update(fixtureSource).digest("hex"),
    policy: challenge.policy,
    model: KnownModelManifests[model],
    runtime: KnownLlamaRuntime,
    configuration,
    answeringModelCalls: 0,
    transport: "actual Codex adapter with controlled RPC; no native process",
    cases,
    disclosureDenied: denied,
    passed: cases.every((item) => item.passed),
    limitation:
      "Selection and actual start/steer input proof only; no claim that an answering model follows guidance.",
  };
} finally {
  semantic.close();
  provider.close();
  ...
  JSON.stringify(
    {
      ...
      cases: cases.map(({ id, mode, selected, missing, unexpected, passed }) => ({
        id,
        mode,
        selected,
        missing,
      ...
      cleanup: "closed and removed own temporary store",
    },
    null,
    2,
  ),
