...
import {
  scoreRetrieval,
  scoreRetrievalByCategory,
  type RetrievalCategoryMetrics,
...
type Latency = {
  ...
  readonly warm30MedianMs: number;
  readonly warm30P95Ms: number;
  readonly cpuMicros: number;
  readonly startRssBytes: number;
  readonly peakRssBytes: number;
...
type HybridRun =
  ...
  | ({
      readonly kind: "real_vectors";
      readonly model: string;
      readonly indexing: PhaseMeasurement;
    } & RetrievalRun);
...
export async function runKnowledgeEvaluation(
  options: KnowledgeEvaluationOptions,
): Promise<KnowledgeEvaluationReport> {
  const size = options.size ?? documents.length;
  if (!Number.isSafeInteger(size) || size < documents.length || size > 100_000)
    throw new Error("size must be between the held-out corpus size and 100000");
    ...
    const hybrid = options.embedding
      ...
      : options.model
        ? await measureHybrid(
            ...
            size,
            options.model,
            options.modelRoot ?? join(root, "models"),
            databasePath,
          )
        : { kind: "not_run" as const, reason: "no_model_requested" as const };
    return {
...
async function ingest(
  ...
  for (const document of records) {
    ...
    const result = await provider.intake.ingest(subject, {
      operation: "upsert",
      expectedRevision: revisions.get(key) ?? null,
      record: {
        ref,
        body: document.text,
        status: "active",
      ...
      links: document.evidence.map((evidence) => ({
        from: ref,
        to: parseRef(evidence),
        relation: "support" as const,
      })),
    });
    if (result.kind !== "accepted" && result.kind !== "duplicate")
      throw new Error(`ingest failed for ${document.id}@${document.revision}: ${result.kind}`);
...
async function measureLexical(
  ...
  return measure(
    async (query) => {
      const result = await provider.retrieval.search(subject, {
        query,
        mode: "lexical",
        limit: 10,
        maxBytes: 256 * 1024,
...
async function measureEmbedding(
  ...
  const semantic = createSemanticRetrieval({
    ...
    work: provider.indexWork,
    index: provider.embeddingIndex,
    embeddings,
    configuration,
    authorizeSearch,
  ...
  try {
    ...
    if (semantic.state !== "ready")
      throw new Error(`semantic indexing did not finish within ${recordCount} bounded steps`);
...
async function measure(
  ...
  for (const [index, question] of heldOutQuestions.entries()) {
    ...
    retrieved[question.id] = await retrieve(question.query);
    measurement.sample();
    reportProgress(phase, index + 1, heldOutQuestions.length);
    if (index === 0) coldMs = performance.now() - started;
  }
  const warm: number[] = [];
  for (let index = 0; index < 30; index++) {
    const started = performance.now();
    await retrieve(heldOutQuestions[index % heldOutQuestions.length]!.query);
    measurement.sample();
    reportProgress(`${phase} warm queries`, index + 1, 30);
  ...
  return {
    ...
    questions: Object.fromEntries(
      heldOutQuestions.map((question) => [
        question.id,
        { category: question.kind, retrieved: retrieved[question.id] ?? [] },
      ]),
    ),
    measurement: phaseMeasurement,
    latency: {
      coldMs,
      warm30MedianMs: median(warm),
      warm30P95Ms: percentile(warm, 0.95),
      cpuMicros: phaseMeasurement.cpuMicros,
      startRssBytes: phaseMeasurement.startRssBytes,
      peakRssBytes: phaseMeasurement.peakRssBytes,
      storeBytes: bytes.total,
    },
    storeBytes: bytes,
  };
}
...
function recordRef(document: EvaluationDocument): SourceRef {
  return {
    type: "source",
...
function parseRef(value: string): SourceRef {
  const [id, revision] = value.split("@");
  if (!id || !revision) throw new Error(`invalid corpus reference ${value}`);
  return { type: "source", origin: "evaluation-public", id, revision };
}
...
interface PhaseSampler {
  sample(): void;
  finish(): PhaseMeasurement;
}
...
function reportProgress(phase: string, completed: number, total: number): void {
  if (completed === total || completed % 1_000 === 0)
    process.stderr.write(`[knowledge-evaluation] ${phase}: ${completed}/${total}\n`);
}
function reportReady(phase: string, workUnits: number): void {
  process.stderr.write(
    `[knowledge-evaluation] ${phase}: ready after ${workUnits} bounded work units\n`,
  );
}
async function storeBytes(path: string): Promise<SQLiteStoreBytes> {
  const size = async (file: string) => {
    try {
      return Number((await stat(file)).size);
    } catch {
      return 0;
    }
  };
...
async function corpusHash(): Promise<string> {
...
export function parseCliOptions(arguments_: readonly string[]): KnowledgeEvaluationOptions {
  ...
  const scale = arguments_.includes("--scale");
  const stress = arguments_.includes("--stress");
  if (scale && stress) throw new Error("--scale and --stress cannot be combined");
  const model = value("--model");
  if (model !== undefined && !(model in KnownModelManifests))
    throw new Error("--model must name a known local embedding model");
  return {
    root,
    size: stress
      ? 100_000
      : scale
        ...
        : value("--size") === undefined
          ? undefined
          : Number(value("--size")),
    ...(model ? { model: model as KnownModelId } : {}),
    ...(value("--models-root") ? { modelRoot: value("--models-root") } : {}),
  };
}

if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  process.stdout.write(
    `${JSON.stringify(await runKnowledgeEvaluation(parseCliOptions(process.argv.slice(2))), null, 2)}\n`,
  );
}
