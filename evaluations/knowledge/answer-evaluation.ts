...
const RetrievalCategorySchema = z.enum([
  "semantic",
  "identifier",
  "chain",
  "contradiction",
...
const RetrievalQuestionsSchema = z.record(
  Id,
  z.strictObject({ category: RetrievalCategorySchema, retrieved: z.array(Id).max(100) }),
);
const CorpusIdentitySchema = z.strictObject({
  version: Id,
  sha256: z.string().regex(/^[a-f0-9]{64}$/),
  records: z.number().int().min(24).max(100_000),
});
const ExistingReportSchema = z.looseObject({
  ...
  hybrid: z.union([
    ...
    z.looseObject({
      ...
      model: z.enum(["qwen3-embedding-0.6b-gguf", "qwen3-embedding-0.6b", "nomic-embed-text-v1.5"]),
      questions: RetrievalQuestionsSchema,
    }),
...
const PreparedRetrievalSchema = z.strictObject({
  version: z.literal(1),
  mode: AnswerEvaluationCaseSchema.shape.mode,
  databasePath: z.string().min(1),
  corpus: CorpusIdentitySchema,
  questions: RetrievalQuestionsSchema.optional(),
  report: z.json(),
  source: z.enum(["generated_smoke", "existing_report"]),
  reportSha256: Id.optional(),
});
...
const ReceiptSchema = z.strictObject({
  ...
  turnId: Id.optional(),
  output: AnswerOutputSchema.optional(),
...
export type AnswerCaseResult =
  | {
      kind: "completed";
      output: AnswerOutput;
      nativeActivity: readonly string[];
      cached: boolean;
  ...
  | {
      kind: "blocked";
      reason: "context_too_large" | "model_unavailable" | "invalid_output";
      nativeActivity: readonly string[];
      cleanupFailure?: string;
    }
...
export function buildAnswerPrompt(raw: AnswerEvaluationCase): string {
  ...
  return [
    ...
    canonical({
      question: { id: value.questionId, text: value.query },
      retrievedRecords: value.records,
      storedEvidenceChains: value.chains,
    }),
  ].join("\n\n");
}
...
export async function runAnswerCase(options: {
    ...
    try {
      return await Promise.race([
        ...
        new Promise<T>((_, reject) => {
          timer = setTimeout(() => reject(Error("Answer evaluation deadline exceeded")), remaining);
        }),
      ]);
    } finally {
  ...
  const maxEvidenceBytes = Math.min(
    Math.max(options.maxEvidenceBytes ?? 256 * 1024, 1024),
    1024 * 1024,
  );
  if (
    Buffer.byteLength(canonical({ records: value.records, chains: value.chains })) >
    maxEvidenceBytes
  )
    return { kind: "blocked", reason: "context_too_large", nativeActivity: [] };
  ...
  try {
    saved = await bounded(() => options.store.get(key));
  } catch {
  ...
  if (saved !== undefined) {
    ...
    if (
      ...
      receipt.effort !== options.effort
    )
      throw Error("Answer evaluation receipt conflicts with the requested case");
  } else {
    receipt = ReceiptSchema.parse({
      ...
      marker: `drawloom-answer:${digest(key)}`,
      milestone: "preflight",
      nativeActivity: [],
    });
    try {
      await bounded(() => options.store.set(key, structuredClone(receipt) as JsonValue));
    } catch {
      return { kind: "uncertain", nativeActivity: [] };
    }
  }
  const cleanTerminalReceipt = async (result: AnswerCaseResult): Promise<AnswerCaseResult> => {
    if (
      !receipt.threadId ||
      receipt.archiveCompleted ||
      (receipt.milestone !== "completed" && receipt.milestone !== "failed")
    )
      return result;
    ...
    if (finished.cleanup.kind === "failed")
      return { ...result, cleanupFailure: finished.cleanup.reason };
      ...
      try {
        await Promise.race([
          ...
          new Promise<never>((_, reject) => {
            timer = setTimeout(
              () => reject(Error("Archive confirmation receipt write timed out")),
              Math.min(timeoutMs, 5_000),
            );
          }),
        ]);
      } catch {
        return { ...result, cleanupFailure: "Archived thread confirmation could not be saved" };
      } finally {
        if (timer) clearTimeout(timer);
  ...
  if (receipt.milestone === "completed" && receipt.output)
    return cleanTerminalReceipt({
      kind: "completed",
      output: receipt.output,
      nativeActivity: receipt.nativeActivity,
      cached: true,
    });
  if (receipt.milestone === "failed")
    return cleanTerminalReceipt({ kind: "failed", nativeActivity: receipt.nativeActivity });
  let rpc: RpcTransport | undefined;
  let unsubscribe = () => {};
  let terminalToClean: AnswerCaseResult | undefined;
  const markTerminal = (result: AnswerCaseResult) => {
    terminalToClean = result;
    return result;
  };
  ...
  const save = async () => {
    ...
    try {
      await bounded(() => options.store.set(key, snapshot));
    } catch (cause) {
      persistenceCompromised = true;
      throw cause;
    }
  };
  try {
    ...
    try {
      ...
      void opening
        .then((late) =>
          Promise.race([late.close(), new Promise<void>((resolve) => setTimeout(resolve, 1_000))]),
        )
        .catch(() => undefined);
      throw cause;
    }
    const request = (method: string, params: unknown) =>
      bounded(() => rpc!.request(method, params));
    unsubscribe = rpc.subscribe(
      (message) => {
        const label = nativeActivityLabel(message);
        if (label) activity.add(label);
      },
      () => {},
    );
    await request("initialize", {
      clientInfo: { name: "drawloom-answer-evaluation", version: "0.0.0" },
      capabilities: { experimentalApi: true },
    });
    rpc.notify("initialized");

    if (receipt.milestone === "preflight") {
      if (!(await modelAvailable(request, options.model, options.effort)))
        return { kind: "blocked", reason: "model_unavailable", nativeActivity: [...activity] };
      const opened = z.object({ thread: z.object({ id: Id }) }).parse(
        await request("thread/start", {
          ...
          developerInstructions:
            "Answer only from supplied evidence. Do not use tools or external knowledge. Return schema-valid JSON only.",
          config: disabledCapabilities,
        }),
      );
      receipt.threadId = opened.thread.id;
      receipt.milestone = "thread_created";
      await save();
    }
    ...
    if (receipt.milestone === "thread_created") {
      ...
      await save();
      try {
        const started = z.object({ turn: z.object({ id: Id }) }).parse(
        ...
        receipt.turnId = started.turn.id;
        receipt.milestone = "accepted";
        await save();
      } catch {
        await save();
        return { kind: "uncertain", nativeActivity: [...activity] };
      }
    }
    ...
    while (receipt.milestone === "accepted" && Date.now() < deadline) {
      const turns = z
        .object({
          data: z.array(z.object({ id: Id, status: z.string() })).max(50),
      ...
      if (turn.status !== "completed") {
        receipt.milestone = "failed";
        await save();
        return markTerminal({ kind: "failed", nativeActivity: [...activity] });
      }
      ...
      const answer = items
        ...
        .find((item) => item.success && item.data.phase !== "commentary");
      if (!answer?.success) {
        receipt.milestone = "failed";
        await save();
        return markTerminal({
      ...
      try {
        parsed = AnswerOutputSchema.safeParse(JSON.parse(answer.data.text));
      } catch {
        /* terminal invalid output below */
      }
      if (!parsed?.success) {
        receipt.milestone = "failed";
        await save();
      ...
      receipt.output = parsed.data;
      receipt.milestone = "completed";
      await save();
      return markTerminal({
        kind: "completed",
        output: parsed.data,
    ...
    return {
      kind:
        receipt.milestone === "submission_attempted" || receipt.milestone === "accepted"
          ? "uncertain"
          : "failed",
      nativeActivity: [...activity],
    };
    ...
    if (terminalToClean && ownerClosed) {
      ...
      terminalToClean.cleanupFailure =
        "Answer writer transport did not close before archive cleanup";
    }
  }
}
  ...
  for (let page = 0; page < 20; page++) {
    ...
    if (
      result.data.some(
        (value) =>
          value.model === model &&
          value.supportedReasoningEfforts.some((candidate) => candidate.reasoningEffort === effort),
      )
    )
      return true;
...
async function findMarkedTurn(
  ...
  const turns = z
    .object({ data: z.array(z.object({ id: Id })).max(5), nextCursor: z.string().nullable() })
    .parse(
      await request("thread/turns/list", {
        threadId,
        limit: 5,
...
async function readItems(
  ...
  for (let page = 0; page < 100; page++) {
    const result = z
      .object({ data: z.array(z.unknown()).max(100), nextCursor: z.string().nullable() })
      .parse(
    ...
    bytes += Buffer.byteLength(canonical(result.data));
    if (bytes > 1024 * 1024) throw Error("Answer history exceeded its byte bound");
    for (const value of result.data) {
      ...
      if (
        itemType.success &&
        !["userMessage", "agentMessage", "reasoning"].includes(itemType.data.type)
      )
        activity.add(`item:${itemType.data.type}`);
...
function nativeActivityLabel(message: RpcMessage): string | undefined {
  if (/(?:^|\/)(?:tool|approval|input|commandExecution|webSearch)(?:\/|$)/i.test(message.method))
    return message.method;
  const item = z
    .object({
      item: z.object({ type: z.string() }).optional(),
      action: z.object({ type: z.string() }).optional(),
    })
    .safeParse(message.params);
  const type = item.success ? (item.data.item?.type ?? item.data.action?.type) : undefined;
  return type && !["userMessage", "agentMessage", "reasoning"].includes(type)
    ? `item:${type}`
    : undefined;
}
...
const expectedAssertions: Readonly<Record<string, readonly Assertion[]>> = {
  s1: [
    { label: "Willow Lane", pattern: /willow lane/i },
    { label: "step-free", pattern: /step[- ]free/i },
  ],
  s2: [
    { label: "closed Monday", pattern: /closed[^.]{0,40}monday|monday[^.]{0,40}closed/i },
    { label: "Tuesday opening", pattern: /tuesday/i },
  ],
  s3: [
    { label: "no recollection", pattern: /\bno\b|do not|don't/i },
    { label: "retained scans", pattern: /retained[^.]{0,30}scans|scans[^.]{0,30}retained/i },
  ],
  s4: [
    { label: "replacement bus", pattern: /replacement bus/i },
    { label: "Orchard Square", pattern: /orchard square/i },
    { label: "3 to 9 April", pattern: /3\D+(?:to|-|–)\D*9\D+april/i },
  ],
  s5: [
    {
      label: "bicycles prohibited",
      pattern: /cannot[^.]{0,30}(?:bike|bicycle)|(?:bike|bicycle)[^.]{0,30}(?:cannot|not allowed)/i,
    },
  ],
  s6: [
    {
      label: "stored rainwater",
      pattern: /stored rainwater|rainwater[^.]{0,30}(?:tank|first|before)/i,
    },
  ],
  s7: [{ label: "induction required", pattern: /induction/i }],
  s8: [
    { label: "no reprint", pattern: /\bno\b|does not|don't/i },
    { label: "cover replaceable", pattern: /cover[^.]{0,40}replac|replac[^.]{0,40}cover/i },
  ],
  s9: [
    { label: "no immediate retry", pattern: /\bno\b|do not|don't/i },
    { label: "inspect original", pattern: /inspect[^.]{0,40}original/i },
  ],
  s10: [
    { label: "no guarantee", pattern: /\bno\b|not guarantee|does not guarantee/i },
    { label: "clouds", pattern: /cloud/i },
  ],
  s11: [{ label: "ventilated cabinet", pattern: /ventilated cabinet/i }],
  i1: [
    { label: "automatic retry disabled", pattern: /(?:disable|no)[^.]{0,50}(?:automatic )?retr/i },
  ],
  i2: [
    {
      label: "completed thumbnails cached",
      pattern: /cache[^.]{0,40}(?:completed )?thumbnail|thumbnail[^.]{0,40}cache/i,
    },
  ],
  ...
  c1: [
    { label: "Willow Lane", pattern: /willow lane/i },
    { label: "Tuesday through Sunday", pattern: /tuesday[^.]{0,40}sunday/i },
    { label: "10 to 18", pattern: /10(?::00)?\D+(?:to|-|–)\D*18(?::00)?/i },
  ],
  c2: [
    { label: "bridge repair", pattern: /bridge repair/i },
    { label: "replacement bus", pattern: /replacement bus/i },
    {
      label: "bicycle prohibited",
      pattern: /cannot[^.]{0,30}(?:bike|bicycle)|(?:bike|bicycle)[^.]{0,30}(?:cannot|not allowed)/i,
    },
  ],
  c3: [
    {
      label: "cover separate",
      pattern: /cover[^.]{0,50}(?:separate|replace)|(?:separate|replace)[^.]{0,50}cover/i,
    },
    { label: "no reprint", pattern: /reprint|printed pages/i },
  ],
  x1: [
    { label: "not cash-only", pattern: /\bno\b|not[^.]{0,30}cash.only/i },
    { label: "contactless", pattern: /contactless/i },
    { label: "since 1 June", pattern: /since\s+1\s+june/i },
  ],
  x2: [
    { label: "not regular closure", pattern: /\bno\b|not[^.]{0,50}(?:always|regular)/i },
    {
      label: "later Sunday collections",
      pattern: /later[^.]{0,50}sunday|sunday[^.]{0,50}(?:success|collection)/i,
    },
  ],
  n1: [],
  n2: [],
...
export function scoreAnswer(
  ...
  const provided = [...value.records, ...value.chains.flatMap((chain) => chain.records)].map(
    (record) => record.ref,
  );
  ...
  const isCurrent = (citation: RecordRef) =>
    current.has(refKey(citation)) ||
    (citation.origin === "evaluation-public" &&
      citation.revision === "r1" &&
      /^inventory-\d+$/.test(citation.id));
  const expected = new Set([...question.relevant, ...question.requiredChain]);
  const invalid = parsed.citations
    .filter((citation) => !provided.some((candidate) => sameRef(candidate, citation)))
    .map(refKey);
  ...
  const checks = assertions.map((assertion) => ({
    ...
    matched: assertion.pattern.test(parsed.answer),
  }));
  return {
    citations: {
      supplied: parsed.citations.length,
      valid: parsed.citations.filter((citation) =>
        provided.some((candidate) => sameRef(candidate, citation)),
      ).length,
    ...
    abstention: {
      ...
      actual: parsed.abstained,
      matched: question.abstain === parsed.abstained,
    },
    limit: "deterministic_surface_checks_are_not_universal_correctness" as const,
  };
}

const evaluationSubject = Object.freeze({
  type: "evaluation",
  id: "answer-owner",
...
export async function loadAnswerCases(options: {
  ...
  const provider = createSqliteKnowledge({
    databasePath: resolve(options.databasePath),
    authorizer: evaluationAuthorizer,
    resolveResource: resolveEvaluationResource,
  });
  try {
    ...
    for (const question of heldOutQuestions) {
      ...
      if (!reported || reported.category !== question.kind)
        throw Error(`Missing retrieval result for ${question.id}`);
      const records = [];
      const chains = [];
      for (const key of reported.retrieved) {
        ...
        if (read.kind !== "ok" || !read.record)
          throw Error(`Reported retrieved record is unavailable: ${key}`);
        ...
        do {
          ...
          if (chainBytes > maxBytes)
            throw Error(`Evidence chain exceeds byte allowance for ${key}`);
          for (const record of page.records)
            if (!chainRecords.some((prior) => sameRef(prior.ref, record.ref)))
              chainRecords.push(record);
          for (const link of page.links)
            if (!links.some((prior) => canonical(prior) === canonical(link))) links.push(link);
          if (chainRecords.length > 100 || links.length > 100)
            throw Error(`Evidence chain exceeds item allowance for ${key}`);
          if (page.cursor && seen.has(page.cursor))
            throw Error(`Evidence cursor repeated for ${key}`);
          cursor = page.cursor;
          if (cursor) seen.add(cursor);
        } while (cursor);
        const retained = (candidate: RecordRef) =>
          chainRecords.some((record) => sameRef(record.ref, candidate));
        const complete =
          chainRecords.every((record) => record.provenance.inputs.every(retained)) &&
          links.every((link) => retained(link.from) && retained(link.to));
        chains.push({ root: ref, records: chainRecords, links, complete });
      }
      answerCases.push(
        AnswerEvaluationCaseSchema.parse({
          mode: options.mode,
...
export interface AnswerEvaluationCliOptions {
  ...
  modelsRoot?: string;
  reports?: Readonly<
    Record<AnswerEvaluationCase["mode"], { reportPath: string; databaseRoot: string }>
  >;
  maxCases: number;
...
export function parseAnswerEvaluationCli(
  arguments_: readonly string[],
): AnswerEvaluationCliOptions {
  const value = (name: string) => {
    const index = arguments_.indexOf(name);
    return index < 0 ? undefined : arguments_[index + 1];
  };
  const root = value("--root");
  const modelsRoot = value("--models-root");
  if (!root) throw Error("--root is required");
  const external = ["lexical", "mlx"].map((name) => ({
    name,
    reportPath: value(`--${name}-report`),
    databaseRoot: value(`--${name}-db-root`),
  }));
  const externalCount = external
    .flatMap((item) => [item.reportPath, item.databaseRoot])
    .filter(Boolean).length;
  if (externalCount !== 0 && externalCount !== 4)
    throw Error("all lexical and MLX report/database paths are required together");
  if (externalCount === 0 && !modelsRoot)
    throw Error("--models-root is required when existing reports are not supplied");
  const maxCases = value("--max-cases") === undefined ? 72 : Number(value("--max-cases"));
  const timeoutMsPerCase =
    value("--timeout-ms") === undefined ? 300_000 : Number(value("--timeout-ms"));
  const maxEvidenceBytes =
    value("--evidence-bytes") === undefined ? 256 * 1024 : Number(value("--evidence-bytes"));
  if (!Number.isInteger(maxCases) || maxCases < 1 || maxCases > 72)
    throw Error("--max-cases must be between 1 and 72");
  if (!Number.isInteger(timeoutMsPerCase) || timeoutMsPerCase < 1 || timeoutMsPerCase > 300_000)
    throw Error("--timeout-ms must be between 1 and 300000");
  if (
    !Number.isInteger(maxEvidenceBytes) ||
    maxEvidenceBytes < 1024 ||
    maxEvidenceBytes > 1024 * 1024
  )
    throw Error("--evidence-bytes must be between 1024 and 1048576");
  const reports =
    externalCount === 4
      ? ({
          lexical: {
            reportPath: resolve(external[0]!.reportPath!),
            databaseRoot: resolve(external[0]!.databaseRoot!),
          },
          "qwen3-embedding-0.6b-gguf": {
            reportPath: resolve(external[1]!.reportPath!),
            databaseRoot: resolve(external[1]!.databaseRoot!),
          },
        } as const)
      : undefined;
  return {
    root: resolve(root),
    ...(modelsRoot ? { modelsRoot: resolve(modelsRoot) } : {}),
    ...(reports ? { reports } : {}),
    maxCases,
    timeoutMsPerCase,
    maxEvidenceBytes,
...
export async function runAnswerQualityEvaluation(options: AnswerQualityEvaluationOptions) {
  ...
  for (const entry of modes) {
    ...
    if (corpusIdentity && canonical(corpusIdentity) !== canonical(prepared.corpus))
      throw Error("Lexical and MLX reports must identify the same corpus");
    corpusIdentity = prepared.corpus;
    retrieval[entry.mode] = prepared.report;
    if (!prepared.questions) continue;
    const loaded = await loadAnswerCases({
  ...
  for (const [index, entry] of selected.entries()) {
    ...
    const result = await runAnswerCase({
      ...
      effort: "low",
      store,
      connect: () => options.connect(workingDirectory),
      workingDirectory,
      timeoutMs: options.timeoutMsPerCase,
      maxEvidenceBytes: options.maxEvidenceBytes,
    });
    ...
    results.push({
      ...
      ...(result.kind === "completed"
        ? { score: scoreAnswer(question, entry.value, result.output) }
        : {}),
    });
    await store.set(
      "answer-evaluation-progress",
      z.json().parse({ completedCases: index + 1, totalCases: selected.length, results }),
    );
    if (result.cleanupFailure)
      throw Error(
        `Answer evaluation cleanup failed after its receipt was saved: ${result.cleanupFailure}`,
      );
  ...
  return {
    ...
    limits: {
      ...
      timeoutMsPerCase: options.timeoutMsPerCase,
      maxEvidenceBytes: options.maxEvidenceBytes,
    },
    retrieval,
    results,
    summary: {
      ...
      nativeActivityCases: results.filter((result) => result.result.nativeActivity.length > 0)
        .length,
      citationValidity: ratio(
        scores.reduce((sum, score) => sum + score.citations.valid, 0),
        scores.reduce((sum, score) => sum + score.citations.supplied, 0),
      ),
      currentCitationRate: ratio(
        scores.reduce((sum, score) => sum + score.citations.current, 0),
        scores.reduce((sum, score) => sum + score.citations.supplied, 0),
      ),
      expectedCitationRate: ratio(
        scores.reduce((sum, score) => sum + score.citations.expected, 0),
        scores.reduce((sum, score) => sum + score.citations.supplied, 0),
      ),
      surfaceAssertionRate: ratio(
        scores.reduce((sum, score) => sum + score.surfaceAssertions.matched, 0),
        scores.reduce((sum, score) => sum + score.surfaceAssertions.total, 0),
      ),
      abstentionAccuracy: ratio(
        scores.filter((score) => score.abstention.matched).length,
        scores.length,
      ),
    },
    limitations: [
...
async function prepareGeneratedRetrieval(
  store: JsonStore,
  mode: AnswerEvaluationCase["mode"],
  root: string,
  modelsRoot: string,
  ...
  if (saved !== undefined) {
    ...
    if (
      prepared.mode !== mode ||
      prepared.databasePath !== join(root, "knowledge.sqlite") ||
      prepared.source !== "generated_smoke"
    )
      throw Error(`Stored retrieval configuration changed for ${mode}`);
    return prepared;
  }
  const report = await runKnowledgeEvaluation({
    root,
    ...(mode === "lexical" ? {} : { model: mode, modelRoot: modelsRoot }),
  });
  const questions =
    mode === "lexical"
...
async function prepareExistingRetrieval(
  mode: AnswerEvaluationCase["mode"],
  source: { reportPath: string; databaseRoot: string },
): Promise<PreparedRetrieval> {
  ...
  const currentHash = createHash("sha256")
    ...
    .digest("hex");
  if (report.corpus.version !== corpusVersion || report.corpus.sha256 !== currentHash)
    throw Error(`Retrieval report corpus does not match the frozen evaluator: ${mode}`);
  const questions =
    mode === "lexical"
      ? report.lexical.questions
      : report.hybrid.kind === "real_vectors" && report.hybrid.model === mode
        ? report.hybrid.questions
        : undefined;
  ...
  return PreparedRetrievalSchema.parse({
    version: 1,
    mode,
    databasePath: join(resolve(source.databaseRoot), "knowledge.sqlite"),
    corpus: report.corpus,
    questions,
    report: z.json().parse(report),
    source: "existing_report",
    reportSha256: digest(bytes.toString("utf8")),
  });
}
...
if (import.meta.url === pathToFileURL(process.argv[1] ?? "").href) {
  if (process.env.DRAWLOOM_KNOWLEDGE_ANSWER_LIVE !== "1")
    throw Error(
      "Set DRAWLOOM_KNOWLEDGE_ANSWER_LIVE=1 to authorize this bounded real Codex answer evaluation.",
    );
  const options = parseAnswerEvaluationCli(process.argv.slice(2));
  const report = await runAnswerQualityEvaluation({
    ...options,
    connect: async (workingDirectory) =>
      createStdioTransport({
        ...codexCommand(),
        cwd: workingDirectory,
        requestTimeoutMs: options.timeoutMsPerCase,
