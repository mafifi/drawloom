...
if (process.env.DRAWLOOM_TEMPORAL_TEST !== "1") {
  process.stdout.write(
    "SKIPPED: set DRAWLOOM_TEMPORAL_TEST=1 to run the local Temporal Nightloom integration.\n",
  );
  process.exit(0);
}
...
const subject = Object.freeze({
  type: "user",
  id: "nightloom-evaluation",
  properties: { locality: "test" },
});
const resource = ({ ref } = {}) => ({
  type: ref ? "knowledge-record" : "knowledge-store",
  id: ref ? `${ref.type}:${ref.origin}:${ref.id}:${ref.revision}` : "nightloom-evaluation",
  properties: { locality: "test" },
});
...
function receiptStore(store) {
  ...
  return {
    async load(requestId) {
      return await store.get(key(requestId));
    },
...
try {
  ...
  for (const [id, body] of [
    ["first", "public first source"],
    ["second", "public second source"],
  ]) {
    const accepted = await provider.intake.ingest(subject, {
      operation: "upsert",
      expectedRevision: null,
      record: {
        ref: { type: "source", origin: "public-nightloom-evaluation", id, revision: "r1" },
        body,
        status: "active",
        confidence: { value: "fixture" },
        provenance: { producer: { type: "evaluation", id: "public" }, inputs: [] },
      },
      links: [],
    });
  ...
  const assessment = {
    ...
    async cancel() {
      return { kind: "too_late", requestId: "not-used", payloadFingerprint: "not-used" };
    },
  };
  ...
  const firstRun = await registration.orchestrator.start(
    "owned-nightloom-run",
    nightloomWorkflow,
    input,
  );
  ...
  const reopenedRun = await registration.orchestrator.start(
    "owned-nightloom-run",
    nightloomWorkflow,
    input,
  );
  ...
  assert.equal(assessments, 1);
  process.stdout.write(
    `${JSON.stringify({ kind: "passed", processed: first.processed, assessments, runId: firstRun })}\n`,
  );
} finally {
  await manager?.close();
  provider?.close();
