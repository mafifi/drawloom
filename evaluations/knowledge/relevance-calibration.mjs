...
if (
  ...
  throw Error(
    "Opt in with DRAWLOOM_GGUF_EVALUATION=1, DRAWLOOM_MODELS_ROOT and an output path; no downloads",
  );
...
if (!(await createModelSetup({ root, manifest: KnownModelManifests[model] }).ready()))
  throw Error("Existing model not ready; no install allowed");
const worker = new LlamaEmbeddingWorker({ root, model });
const embeddings = createKnowledgeEmbeddings({
  model,
  worker,
...
async function encode(role, values) {
  const result = await embeddings.embed(subject, {
    configuration,
    role,
    items: values.map((item) => ({
      id: item.id,
      text: role === "query" ? item.query : item.document,
    })),
  });
...
try {
  ...
  const queries = await encode("query", fixture.pairs),
    passages = await encode("document", documents);
  const pairs = queries.flatMap((query) =>
    passages.map((document) => ({
      queryId: query.id,
      documentId: document.id,
      expectedRelevant: query.id === document.id,
      cosine:
        query.vector.reduce((sum, v, i) => sum + v * document.vector[i], 0) /
        (Math.hypot(...query.vector) * Math.hypot(...document.vector)),
    })),
  );
  ...
  const report = {
    ...
    configuration,
    preprocessing:
      "Product role=query/document formatting, last-token pooling and L2 normalization; one passage per document, <=512 Unicode code points.",
    pairs,
    positiveMin,
    negativeMax,
    selected,
    candidate05: {
      rejectedPositives: pairs.filter((p) => p.expectedRelevant && p.cosine < 0.5),
      admittedNegatives: pairs.filter((p) => !p.expectedRelevant && p.cosine >= 0.5),
    },
  };
  writeFileSync(process.argv[2], JSON.stringify(report, null, 2) + "\n");
  console.log(
    JSON.stringify(
      { positiveMin, negativeMax, selected, candidate05: report.candidate05, pairs: pairs.length },
      null,
      2,
    ),
  );
