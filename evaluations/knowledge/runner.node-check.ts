...
test("default evaluation is a deterministic lexical smoke run without semantic claims", async () => {
  ...
  try {
    ...
    assert.equal(
      report.lexical.storeBytes.total,
      report.lexical.storeBytes.database +
        report.lexical.storeBytes.wal +
        report.lexical.storeBytes.shm,
    );
    assert.ok(report.lexical.storeBytes.wal > 0);
    assert.ok(report.lexical.storeBytes.shm > 0);
    assert.deepEqual(
      Object.keys(report.lexical.questions).sort(),
      heldOutQuestions.map((question) => question.id).sort(),
    );
...
test("scale and stress flags select their documented corpus sizes", () => {
  assert.deepEqual(parseCliOptions(["--root", "/tmp/evaluation", "--scale"]), {
    root: "/tmp/evaluation",
    size: 10_000,
  });
  assert.deepEqual(parseCliOptions(["--root", "/tmp/evaluation", "--stress"]), {
    root: "/tmp/evaluation",
    size: 100_000,
  });
});
...
test("an external embedding implementation exercises the unchanged path with separate indexing timing", async (context) => {
  ...
  const configuration = {
    id: "evaluation:external",
    fingerprint: "deterministic-not-model-quality",
    dimensions: 2,
  };
  let documents = 0,
    queries = 0;
  try {
    ...
    assert.equal(
      report.hybrid.kind === "real_vectors" && report.hybrid.model,
