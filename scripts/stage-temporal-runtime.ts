 ...
 */
export async function stageTemporalRuntime(options: {
  ...
  if (
    basename(destination) !== destinationName ||
    destination === repository ||
    contained(destination, repository)
  )
    throw Error("Invalid generated runtime destination");
  ...
  async function copyPackage(source: string): Promise<void> {
    ...
    const internal =
      contained(join(repository, "packages"), source) && manifest.name.startsWith("@drawloom/");
    if (!internal && !contained(modules, source))
      throw Error(`Runtime dependency escapes public installation: ${manifest.name}`);
    ...
    if (localPath.startsWith(".bun/"))
      throw Error("Runtime staging requires the canonical hoisted dependency layout");
    ...
    for (const entry of entries) {
      ...
      try {
        await cp(path, join(target, entry), {
          recursive: true,
        ...
        if (!(internal && error instanceof Error && "code" in error && error.code === "ENOENT"))
          throw error;
      }
    }
    packages.push({
  ...
  try {
    for (const packageDirectory of options.packageDirectories ?? [
      join(repository, "packages/orchestration/temporal-orchestration"),
    ])
      await copyPackage(packageDirectory);
    await writeFile(
      join(temporary, "runtime-manifest.json"),
      JSON.stringify(
        { format: 1, platform: process.platform, arch: process.arch, packages },
        null,
        2,
      ),
    );
...
if (import.meta.main) {
  ...
  const result = await stageTemporalRuntime({
    repositoryRoot,
    destination: join(repositoryRoot, "apps/desktop/src-tauri/binaries/orchestration"),
  });
