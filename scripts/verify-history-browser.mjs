...
if (!process.env.DRAWLOOM_PLAYWRIGHT_PATH || !process.argv[2])
  throw Error("Set DRAWLOOM_PLAYWRIGHT_PATH and pass the isolated fixture bootstrap URL.");
...
const output =
  process.env.DRAWLOOM_BROWSER_EVIDENCE_DIR ||
  (await mkdtemp(join(tmpdir(), "drawloom-history-browser-evidence-")));
await mkdir(output, { recursive: true });
const browser = await chromium.launch({
  headless: true,
  channel: process.env.DRAWLOOM_BROWSER_CHANNEL || "msedge",
});
...
try {
  ...
  await page
    .getByRole("button", { name: "Load earlier", exact: true })
    .waitFor({ state: "visible" });
  await page.waitForFunction(() =>
    [...document.querySelectorAll("button")].some(
      (button) =>
        button.textContent?.trim() === "Load earlier" &&
        getComputedStyle(button).color === "rgb(245, 245, 245)",
    ),
  );
  await page.screenshot({ path: join(output, "dark.png") });
  await page.waitForResponse(
    (response) => response.url().includes("/api/state") && response.status() === 204,
  );
  await page.waitForResponse(
    (response) => response.url().includes("/api/history/changes") && response.status() === 204,
  );
  let release;
  ...
  await page.route("**/api/history?*before=*", async (route) => {
    reached();
    await delayed;
  ...
  console.log(
    JSON.stringify({
      ...
      initialEntries: 50,
      maxRendered: 200,
      scrollDeltaPx: after - before,
      navigationIsolation: "pass",
      themes: ["light", "dark"],
      viewports: ["1440x1000", "390x844"],
      errors,
      unchangedApplicationPolls: responses.filter(
        (r) => r.path === "/api/state" && r.status === 204,
      ).length,
      unchangedHistoryPolls: responses.filter(
        (r) => r.path === "/api/history/changes" && r.status === 204,
      ).length,
    }),
  );
} finally {
