...
const page =
  '<html><head></head><body><svg><g id="node-knowledge" data-node-id="knowledge" tabindex="0" role="button" aria-pressed="false"><title>Knowledge</title><g><rect width="250"/></g><text>Knowledge</text></g></svg></body></html>';
const maps = [
  { name: "overview", title: "Ownership map" },
  { name: "knowledge", title: "Knowledge & memory" },
];

test("overview links are not hidden inside a single accessible image", () => {
  const result = addAtlasNavigation(page.replace("<svg>", '<svg role="img">'), maps, "overview", [
    { id: "knowledge", name: "knowledge", title: "Knowledge" },
  ]);
  expect(result).toContain('<svg role="group">');
});

test("overview family tiles become native links without intercepting focus controls", () => {
  const result = addAtlasNavigation(page, maps, "overview", [
    { id: "knowledge", name: "knowledge", title: "Knowledge & memory" },
  ]);
  expect(result).toContain(
    '<a href="knowledge.html" aria-label="Open Knowledge &amp; memory map" class="atlas-tile">',
  );
...
test("renderer drift or missing destinations fail the build instead of silently losing navigation", () => {
  expect(() =>
    addAtlasNavigation(page, maps, "overview", [
      { id: "missing", name: "knowledge", title: "Knowledge" },
    ]),
  ).toThrow();
  expect(() =>
    addAtlasNavigation(page, maps, "overview", [
      { id: "knowledge", name: "missing", title: "Missing" },
    ]),
  ).toThrow();
});
