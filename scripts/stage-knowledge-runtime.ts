...
export function stageKnowledgeRuntime(options: { repositoryRoot: string; destination: string }) {
  return stageTemporalRuntime({
    ...options,
    destinationName: "knowledge",
    packageDirectories: [
...
if (import.meta.main) {
  ...
  const result = await stageKnowledgeRuntime({
    repositoryRoot,
    destination: join(repositoryRoot, "apps/desktop/src-tauri/binaries/knowledge"),
  });
