...
try {
  ...
  const document = async (name: string) =>
    JSON.parse(await readFile(join(root, "v2", name + ".json"), "utf8")) as unknown;
  const schema = async (name: string) => object.parse(await document(name)).properties;
  const resume = await schema("ThreadResumeParams"),
    turns = await schema("ThreadTurnsListParams"),
    items = await schema("ThreadItemsListParams");
  if (
    !resume.excludeTurns ||
    !turns.itemsView ||
    !turns.cursor ||
    ...
    throw Error("Installed Codex lacks required bounded history fields.");
  const response = await schema("ThreadTurnsListResponse");
  if (!response.nextCursor || !response.data)
    throw Error("Installed Codex history response differs from the supported shape.");
  ...
  if (!turn.completedAt || !turn.startedAt || !turn.durationMs || !turn.status)
    throw Error("Installed Codex lacks the inspected completion metadata.");
  const item = object.parse(
    definitions.parse(await document("ThreadItemsListResponse")).definitions.ThreadItemEntry,
  ).properties;
  if (!item.turnId || !item.item)
    throw Error("Installed Codex item correlation differs from the supported shape.");
  console.log(
    JSON.stringify({
      version: version.stdout.toString().trim(),
      checked: [
        "thread/resume.excludeTurns",
        "thread/turns/list.itemsView",
        "thread/turns/list.cursor",
        "thread/items/list.cursor",
        "thread/items/list.limit",
        "thread/items/list.turnId",
        "thread/turns/list.nextCursor",
        "Turn completion metadata",
        "ThreadItemEntry turn/item correlation",
      ],
      result: "pass",
      liveProviderPerformance: false,
    }),
  );
} finally {
