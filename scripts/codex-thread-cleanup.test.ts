...
function fixture(failArchive = false) {
  ...
  const transport: RpcTransport = {
    ...
    notify() {}, respond() {}, subscribe() { return () => {}; },
...
test("archives only the exact disposable thread identity after preserving a successful result", async () => {
  ...
  const finished = await finishDisposableCodexThread(primary, { threadId: "owned-thread", connect: f.connect });
...
test("preserves a primary failure while still archiving its exact owned thread", async () => {
  ...
  const finished = await finishDisposableCodexThread(primary, { threadId: "failed-proof-thread", connect: f.connect });
  expect(finished.primary).toBe(primary);
...
test("reports archive failure without replacing or mutating the saved primary receipt", async () => {
  ...
  expect(finished.cleanup).toEqual({ kind: "failed", threadId: "owned-thread", reason: "archive unavailable" });
  expect(f.closes()).toBe(1);
});
...
test("does not connect or archive when the caller has no exact owned thread identity", async () => {
  ...
  const finished = await finishDisposableCodexThread(primary, { connect: async () => { connected = true; throw Error("must not connect"); } });
...
test("closes a cleanup transport that resolves after connection timeout", async () => {
  let resolve!: (transport: RpcTransport) => void; let closes = 0;
  const late = new Promise<RpcTransport>(done => { resolve = done; });
  const finishedPromise = finishDisposableCodexThread({ kind: "completed" }, { threadId: "owned-thread", connect: () => late, timeoutMs: 5 });
  await Bun.sleep(10);
  resolve({ request: async () => ({}), notify() {}, respond() {}, subscribe() { return () => {}; }, async close() { closes++; } });
