...
export function addAtlasNavigation(html, maps, current, destinations = []) {
  ...
  if (!html.includes("</head>") || !html.includes("<body>"))
    throw Error("Unsupported atlas document");
  if (maps.some((map) => !/^[a-z0-9-]+$/.test(map.name))) throw Error("Invalid map name");
  if (destinations.length)
    html = html.replace(/<svg\b[^>]*>/, (tag) => tag.replace('role="img"', 'role="group"'));
  for (const destination of destinations) {
    ...
    const group = html
      .slice(start, end)
      .replace(/^<g[^>]*>/, (opening) =>
        opening.replace(/ (?:data-node-id|tabindex|role|aria-label|aria-pressed)="[^"]*"/g, ""),
      );
    html =
      html.slice(0, start) +
      `<a href="${destination.name}.html" aria-label="Open ${escape(destination.title)} map" class="atlas-tile">${group}</a>` +
      html.slice(end);
  }
  const links = maps
    .map(
      (map) =>
        `<a href="${map.name}.html"${map.name === current ? ' aria-current="page"' : ""}>${escape(map.title)}</a>`,
    )
    .join("");
