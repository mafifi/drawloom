...
| Orchestration | Architecture capability partition; no need to implement every capability now. | Goals, children, process-local jobs, workflow VMs and experimental teams have different guarantees. | Defer generic engines. Consumer/plugin uses supported runtime behavior first. |
